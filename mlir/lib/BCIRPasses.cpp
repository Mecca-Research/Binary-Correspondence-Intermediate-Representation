...
//===----------------------------------------------------------------------===//
// -bcir-rcsp : K_BCIR constrained selection (RCSP) -- the deterministic optimizer
// core ported from bcir/kbcir/rcsp.py. Over a claim's candidate paths it runs the
// budget-feasible min-plus argmin via label dominance and computes the Pareto front
// over (score, thermal, power) -- the non-convex points no single weight vector can
// reach (Geoffrion). Annotates kbcir.computed_{selected,score} and kbcir.pareto_size
// and cross-checks the declared selection (reproduces the oracle's 9472 under the cap
// + the {vec16, vec8} front of size 2). C++23.
//===----------------------------------------------------------------------===//
