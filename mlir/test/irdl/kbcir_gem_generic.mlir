...
"bcir.kbcir_policy"() {sym_name = "perf", mode = "latency",
...
%p16 = "bcir.kbcir_path"() {sym_name = "add_cpu_u16", claim = "add",
  realization = "cpu.vector.u16", lane = "u", layout = "soa"} : () -> !bcir.handle
%sel = "bcir.kbcir_select"() {claim = "add", from = ["add_cpu_u8", "add_cpu_u16"],
...
"bcir.gem_prefetch"() {sym_name = "pf0", distance = 4 : i32, targets = ["A", "B"],
  hint = "T0", pattern = "linear"} : () -> ()
"bcir.gem_lane_segment"() {sym_name = "seg0", claim = "add", phase = "p0", lane = "u",
  stride_k = 1 : i32, width = 16 : i32, opcode = "f32.add", reads = ["A", "B"],
  writes = ["C"], fence_before = [], fence_after = []} : () -> ()
"bcir.trace_note"() {sym_name = "add_trace", src_hash = 0 : i64, trace_hash = 0 : i64} : () -> ()

// CHECK: "bcir.kbcir_policy"
// CHECK: "bcir.kbcir_path"
// CHECK: "bcir.kbcir_select"
// CHECK: "bcir.gem_stream_pack"
// CHECK: "bcir.gem_lane_segment"
// CHECK: "bcir.trace_note"
