...
"bcir.target_capability"() {sym_name = "riscv", triple = "riscv64",
  isa_features = ["rvv"], lane_widths = [1, 8, 16], warp = 0 : i32, cacheline = 64 : i32} : () -> ()
"bcir.target_lower_contract"() {sym_name = "lc_riscv", target = "riscv", bcir_op = "vector.add",
  lane = "u", stride_class = "unit", opcode = "vadd.vv", legal_if = "rvv",
  preserves = "bounds,hazard,precision"} : () -> ()

"bcir.target_capability"() {sym_name = "gpu", triple = "nvptx64-nvidia-cuda",
  isa_features = ["ptx"], lane_widths = [1, 32], warp = 32 : i32, cacheline = 128 : i32} : () -> ()
"bcir.target_lower_contract"() {sym_name = "lc_gpu", target = "gpu", bcir_op = "vector.add",
  lane = "u", stride_class = "unit", opcode = "add.f32", legal_if = "ptx",
  preserves = "bounds,hazard,precision"} : () -> ()

"bcir.target_capability"() {sym_name = "bpf", triple = "bpfel",
  isa_features = [], lane_widths = [1], warp = 0 : i32, cacheline = 64 : i32} : () -> ()
"bcir.target_lower_contract"() {sym_name = "lc_bpf", target = "bpf", bcir_op = "vector.add",
