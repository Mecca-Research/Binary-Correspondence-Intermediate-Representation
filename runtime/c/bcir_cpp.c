...
static long eval_if(const char *expr) {
  ...
  while(1){int k=ntok(expr,&i,t,sizeof t); if(!k)break;
    if(k=='i'&&!strcmp(t,"defined")){char p[256]; int j=i; int pk=ntok(expr,&j,p,sizeof p);
      ...
      has = find_macro(nm,(int)strlen(nm))>=0;
  ...
  while(*p){
    ...
    if(line[q]=='#'){
      ...
      if(!strcmp(dir,"ifdef")||!strcmp(dir,"ifndef")||!strcmp(dir,"if")){
        int tk; if(!strcmp(dir,"ifdef")){char n[64];int j=0;ntok(rest,&j,n,sizeof n);tk=find_macro(n,(int)strlen(n))>=0;}
        else if(!strcmp(dir,"ifndef")){char n[64];int j=0;ntok(rest,&j,n,sizeof n);tk=find_macro(n,(int)strlen(n))<0;}
        ...
        if(ncs){ int par=cs[ncs-1].parent; int take;
          ...
          else if(!strcmp(dir,"elifdef")){char n[64];int j=0;ntok(rest,&j,n,sizeof n);take=!cs[ncs-1].taken&&find_macro(n,(int)strlen(n))>=0;}
          else if(!strcmp(dir,"elifndef")){char n[64];int j=0;ntok(rest,&j,n,sizeof n);take=!cs[ncs-1].taken&&find_macro(n,(int)strlen(n))<0;}
