...
| Function/call-site attributes such as `noundef`, `noalias`, `readonly`, `memory(read)`, `memory(argmem: readwrite)`, `memory(inaccessiblemem: readwrite)`, `sret`, or `byval` | [`../13-advanced-ir/04-attributes.md`](../13-advanced-ir/04-attributes.md) | Attributes are optimizer-visible promises; copying unproven attributes miscompiles code. |
...
6. Audit `memory(...)` effects on BCIR runtime wrappers: `memory(none)` permits
   deletion of unused calls, scoped forms omit untouched locations, and legacy
   `readnone`/`readonly`/`writeonly` are coarser whole-function promises.
