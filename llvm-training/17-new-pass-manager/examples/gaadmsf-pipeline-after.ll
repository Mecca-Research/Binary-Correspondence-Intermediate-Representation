; Plausible post-pipeline shape after verification, SCCP, and loop canonicalization.
; This is checked as standalone IR, not as an exact golden output for every LLVM version.
...
entry:
  ...
  br i1 %has.work, label %loop.preheader, label %exit

loop.preheader:
  br label %loop

loop:
  %iv = phi i32 [ 0, %loop.preheader ], [ %iv.next, %loop ], !bcir.reg !3
  %sum = phi i32 [ 0, %loop.preheader ], [ %sum.next, %loop ], !bcir.reg !4
  ...
  br i1 %done, label %exit.loopexit, label %loop, !llvm.loop !7

exit.loopexit:
  br label %exit

exit:
  %result = phi i32 [ 0, %entry ], [ %sum.next, %exit.loopexit ]
