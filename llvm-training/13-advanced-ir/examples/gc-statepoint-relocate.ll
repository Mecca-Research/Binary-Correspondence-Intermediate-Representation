...
define i64 @load_field_after_statepoint(ptr addrspace(1) %obj) gc "statepoint-example" {
entry:
  ; %obj is the base object pointer. %field is a derived interior pointer.
  %field = getelementptr inbounds %Obj, ptr addrspace(1) %obj, i32 0, i32 1
  ...
  %sp = call token (i64, i32, ptr, i32, i32, ...) @llvm.experimental.gc.statepoint.p0(
      ...
      i32 0, i32 0, i32 0, i32 0) [ "gc-live"(ptr addrspace(1) %obj, ptr addrspace(1) %field) ]
  ...
  %obj.relocated = call ptr addrspace(1) @llvm.experimental.gc.relocate.p1(token %sp, i32 0, i32 0)
  %field.relocated = call ptr addrspace(1) @llvm.experimental.gc.relocate.p1(token %sp, i32 0, i32 1)

  ; Correct post-statepoint use: load through the relocated derived pointer, not
  ; through the original %field SSA value.
  %value = load i64, ptr addrspace(1) %field.relocated, align 8

  ; Keep the relocated base visibly live in this teaching example.
  %same.object = icmp eq ptr addrspace(1) %obj.relocated, %obj.relocated
