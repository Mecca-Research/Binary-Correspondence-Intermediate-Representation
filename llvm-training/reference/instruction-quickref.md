...
| `invoke` | `%r = invoke <ty> @f(...) to label %N unwind label %U` | Normal and unwind successors; result available on normal edge | Call that transfers exceptions to an EH pad. Use instead of `call` when the callee may unwind into IR EH. |
...
| `call` | `%r = call <ty> @f(<args>)` | Function or intrinsic call. Normal calls do not have unwind successors; use `invoke` if unwinding is represented. |
