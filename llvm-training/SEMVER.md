...
`llvm-training/examples/MANIFEST.txt` is the source-of-truth list of known-good standalone examples. `llvm-training/tools/verify-manifest.sh` is part of the compatibility contract: every checked-in standalone `*/examples/*.ll` file must appear in that manifest and in the README mirror, while intentionally invalid `.ll.txt` fixtures remain excluded.
