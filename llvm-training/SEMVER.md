...
`llvm-training/tools/verify-manifest.sh` is part of the compatibility contract: every checked-in standalone `*/examples/*.ll` file must appear in `llvm-training/examples/README.md`, while intentionally invalid `.ll.txt` fixtures remain excluded.
