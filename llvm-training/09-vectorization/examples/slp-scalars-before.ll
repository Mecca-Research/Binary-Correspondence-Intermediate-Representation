...
;
; Try:
;   opt -S -passes=slp-vectorizer slp-scalars-before.ll -o -
;   opt -S -passes=slp-vectorizer -slp-threshold=-999 slp-scalars-before.ll -o -
;
; Expected observation:
;   Before SLP, the code contains independent scalar i32 loads, adds, and
;   stores that are close enough for SLP to consider packing into vector lanes.

source_filename = "slp-scalars-before.ll"
...
entry:
  ...
  store i32 %s0, ptr %c0p, align 4
  store i32 %s2, ptr %c1p, align 4
  store i32 %s1, ptr %c2p, align 4
  store i32 %s3, ptr %c3p, align 4
  ret void
}
