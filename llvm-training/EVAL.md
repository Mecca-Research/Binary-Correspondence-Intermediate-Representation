...
- Which BCIR facts should remain as dialect operations or attributes before descriptor lowering?
- What does a type converter do with `!bcir.vertex` or `!bcir.edge` values before the LLVM dialect boundary?
- Why is a leftover `builtin.unrealized_conversion_cast` usually evidence of an incomplete conversion?
- In `14-mlir-bridge/examples/bcir-final.ll`, where did the graph edge weight become a concrete LLVM struct field load?
