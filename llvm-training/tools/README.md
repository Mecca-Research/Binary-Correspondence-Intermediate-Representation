...
| --- | --- | --- |
| `verify-examples.sh` | Reads `examples/MANIFEST.txt`, assembles each listed standalone `.ll` file with `llvm-as`, and runs `opt -passes=verify`. It also checks the broken `.ll.txt` sentinel so intentionally invalid examples do not drift into the manifest. | `llvm-as`, `opt` |
| `verify-manifest.sh` | Compares discovered standalone `*/examples/*.ll` files, the source-of-truth `examples/MANIFEST.txt`, and the human-readable `examples/README.md` table so new examples cannot land without manifest documentation. | `bash`, `python3` |
...
`llvm-training-verify-manifest` has no LLVM tool dependency; it only
checks documentation drift. Each other CMake target checks for the LLVM tools
needed by its underlying script before running it. If the host image does not
provide those tools, the target prints the same kind of clean skip message used by CI and exits successfully.
...
`verify-examples.sh` is driven by the positive source manifest in
`examples/MANIFEST.txt` rather than a global `*.ll*` sweep:
...
- only files below an `examples/` directory are considered standalone training
  modules;
- `verify-manifest.sh` compares discovered files with `examples/MANIFEST.txt`
  and the README mirror so the machine-readable manifest remains authoritative.

CI runs `verify-manifest.sh` after the standalone example verifier so PRs
cannot add a checked `.ll` without updating `examples/README.md`. CI then runs
`smoke-llc.sh` when `llc` is available, guaranteeing that curated
runnable-through-codegen training IR continues to lower successfully. Minimal images without `llc` report an
