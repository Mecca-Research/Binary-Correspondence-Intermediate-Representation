...
the canonical example verification script. It reads the source-of-truth list in
`llvm-training/examples/MANIFEST.txt` rather than discovering examples ad hoc:
...
Check both manifests for drift whenever adding or removing `*/examples/*.ll`
files. This compares checked-in files, `examples/MANIFEST.txt`, and the
human-readable `examples/README.md` table:
