...
| F9 | `mirrors-clang-format` v18.1.8 while the rail's toolchain is LLVM 23 | low (style drift) | **Revision moved to v23.1.0 in the follow-up PR; no reformat.** Measured before moving: 18.1.8 and 23.1.0 disagree on 13 of the 340 hook-matched files (69 lines), so the version itself is a small drift — but *either* version would rewrite all 340 files (64,000 lines), because `.clang-format` (LLVM base with `PointerAlignment: Left`, joined argument lines) and the code (LLVM's right-aligned `Type &x`, one argument per line) have never agreed. The hook therefore restyles any C/C++ file it touches into a style the tree does not use. That is a policy decision for the maintainer — adopt the configuration tree-wide in one dedicated commit, or change `.clang-format` to describe the prevailing style — and not a version move, so it is left as found |
