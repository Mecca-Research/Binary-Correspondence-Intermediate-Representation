...
| torch's CPU-wheel closure (`download.pytorch.org/whl/cpu`, the hosted jobs' actual install) | `ci.yml` hosted train-to-C jobs | — | **not run** here: the index is blocked on this host (§5 F12); audited in CI by `--installed`, whose first run on both runners found `setuptools` 78.1.0 — pulled from the PyTorch index by torch's `setuptools>=77.0.3` — with PYSEC-2025-49 (CVE-2025-47273) and PYSEC-2026-3447 (CVE-2026-59890) open; now pinned at 84.0.0 from PyPI ahead of torch, and the audit passes |
...
| `nodejs` (WASM tests) — *no longer installed after the follow-up PR (F6)* | nodejs | 18.19.1+dfsg-6ubuntu5 | 34 | 14 | 0 | **20** (high: CVE-2023-44487, CVE-2024-22017; 18 medium) |
...
(Refreshed to `05deced` and re-verified against LLVM 23.1.0 in the second follow-up PR — F11.)
...
| F6 | `nodejs` from apt is the 18 line: upstream EOL, 20 open advisories in noble (2 high) | medium (runner hygiene) | **Landed in the follow-up PR**: `nodejs` removed from every apt package list (only the oracle jobs execute `node`, for the WASM tests); those jobs run Node 24 (Active LTS) through `actions/setup-node` v7.0.0, SHA-pinned, with the package-manager cache off, and print `command -v node && node --version` as evidence. The other apt packages' open advisories (F7, F8) are unchanged |
...
| F9 | `mirrors-clang-format` v18.1.8 while the rail's toolchain is LLVM 23 | low (style drift) | **Revision moved to v23.1.0 in the first follow-up PR; the style policy decided by measurement and landed in the second.** Measured (clang-format 23.1.0, changed lines over the hook-matched files): 18.1.8 vs 23.1.0 disagree on 13 of 340 files (69 lines), so the version itself was a small drift — but `.clang-format` as it stood rewrote all 340 files (65,088 lines), and no LLVM-based configuration does better than 57,849, because the two trees the hook covered are not one style. `runtime/c` (301 files, 31k lines) is deliberately dense hand-formatted C (several statements per line, no space after `if`, `,` or operators): every configuration tried rewrites 58k+ of its lines, i.e. it is not a clang-format style at all. `mlir/lib` + `mlir/include` (39 files, 12k lines) and `runtime/cpp` (11 files, 1.8k lines) are LLVM style by intent (`Type *x` 2090:4 in the tree, 2-space indent, LLVM file banners); the configuration that describes them best is the existing one with LLVM's `PointerAlignment: Right` (the file said `Left`, which the code never used) and `ReflowComments: false` (hand-wrapped comment blocks stay): 2,480 + 641 + 8 residual lines, all hand-wrapping choices clang-format cannot be configured to reproduce. **Policy**: the hook covers the C++ rails only (`mlir/lib`, `mlir/include`, `mlir/tools`, `runtime/cpp`), those trees were reformatted once under the corrected `.clang-format` in a dedicated mechanical commit (validated by the LLVM 23 bcir-opt build, the pass/ODS/bytecode gates and the C++ hand-off gate), and `runtime/c` is out of the hook's scope by declaration in both files — a hook must not restyle a rail into a style it does not use; reformatting `runtime/c` would be its own decision and PR |
| F10 | `pre-commit-hooks` v5.0.0 → v6.0.0 available | low | **Landed in the follow-up PR**: rev v6.0.0 (changelog: Python ≥ 3.9; two hooks this repository never used removed; all eight used hooks present). Running the eight hooks over the whole tree found two pre-existing conflicts, both fixed in the hook configuration rather than in the files: `check-yaml` rejected `.clang-format`'s two-document stream (now `--allow-multiple-documents`), and `end-of-file-fixer` would rewrite the differential's `.mlir` corpora (emitted with a trailing blank line and drift-gated byte for byte in CI) and `cfront_sec_lextail.c` (intentionally unterminated, a lexer-tail sanitizer fixture) — both now excluded. Four files the hooks legitimately fixed (a stray trailing blank line in `bcir/bench.py`, missing final newlines in the two example channel manifests and the audit's own JSON evidence, whose tools now emit one) are included |
| F11 | Vendored `llvm-ir.tm` is six commits behind a dormant upstream; header verified against LLVM 18 | low (corpus currency) | **Landed in the second follow-up PR**: the snapshot is now `ll.tm` at upstream HEAD `05deced0a4bcf9699a2e4b04f2f1e36beff33d69` (byte-identical below the six-line provenance header; +80/−126 lines, all LLVM 15 syntax), the header stamps verification against `llvm-as-23` 23.1.0 on 2026-09-04, `NOTICE.md` and the grammar README carry the new commit, and `10-grammar/examples/llvm15-constructs.ll` exercises exactly the syntax the delta added (`allockind`/`allocptr`/`allocalign`, global sanitizer kinds, `presplitcoroutine`, `fn_ret_thunk_extern`, `nosanitize_bounds`, `atomicrmw fmin`/`fmax`, `DISubprogram.targetFuncName`) — it assembles and passes `opt -passes=verify` under LLVM 18.1.3 (the corpus runner) and 23.1.0, and the corpus manifest and example verifier (158 examples) hold it. The delta's removal of the `fadd`/`fsub`/`fmul`/`udiv`/`sdiv`/`fdiv`/`urem`/`srem`/`frem` constant expressions matches the toolchain: `llvm-as-23` refuses `fadd (float 1.0, float 2.0)` with "fadd constexprs are no longer supported". Upstream stopped at LLVM 15, so syntax added after 15 remains absent from the grammar; the README now says so |
| F12 | The hosted jobs' real torch closure (CPU wheels from `download.pytorch.org`) is not what PyPI resolves, and that index is unreachable here | low (coverage gap) | **Landed in the follow-up PR**: `audit_dependencies.py --installed` audits what the running interpreter has installed — every distribution by exact *public* pin (`2.13.0+cpu` is a build of the 2.13.0 release; the advisory database is keyed by release), no resolver, coverage reconciled against the installed set, the repository's own distribution excluded, `--expect torch --expect safetensors --expect numpy` refused if absent (the audit must be of the environment it claims), the engine required. Both hosted train-to-C jobs (Ubuntu and Windows) install `pip-audit==2.10.1` into a scratch venv of their own, hand it to the rail as `PIP_AUDIT`, and run the audit after installing the wheels and before the gates; `pip` is pinned in that environment too. On the analysis host the same mode immediately flagged a scratch venv's bundled `pip` 24.0 and `setuptools` 79.0.1 (six and one open advisories), which is exactly the class of finding environment mode exists for. **First CI run (both runners): FAIL, 4 findings in 1 distribution** — `setuptools` 78.1.0, which torch 2.13.0's `setuptools>=77.0.3` pulls from the PyTorch index (its mirror stops at 78.1.0), carrying PYSEC-2025-49 / CVE-2025-47273 (path traversal in `PackageIndex`, fixed 78.1.1) and PYSEC-2026-3447 / CVE-2026-59890 (fixed 83.0.0), each reported under two records. Reproduced on the analysis host over the runner's exact 13-distribution closure (4 findings in 1 package), and clean with `setuptools==84.0.0` (released 2026-08-08; no `pkg_resources`, which nothing in this tree imports). Fix: `setuptools==84.0.0` is installed from PyPI together with `pip`, before torch, so the PyTorch index never resolves it; `test_ci_owns_the_installed_audit` holds that order. The rail's console line now names each vulnerable distribution with its advisory ids and fix versions, bounded (`_findings_summary`, L7), so the next job log says what to bump without the JSON report |
