...
| torch's CPU-wheel closure (`download.pytorch.org/whl/cpu`, the hosted jobs' actual install) | `ci.yml` hosted train-to-C jobs | — | **not run** here: the index is blocked on this host (§5 F12); audited in CI by `--installed` from the follow-up PR |
