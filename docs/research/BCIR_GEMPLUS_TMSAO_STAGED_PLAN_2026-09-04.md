...
| Control | lease, generation, quiesce, activate, rollback, cancel | `ControlRecordV1` (G14, S3-A): bounded, versioned, MACed records decided by a resident plane on both rails; carried by a BACKPRESSURE control ring (G15, S3-B) | generation gating at the C++ `admit()` seam (G16) |
