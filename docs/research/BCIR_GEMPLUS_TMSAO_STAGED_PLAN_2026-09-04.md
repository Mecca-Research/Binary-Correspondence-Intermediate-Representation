...
| Control | lease, generation, quiesce, activate, rollback, cancel | `ControlRecordV1` (G14, S3-A): bounded, versioned, MACed records decided by a resident plane on both rails | carried by the SPSC ring (G15); generation gating at the C++ `admit()` seam (G16) |
...
| L4 | process ↔ process | SPSC shared rings for telemetry and control records | the records: G14 (landed, S3-A); the ring: G15 |
