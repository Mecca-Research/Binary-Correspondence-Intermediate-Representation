...
| P2 bounded TMSAO | Stage 2: G2 **landed (S2-A)**, G4, G12, G6, G13 |
...
| **2 — best-fit solver portfolio** | G2 **(landed, S2-A)**, G4, G12, G6, G13 | first TMSAO-2 certificate; `solver.exact.coverage` and `solver.gap.p95` reported over the corpus; identical assignment under delta pricing **(met, S2-A: the delta search returns the reference sweep's assignment claim by claim on 1,344 cases; a trial re-places the affected phase from a checkpoint and skips every later phase that touches nothing the replay moved)**; every region expands conservatively (differential per region); `native.*` no regression |
