...
| **4 — performance program** | G17 **(landed, S4-A)**, G18 | rows moved with the mechanism named and the plan unchanged (`planner.parity`, `verify.delta.identity` exact) **(`planner.parity` met, S4-A: 8,430 → 0, the plan unchanged byte for byte on both rails)** |
...
| **S4-B** | G18 incremental re-verification and delta StreamPack | S4-A, S2-A |
