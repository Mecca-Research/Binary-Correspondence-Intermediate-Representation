...
|---|---|---|---|
| 8 | MLIR module scope violated (root-global maps in `-bcir-verify`, selection and GEM passes) | **open** | `BCIRVerifyPass.cpp` is an `OperationPass<>` and builds its maps with `root->walk` from whatever root it is run on. S0-5 anchors the walks at `bcir.module` |
| 9 | Phase identity and ordering inconsistent across rails (dangling/duplicate phase ids; five different orders in use) | **open** | S0-6, the shared structural-law corpus run against both rails, includes phase identity and ordering |
| 10 | Advertised verifier checkpoints missing from `bcir-optimize` and `bcir-hydrate` | **open** | `mlir/lib/BCIRPasses.cpp`: `bcir-audit`, `bcir-lower-llvm` and `bcir-aot` add `createVerifyPass()`; `bcir-optimize` and `bcir-hydrate` do not. S0-3 |
...
| # | Finding | Status | Evidence / owner |
|---|---|---|---|
| 18 | IRDL coverage incomplete with no ODS→IRDL inventory gate | **open** | `tools/irdl/check_corpus.sh` round-trips the corpus only. S0-9 adds the supported-subset manifest and the parity check |
| 19 | Address width target-insensitive (i32 accepted, later `inttoptr`) | **not re-verified** | S0-6 |
| 20 | MAP/ROP mishandle non-RAM resources (default domain RAM) | **not re-verified** | S0-6 |
| 21 | M5 descriptors need construction-time validation | **not re-verified** | S0-6 |
...
| **S0-E** (ABI) | S0-2 R11 per-resource generation vectors as a StreamPack v4 append-only record, its C twin and ASN.1 projection, landed with `BCIR_STREAMPACK_ABI.md` | — |
