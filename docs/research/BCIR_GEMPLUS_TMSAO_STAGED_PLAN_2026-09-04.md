...
| 10 | Advertised verifier checkpoints missing from `bcir-optimize` and `bcir-hydrate` | **closed (S0-B)** | `bcir-optimize` is verifier-checkpointed on entry and on the plan it emits; `bcir-hydrate` on entry (`BCIRPasses.cpp`). `pipeline_checkpoints.mlir` runs an illegal module under both and expects the same R2 at the door; both pipelines ran it to completion before |
