...
| 6 | The decoupled GGG tail can overlap conflicting or barriered work (cross-stream RAW/WAR/WAW and fence edges not built before the split) | **closed (S1-A)** | the defect stood on both rails: a gather reading what a wave claim wrote started at the phase start, before its producer finished, and a data-independent claim overlapped a `barriered` fence (`schedule_eft`, `execute_tokens`, `-bcir-schedule-eft`, `-bcir-async`, and both pricers overlapped the tail with its own producer). `concurrency.hazard_predecessors` is the ONE hazard DAG of the GEM rails — RAW/WAR/WAW plus the fence edges (`is_fence`: `barriered` or `volatile`; a fence waits for everything since the previous fence and holds everything after it) — built over every claim of a phase BEFORE the stream split, and `schedule.py` dispatches the sparse tail on its own stream inside the same event loop (`BCIRSchedule.h` is the twin). Witnesses: `test_schedule.py` (a dependent tail waits in both directions, a fence is overlapped by nothing, tokens honor a fence across phases), `schedule_hazards.mlir` (the gather starts at its producer's finish on stream −1, `async_awaits = [1, 2]` on the fence, gain 0) |
| 7 | CSE can merge semantically different or effectful claims (signature = op + read versions; precedes the barrier guard) | **closed (S1-A)** | the defect stood on both rails: a duplicate over a different count, offset, immediate or dynamic bound, and an atomic, barriered or volatile duplicate, all took the copy credit. `realize.cse_identity` carries the complete value identity (op + opcode, lane, stride class/multiplier, offset, count, domain, dynamic bound, operand versions) and `realize.cse_eligible` excludes categorically — atomic, barriered, volatile, effect opcodes, indirect calls, lifetime/timing metadata, sparse accesses, isolated domains, non-default numeric contracts, and the oracle-only fields the law rail cannot see — with an ineligible claim never seeding a match; `BCIRCostModel.h` `cseEligible`/`cseIdentity` mirror both byte-for-byte. Witnesses: `test_fusion.py` (six identity negatives, twelve exclusion negatives, the ineligible seed, the exact duplicate still credited), `cost_model_cse_neg.mlir` |
...
| 16 | Telemetry replay evidence is not transport replay evidence (static ids as sequencing; no integrity witness retained) | **partly** | The BTLM v1 frame ABI carries `seq`, per-frame CRC and resync, byte-identical on both rails (`docs/kernel/TELEMETRY_FRAME_ABI.md`); since S3-B the *live* ring carries a per-slot sequence with exact loss accounting (`docs/kernel/BCIR_LIVE_RING_ABI.md`), each `TelemetryEnvelopeV0` its source, session, sequence and the producer's own loss count, and the intake's report is the transport witness. Open: replay does not yet consume that witness |
...
| P0 scope and evidence schemas | G0 **landed** (scope identity, class ladder, gap arithmetic); the frozen baseline harness exists; the plan's *binary* form is G11 **(landed, S1-C)** |
| P1 canonical GEM+ plan | Stage 1 **complete**: G1 **landed (S1-A)**, G3 **landed (S1-B)**, G11 **landed (S1-C)**, G5 **landed (S1-D)** |
| P2 bounded TMSAO | Stage 2: G2 **landed (S2-A)**, G4 **landed (S2-B — the first TMSAO-2 and TMSAO-1 certificates)**, G12 **landed (S2-C)**, G6 **landed (S2-D, affine first)**, G13 |
...
2. **A binary plan ABI.** The canonical plan cannot be Python objects if the C twin, the MLIR
   ...
   reader prices. (G11 — **landed as S1-C**)
...
|---|---|---|---|
| Data | StreamPack v1 (+v2/v3/v4), BCAB bundles, tensors | frozen ABI, C twin, byte-identity gates; `ExecutionPlanV1` records (G11); the pack table (G16, S3-C): artifacts written once and read in place through borrowed views with explicit lifetime, admitted by the live plane, dispatched as a plane phase; the per-step freeze; the BSHM v0 manifest-of-shards | a pack table shared across processes; a cross-node transport |
| Control | lease, generation, quiesce, activate, rollback, cancel | `ControlRecordV1` (G14, S3-A): bounded, versioned, MACed records decided by a resident plane on both rails; carried by a BACKPRESSURE control ring (G15, S3-B); generation gating at the C++ `admit()` seam and at the manifest gate (G16, S3-C) | transports beyond one host |
| Telemetry | frames and readings | BTLM v1 frame ABI (byte-identical), `TelemetryIntegrity`, signal registry T1; the version-zero triple (G15, S3-B): the generated signal table, `TelemetryEnvelopeV0` with its host intake, the live SPSC ring with exact loss accounting | transports (UART, OTLP, Redfish); the v1 freeze after the UART and virtio-blk traces |
...
| L2 | C++ orchestrator ↔ C runtime | borrowed views with explicit lifetime; generation gating at `admit()`; transactional builders | G16 (landed, S3-C: `PackArena`/`PackOwner`/`PackView`/`Borrow` over the C pack table; a freeze straight into a slot; dispatch in place — `handoff.copies` 0) |
| L3 | host ↔ device | `bcir_channel_open/claim/map/submit/sync/next_event/close` (`runtime/c/bcir_runtime_channel.h`), loopback and simulator; HAM movement edges; G8's explicit `mem.move.*` claims and v3 plan edges | verbs exist; movement as a first-class edge **landed (G8, S5-C)**: the plan carries every move, a real channel executing them is Stage 6 |
| L4 | process ↔ process | SPSC shared rings for telemetry and control records | the records: G14 (landed, S3-A); the ring: G15 (landed, S3-B — two processes over a shared mapping, a peer SIGKILLed and taken over) |
| L5 | node ↔ node | the MPI/NCCL orchestrator, `shard()` real, dispatch a declared stub | stays a stub until a cluster exists; the manifest-of-shards format (G16, landed S3-C: every shard a pack a rank admits and runs by itself, the set reassembling to the whole by digest) is the honest preparation |
...
|---|---|---|
| Delta pricing replaces makespan recomputation (identical assignment) | `optimize_scheduled.slowdown.512` 65.5× → ≤ 8× **(met: 3.5×, S2-A)**; `optimize_scheduled.{256,512}` **(23 / 60 ms)**; the general case `optimize_scheduled.general.slowdown.512` 44.5× → **4.7×**, `sweep.replacement.fraction` 1.0 → **0.0625** | G2 |
| One digest, identity-bound, mutation-invalidated | `static_memory.{digest,verify,plan}.2048` | G3 |
| Compact indexed planner (arrays, not per-claim dataclasses), byte-identical plans | `audit.kbcir-streampack.scale4`; a new `planner.calls` exact row **(met, S4-A: 3,419,172 → 589,858 at scale 8, 5.80×; `planner.parity` 8,430 → 0)** | G17 |
| Incremental re-plan + delta StreamPack | `kbcir-streampack.delta` (new, `ratio`) **(met, S4-B: ~0.0098 at scale 4, ~0.007 at scale 8; a one-claim delta 491 calls against 10,855,666 from scratch; `pack.delta.identity` 2,064 → 0)** | G18 |
| Native planner after parity | `wall` rows, indicative; `exact` parity row **(met, S4-A: `runtime/c/bcir_kplan.c` byte-identical over 3,290 cases; `planner.native.scale4` ~3.4 ms against ~33 ms compact)** | G17 (CXX2/3) |
| Structural wins preserved through the regions refactor | `native.*` guardrails **(held, S2-D: A/B on this host 4.53 → 4.44×, 15.00 → 15.56×, 1.315 → 1.314×, 1.003 → 1.004×; the region layer is read-only over the claim DAG)** | G6 |
...
| **1 — one canonical plan and its ABI** — **complete** | G1 **(landed, S1-A)**, G3 **(landed, S1-B)**, G11 **(landed, S1-C)**, G5 **(landed, S1-D)** | `pricing.eft.divergence` = 1.0 **(met: 1.0 exactly; the retired pricer still reproduces 1.9922 as the witness)**; pricing, token execution, static memory and StreamPack lowering read one artifact and produce identical traces **(met — `schedule_plan` is the artifact and `ExecutionPlanV1` its bytes: the pricer, both executors and the pack hydrated from the plan's bytes reproduce their traces on every corpus program in both placements, and the plan carries the static-memory planner's lifetimes exactly; since S1-D the lifetimes are the placement's own intervals under schedule liveness)**; `plan.abi.roundtrip` byte-identical Python ↔ C **(met: 26/26 corpus plans; `plan.abi.mismatches` 26 → 0)**; the two-phase alias fixture rejected **(met, S1-D: refused against the token placement, disjoint storage when planned from it; `memory.*` at their bounds on the proof rail)** |
| **2 — best-fit solver portfolio** | G2 **(landed, S2-A)**, G4 **(landed, S2-B)**, G12 **(landed, S2-C)**, G6 **(landed, S2-D)**, G13 **(landed, S2-E — Stage 2 complete)** | first TMSAO-2 certificate **(met, S2-B: `gem.exact.certify_schedule` — L, U, both gaps, stop reason, budget and the bound stack bound to the scope digest; TMSAO-1 when the search closes)**; `solver.exact.coverage` and `solver.gap.p95` reported over the corpus **(met: `solver.unproved.fraction` 0 and `solver.gap.p95` 0 over the 3,432-instance pooled corpus)**; identical assignment under delta pricing **(met, S2-A: the delta search returns the reference sweep's assignment claim by claim on 1,344 cases; a trial re-places the affected phase from a checkpoint and skips every later phase that touches nothing the replay moved)**; every region expands conservatively **(met, S2-D: `regions.unexpanded.claims` 542 → 0, the expansion the identity on the carrier)**; `native.*` no regression **(met, S2-D: A/B unchanged on this host)**; `W` in every certificate's scope and the corpus-bound replay gate **(met, S2-E: `scope.workload.collisions` 36 → 0, `replay.subset.admitted` 24 → 0, `dispatch.measured.unavailable` 12 → 0)** |
| **3 — IPC at every level** — **complete** | G14 **(landed, S3-A)**, G15 **(landed, S3-B)**, G16 **(landed, S3-C — Stage 3 complete)** | one artifact generation flows plan → control → data → telemetry → evidence on the loopback/simulator with generation handles end to end **(met, S3-C: the Stage 3 exit flow — `runtime/c/test_stage3.h` on the C twin and the C++ seam, `run_stage3_python` on the oracle, 31 identical lines; the sample's generation is the one the data plane admitted)**; stale generations refused at every boundary **(met: the plane's compare-and-swap, the plan, the pack's dispatch and admission, the manifest and the intake each refuse the old generation with their own status — `handoff.stage3.stale.accepted` 18 → 0)**; `ring.loss.accounting` exact **(met: `ring.loss.unaccounted` 140 → 0, and in the flow the telemetry ring's loss equals the intake's gap count)**; `control.record.bytes` bounded **(met, S3-A/S3-B: a 192-byte declared record bound that every CONTROL ring slot carries)** |
| **4 — performance program** — **complete** | G17 **(landed, S4-A)**, G18 **(landed, S4-B — Stage 4 complete)** | rows moved with the mechanism named and the plan unchanged (`planner.parity`, `verify.delta.identity` exact) **(met: `planner.parity` 8,430 → 0, S4-A, the plan unchanged byte for byte on both rails; `verify.delta.identity` 4,128 → 0 and `planner.delta.parity` 2,064 → 0, S4-B, the incremental chain equal to the chain from scratch over 258 cases × 8 steps and a forged plan and pack on every step)** |
| **5 — movement, alias, escape** | G8 **(landed, S5-C)**, G9 remainder **(landed, S5-A)**, G10 **(landed, S5-B)** | joint movement/compute pricing **(met, S5-C: `movement.implicit_cross_tier` 9 → 0; `movement.excess` 597,840 → 0 ticks, every fixture at the exact optimum of the joint objective, TMSAO-1; MV1–MV11 each refusing exactly its own variant, `movement.laws.misattributed` 30 → 0; `movement.plans.unlawful` 9 → 0; ExecutionPlan v3 on both rails, `movement.parity.mismatch` 11 → 0, `movement.asn1.accepted` 18 → 0)**; alias scopes, TBAA and `volatile` to LLVM **(met, S5-A: `alias.scope.mismatch` 2,646 → 0, `alias.tbaa.mismatch` 2,646 → 0, `alias.volatile.mismatch` 1,533 → 0, `alias.differential.silent` 1,072 → 0, LLVM's own analysis proving every declared-disjoint pair from the scopes alone, `native.*` sources byte-identical)**; non-escaping resources counted **(met, S5-B: all 39 candidates proved nonescaping, `escape.unproved` 39 → 0; 7 of 22 indirect sites narrowed, `icall.unknown` 22 → 15 at the open world's floor; 4 resolved, `icall.unresolved` 22 → 18; `effects.commute.unsound` 119 → 0)** |
...
| **S1-C** — **landed** | G11 `ExecutionPlanV1`, the plan as bytes: the record family (one step per claim with the realization and the canonical placement, the static-memory lifetimes, the G8 movement-edge family, the generation vector) under a frozen v1 wire format, the freestanding C decoder/verifier (`bcir_ep_verify`, `bcir_ep_check_generation_vector`, `bcir_ep_check_pack`), `verify_execution_plan` on the oracle, BCAB kind 25, the `BCIR-ExecutionPlan` ASN.1 module; the four exact gates at their bounds (`plan.abi.mismatches` 26 → 0, `plan.readers.disagreements` 27 → 0, `plan.stale.accepted` 6 → 0, `plan.malformed.accepted` 39 → 0) | S1-A, S0-E |
| **S1-D** — **landed** | G5 schedule-aware liveness and bounded exact memory: `static_memory.schedule_intervals` (half-open ticks from the canonical placement), plans that name their liveness domain and bind to the placement by digest, `verify_static_memory_plan(schedule=)` refusing a phase plan the placement does not refine, the bounded exact solver `exact_layout` behind first-fit with the concurrent-live lower bound, the gap and the stop reason recorded and recomputed by the verifier, ExecutionPlanV1 v2 (liveness byte + lifetime ticks, the alias law by bytes on both rails), the section 6.4 corpus and witness in the tree; the three `memory.*` rows at their bounds (0% / 1.0 / 832 B) | S1-A, S1-C |
| **S2-A** — **landed** | G2 incremental delta pricing: `gem.schedule.EftPlacer` (the base placement recorded once per phase, a trial replayed from the checkpoint before the first pop it can move, later phases skipped unless they touch a moved rid), the dispatch as one predicate (`_PhaseDispatch.run`), the sweep reading the candidate map `optimize` built and the artifact the placer holds; the general-case fixture (one step-shortening trial per pair) and the adoption fixture in the tree; `optimize_scheduled.slowdown.512` 6.4× → 3.5×, the general case 44.5× → 4.7×, `sweep.replacement.fraction` 1.0 → 0.0625; identical assignment on 1,344 cases | S1-A |
| **S2-B** — **landed** | G4 bounded exact solvers and the lower-bound stack: `gem.exact.exact_schedule` (branch-and-bound over active schedules under the artifact's eligibility rules, the heuristic as the seed incumbent, symmetry breaking, an expansion budget, a valid `L` on a budget stop), the named bound stack, `exact_selection` for the section 6.3 sweep, `certify_schedule` (the first TMSAO-2 / TMSAO-1 certificates over `ExecutionScopeV1`); the section 6.1 corpus pinned and reproduced exactly, the solver held to code-free oracles; the four G4 rows and the Stage 2 exit rows at their bounds | S1-C |
| **S2-C** — **landed** | G12: `gem.dispatch` (the law as a table over region kind, size, requested class and budget; one runner per kind; `DispatchRecord` in every certificate), work-unit budgets everywhere (the exact scheduler's now the module's, consumed phase by phase), content-addressed resumable states for the schedule, memory and selection solvers (run(b₁) + resume(b₂) = run(b₁+b₂)), an incumbent at every interruption point (the exact layout accepts a zero budget through the runner, the enumeration is seeded with the sweep), the ranker held to a permutation of the census, `gem.diff.plan_diff`; `search.resume.unavailable` 393 → 0, `dispatch.incumbent.missing` 26 → 0, `dispatch.unrecorded` 12 → 0 | S2-B |
| **S2-D** — **landed** | G6: `kbcir.regions` (affine and opaque regions, each with a verifier, a conservative expansion that is the identity on the carrier, a cost floor and named refusals; `region_graph` partitions every phase), `kbcir.objectives` (the typed registry — six objectives admitted only with their laws proved, a declared overflow policy, one layered relaxation reproducing the planner's min-plus path and the scheduler's max-plus critical path), `gem.exact.certify_selection` and the dispatch law's `path` kind; `regions.unexpanded.claims` 542 → 0, `objectives.unverified` 2 → 0, the native guardrails unchanged A/B on this host | S2-C |
| **S2-E** — **landed** | G13: `kbcir.workload.Workload` (the declared `W`: shapes, batch, concurrency, service level, horizon, the expected counts of dynamic claims; validated, held to the module, digested with the scope and carried by every certifier), `kbcir.measured` (the append-only, content-addressed measured-candidate corpus with raw samples and the host attestation; `measure_plans`, B1's artifacts adapted, `replay_measured` over every logged episode), the portfolio's (runtime, workload) table and the corpus-bound `ReplayCertificate`, the dispatch law's `measured` rail with `solve_measured` re-deriving the plan from the planner, `certify_measured`, the ladder's `MEASURED_COMPONENTS` and two-target rule; `scope.workload.collisions` 36 → 0, `replay.subset.admitted` 24 → 0, `dispatch.measured.unavailable` 12 → 0; every measured certificate on this host TMSAO-4 by the rule. Stage 2 complete | S2-C |
| **S3-A** — **landed** | G14 control-plane record ABI: `ControlRecordV1` (lease, generation, quiesce, activate, rollback, cancel as fixed 64-byte-header records with an `expect` compare-and-swap witness, a lease-scoped HMAC-SHA256 and a CRC; eleven wire laws in one order on both rails), the resident `ControlPlane` / `bcir_ctl_state` deciding every record applied, deferred or refused by its bytes (one pending switch applied exactly once at a boundary, an eight-lease table, packs and plans admitted by the installed registry), the shared `is_stale` predicate, `verify_control_record` (R11), the `BCIR-ControlPlane` ASN.1 module, a sealing libFuzzer target; `control.abi.mismatches` 29 → 0, `control.malformed.accepted` 82 → 0, `control.stale.accepted` 25 → 0, `control.deferred.lost` 16 → 0, `control.decisions.nonconforming` 64 → 0, `control.traces.divergent` 52 → 0 | S1-C |
| **S3-B** — **landed** | G15 the live SPSC ring and the version-zero triple: `TelemetryEnvelopeV0` (source, session, generation, signal, sequence, the producer's loss count and a clock with its unit; wire laws in one order on both rails, one spelling per record), the generated fixed-width signal table with the ID-range policy and the unknown-required-signal law, the host intake (a bounded stream table, `SequenceTracker` — the frame ABI's continuity predicate, now one — classifying before any refusal, stale generations refused by `is_stale`), and the ring (`gem.ring` ↔ `bcir_ring.{h,c}`: seven one-writer lines, a per-slot seqlock under acquire/release publication, BACKPRESSURE or OVERWRITE with exact loss accounting, double-buffered accounting, epochs with takeover, CONTROL rings backpressure with slots that carry the control bound); the lease-key cache; `ring.abi.mismatches` 29 → 0, `ring.malformed.accepted` 112 → 0, `ring.loss.unaccounted` 140 → 0, `ring.torn.delivered` 140 → 0, `ring.sequence.misreported` 22 → 0, `ring.stale.accepted` 10 → 0, `ring.traces.divergent` 70 → 0, `ring.control.divergent` 104 → 0, `ring.concurrent.violations` 6 → 0; `ring.throughput` 26× against `memcpy` (INDICATIVE) | S3-A |
| **S3-C** — **landed** | G16 data-plane hand-off: the pack table (`gem.handoff` ↔ `bcir_handoff.{h,c}`: write-once slots, epoch-bearing handles, pins and retirement, admission recording generation and registry, dispatch as a plane phase, the state digest), the per-step freeze (`bcir_hydrate_generations` ↔ `freeze_claims`), BSHM v0 (`abi.shard_manifest` ↔ `bcir_shard_manifest.{h,c}`), the C++ RAII seam (`PackArena`/`Reservation`/`PackOwner`/`PackView`/`Borrow`, a non-virtual `admit`, the dynamic-graph backend real, the distributed `cut()` real) and the Stage 3 exit flow; `handoff.stale.admitted` 42 → 0, `handoff.stale.dispatched` 90 → 0, `handoff.fresh.refused` 90 → 0, `handoff.lifetime.unrefused` 68 → 0, `handoff.copies` 80 → 0, `handoff.builder.violations` 422 → 0, `handoff.decisions.nonconforming` 349 → 0, `handoff.shards.mismatches` 392 → 0, `handoff.shards.malformed.accepted` 68 → 0, `handoff.reentry.divergent` 115 → 0, `handoff.traces.divergent` 77 → 0, `handoff.stage3.stale.accepted` 18 → 0, `handoff.stage3.flow.divergent` 75 → 0; `handoff.dispatch.overhead` 1.95× → ~1.01× | S3-A |
| **S4-A** — **landed** | G17: the compact offer (`realize.fused_offer` — one enumeration, one arithmetic, the discounts in the same pass, no candidate objects), the indexed min-plus over the cheapest narrow and wide predecessor per column, `result.cand_map` a lazy view, the pre-G17 planner kept verbatim as `realize_reference`; R9's single-candidate re-derivation and the phase-binding law it lacked; the native planner `bcir_kplan.{h,c}` over the version-zero BKPI/BKPR records (`BCIR_ERR_PLANNER` 25), exact in 128 bits; `planner.parity` 8,430 → 0, `planner.malformed.accepted` 148 → 0, `planner.r9.misjudged` 232 → 0, `planner.calls` 3,419,172 → 589,858, `verify.plan.scope.overhead` 1.07 → ~0.73 | S2-A, S1-B |
| **S4-B** — **landed** | G18: the declared `Delta` (replacements only; `DeltaError` before anything moves), `IncrementalOffer` (the dependency cone, one discount table) and `IncrementalPlan` (the one relaxation, a cutoff on the next column's input, lazy shifts, the path spliced), the delta StreamPack (`PackState`: per-record encodings, the encoder's own contract and writers, a refusal re-emitted in full), the incremental verdict (`VerifyState`: the verifiers as units, change found by identity, an offer of its own), `DeltaChain`; `planner.delta.parity` 2,064 → 0, `pack.delta.identity` 2,064 → 0, `verify.delta.identity` 4,128 → 0, `delta.malformed.accepted` 51 → 0, `kbcir-streampack.delta.calls` 10,855,666 → 491 | S4-A, S2-A |
| **S5-A** — **landed** | G9 remainder: one derivation of the declared alias facts (`lower.alias_facts`) for every emitter of the elementwise claim — the LLVM kernel's `noalias`, alias scopes from the RID partition, clang's TBAA tag for the element type, `volatile`, a barrier's fences; the C kernel, gather form, specialist, ABI header and Q-fixed kernel's `restrict`, `volatile` and fences — R12 holding each on both backends, the subset refusing atomic and unknown hazards and undeclared or mis-sized operands, every self-check binding one buffer per resource; `alias.noalias.mismatch` 648 → 0, `alias.refusal.accepted` 602 → 0, `alias.r12.rejected` 336 → 0, `alias.r12.forgery.accepted` 1,084 → 0, `alias.harness.unaliased` 75 → 0, `alias.backends.disagree` 132 → 0, `alias.exec.failed` 9 → 0, `alias.llvm.caller_memory` 56 → 0 (the kernel inlined into a C caller: one dead store and one reload of the caller's own data removed per call site, the barrier's fences keeping both) | S2-D |
| **S5-B** — **landed** | G10: one points-to analysis over the unit (`frontends.cfront.escape` ↔ the twin's `esc_*`, `bcir-cc --emit-effects` / `--emit-escape`), Andersen's, open world, monotone and so order-free — escape verdicts for named locals (nonescaping / lent / escaping), indirect sites narrowed to their target sets and added to the call graph, and the footprint behind `CompileResult.commute` rebuilt on it (every store a write; writes through pointers, to statics and to the heap; a device access by its base resource; a call the twin cannot hold refused on both rails); judged by generated units with verdicts and targets known by construction, a dynamic commute witness, and 386 forms of every declaration kind against every access form; `escape.unproved` 39 → 0 (at its floor), `icall.unknown` 22 → 15 (at the open world's floor), `icall.unresolved` 22 → 18 (floor 16), `escape.verdict.mismatch` 73 → 0, `icall.target.mismatch` 20 → 0, `effects.commute.unsound` 119 → 0, `effects.parity.mismatch` 24 → 0, `escape.parity.mismatch` 200 → 0 | S2-D |
| **S5-C** — **landed** | G8 data movement as a first-class transformation: the movement spec and Semantic Swap classes, the module transform M → M′ (explicit `mem.move.*` claims over per-(resource, bank, episode) copies, inbound/outbound/final phases), the joint planner (exact within budget: TMSAO-1; greedy: TMSAO-4), MV1–MV11, ExecutionPlan v3 (move tail + source/spec binding) on both rails, BCAB, the control plane and the ASN.1 projection; RED on 2221db5e | S2-D |
