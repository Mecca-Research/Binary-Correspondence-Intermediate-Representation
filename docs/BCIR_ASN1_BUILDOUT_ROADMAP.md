...
**§21.7.6 and §21.7.7's continuation flag is built, and clause 21.7 is complete** — all eight
repetition-space determinations encode.

These two outlived the other six for a reason worth recording: **every other determination
writes its determinant outside the repeated elements**, where `BitWriter`'s name-keyed patch is
enough. §21.7.6's is "a REFERENCE to a field **that is part of the repeated element**", so N
elements carry N fields of one name and a name-keyed patch cannot say which it means. What was
missing was not more machinery but the right machinery: each element now gets its own reference
scope — §9.24.2's, reused rather than a second one invented for one clause.

The polarity is the clause's whole content and is asserted on octets. The boolean "is false if
the element is the last in the repetition, and is true otherwise", so the flag says *continues*
and the last element carries zero; an implementation that inverted it would still round-trip
against itself.

§21.7.7 splits from §21.7.6 exactly as §21.3.5 splits from §21.3.4 — the value comes "from the
abstract syntax ... or ... some other encoder actions", so the encoder **checks** rather than
writes, per "a conforming encoder shall not produce encodings in which the decoder's transforms
of this field do not correctly identify the last element". `record` returns early for both:
the end is announced inside the elements, so no determinant outside them carries a count.

**One rule came for free, from machinery written before the clause was reachable.** §21.7.6
...
>
> **The gate is answered.** The directed table had **one row** while `bcir_oer.c` was the only
> plan-driven decoder in the C rail, which made "selects OER" true and *vacuous* — a selection
> over one candidate is not a selection. [`bcir_per_plan.c`](../runtime/c/bcir_per_plan.c)
> closes it: X.691 §7.2 bars only the schema-*free* decode, so the missing rows were a gap in
> this repository rather than a prohibition in the standard, and closing it needed no hardware.
>
> The table now carries **three** rows and the winner is not the one the single-row version
> asserted:
>
> | candidate | encode (ns) | decode (ns) |
> |---|---|---|
> | CANONICAL-PER-UNALIGNED | 58 | 25 |
> | CANONICAL-PER-ALIGNED | 55 | **24** |
> | COER | **41** | 26 |
>
> Which is the useful result: OER is the cheaper *encode* and PER the cheaper *directed decode*
> on this schema, so the two axes disagree and a table carrying only one of them would have
> answered the wrong question confidently. The two PER variants are timed separately because
> `bcir_per_align` is the only difference at a field boundary — and it is at *every* boundary,
> so a rail that timed one and reported it for both would report a number it never measured.
>
> The `#per` gate is a real differential rather than a self-consistency check: its corpus is
> generated by BCIR's **own** PER encoder and read back by the C twin across both variants, so
> a disagreement about a bit means the decoder is wrong about the rule the encoder implements.
> It also refuses a vacuous pass — a corpus that decoded nothing would compare `-O0` against
> `-O3` equal too, so the gate counts the decoded lines and fails below a floor. That check
> earned itself on its first two runs, both of which surfaced a broken corpus generator as
> "only 0 of 0 PER cases decoded" rather than as a green gate over an empty file.
>
