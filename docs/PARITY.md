...
| real-signal probes | `bcir.silicon` (`cache_topology`/`tier_capacities`, `cpufreq_info`, `CounterSampler`, `perf_counters_available`, `summary`) | *(host capability)* read-only `/sys` cache + cpufreq + `getrusage`; honest about what the host exposes (no PMU / no cpufreq actuation ⇒ reported, not faked) |
