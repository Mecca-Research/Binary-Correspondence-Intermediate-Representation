...
| scheduled price M(π,Θ) | `gem.overlap.price_scheduled` / `optimize_scheduled` — the makespan of the ONE canonical schedule artifact `gem.schedule.schedule_plan` (G1 / S1-A): the plan's step costs placed by the hazard-honoring LPT/EFT dispatch the executors run, the hazard DAG (RAW/WAR/WAW + `barriered`/`volatile` fences, `concurrency.hazard_predecessors`) built over every claim before the tail split; `price_scheduled(...).schedule` IS the artifact. The retired wave pricer is `price_waves_legacy`, the §6.2 divergence witness (1.9922), read by nothing else | `bcir.kbcir.scheduled_price` (R9) + **`-bcir-overlap`** (`BCIROverlapPass.cpp` over `BCIRSchedule.h`, the artifact's twin): prices the placement `-bcir-schedule-eft` annotates (matmul 253952 / gain 761856, corpus; `overlap.mlir`; a dependent tail claim and a fence serialize — `schedule_hazards.mlir`; the gate checks the priced makespan equals the placed one on the corpus); **`-bcir-overlap-optimize`** ports `optimize_scheduled` — the makespan-driven re-selection sweep (only a step-shortening alternative is placed; adopt the one that strictly lowers the makespan, re-priced serially for R9); matches the oracle's `(makespan, serial)` on all 11 corpus programs (`overlap_optimize.mlir`) |
