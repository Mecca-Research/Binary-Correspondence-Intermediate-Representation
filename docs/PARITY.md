...
| ASN.1 ExecutionPlan projection | `asn1.execution_plan` (the `BCIR-ExecutionPlan` module, OID `{ 1 3 6 1 4 1 62596 3 }`; DER, OER and JER over one type model; additive, native octets frozen) | *(interop rail)* A1 faithful / A2 canonical / A3 additive over every corpus plan in both placements in `test_asn1_execution_plan.py`; the compiled `.asn1` encodes byte-identically to the hand-built model; `docs/BCIR_ASN1_X690_ABI.md` §3b |
