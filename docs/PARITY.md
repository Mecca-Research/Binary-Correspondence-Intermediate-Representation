...
| the compact offer and the native planner (G17, S4-A) | `kbcir.realize.fused_offer` (the offer as compact rows: one enumeration `_offer_rows` over one arithmetic `_base_cost`, the CSE/deforestation discounts in the same pass, no candidate objects; `fused_candidates` and `result.cand_map` its views), `optimize` (the indexed min-plus over the cheapest narrow and wide predecessor per column, first on a tie), R9's single-candidate re-derivation (`verify_plan` prices only what the plan names, from the same offer, and binds each step to its claim's phase); `abi.planner_abi` (BKPI / BKPR, version zero: `docs/kernel/BCIR_PLANNER_ABI.md`); the pre-G17 planner kept verbatim as `kbcir.realize_reference`, read only by the parity gate | *(C rail)* `runtime/c/bcir_kplan.{h,c}`: `bcir_kp_decode_input` / `_scratch_size` / `_realization_size` / `_plan` / `_decode_realization`, freestanding, exact in 128 bits over the declared domain; the realization byte-identical to the compact planner's (or the same refusal) over 3,290 cases -- the fixed corpus under every target, Theta and policy plus the edge scopes, and 240 generated modules under every target -- and the compact planner byte-identical to the reference in its plan, its BKPR bytes and its ExecutionPlanV1 bytes; one refusal per law on both rails; -O0 == -O3 == the oracle (`bcir/tests/test_planner_compact.py`, `tools/c/check_runtime.sh`, the exact `planner.*` rows); `fuzz_kplan.c`; *(law rail)* `-bcir-plan` unchanged -- it reproduces the scores on its frozen corpus and does not read BKPI |
| the delta chain (G18, S4-B) | `kbcir.delta` (`Delta`, the declared replacements, refused with `DeltaError` before anything moves; `apply_delta`, the reference application; `IncrementalOffer`, the offer's dependency cone over the one discount table `realize._DISCOUNT`; `IncrementalPlan`, the one relaxation `realize._relax_column` with a cutoff, lazy shifts and the path spliced), `gem.delta_pack.PackState` (per-record encodings; the records `streampack.step_records` and `double_buffer` build; the encoder's own contract functions and writers), `verify.delta.VerifyState` (the three verifiers as units, change found by object identity, an offer of its own), `gem.delta_chain.DeltaChain` / `full_chain` (`docs/kernel/BCIR_DELTA_CHAIN.md`) | *(oracle)* every link equal to the chain from scratch on the declared module over 258 cases x (a build + 7 deltas) and a forged plan and pack on every step: `planner.delta.parity`, `pack.delta.identity`, `verify.delta.identity` and `delta.malformed.accepted` at 0 (`bcir/tests/test_delta_chain.py`, `tools/perf/check_delta.py`, `tools/testing/faults/delta.json`); *(C rail, law rail)* none -- the incremental chain is oracle-only (declared); what it produces is the full chain's plan, pack and verdict, so every twin of those (`bcir_kplan.c`, the StreamPack codecs, `-bcir-plan`, the verifier passes) still holds it |
...
| scheduled price M(π,Θ) | `gem.overlap.price_scheduled` / `optimize_scheduled` — the makespan of the ONE canonical schedule artifact `gem.schedule.schedule_plan` (G1 / S1-A): the plan's step costs placed by the hazard-honoring LPT/EFT dispatch the executors run, the hazard DAG (RAW/WAR/WAW + `barriered`/`volatile` fences, `concurrency.hazard_predecessors`) built over every claim before the tail split; `price_scheduled(...).schedule` IS the artifact. The retired wave pricer is `price_waves_legacy`, the §6.2 divergence witness (1.9922), read by nothing else. Since G2 (S2-A) the sweep is priced incrementally by `gem.schedule.EftPlacer` (the base placement recorded once per phase; a trial replays the changed phase from a checkpoint and skips later phases that touch nothing it moved) and `optimize_scheduled(delta=False)` is the full re-placement reference it is held to claim by claim | `bcir.kbcir.scheduled_price` (R9) + **`-bcir-overlap`** (`BCIROverlapPass.cpp` over `BCIRSchedule.h`, the artifact's twin): prices the placement `-bcir-schedule-eft` annotates (matmul 253952 / gain 761856, corpus; `overlap.mlir`; a dependent tail claim and a fence serialize — `schedule_hazards.mlir`; the gate checks the priced makespan equals the placed one on the corpus); **`-bcir-overlap-optimize`** ports `optimize_scheduled` — the makespan-driven re-selection sweep (only a step-shortening alternative is placed; adopt the one that strictly lowers the makespan, re-priced serially for R9); matches the oracle's `(makespan, serial)` on all 11 corpus programs (`overlap_optimize.mlir`); the port re-places per trial — parity holds the results, not the cost |
| the exact rail and the certificate (G4, S2-B) | `gem.exact`: `exact_schedule` (bounded branch-and-bound over active schedules under the artifact's eligibility rules; L, U, stop reason, expansions, the named bound stack), `exact_selection` (the section 6.3 sweep against the exhaustive enumeration), `certify_schedule` (TMSAO-1 / TMSAO-2 / TMSAO-4 over `ExecutionScopeV1`); the section 6.1 corpus and its code-free oracles in `bcir/tests/exact_fixtures.py` | *(oracle only)* the artifact is not moved, so the C and MLIR twins keep reproducing `schedule_eft`'s placement; a native solver may own a certificate only after G17's byte-for-byte parity over a generated corpus |
| the dispatch law, resumable search and the plan diff (G12, S2-C) | `gem.dispatch`: `dispatch` (the table), `solve_schedule` / `solve_memory` / `solve_selection`, `DispatchRecord` (in every certificate), `ranked` / `policy_ranking` (the census rule); content-addressed states `exact.SearchState`, `static_memory.LayoutSearchState`, `exact.SelectionSearchState` with `resume=`; `gem.diff.plan_diff` | *(oracle only)* no rail below the Python one runs a search; the states are JSON artifacts with SHA-256 digests, not yet a wire record (a control-plane record ABI is G14) |
| typed regions and the objective registry (G6, S2-D) | `kbcir.regions`: `region_graph` / `verify_region` / `expand` / `region_floor` / `affine_refusal` (affine and opaque kinds); `kbcir.objectives`: `registry` / `objective` / `verify_objective` / `dag_best_path` (min_plus, max_plus, min_max, boolean, lexicographic, pareto); `gem.exact.certify_selection` and the dispatch law's `path` kind | *(oracle only)* the law rail's `BCIR_Semiring` (`min_plus`, `max_plus`) is the shared subset of the registry's names; no region attribute on the law rail — a region is a read-only view over the claims every rail already verifies, and its expansion is the identity |
| the workload component `W`, the measured corpus and the measured rail (G13, S2-E) | `kbcir.workload` (`Workload` / `workload_for` / `verify_workload`), `kbcir.measured` (`MeasuredPlan` / `MeasuredCorpus` / `measure_plans` / `from_schedule_artifact` / `replay_measured`), `kbcir.portfolio.PolicyPortfolio.select(theta, workload)` and the corpus-bound `ReplayCertificate`, `gem.dispatch.solve_measured` (the `measured` rail), `gem.exact.certify_measured`, `kbcir.scope.MEASURED_COMPONENTS` and the two-target rule in `certificate_class_allowed` | *(oracle only)* `W` is a scope component on the Python rail, not a field of the cross-rail manifest `-bcir-verify` recomputes — carrying it there is an S0-D-pattern change on both rails; the corpus never changes what `-bcir-verify` accepts or `-bcir-select-realization` scores (the measured rail chooses among policies, each plan the planner's) |
...
| duration-aware schedule | `gem.schedule.schedule_eft` (LPT+EFT+locality+knee; the hazard DAG over main AND tail claims before the split — a dependent gather waits for its producer, a fence is overlapped by nothing) | `bcir.gem.schedule` mode `eft` (R9); **`-bcir-schedule-eft`** over `BCIRSchedule.h` (`schedule_eft.mlir`, `schedule_hazards.mlir`) |
| token-DAG execution | `gem.schedule.execute_tokens` (pipelined phases; awaits = `hazard_predecessors` over the module, fences included) | `bcir.gem.schedule` mode `tokens` (R9); **`-bcir-async`** (`async.mlir`, `schedule_hazards.mlir`: `async_awaits` carries the fence edges) |
| bandwidth knee | `gem.schedule.bandwidth_knee` / `TargetProfile.mem_channels` | `bcir.target.capability` `mem_channels` |
| pipelined/routed StreamPack (ABI v2/v3/v4) | `gem.streampack.hydrate_pipelined` / append-only codec; `generation_vector` (v4, R11 per resource) | `bcir.gem.stream_pack` `pipeline_depth`, `bcir.gem.prefetch` `buffers`, v3 segment dispatch/channel metadata (R10), and the v4 `generations` triples (`GEMStreamPackOp::verify` + `-bcir-verify` R11 per resource) |
| static memory — schedule-aware liveness and the bounded exact layout (G5, S1-D) | `kbcir.static_memory`: `schedule_intervals` (half-open ticks from a `GemSchedule` / `ExecutionPlan`), `plan_static_memory(schedule=, layout=, budget=)` (plans name their `liveness` domain and bind to the placement by `schedule_digest`), `first_fit_layout` / `exact_layout` (complete branch-and-bound over aligned offsets below the first-fit incumbent, work-unit budget) / `layout_lower_bound`, bank summaries with `lower_bound_bytes` / `gap_bytes` / `stop_reason`, `verify_static_memory_plan(schedule=)` (refuses a phase plan the placement does not refine; recomputes an exact layout under the plan's budget) | *(bytes)* ExecutionPlanV1 v2: header `liveness` byte + lifetime `first_tick`/`last_tick`, the alias law by bytes in `bcir_ep_verify` and the codec; `verify_execution_plan` refuses lifetimes that do not cover the schedule; *(law rail)* no static-memory op — the plan bytes are the reader every rail links |
| ControlRecordV1 — the control plane as bytes (G14, S3-A) | `gem.control` (`ControlRecord` and the six bodies, `ControlPlane` — `submit` / `enter` / `leave` / `advance` / `admit_pack` / `admit_plan` / `state_digest` — and the shared `is_stale`, `lease_key`, `registry_digest`, `rollback_token`), `abi.control_abi` (`issue_control` / `encode_control` / `decode_control` / `validate_control` / `check_control_mac`, frozen v1: `docs/kernel/BCIR_CONTROL_PLANE_ABI.md`), `verify.verify_control_record` (R11: a generation record held to the module's registry) | *(C rail)* `runtime/c/bcir_control_plane.{h,c}`: `bcir_ctl_decode` / `bcir_ctl_verify` (the eleven wire laws in the same order, `BCIR_ERR_CONTROL` / `BCIR_ERR_MAC`), `bcir_ctl_check_mac`, `bcir_ctl_lease_key`, `bcir_ctl_registry_digest`, `bcir_ctl_rollback_token` and the plane `bcir_ctl_init` / `_submit` / `_enter` / `_leave` / `_advance` / `_admit_pack` / `_admit_plan` / `_state_digest`, freestanding over the shared SHA-256/HMAC; byte-identical Python encode → C decode → Python re-encode on every corpus record, one status per malformed variant on both rails, and identical two-rail traces (verdict, refusal, status, resident state digest) over 52 scenarios (`test_control_plane.py`, `tools/c/check_runtime.sh`, the six `control.*` rows); `fuzz_control_plane.c` seals records under the plane's key and asserts the plane's invariants; *(bundle)* none — a control record is a decision with a lifetime, not an immutable artifact; *(law rail)* no control op — nothing in a record is a legality verdict |
| the live SPSC ring, version zero (G15, S3-B) | `gem.ring` (`RingGeometry`, `RingProducer` — `attach` / `open` / `fill` / `close` / `publish` / `beat` / `detach`, `RingConsumer` — `attach` / `open` / `copy` / `close` / `consume` / `beat` / `detach`, `format_ring`, `decode_geometry`, `committed_accounting`, `ring_state`): the algorithm over the region's bytes one operation at a time (`docs/kernel/BCIR_LIVE_RING_ABI.md`) | *(C rail)* `runtime/c/bcir_ring.{h,c}`: `bcir_ring_format` / `_decode_geometry` / `_accounting_of`, the producer (`_producer_attach`, `_write_open` / `_write_fill` / `_write_close`, `_publish`) and the consumer (`_consumer_attach`, `_read_open` / `_read_copy` / `_read_close`, `_consume`) over C11 atomics, freestanding; identical two-rail traces over 70 scripted scenarios — verdict, status, position, count, epoch, payload and the region's CRC after every operation — and -O0 == -O3 == the oracle (`test_live_ring.py`, `tools/c/check_runtime.sh`, the nine exact `ring.*` rows); the concurrent runs (threads; processes with a peer SIGKILLed and taken over) under ThreadSanitizer; `fuzz_ring.c` over hostile and honest regions; *(bundle, law rail)* none — a version-zero transport, not an artifact or a law |
| TelemetryEnvelopeV0, the generated signal table and the intake (G15, S3-B) | `abi.telemetry_envelope` (`encode_envelope` / `decode_envelope` / `validate_envelope`), `signal_table` (`encode_row`, `signal_range`, `admit_signal`, `emit_c_header` — the generator), `telemetry.SequenceTracker` (the one continuity predicate, also the BTLM decoder's), `telemetry_intake.TelemetryIntake` (`docs/kernel/TELEMETRY_ENVELOPE_ABI.md`) | *(C rail)* `runtime/c/bcir_telemetry_envelope.{h,c}`: `bcir_tev_decode` / `_encode` (the same wire laws in the same order), `bcir_seq_observe`, `bcir_signal_find` over the **generated** `bcir_signal_table.h`, `bcir_tev_intake_admit` / `_report`; every corpus envelope Python encode → C decode → both re-encodes byte-identical, one status per malformed variant, the table's rows byte-identical, the intake's verdicts in the ring traces (`test_telemetry_envelope.py`, `test_signal_table.py`); the envelope mode of `fuzz_ring.c` and the decoder campaign's `envelope` surface |
| the data-plane hand-off: the pack table and the per-step freeze (G16, S3-C) | `gem.handoff` (`PackTable` — `reserve` / `write` / `commit` / `abort` / `borrow` / `give_back` / `release` / `admit` / `dispatch` / `freeze` / `state_digest`; `freeze_claims`, `admit_manifest`), graded against the live `gem.control.ControlPlane` (`docs/kernel/BCIR_DATA_PLANE_HANDOFF.md`) | *(C rail)* `runtime/c/bcir_handoff.{h,c}`: `bcir_ho_init` / `_reserve` / `_reserved` / `_commit` / `_abort` / `_borrow` / `_give_back` / `_release` / `_admit` / `_dispatch` / `_admit_manifest` / `_state_digest`, freestanding; `bcir_hydrate_generations` (byte-identical to `freeze_claims` over 187 freeze cases); identical three-rail traces over 40 scenarios — verdict, refusal, status, handle, generation, the claims walked and the table's and the plane's digests after every operation — and -O0 == -O3 == the oracle (`test_handoff.py`, `tools/c/check_runtime.sh`, the exact `handoff.*` rows); *(C++ seam)* `runtime/cpp/bcir_handoff.{hpp,cpp}` — `PackArena` / `Reservation` / `PackOwner` / `PackView` / `Borrow` / `GraphBuilder` — traced line for line over every scenario it can spell, plus ten lifetime witnesses, under ASan and UBSan (`tools/cpp/check_handoff.sh`); the Stage 3 exit flow (`runtime/c/test_stage3.h`) identical on all three rails; `fuzz_handoff.c` table and freeze modes; *(bundle, law rail)* none — a version-zero runtime structure, not an artifact or a law |
| the manifest-of-shards, BSHM version zero (G16, S3-C) | `abi.shard_manifest` (`encode_manifest` / `decode_manifest`, `split`, `sub_pack`, `frame_of`, `partition`, `reassemble`, `hydrated_layout`; `docs/kernel/BCIR_SHARD_MANIFEST_ABI.md`) | *(C rail)* `runtime/c/bcir_shard_manifest.{h,c}`: `bcir_shm_decode` / `_encode` (the same wire laws in the same order), `_sub_pack` / `_frame` / `_partition` / `_check_whole` / `_reassemble` (one total predicate); manifest, frame and shards byte-identical to the oracle over 73 split cases, every whole reassembled byte for byte, one status per malformed variant and tampered set; *(C++ seam)* `cut_shards` / `reassemble_shards` and the distributed `cut()`, each shard admitted and run by itself reproducing the whole's dispatch; `fuzz_handoff.c` manifest and shard-set modes and the decoder campaign's `manifest` surface |
| ExecutionPlanV1 — the plan as bytes (G11, S1-C) | `gem.execution_plan` (`ExecutionPlan` / `PlanStep` / `Lifetime` / `MovementEdge`, `plan_from_realization`, `schedule_of`, `realization_of`), `abi.execution_plan_abi` (`encode_plan` / `decode_plan` / `validate_plan`, frozen v1: `docs/kernel/BCIR_EXECUTION_PLAN_ABI.md`), `verify.verify_execution_plan` (R9 structure, R13 binding, the placement re-derived from the plan's own costs, R11 against the registry, R10/R11 against the pack) | *(C rail)* `runtime/c/bcir_execution_plan.h` + `bcir_runtime.c`: `bcir_ep_validate` / `bcir_ep_verify` (the same wire laws, `BCIR_ERR_PLAN` and the range gate), the record walks, `bcir_ep_check_generation_vector` (R11), `bcir_ep_check_pack` (the pack is the lowering of its plan: one segment per step with the step's claim/phase/lane/width, identical vectors — an older vector is `BCIR_ERR_STALE`); byte-identical Python encode → C decode → Python re-encode on every corpus plan (`test_execution_plan.py`, `tools/c/check_runtime.sh`); *(bundle)* BCAB kind 25 / format 13 with the full wire verification in both readers and `bcir.artifact.variant` kind `execution_plan`; *(law rail)* no plan op yet — the C decoder is the reader every rail links |
| data movement as a first-class transformation (G8, S5-C) | `kbcir.movement`: `MovementSpec` (banks and directed links, homes, the Semantic Swap classes, codecs, site restrictions, deadline), `transform` (M → M′: explicit `mem.move.{far,near}` claims over per-(resource, bank, episode) copies in inbound/outbound/final phases), `plan_movement` (the joint objective over the transformed module: exact within the budget, TMSAO-1; greedy, TMSAO-4), `execution_plan_of` (one v3 edge per move and remat, the source/spec binding), `verify_movement` (MV1–MV11, total); `verify_execution_plan(movement=(source, spec))` | *(bytes)* ExecutionPlan v3 on the C rail: the move tail and the binding trailer in `bcir_ep_verify` (every v3 move law, the same predicate as `validate_plan`; the wire version decides), `bcir_ep_binding`, one generation locator for the R11 check and the plan/pack binding; every planned plan byte-identical through the C decode and bound to its pack and registry, every malformed move and binding refused on both rails (`test_execution_plan.py`, `tools/c/check_runtime.sh`, `movement.parity.mismatch`); BCAB and the control plane admit v3 on both rails; the ASN.1 projection v3 holds the wire laws (`movement.asn1.accepted`); *(oracle only)* the transform, the planner and MV1–MV11, which need the source module and the spec; *(law rail)* no movement op -- the plan bytes are the reader every rail links |
...
| provenance manifest | `kbcir.provenance` (`build_manifest` / `replay` / `reproduces` / `verify_manifest`; `_fnv`/`_digest`/`hash_module`/`hash_target`/`hash_theta`/`hash_policy` — since S0-D `hash_target` folds the memory hierarchy and `hash_module` the claims in declared order; since S1-B `canonical_stream` is the one iterative walk `hash_module` chains, `module_identity` computes the digest once per `Module.revision`, and `digest_of(module, identity)` is the identity-bound verifier API — accepted only when the module's canonical stream is exactly what the identity describes, recomputed otherwise; `verify_manifest`/`replay`/`reproduces` always recompute) | `bcir.kbcir.provenance_manifest` (R13: deployed ⇒ reproduced; manifest equality ⇒ identical plan). When the op carries the four component hashes + artifacts, `-bcir-verify` **recomputes the digest** byte-identically to `provenance._digest`, and **cross-checks every component hash** against the IR — `m_module` from the `bcir.module` (resources/claims incl. the `opcode`), `m_target` from the `target.capability` (incl. `target_name`/`scalable`), `m_theta` from the `kbcir.theta`, `m_policy` from the `kbcir.policy` unfolded `base_weights` — each recomputed byte-identically to the oracle's `hash_*`. So a manifest can be re-pointed at neither a different goal graph, target, Θ, nor policy (`verify_provenance.mlir`; pinned by `test_provenance.py`) |
...
| ASN.1 ExecutionPlan projection | `asn1.execution_plan` (the `BCIR-ExecutionPlan` module, OID `{ 1 3 6 1 4 1 62596 3 }`, projection version 3: the v3 move tail and binding; DER, OER and JER over one type model, each held to the native wire laws; additive, native octets frozen) | *(interop rail)* A1 faithful / A2 canonical / A3 additive over every corpus plan in both placements in `test_asn1_execution_plan.py`; the compiled `.asn1` encodes byte-identically to the hand-built model; `docs/BCIR_ASN1_X690_ABI.md` §3b |
| ASN.1 ControlPlane projection | `asn1.control_plane` (the `BCIR-ControlPlane` module, OID `{ 1 3 6 1 4 1 62596 4 }`; the body a CHOICE whose alternative is the kind, the widths as constraints; DER, canonical OER and JER over one type model; additive, native octets — MAC and CRC — frozen; decoded values held to `validate_control`) | *(interop rail)* A1 faithful / A2 canonical / A3 additive over every corpus record in `test_asn1_control_plane.py`; the compiled `.asn1` encodes byte-identically to the hand-built model under DER and canonical OER; `docs/BCIR_ASN1_X690_ABI.md` §3c |
...
| partial LLVM AOT/JIT subset | `lower.llvm` (clang/lli; exactly one 2-read/1-write add/sub/mul claim, otherwise reject; the runtime-`n` tail contract: vector loop over `n & -W` plus a scalar epilogue, R12-held, harness canaries at `count + 7`, a sub-width count and 0; the declared alias facts (S5-A, `lower.alias_facts`): `noalias` on exactly the exclusive pointers, one alias scope per resource, clang's TBAA tag for the element type, `volatile`, a barrier's fences — R12-held (`verify.alias`), judged by LLVM's own alias analysis, equal to clang's IR for the C kernel; atomic and unknown hazards and undeclared or mis-sized operands refused; every harness binds one buffer per resource) | `bcir.target.lower_contract`; not arbitrary-graph lowering. The law rail emits no LLVM kernel (`-bcir-lower-to-llvm` checks the segment contract), so the alias facts have no second emitter to keep in parity: the C backend is the second path to LLVM, and `alias.backends.disagree` holds the two to one fact set |
| C kernel backend (C23) | `lower.c_kernel` (`emit_kernel_c` / `emit_header_c` / `emit_selfcheck_c` / `compile_and_run_c`) + `verify.verify_c_lowering` | `bcir.target.lower_contract` (R12: selected width → loop — a *floor* at the full hardware lane (idiomatic loop), a *cap* when sub-maximal (a thermal/power throttle); `restrict` on exactly the exclusive pointers, `volatile` pointees, a barrier's `atomic_thread_fence` (S5-A — the gather form, the hot-shape specialist, the Q-fixed kernel and the ABI header from the same derivation), bounds tail, precision; portable C23 for any resident toolchain) |
...
| native calibration (attested tenancy) | `runtime/c/bcir_microbench.c` + `kbcir.microbench.calibrate_native` (`strided_order` is the Python twin of the rig's full-cycle walk) | feeds the frozen `CalibratedProfile` schema with real cache latency plus `NativeEvidence` (census, raw samples, host attestation); "bare-metal" only where the evidence proves it (S0-F / G7) |
...
| C-side zero-copy ring (real bridge) | `lower.memory_model.emit_ring_header_c` (C producer: atomic release-store to fixed offsets, `hazard_to_ordering`) + `telemetry.parse_shared_ring` | *(host)* C writes / Python reads the **same mmap** with no syscall/serialization — measured round-trip in `test_persistent_oracles`. The frozen v1 quiescent snapshot; the live transport is the version-zero ring (the G15 row), whose C API claims none of the names this emitter emits |
...
| CSE / duplicate elimination | `realize.fused_candidates` value numbering over the complete semantic identity (`cse_identity`: op + opcode, lane, stride class/multiplier, offset, count, domain, dynamic bound, operand versions ⇒ a copy, no recompute) with the categorical exclusions (`cse_eligible`: never atomic, barriered, volatile, effectful, an indirect call, lifetime/timing-bearing, sparse, isolated-domain or non-default-numeric; an ineligible claim never seeds a match) — the egraph's liked-pair (`egraph.module_exprs`/`shared_blocks`) finally priced into the plan (S1-A, assessment row 7) | `BCIRCostModel.h` `cseEligible` / `cseIdentity` mirror both predicates byte-for-byte (`cost_model_cse_neg.mlir`: a different count/offset, an atomic, a volatile and an ineligible seed take no credit; the exact duplicate still 5100); a duplicate claim is priced as a copy (~15% cheaper); a write between duplicates bumps the version and withdraws the credit (sound); no-op on single-claim programs (7808 intact) |
...
| StreamPack v1–v4 | `bcir.abi.streampack_abi` encode/decode and semantic validation; `verify_pack` R11 per resource | `bcir_runtime.c` + `bcir_encode.c` + `bcir_exec.c` (`bcir_sp_execute_checked_vector`); byte-identical re-encode, exact-version, CRC, bounds, trailing-byte, dispatch/channel, generation-vector, and malformed-corpus gates (`--live` in `check_streampack_semantic.sh`) |
...
| `volatile` (CF-VOL): `CType.touches_mmio` decides a device region (a parameter, local, static, global or temp whose type holds volatile storage or points at it), an access is volatile when its lvalue is, `_order_device_claims` is R3's pass, a loaded value is unqualified; the emit's `_volatile_ptr` / `_slot_ctype` spell each access at its exact type | `bcir_cfront.c` `ty_mmio`, `mark_access`, `order_device_claims`, `named_read`; `vol_ptr` / `vol_load_ty` / `vol_slot_ty`; `bcir_claim.is_volatile` (digest-excluded, as `Claim.volatile`) | `bcir/tests/volatile_fixtures.py` (548 forms, Clang-judged: volatile op lists, a seeded behaviour harness, the digest, the effect reports); `tools/perf/check_volatile.py --require-cc`; `cfront_volatile_width.c`; rows `volatile.*` |
| a pointer cast `(T *)x`: a pointer temp of the target type, spelled faithfully (`lower._pointer_spelling`, in the digest); `*(T *)x` dereferences it | `bcir_cfront.c` `cast_name` (kind 2) + `temp_ptr`; the general deref | the `vcast` forms; `test_the_register_idiom_is_one_access_of_its_width_on_both_rails` |
| a file-scope variable's value type (sign, float, `_Bool`, plain `char`) rides on its resource | `bcir_cfront.c` `use_global` | `cfront_globaltype.c` |
| a subscript chain (CF-IDX): a base takes one subscript per declared dimension (a multi-dimensional VLA's too), else one; while subscripts remain its element must be a pointer, which is loaded, and the rest index what it holds (`lower._lvalue(Index)`); `*q[j]` and `*(q[j] + i)` take the same step | `bcir_cfront.c` `index_chain` / `subscript_dims` / `step_to_elem_ptr`, `ptr_array` (an array of pointers, one element included), `decl_array`, and `T *rows[]` decaying to `T **` | `cfront_ptrindex.c`; the volatile corpus's `aptr`, `aptr1` and `aparam` forms |
| C's assignment conversion at a store spelled as a byte copy (CF-MEMCONV): a value of another arithmetic class (integer, real floating, complex) converts first through the `c.cast` an explicit cast lowers to (`lower._store_conversion` over `_cast_value`, `_arith_class`); `_write` returns the value stored, which a compound assignment yields | `bcir_cfront.c` `store_conv` over `emit_cast` (`res_arith_class`), the slot types `field_slot` / `pointee_slot` / `res_pointee_slot`; the store helpers convert and return the value stored | `cfront_memberconv.c`; `test_a_byte_copy_store_converts_the_value_to_the_slots_type` |
| a `static` local array or aggregate (CF-STATIC) is declared with its shape, `static` and a zero initializer (`emit._static_decl`); a static scalar or pointer keeps its constant; any other initializer on an array or aggregate is refused on both rails | `bcir_cfront.c` `emit_func`'s declaration chain (the local array and aggregate branches take the storage class) | `cfront_staticarr.c` |
| a scalar's layout under the target ABI (CF-CALIGN): `CType.align` -- a complex type its element's, a `long double` (and a `long double _Complex`) the ABI's `long_double_align`, any other scalar its size; the ABI's atomic promotion (`ctype_model.with_atomic` over `TargetABI.atomic_promote_size`, applied to a local's, a parameter's, a member's and a global's type by `lower._atomic_type`, which refuses an `_Atomic` struct or union); `sizeof` / `typeof` take an `_Atomic` type-name (`_is_decl_start`), a cast does not (`_is_cast`) | `bcir_cfront.c` `scalar_align` (a member's placement) and `type_layout` (`sizeof`, `_Alignof`), `atomic_layout` over the table's `atomic_promote_size`, the `_Atomic`-aggregate refusal in `p_type_base`; `starts_decl_type` / `starts_type_name`; a complex spelled in its own specifier is sized once (`cplxkw`) | `cfront_complexalign.c`; `test_scalar_alignment_matrix_dual_rail` (thirteen constants on five targets); `test_an_atomic_aggregate_or_cast_is_refused_on_both_rails` |
...
duration-aware (HEFT-lite) wave scheduler over the ONE canonical schedule artifact
(`BCIRSchedule.h` ↔ `gem.schedule.schedule_plan`, G1 / S1-A): the hazard DAG over every claim
of the phase (data hazards and `barriered`/`volatile` fences) built before the tail split, then
LPT priority + earliest-finish placement + locality + the bandwidth-knee clamp with the sparse
tail on its own stream inside the same dispatch, annotating `kbcir.sched_domain`/`sched_start`/
`sched_finish` per claim + `sched_makespan`/`sched_knee` (two shared-read compute claims run
parallel on domains 0/1, makespan 7808 — `schedule_eft.mlir`; a gather waits for its producer on
the tail stream and a fence is overlapped by nothing — `schedule_hazards.mlir`; both pinned by
`test_schedule.py`). `-bcir-overlap` prices exactly that placement, so the objective and the
executor annotations never describe two schedules (the gate checks the priced makespan equals
the placed one on the corpus). `-bcir-async` ports `gem.async_tokens.async_plan` +
`schedule.execute_tokens` — the `!bcir.token` fork/await DAG (hazards and fences over the whole
module) drives a single cross-phase dispatch (no phase barriers), so a later-phase independent
claim overlaps an earlier one (software pipelining); annotates `kbcir.async_awaits`/`async_domain`/
`async_start`/`async_finish`/`async_makespan` (a phase-1 independent claim starts at 0, makespan
15616 vs 2·7808 — `async.mlir`; pinned by `test_schedule.py`). `-bcir-power-rail` ports
