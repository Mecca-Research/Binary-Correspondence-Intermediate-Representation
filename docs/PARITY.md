...
| partial LLVM AOT/JIT subset | `lower.llvm` (clang/lli; exactly one 2-read/1-write add/sub/mul claim, otherwise reject; the runtime-`n` tail contract: vector loop over `n & -W` plus a scalar epilogue, R12-held, harness canaries at `count + 7`, a sub-width count and 0) | `bcir.target.lower_contract`; not arbitrary-graph lowering |
