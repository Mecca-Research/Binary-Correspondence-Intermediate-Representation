...
| verifier **R14–R16** (smart lowering) | `verify.{verify_cim (R14), verify_dvfs (R15), verify_allocator (R16), verify_smart_lowering}` — dual-rail with the law, negative-tested per law | `-bcir-lower-to-llvm` R14/R15/R16 (`gem_passes_neg.mlir`); `dispatch="pim"` only on `reduce.*`; clock_q8 ∈ [64,512] + no pim overclock; L1 ≤ 64 KiB / L2 ≤ 4 MiB |
| verifier differential (illegal modules) | `kbcir.differential.{gen_illegal_module, check_verifier, run_verifier_campaign}` — fault-inject one law (R2/R3/R5/R6/R7), confirm the verifier flags it | *(seed corpus for a future structure-aware `-bcir-verify` fuzz)* |
...
| multi-version LLVM matrix | — | `.github/workflows/ci.yml` `mlir-rail-validate` matrix (LLVM 18 gating + 19 informational); scripts auto-resolve toolchain + `FileCheck-NN` |
