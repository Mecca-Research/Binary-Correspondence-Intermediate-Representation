...
| C kernel backend (C23) | `lower.c_kernel` (`emit_kernel_c` / `emit_header_c` / `emit_selfcheck_c` / `compile_and_run_c`) + `verify.verify_c_lowering` | `bcir.target.lower_contract` (R12: selected width → loop, `restrict`, bounds tail, precision; portable C23 for any resident toolchain) |
...
| gather/blocked reduction (end-to-end) | `examples.gather_reduce` (`reduce.gather`) + `realize.candidates_for` (blocked vs gather) + `lower.c_kernel.emit_reduce_c` + `bench.compare_reduce` (`--bench-reduce`) | *(host measurement)* the cost model selects blocked; blocked == gather sum (correct), ~16× faster — a real gather program BCIR lowers two ways |
