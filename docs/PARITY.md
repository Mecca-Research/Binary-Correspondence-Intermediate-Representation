...
| modular mapping function | `kbcir.mapping` (`support` / `MappingFunction` / `CommutingSquare`) + `verify.verify_support_preservation` / `verify_commutativity` | `bcir.target.lower_contract` (R12: the lowering-discharge contract attr is checked today; the full `f(Supp(J)) ⊆ Supp(J')` set-containment + `Λ∘Ψ = Φ` path-independence is a tracked R12 refinement — see the roadmap) |
