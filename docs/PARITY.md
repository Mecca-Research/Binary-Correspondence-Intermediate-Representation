...
| `volatile` (CF-VOL): `CType.touches_mmio` decides a device region (a parameter, local, static, global or temp whose type holds volatile storage or points at it), an access is volatile when its lvalue is, `_order_device_claims` is R3's pass, a loaded value is unqualified; the emit's `_volatile_ptr` / `_slot_ctype` spell each access at its exact type | `bcir_cfront.c` `ty_mmio`, `mark_access`, `order_device_claims`, `named_read`; `vol_ptr` / `vol_load_ty` / `vol_slot_ty`; `bcir_claim.is_volatile` (digest-excluded, as `Claim.volatile`) | `bcir/tests/volatile_fixtures.py` (432 forms, Clang-judged: volatile op lists, a seeded behaviour harness, the digest, the effect reports); `tools/perf/check_volatile.py --require-cc`; `cfront_volatile_width.c`; rows `volatile.*` |
