...
| scheduled price M(π,Θ) | `gem.overlap.price_scheduled` / `optimize_scheduled` | `bcir.kbcir.scheduled_price` (R9) + **`-bcir-overlap`** (`BCIROverlapPass.cpp`, C++23): recomputes the (max,+) wave makespan (matmul 253952 / gain 761856, corpus; `overlap.mlir`) |
...
| bundle (joint) optimization | `kbcir.bundle.optimize_bundled` (jointly reorder input-sharing claims; bounded, dependency-preserving; `BundleCertificate` per gain) | *(oracle optimizer; MLIR port is roadmap)* a real 12% matmul gain the pairwise shortest path misses; corpus otherwise pinned (`test_bundle.py`) |
...
| verifier differential (illegal modules) | `kbcir.differential.{gen_illegal_module, check_verifier, gen_illegal_plan, check_plan_verifier, run_verifier_campaign}` — fault-inject one law and confirm the verifier flags it, across two rails: module/claim laws **R2–R8** (`verify`) and the plan law **R9** (`verify_plan`). R1 is enforced by construction (RID-keyed registry), so it is documented, not injected. | *(seed corpus for a future structure-aware `-bcir-verify` fuzz)* |
...
| a-priori telemetry gating | `kbcir.accel.FrozenRanker.confidence` (top-2 z-margin) + `kbcir.sensing.sense_by_ranker` | *(planning)* instruments only columns the ranker cannot resolve (narrow margin); complements the a-posteriori `RegretSensor.sense` |
