...
### 1.7 Byte-native architecture laboratory (2026-07-22)

BCIR now has a separate **token-free model laboratory**. “Token-free” here means that model input
ids `0..255` are the original octets; latent patches are internal compute units and never a
symbolic BPE/word-piece vocabulary. Four control ids provide BOS, EOS, PAD, and MASK. A Python
string crosses the boundary by strict UTF-8 encoding with **no implicit normalization**. Its
Unicode-scalar-to-byte spans are optional diagnostic metadata only; arbitrary byte streams need
no character interpretation, and generated bytes are decoded by an explicit strict-or-replace
caller policy. Canonically equivalent strings therefore remain different training examples unless
the corpus manifest declares a normalization pass before this boundary.

The dependency-free contracts, exact size accounting, and verified lowering live in
[`kbcir/byte_latent.py`](../../bcir/kbcir/byte_latent.py). The import-quarantined PyTorch references
live in [`hosted/training/byte_latent.py`](../../bcir/hosted/training/byte_latent.py). They are an
independent implementation of paper-level mechanisms; no external source, checkpoint, tokenizer,
or dataset is copied into BCIR.

| Research rail | Landed bounded contract | Semantic boundary |
|---|---|---|
| [Byte Latent Transformer](https://arxiv.org/abs/2412.09871) | Causal RoPE local byte encoder, optional hashed byte n-grams, fixed/entropy/learned incremental patches, cross-attention pooling, causal RoPE global latent Transformer, and causal local decoder conditioned only on completed patches | The report has exact parameter/cache sizes and analytic FLOP lower bounds, not measured throughput or quality |
| [Fast BLT](https://arxiv.org/abs/2605.08044) BLT-D | Joint clean next-byte loss and masked block reconstruction; deterministic sampled diffusion time, `1/t` loss scaling, bidirectional corrupted-block decode, confidence/entropy-bounded progress, and MASK-free block generation | Pure diffusion is approximate and is never relabeled as exact autoregressive decoding |
| Fast BLT BLT-S / BLT-DV | Local or diffusion draft followed by one full-model verification; accept the matching prefix, replace the first mismatch, and emit the free verifier byte after a full match | Both verified modes are regression-gated to produce exactly the greedy target ids; draft acceptance may change work, never output semantics |
| [MambaByte](https://arxiv.org/abs/2401.13660) | Direct-byte selective diagonal SSM with input-dependent step/B/C values, causal depthwise convolution, fixed recurrent-state accounting, and tiny pretraining | The readable recurrent reference is not a fused parallel-scan kernel and makes no infinite-context claim |
| [End-to-end learned tokenization](https://arxiv.org/abs/2602.13940) | Bernoulli boundary policy over causal byte state and previous boundaries; discounted future advantage, per-position batch centering, score-function loss, target boundary-rate loss, and early-exit auxiliary LM | Sampled boundaries influence architecture/training only; they cannot alter R-law legality |
| [GPUTOK](https://arxiv.org/abs/2603.02597) | Measured host/device ingest-provider selection with launch cost, chunk limit, pool capacity, exact-roundtrip prerequisite, and a deterministic crossover decision | GPUTOK is a GPU BPE merge engine, not a token-free model; its paper keeps UTF-8 conversion, chunking, and assembly on the CPU, so BCIR does not claim a landed GPU Unicode pipeline |

Entropy and learned patch scores are causal and content-addressed. Every patch except the final
short tail obeys the configured minimum, all patches obey the maximum, and fixed stride is the
deterministic baseline. The learned policy uses the paper's batch-relative score-function form:
full-model log probability minus early-exit log probability becomes discounted future reward;
actions are detached while their Bernoulli log probabilities carry the gradient. The target-rate
term cannot silently choose legality or a hardware plan.

The **byteified transplant** rail is deliberately narrower than “convert any Llama.” External
weights must first pass BCIR's strict checkpoint ingestion. The dependency-free planner bounds and
validates the supplied shape inventories, provenance digest, one-to-one names, exact shapes, and
complete coverage of the supplied `global_blocks.*` inventory. The hosted application boundary
then requires every actual global-block parameter, validates mapped tensor dtype and finiteness,
and stages every copy before the first destination mutation. Its canonical inventory counts tied
parameters once. Local byte encoder/patcher/decoder organs remain newly initialized. The default
global learning rate is one tenth of the byte-organ rate, and a frozen-global option is explicit.
Architecture-specific Llama/Gemma name mapping, positional conversion, and quality recovery remain
checkpoint-specific acceptance work.

The BCIR integration is a real execution contract rather than an isolated notebook:

- BLT and MambaByte parameter inventories exactly equal the hosted tensor inventories; cache,
...
- All model learning is L2/L3 hosted evidence. L0 receives only a preverified immutable plan; no
  entropy model, boundary policy, diffusion confidence, or SSM state can become a legality oracle.

The always-on Ubuntu/Windows hosted gate runs twice with one CPU thread. It proves exact symbolic ↔
PyTorch parameter counts, finite BLT-D and learned-patcher gradients, tiny BLT and MambaByte loss
reduction, selective-SSM/local-BLT gradients, failure-atomic global transplantation, BLT-S/BLT-DV
greedy-id identity, bounded BLT-D generation, deterministic claims/StreamPack, and host/device
ingest crossover selection. It downloads nothing and writes only a timestamp-free report.

Promotion remains ordered: corpus/normalization manifest and byte-quality evaluation → safe
checkpoint/export schema → measured patch distribution and wall-clock baseline → fused local
attention/selective-scan and patch kernels → verified static KV/diffusion buffers → two-target
evidence → optional native C/GPU inference. GPU-resident UTF-8 validation, pinned memory pools,
asynchronous H2D overlap, vendor kernels, firmware DMA, and kernel bypass are not implemented;
the driver/kernel portions stay in
