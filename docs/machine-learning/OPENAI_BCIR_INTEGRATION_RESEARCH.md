...
### 3.9 Open-weight model ingestion (moved)

Open-weight model ingestion (GLM/Gemma/Qwen family fit, the missing LLM-runtime layers, and the staged manifest → tokenizer → reference-decode → quantized-artifact → serving path) is not an OpenAI integration — it is an ML/AI-substrate program. It now lives in [`BCIR_ML_AI_INTEGRATION_ROADMAP.md`](BCIR_ML_AI_INTEGRATION_ROADMAP.md) §7. The one OpenAI-relevant boundary stays here: open weights and OpenAI APIs are complementary paths (BCIR may eventually *own* an open-weight endpoint, while hosted GPT remains an external stochastic teacher), and both are subject to the same deterministic gates before anything they produce is trusted.
...
- (The open-weight endpoint track — small Gemma/Qwen first, GLM-class later — is specified in [`BCIR_ML_AI_INTEGRATION_ROADMAP.md`](BCIR_ML_AI_INTEGRATION_ROADMAP.md) §7.)
...
10. For the open-weight track (manifest-only ingestion first, small Gemma/Qwen dense models before GLM-class), follow [`BCIR_ML_AI_INTEGRATION_ROADMAP.md`](BCIR_ML_AI_INTEGRATION_ROADMAP.md) §7.4.

## 7. Core conclusion

ChatGPT can integrate deeply with BCIR as an orchestrator, researcher, teacher, patch author, trace critic, UI host, cloud-teacher data generator, and meta-learning proposer. The frontier model can bootstrap new BCIR neural networks and endpoint models by producing validated training episodes, but it should not be integrated as a verifier, hot-path planner, unmediated source of truth, or hidden gradient oracle. (Open-weight models add a second, non-OpenAI path — BCIR eventually owning the weights and endpoint — specified in [`BCIR_ML_AI_INTEGRATION_ROADMAP.md`](BCIR_ML_AI_INTEGRATION_ROADMAP.md) §7.) The correct architecture is a layered one: OpenAI APIs expand exploration and synthesize experience; BCIR's oracle/law/parity/training system decides what becomes a frozen, replayable machine artifact.
