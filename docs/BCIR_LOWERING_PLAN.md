...
1. ✅ **A target.capability matrix. DONE.** All six TARGETS are now emitted with their
   ...
   `mlir/test/passes/target_matrix.mlir` (generated by `differential.emit_target_matrix`
   / `--emit-matrix`, drift-gated like `gem_corpus.mlir`) runs each program once per
   target through `-bcir-plan -bcir-overlap -bcir-rcsp-plan`, and the passes recompute
   the oracle's per-target plan score, (max,+) makespan/gain, and constrained optimum
   **from the capability alone** (the emitted path costs are decoration the plan pass
   ignores). The emitter now writes a *full* descriptor (`warp` + `mem_channels` added,
   so a non-x86 target plans from its own constants). Headline parity, recomputed in
   C++ from the seeds: `vector_add` realizes at the per-target max lane (avx512/sve/rvv
   **16 → 7808**, avx2 **8 → 9472**, neon **4 → 12800**, ptx **32 → 6976**); the GPU's
   coalesced gather (penalty 16) **halves** `histogram_gather` (266240 vs the CPUs'
   528384); `fused_chain` overlaps with a per-target gain (avx2 7168 / avx512 5888 /
   neon 9728 / ptx 5248); and the plan-level RCSP thermal cap narrows `{16,16}→{16,8}`
