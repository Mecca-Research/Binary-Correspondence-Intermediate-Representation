# BCIR Machine-Code / HAL / ABI / ISA Audit — verified gap register (2026-07-22)

> Status in this document is derived from repository implementation and tests, not from roadmap
> prose. The original wave-14 analysis is retained, while MC1/MC11/MC14 now include BCAB v1,
> MC2/MC6 and the x86 asm-edge reflect the current tree, MC10–MC14 preserve the resident/native
> backend boundary, and MC15 records the measurement/trace feedback ABI required by drivers.
...
| V3 | **ABI maturity / POSIX-Linux compat?** | **Seven bounded contracts exist** (§5): StreamPack v1–v3 (BSPK), BCAB v1 artifact bundles, the `bcir_exec` calling convention, the R12 `TargetABI` matrix, Q8 tier tables, frozen BTLM v1, and direct RuntimeChannel v1. BCAB selects standard payloads; it is not a Linux UAPI. POSIX coupling today is **read-only** (two syscalls + sysfs/procfs + one gated DVFS write). Full compatibility still needs device-fd lifecycle, BAR/UIO mmap, ioctl submission, eventfd/poll delivery, errno bridging, and MC15's driver telemetry contract. |
| V4 | **Hex/dump/loader, listing, mnemonics, disassembly?** | **StreamPack and multi-image bundle baselines landed.** `bcir-pack` validates/disassembles StreamPack; `bcir-bundle` validates, lists, span-dumps, selects, extracts, and delegates standard payload disassembly to resident tools. Python and allocation-free C BCAB readers agree on checksums and selection. Remaining: C StreamPack text rendering, source-module symbol resolution, and per-device command-ISA disassemblers. |
...
| V6 | **Assembler/linker/object code?** | **Resident path plus packaging: done** for the scoped targets (`bcir-cc`, link flags, real GCC/Clang/LLVM objects and linker images, JVM class assembly, exact BCAB extraction). BCAB preserves ELF/COFF/PE/Mach-O/archive/WASM/JVM/SPIR-V bytes; it does not link them. **Native isel remains gated**, and **StreamPack-level linking** (multi-pack compose, RID relocation, symbol section) remains MC7. |
...
| Multi-image packaging and selection | **BCAB v1 landed**: deterministic directory, standard payload preservation, target/feature/manifest/calibration/R12 selection, SHA-256 + CRC, Python/C/C++/MLIR surfaces | `abi/artifact_bundle.py`, `runtime/c/bcir_artifact_bundle.*`, `BCIR_ARTIFACT_BUNDLE_ABI.md` |
...
| Portable execution artifact | **StreamPack, frozen v1 + append-only v2/v3**, dual-rail codec, CRC + R10/R11 semantic trust boundary, adversarial corruptor harness | `abi/streampack_abi.py`, `runtime/c/bcir_runtime.h:46-107`, `tools/c/streampack_corrupt.py` |
...
| **Disassembler / listing (artifact → text)** | **Validated Python baseline landed** — StreamPack and BCAB listing/hexdump plus mnemonic/extraction paths; native/VM formats delegate to standard disassemblers. The C BCAB twin validates/selects but does not render text. Device-command ISA disassembly remains open | `abi/streampack_tool.py`, `abi/artifact_tool.py`, `tests/test_artifact_bundle.py` |
...
7. **Artifact bundle BCAB v1** — fixed little-endian directory and bounded preserved payloads,
   dual-rail integrity/identity checks, deterministic target selection, generation/provenance,
   and R12 admission. It is a distribution/selection ABI, not an object linker or code signature.
...
|---|---|---|---|
| **MC1** | **Partial landed** | Python StreamPack + BCAB listing/hexdump/mnemonic/extraction tools; allocation-free C BCAB validate/select twin; validation precedes display/extraction and exact spans partition the wire | Add C StreamPack text rendering, source-module/symbol resolution, and per-device command-ISA encode/decode/disassembler round trips |
...
| **MC7** | **Open** | StreamPack-level linking, RID/claim relocation, and symbol section. BCAB packages already-linked standard images and deliberately does not satisfy this graph-linking gap | Append-only StreamPack wire version, collision/overflow checks, deterministic compose, provenance-preserving unlink/list tests |
...
| **MC11** | **Resident + BCAB packaging** | GCC/Clang/LLVM retain sections/symbols/relocations and produce standard objects/archives/linker images; BCAB preserves exact bytes, validates header identity, selects by target envelope, and extracts for system-loader interop | BCIR-native emission and a broad relocation/COMDAT/TLS corpus are absent. This remains distinct from MC7 |
...
| **MC14** | **Partial resident evidence** | BCAB adds bounded geometry, CRC/SHA, format identity, R12/feature/manifest/calibration policy, corruption corpus, and allocation-free selection; resident loaders and differential ISA gates remain the execution authorities | Add authenticity signatures, W^X/relocation/errata policy, deeper malformed-object corpora, independent assembler/disassembler comparison, and simulator/hardware parity per native/device ISA |
