...
| **Plug-in C compiler / frontend** (C source → claim graph) | **C** `runtime/c/bcir_cfront.c` (IR: `bcir_cir.h`) + Python prototype `bcir/frontends/cfront/` | **C** (the driver-embeddable compiler) | ◑ **porting** — ported to C + Python↔C parity-gated: **L1** integer expr, **L2** struct/bitfield layout, **L3** pointers/arrays (GEP), **L4** functions + the call graph (**R18** in C), **L5** volatile/MMIO, **L6** control flow (`if`/`else`/`while`), **L7** a real C preprocessor (`bcir_cpp.c` — object/function macros with `#`/`##`, `#if`/`#ifdef`/`#elifdef`, `#include`, C23 `#embed`); an **R1–R8 + R18 verifier**, a **faithful C emitter** (Clang-behaviour-equivalent), and the **closed loop** `C → bcir_cpp → bcir_cfront → bcir_plan → bcir_hydrate → bcir_exec` (no Python). Remaining: L8 full ABI + R9–R17 / attestation / atomics (§5.8) |
