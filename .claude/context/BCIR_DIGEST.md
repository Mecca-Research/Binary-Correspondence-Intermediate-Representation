...
- `bcir/` is the dependency-free Python conformance oracle: R1–R25 semantics,
  K_BCIR planning/certified learned organs, GEM scheduling and StreamPack v1–v4,
...
- `runtime/c/` contains three enforced classes: heap-free freestanding code; hosted
  ...
  structural index) over the freestanding pack table (G16): artifacts cross as borrowed
  views admitted by the live control plane; single-node and dynamic-graph backends are
  real; the distributed partition and BSHM manifest-of-shards are real, and only its
  cross-node dispatch remains an explicit stub.
...
S1-A — one canonical schedule artifact `gem.schedule.schedule_plan` / `BCIRSchedule.h` read by
the price, both executors and the four law-rail passes, the hazard DAG (`hazard_predecessors`:
data hazards + barriered/volatile fences) built before the tail split, the CSE identity complete
and its exclusions categorical on both rails, `pricing.eft.divergence` 1.9922 → 1.0; G3 landed as
S1-B — `provenance.canonical_stream` / `module_identity` / `digest_of`: the module digest computed
once per `Module.revision` from one iterative walk, validated by content by any verifier and
recomputed at the trust boundaries, `static_memory.digests.2048` 3 → 1; G11 landed as S1-C —
`ExecutionPlanV1`, the plan as bytes: `gem.execution_plan` / `abi.execution_plan_abi` /
`runtime/c/bcir_execution_plan.h` / the `BCIR-ExecutionPlan` ASN.1 module / BCAB kind 25, the
pack bound to its plan by bytes on both rails, the four G11 rows at 0; G5 landed as S1-D —
`static_memory.schedule_intervals` / `plan_static_memory(schedule=, layout="exact")` /
...
`sweep.replacement.fraction` 1.0 → 0.0625; G4 landed as S2-B — `gem.exact`: `exact_schedule` (bounded
branch-and-bound over active schedules, the heuristic as the seed, a valid L on a budget stop),
the named bound stack, `exact_selection`, `certify_schedule` (the first TMSAO-2 / TMSAO-1
certificates over `ExecutionScopeV1`); the section 6.1 corpus pinned (`bcir/tests/exact_fixtures.py`)
and proved 1,716/1,716 on 2 and 3 domains against code-free oracles; `eft.*` rows 0 / 1.0 / 1.0
on the proof rail, `optimize_scheduled.quality` 1.0, `solver.unproved.fraction` 0,
`solver.gap.p95` 0; the artifact is never replaced; G12 landed as S2-C — `gem.dispatch` (the law
as a table, one runner per region kind, `DispatchRecord` in every certificate), content-addressed
resumable states for the schedule, memory and selection solvers (run(b₁) + resume(b₂) = run(b₁+b₂)),
an incumbent at every interruption point, the ranker held to a permutation of the census,
`gem.diff.plan_diff`; `search.resume.unavailable` 393 → 0, `dispatch.incumbent.missing` 26 → 0,
`dispatch.unrecorded` 12 → 0; G6 landed as S2-D (affine first) — `kbcir.regions` (affine and
opaque regions: verifier, identity expansion, floor, named refusals; `region_graph`) and
`kbcir.objectives` (the typed registry admitted only with laws proved; `dag_best_path` reproduces
the min-plus path and the max-plus critical path), `gem.exact.certify_selection` + the dispatch
`path` kind; `regions.unexpanded.claims` 542 → 0, `objectives.unverified` 2 → 0, the native
guardrails unchanged A/B on this host; G13 landed as S2-E — `kbcir.workload.Workload` (the
declared `W`, digested with the scope, carried by every certifier), `kbcir.measured` (the
append-only content-addressed measured corpus with raw samples and the host attestation,
`replay_measured` over every logged episode, a corpus-bound `ReplayCertificate`), the portfolio's
(runtime, workload) table, the dispatch law's `measured` rail and `certify_measured` under the
two-target rule; `scope.workload.collisions` 36 → 0, `replay.subset.admitted` 24 → 0,
`dispatch.measured.unavailable` 12 → 0; Stage 2 complete, next Stage 3 (G14, S3-A))
...
`.clang-format`), while `runtime/c` is deliberately dense hand-formatted C that no configuration
describes and is out of the hook's scope by declaration.
...
| Metric | Value |
|---|---|
| Static Python `test_*` function inventory | **3966** across 281 files |
...
| Static runtime C source/header inventory (`runtime/c/`) | **334** files |
...
- **docs/DEVELOPMENT_HISTORY.md** (1367L): 1. The development method · 2. The PR arc (eras) · 3. Condensed dated changelog · 4. Capability closure ledger migrated from the former master · 5. Where the detailed notes live now
...
- **docs/PARITY.md** (429L): Enum value parity (normative) · Concept parity · Python ↔ C artifact and runtime parity · Python ↔ C frontend twin (`runtime/c/`) · Worked-example parity · Generated, adversarial parity (the proof, not the hope) · How parity is enforced today
...
- **docs/REPO_CURRENT_STATE_AUDIT.md** (364L): Snapshot · Confirmed strengths · Confirmed limitations · Recommended next milestones · Changelog
- **docs/STATUS.md** (50L): Verifier-law negative-fixture inventory (R1–R25) · Hardware channel / target matrix · Runtime C components
- **docs/VISION_ALIGNMENT_AUDIT.md** (241L): 1. Thesis under audit · 2. Scorecard · 3. C as registry definition and macro target · 4. IR ownership, machine edges, and backend boundary · 5. Certified optimization and AI substrate · 6. Model inference, training, and C++ boundary · 7. Driver, kernel, telemetry, and IPC alignment · 8. Highest-leverage remaining work · 9. Bottom line
- **docs/kernel/BCIR_ALIAS_FACTS.md** (214L): The facts · Every emitter · R12 · The self-checks · The rows · What LLVM gains, measured · Not claimed
...
- **docs/kernel/BCIR_ARTIFACT_BUNDLE_ABI.md** (270L): 1. Wire conventions and limits · 2. Header (128 bytes) · 3. Directory entry (448 bytes) · 4. Kinds and payload formats · 5. Compatibility and deterministic selection · 6. Interfaces and backend boundary · 7. Additive ASN.1 transfer syntax · 8. Binary compatibility strategy · 9. Conformance
- **docs/kernel/BCIR_CONTROL_PLANE_ABI.md** (329L): Conventions · Envelope · Header (64 bytes, one cache line; every `u64` at an 8-aligne · Kinds, bodies, capabilities and reasons · Digests · The wire laws (keyless; both rails, before publication and a · The resident plane (the semantic trust boundary) · Gates · Where the record travels · Versioning (the freeze) · Not claimed
- **docs/kernel/BCIR_DATA_MOVEMENT.md** (265L): The spec · The transform M → M′ · Semantic Swap · The planner · The laws (`verify_movement`) · The plan as bytes: v3 · What is not claimed · Evidence
...
- **docs/kernel/BCIR_ESCAPE_ANALYSIS.md** (266L): The analysis · The answers · The reports (both rails, byte for byte) · What is not claimed · Evidence
- **docs/kernel/BCIR_EXECUTION_PLAN_ABI.md** (228L): Conventions · Header (64 bytes, cache-line aligned; every `u64` at an 8-al · Body (sequential, length-prefixed) · Trailer · The wire laws (both rails, before publication and again on r · Semantic trust boundary · Readers · Where the plan travels · v2 (append-only): the liveness domain and the lifetime ticks · v3 (append-only): the move tail and the binding (G8) · Versioning (the freeze)
...
- **docs/kernel/BCIR_PLANNER_ABI.md** (235L): Conventions · BKPI — the planner input · BKPR — the realization · The planner, step by step · Gates · Not claimed
- **docs/kernel/BCIR_SHARD_MANIFEST_ABI.md** (177L): Conventions · What shards · The frame and the shards · The manifest · Wire laws (in this order, both rails) · Reassembly: one total predicate · Admission at the node · Gates · Relation to the other surfaces · Not claimed
- **docs/kernel/BCIR_STREAMPACK_ABI.md** (229L): Conventions · Header (64 bytes, cache-line aligned) · Body (sequential, length-prefixed) · Trailer · v2 (append-only): pipelined phases + double-buffer prefetch · v3 (append-only): on-wire segment dispatch + channel · v4 (append-only): per-resource generation vectors (R11) · Semantic trust boundary (R10/R11 in C) · Versioning (the freeze) · Why a frozen ABI now
...
- **docs/kernel/SIGNAL_REGISTRY.md** (130L): What it is · Core types · Providers · Builders + the channel↔provider mapping · Honest real/unavailable split (typical sandbox) · Status and pre-driver boundary
...
- **docs/kernel/TELEMETRY_ENVELOPE_ABI.md** (190L): The envelope · The wire laws (one order, both rails, encoder and decoder) · The signal definition table · Continuity: one predicate · Intake · Gates · Not claimed
- **docs/kernel/TELEMETRY_FRAME_ABI.md** (163L): Conventions · Frame · Resync semantics · Host decode reuses RT3 (two-truth) · Frozen-v1 scope and pre-driver extension · Egress over UART (documented adapter, not built here)
- **docs/kernel/TELEMETRY_PIPELINE_RESEARCH.md** (314L): 0. What BCIR already has (the substrate this builds on) · 1. Layered model of telemetry sources (where each tool sits) · 2. Metric taxonomy → BCIR cost dimensions · 3. Abstractions worth copying (the design DNA) · 4. Gaps to add to BCIR's existing surfaces · 5. Recommended architecture (vendor-neutral, two-truth-safe) · 6. Suggested build order (each a gated segment) · 7. Driver/kernel integration gate · Sources
...
- **docs/languages/CPP_HANDOFF_BOUNDARY.md** (215L): Honest depth (read this first) · Why a boundary at all · What stays on the C/IR rail (below the boundary) · What crosses to C++ (above the boundary) · The seam · Why C++ (and not C / IR) · What is built · Risks and follow-ups (what remains, deliberately)
...
- **docs/research/BCIR_GEMPLUS_ROADMAP.md** (1270L): 0. The measurement discipline, and why it comes first · 1. Scope identity: `S` · 2. Certificate classes · 3. The slices · 4. The sublinearity question, answered precisely · 5. The learned-optimization boundary · 6. Order of work — the six stages · 7. What this roadmap will not claim · 8. Current state, 2026-09-04
- **docs/research/BCIR_GEMPLUS_TMSAO_STAGED_PLAN_2026-09-04.md** (346L): 0. Inputs · 1. Executive summary · 2. Disposition of the assessment against `main` at #757 · 3. Measurement today · 4. What the assessment lacks · 5. The three emphases · 6. The re-staged program · 7. The sections, in order · 8. Decision rules and what this plan will not claim · 9. Sources
...
- **docs/security/laws.md** (1852L): The harvest protocol · The staleness rule (declared, not discretionary) · The laws · Campaign classification summary
