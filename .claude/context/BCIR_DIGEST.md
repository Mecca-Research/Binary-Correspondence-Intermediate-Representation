...
planner parity, incremental re-verification) and its Stage 0 closure items S0-1…S0-10 (S0-4, S0-7
and S0-10 landed as S0-A: EV laws in `verify()`, scope-aware R9 on both rails, the audit command's
honest name)
...
| Metric | Value |
|---|---|
| Static Python `test_*` function inventory | **3576** across 258 files |
| Static MLIR ODS op-definition inventory (`mlir/include/BCIR/*.td`) | **133** |
| Static registered-pass inventory | **37** |
| Static MLIR fixture inventory (`mlir/test/`) | **123** files; 367 `expected-error` markers |
...
- **docs/DEVELOPMENT_HISTORY.md** (486L): 1. The development method · 2. The PR arc (eras) · 3. Condensed dated changelog · 4. Capability closure ledger migrated from the former master · 5. Where the detailed notes live now
...
- **docs/security/laws.md** (820L): The harvest protocol · The staleness rule (declared, not discretionary) · The laws · Campaign classification summary
