...
class BankEnvelope:
    ...
    def __post_init__(self) -> None:
        _str(self.name, "bank name")
        _str(self.channel, "bank channel")
...
class LinkEnvelope:
    ...
    def __post_init__(self) -> None:
        _str(self.source, "link source")
        _str(self.destination, "link destination")
...
class HardwareEnvelope:
    ...
    def __post_init__(self) -> None:
        ...
        _str(self.name, "hardware name")
        _str(self.architecture, "hardware architecture")
        ...
        if any(not isinstance(row, LinkEnvelope) for row in links) or any(
            not isinstance(row, KernelBenchmark) for row in benchmarks
        ):
...
class FormatSize:
    ...
    def __post_init__(self) -> None:
        ...
        if self.name == "bcirq8-group32" and (
            self.classification != "quantized" or self.group_size != 32
        ):
...
class PredictionInterval:
    ...
    def __post_init__(self) -> None:
        ...
        for field in (
            "tokens_per_second_lower_q16",
            "tokens_per_second_median_q16",
        ...
        if not (
            self.tokens_per_second_lower_q16
            <= self.tokens_per_second_median_q16
...
class PlacementCandidate:
    ...
    def __post_init__(self) -> None:
        _str(self.candidate_id, "candidate_id")
        _str(self.kind, "candidate kind")
        ...
        if not self.compute_banks or any(
            not isinstance(name, str) or not name for name in self.compute_banks
        ):
        ...
        if (
            any(not isinstance(row, BankPeak) for row in self.peaks)
...
class ModelCostReport:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            any(not isinstance(row, FormatSize) for row in self.formats)
        ...
        if len({row.name for row in self.formats}) != len(self.formats) or len(
    ...
    def from_json(cls, text: str) -> "ModelCostReport":
        ...
        try:
            ...
            for row in doc.pop("candidates"):
                ...
                values["predictions"] = tuple(
                    PredictionInterval(**prediction) for prediction in values["predictions"]
                )
                values["blockers"] = tuple(values["blockers"])
                candidates.append(PlacementCandidate(**values))
            return cls(
                **doc, formats=formats, training_state=training, candidates=tuple(candidates)
            )
        except (KeyError, TypeError) as exc:
...
class ModelExecutionPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            "prefill" not in stages
            or "decode" not in stages
...
def _candidate_dict(row: PlacementCandidate) -> dict:
    return {
        ...
        "compute_banks": list(row.compute_banks),
        "backing_bank": row.backing_bank,
        "split_layer": row.split_layer,
...
def _bcirq8_compatible(inventory: TensorInventory) -> tuple[bool, str]:
    ...
    if not all(
        (
            ...
            inventory.intermediate_size,
            inventory.num_hidden_layers,
            inventory.num_attention_heads,
    ...
    if (
        inventory.hidden_size % inventory.num_attention_heads
    ...
    expected_shapes = {
        ...
        "norm.attention": (d,),
        "norm.mlp": (d,),
        "embedding": (inventory.vocab_size, d),
        "norm.final": (d,),
        "lm_head": (inventory.vocab_size, d),
    }
    per_layer = {
        layer: [row for row in rows if row.layer == layer]
    ...
    expected_globals = {"embedding", "norm.final"} | (
        set() if inventory.tied_embeddings else {"lm_head"}
    )
...
def _bcirq8_tensors(inventory: TensorInventory) -> list[TensorRecord]:
    by_role_layer = {
        (row.role, row.layer): row for row in inventory.tensors if not row.role.startswith("aux.")
    }
...
def format_sizes(inventory: TensorInventory, formats: tuple[str, ...]) -> tuple[FormatSize, ...]:
    ...
    unsupported_dtypes = sorted(
        {row.dtype for row in inventory.tensors if row.dtype not in {"F64", "F32", "F16", "BF16"}}
    )
    for name in formats:
        if name == "source":
            reasons = []
            ...
            out.append(
                FormatSize(
                    name,
                    "exact",
                    inventory.source_file_bytes,
                    inventory.payload_bytes,
                    inventory.header_bytes,
                    0,
                    not reasons,
                    "; ".join(reasons),
                )
            ...
            continue
        if not compatible:
            out.append(FormatSize(name, "quantized", 0, 0, 0, 32, False, compatibility_reason))
            continue
        if unsupported_dtypes:
            out.append(
                FormatSize(
                    ...
                    32,
                    False,
                    "BCIRQ8 source ingest cannot load dtypes " + ", ".join(unsupported_dtypes),
                )
            )
            continue
...
def _kv_bytes(inventory: TensorInventory, workload: ModelWorkloadSpec) -> int:
    ...
    return _checked_mul(
        ...
        inventory.num_key_value_heads,
        head_dim,
        _DTYPE_BYTES[workload.kv_dtype],
    )
...
def _activation_bytes(inventory: TensorInventory, workload: ModelWorkloadSpec) -> int:
    ...
    concurrent = (
        max(workload.sessions, _checked_mul(workload.batch_size, workload.prompt_tokens))
        if workload.mode == "inference"
        else _checked_mul(workload.batch_size, workload.context_tokens)
    )
    scratch_per_token = _checked_add(
        _checked_mul(4, inventory.hidden_size), _checked_mul(2, inventory.intermediate_size)
    )
...
def _training_state(inventory: TensorInventory, workload: ModelWorkloadSpec) -> TrainingState:
    ...
    if workload.mode == "lora":
        ...
        for tensor in inventory.tensors:
            if tensor.role in targets and len(tensor.shape) == 2:
                trainable = _checked_add(
                    trainable, _checked_mul(workload.lora_rank, tensor.shape[0] + tensor.shape[1])
                )
    ...
    optimizer = (
        _checked_add(_checked_mul(trainable, 8), optimizer_steps)
        if workload.optimizer == "adamw"
        else 0
    )
    master = _checked_mul(trainable, 4) if workload.activation_dtype != "f32" else 0
    return TrainingState(
        trainable, trainable_tensors, gradients, optimizer, optimizer_steps, master, adapter
    )


def _training_bytes_by_layer(
...
def _model_operation_parts(
    inventory: TensorInventory, workload: ModelWorkloadSpec, operation: str
) -> tuple[int, int, int]:
    d = inventory.hidden_size
    f = inventory.intermediate_size
    ...
    kvd = (d // inventory.num_attention_heads) * inventory.num_key_value_heads
    projection_macs = _checked_add(
        _checked_mul(2, d, d), _checked_mul(2, d, kvd), _checked_mul(3, d, f)
    )
    if operation == "prefill":
        tokens = _checked_mul(workload.batch_size, workload.prompt_tokens)
        attention_macs = _checked_mul(
        ...
        attention_macs = _checked_mul(
            2, workload.sessions, workload.max_new_tokens, workload.context_tokens, d
        )
    layer_operations = _checked_mul(
        2, _checked_add(_checked_mul(tokens, projection_macs), attention_macs)
    )
...
def _prediction(
    ...
    for operation in ("prefill", "decode"):
        ...
        if kind == "cpu-gpu-split":
            ...
            assignments = (
                (
                    compute[-1],
                    _checked_add(
                        _checked_mul(layer_operations, inventory.num_hidden_layers), head_operations
                    ),
                ),
            )
        bounds = [0, 0, 0]
        ...
        for bank, operations in assignments:
            evidence = [
                row
                for row in hardware.benchmarks
                if row.channel == bank.channel
            ...
            target_weight_bytes = fmt.file_bytes if kind == "layer-stream" else peak.weights_bytes
            ...
            scale_q32 = max(
                ...
                ((target_weight_bytes << 32) + bench.weight_bytes - 1) // bench.weight_bytes,
        ...
        evidence_id = hashlib.sha256(
            _canonical({"kernels": used_evidence, "transfer_ns": transfer}).encode("utf-8")
        ).hexdigest()
        out.append(
            PredictionInterval(
                operation, lower, median, upper, tokens, tps_low, tps_mid, tps_high, evidence_id
            )
        )
    return tuple(out)


def _peak(bank: BankEnvelope, *, weights=0, kv=0, activation=0, training=0, staging=0) -> BankPeak:
    total = _checked_add(weights, kv, activation, training, staging)
    return BankPeak(
        bank.name, weights, kv, activation, training, staging, total, bank.allocatable_bytes
    )
...
def _candidate(
    ...
    fmt: FormatSize,
    compute: tuple[BankEnvelope, ...],
    backing: BankEnvelope,
    split: int,
    double_buffer: bool,
    peaks: tuple[BankPeak, ...],
    prefill_transfer: int,
    decode_transfer: int,
    hardware: HardwareEnvelope,
    inventory: TensorInventory,
    workload: ModelWorkloadSpec,
    ...
    for peak in peaks:
        if peak.peak_bytes > peak.capacity_bytes:
            blockers.append(
                f"bank {peak.bank} needs {peak.peak_bytes} bytes; "
    ...
    if any(
        bank.name != backing.name and hardware.link(backing.name, bank.name) is None
        for bank in compute
    ):
    ...
    if compute[-1].name != backing.name:
        ...
        if link is not None:
            ...
            dec_events = (
                _checked_mul(inventory.num_hidden_layers, workload.max_new_tokens)
                if kind == "layer-stream"
                else workload.max_new_tokens
            )
            transfer_ns["prefill"] = (
                prefill_transfer * 1_000_000_000 + link.bandwidth_bps - 1
            ) // link.bandwidth_bps + pre_events * link.latency_ns
            transfer_ns["decode"] = (
                decode_transfer * 1_000_000_000 + link.bandwidth_bps - 1
            ) // link.bandwidth_bps + dec_events * link.latency_ns
    if fmt.executable:
        predictions = _prediction(
            hardware, compute, fmt, kind, split, peaks, inventory, workload, transfer_ns
        )
        missing_evidence = {"prefill", "decode"} - {row.operation for row in predictions}
        if missing_evidence:
            blockers.append(
                "no matching bounded benchmark evidence for " + ", ".join(sorted(missing_evidence))
            )
    else:
        predictions = ()
    ...
    return PlacementCandidate(
        ...
        fmt.name,
        fmt.classification,
        tuple(bank.name for bank in compute),
        backing.name,
        split,
        double_buffer,
        peaks,
...
def assess_model(
    inventory: TensorInventory, hardware: HardwareEnvelope, workload: ModelWorkloadSpec
) -> ModelCostReport:
    """Enumerate deterministic resident, layer-streaming, and two-bank split plans."""
    if (
        not isinstance(inventory, TensorInventory)
        or not isinstance(hardware, HardwareEnvelope)
    ...
    if (
        workload.context_tokens > inventory.max_position_embeddings
    ...
    estimate = (len(formats) - executable_formats) + executable_formats * (
        n_banks + n_banks * (n_banks - 1) + split_pairs * max(0, inventory.num_hidden_layers - 1)
    )
    if estimate > _MAX_CANDIDATES:
        raise ValueError(
            f"model placement space has {estimate} candidates; bounded limit is {_MAX_CANDIDATES}"
        )
    ...
    for fmt in formats:
        ...
        training_blockers = (
            ()
            if workload.mode == "inference"
            else ("training memory is accounted, but training-plan lowering is not implemented",)
        )
        ...
        for bank in hardware.banks:
            peak = _peak(bank, weights=weights, kv=kv, activation=activation, training=state_total)
            candidates.append(
                _candidate(
                    f"{fmt.name}:resident:{bank.name}",
                    "resident",
                    fmt,
                    (bank,),
                    bank,
        ...
        max_layer = max(
            (_checked_add(weight, state) for weight, state in zip(layer_bytes, layer_training)),
            default=0,
        )
        for backing in hardware.banks:
            for compute in hardware.banks:
                ...
                candidates.append(
                    _candidate(
                        f"{fmt.name}:stream:{backing.name}->{compute.name}",
                        "layer-stream",
                        fmt,
                        (compute,),
                        backing,
                        -1,
                        True,
                        (backing_peak, compute_peak),
                        transferred,
                        _checked_mul(transferred, workload.max_new_tokens),
                        hardware,
                        inventory,
                        workload,
                        extra_blockers=training_blockers,
            ...
            for left in hardware.banks:
                for right in hardware.banks:
                    ...
                    for split in range(1, inventory.num_hidden_layers):
                        ...
                        right_train = _checked_add(global_training, sum(layer_training[split:]))
                        left_peak = _peak(
                            left,
                        ...
                        right_peak = _peak(
                            ...
                            activation=activation,
                            training=right_train,
                        )
                        boundary_tokens = (
                            workload.batch_size * workload.prompt_tokens
                            if workload.mode == "inference"
                            else workload.batch_size * workload.context_tokens
                        )
                        boundary = _checked_mul(
                            boundary_tokens,
                            inventory.hidden_size,
                        ...
                        candidates.append(
                            _candidate(
                                f"{fmt.name}:split:{left.name}:{split}:{right.name}",
                                "cpu-gpu-split",
                                fmt,
                                (left, right),
                                left,
...
def select_execution_candidate(report: ModelCostReport) -> PlacementCandidate:
    ...
    return min(
        ...
        key=lambda row: (
            ...
            sum(peak.peak_bytes for peak in row.peaks),
            row.candidate_id,
        ),
    )
