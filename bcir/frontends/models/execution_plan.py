...
from ...kbcir.device_manifest import (DeviceManifest, MemoryBank, check_bank_moves,
...
from .assessment import (HardwareEnvelope, ModelCostReport, ModelExecutionPlan,
                         ModelWorkloadSpec, PlanAction, PlacementCandidate,
                         format_tensor_sizes)
...
def _device_manifest(hardware: HardwareEnvelope) -> DeviceManifest:
    ...
    distance = tuple(tuple(0 if i == j else 256 for j in range(len(banks)))
...
def _layer_sizes(inventory: TensorInventory, fmt: str) -> tuple[tuple[int, ...], int]:
    ...
    layers = tuple(sum(sizes[row.name] for row in inventory.tensors
                       if row.layer == layer and row.name in sizes)
                   for layer in range(inventory.num_hidden_layers))
    globals_ = metadata + sum(sizes[row.name] for row in inventory.tensors
                              if row.layer < 0 and row.name in sizes)
    return layers, globals_


class _Builder:
    def __init__(self, hardware: HardwareEnvelope, candidate: PlacementCandidate,
                 inventory: TensorInventory, workload: ModelWorkloadSpec,
                 report: ModelCostReport) -> None:
        self.hardware = hardware; self.candidate = candidate
        self.inventory = inventory; self.workload = workload
        self.report = report
        self.manifest = _device_manifest(hardware)
        self.module = Module(name=f"model-plan:{candidate.candidate_id}",
    ...
    def phase(self, claim: Claim, action: PlanAction) -> None:
        pid = self.next_phase; self.next_phase += 1
    ...
    def cid(self) -> int:
        value = self.next_cid; self.next_cid += 1; return value

    def barrier(self, bank_name: str, layer: int, stage: str, repeats: int) -> None:
        bank = self.hardware.bank(bank_name)
        claim = Claim(self.cid(), Opcode.BARRIER, Lane.H, StrideClass.SCALAR,
                      hazard="barriered", domain=Domain[bank.domain],
                      bounds="assumed_safe", op=f"model.{stage}.barrier", cost_class="latency")
        self.phase(claim, PlanAction(0, "barrier", stage, layer, bank_name, bank_name, 0,
                                     0, repeats, bank.channel))

    def compute(self, bank_name: str, layer: int, weight_rid: int, activation_rid: int,
                kv_rid: int, weight_bytes: int, operations: int, stage: str,
                repeats: int) -> None:
        bank = self.hardware.bank(bank_name)
        ...
        claim = Claim(self.cid(), Opcode.T_MACC, Lane.T, StrideClass.TILE, count=count,
                      rd=(weight_rid, activation_rid, kv_rid),
                      wr=(activation_rid, kv_rid), domain=Domain[bank.domain],
                      bounds="assumed_safe", op=f"model.{stage}.compute.layer.{layer}",
                      cost_class="compute",
                      quantized_bits=8 if self.candidate.weight_format.startswith("bcirq8") else 0)
        self.phase(claim, PlanAction(0, "compute", stage, layer, bank_name, bank_name,
                                     weight_bytes, operations, repeats, bank.channel))

    def transfer(self, source: str, destination: str, source_rid: int, destination_rid: int,
                 nbytes: int, layer: int, stage: str, repeats: int,
                 *, allocate: bool = False) -> None:
        claim = move_claim(self.manifest, source, destination, source_rid, destination_rid,
                           max(1, nbytes), self.cid())
        if allocate:
            claim = replace(claim, lifetime=Lifetime("alloc"))
        channel = self.hardware.bank(destination).channel
        self.phase(claim, PlanAction(0, "move", stage, layer, source, destination,
                                     nbytes, 0, repeats, channel))

    def prefetch(self, bank_name: str, rid: int, nbytes: int, layer: int,
                 stage: str, repeats: int) -> None:
        bank = self.hardware.bank(bank_name)
        claim = Claim(self.cid(), Opcode.LOAD, Lane.U, StrideClass.UNIT,
                      count=max(1, nbytes), rd=(rid,), domain=Domain[bank.domain],
                      bounds="assumed_safe", op=f"model.{stage}.prefetch", cost_class="bandwidth")
        self.phase(claim, PlanAction(0, "prefetch", stage, layer, bank_name, bank_name,
                                     nbytes, 0, repeats, bank.channel))

    def evict(self, bank_name: str, rid: int, nbytes: int, layer: int,
              stage: str, repeats: int) -> None:
        bank = self.hardware.bank(bank_name)
        claim = Claim(self.cid(), Opcode.NOP, Lane.H, StrideClass.SCALAR, rd=(rid,),
                      domain=Domain[bank.domain], bounds="assumed_safe", op=f"model.{stage}.evict",
                      lifetime=Lifetime("free"))
        self.phase(claim, PlanAction(0, "evict", stage, layer, bank_name, bank_name,
                                     nbytes, 0, repeats, bank.channel))


def _compute_counts(builder: _Builder) -> tuple[tuple[int, int], tuple[int, int]]:
    inventory = builder.inventory; workload = builder.workload
    ...
    prefill_attention = (2 * workload.batch_size * workload.prompt_tokens
                         * workload.prompt_tokens * d)
...
def _resident(builder: _Builder, layer_sizes: tuple[int, ...], global_bytes: int) -> None:
    ...
    activation = builder.resource(bank_name, builder.peak_component(bank_name, "activation_bytes"),
                                  "activation")
    kv = builder.resource(bank_name, builder.peak_component(bank_name, "kv_bytes"), "kv-cache")
    weights = [builder.resource(bank_name, nbytes, f"layer-{layer}-weights")
    ...
    for stage, (layer_operations, head_operations), repeats in (
        ...
        for layer, (nbytes, weight) in enumerate(zip(layer_sizes, weights)):
            builder.compute(bank_name, layer, weight, activation, kv, nbytes,
                            layer_operations, stage, repeats)
            builder.barrier(bank_name, layer, stage, repeats)
        builder.compute(bank_name, -1, final_weight, activation, kv, global_bytes,
                        head_operations, stage, repeats)
...
def _stream(builder: _Builder, layer_sizes: tuple[int, ...], global_bytes: int) -> None:
    ...
    activation = builder.resource(compute_name,
                                  builder.peak_component(compute_name, "activation_bytes"),
                                  "activation")
    kv = builder.resource(compute_name, builder.peak_component(compute_name, "kv_bytes"),
                          "kv-cache")
    ...
    stages = tuple(builder.resource(compute_name, max(layer_sizes, default=1),
                                    f"layer-stage-{index}")
                   for index in range(2))
    backing_weights = [builder.resource(backing_name, nbytes, f"layer-{layer}-backing")
                       for layer, nbytes in enumerate(layer_sizes)]
    ...
    for stage_name, (layer_operations, head_operations), repeats in (
        ...
        for layer, (nbytes, backing) in enumerate(zip(layer_sizes, backing_weights)):
            ...
            builder.transfer(backing_name, compute_name, backing, stage, nbytes, layer,
                             stage_name, repeats, allocate=True)
            builder.prefetch(compute_name, stage, nbytes, layer, stage_name, repeats)
            builder.barrier(compute_name, layer, stage_name, repeats)
            builder.compute(compute_name, layer, stage, activation, kv, nbytes,
                            layer_operations, stage_name, repeats)
            builder.barrier(compute_name, layer, stage_name, repeats)
            builder.evict(compute_name, stage, nbytes, layer, stage_name, repeats)
        builder.compute(compute_name, -1, final_weight, activation, kv, global_bytes,
                        head_operations, stage_name, repeats)
...
def _split(builder: _Builder, layer_sizes: tuple[int, ...], global_bytes: int) -> None:
    ...
    left_activation = builder.resource(left_name,
                                       builder.peak_component(left_name, "activation_bytes"),
                                       "activation-left")
    right_activation = builder.resource(right_name,
                                        builder.peak_component(right_name, "activation_bytes"),
                                        "activation-right")
    left_kv = builder.resource(left_name, builder.peak_component(left_name, "kv_bytes"), "kv-left")
    right_kv = builder.resource(right_name, builder.peak_component(right_name, "kv_bytes"), "kv-right")
    left_weights = [builder.resource(left_name, nbytes, f"layer-{layer}-weights")
                    for layer, nbytes in enumerate(layer_sizes[:split])]
    right_weights = [builder.resource(right_name, nbytes, f"layer-{layer}-weights")
                     for layer, nbytes in enumerate(layer_sizes[split:], start=split)]
    dtype_bytes = {"u8": 1, "i8": 1, "f16": 2, "bf16": 2,
                   "f32": 4, "f64": 8}[builder.workload.activation_dtype]
    ...
    for stage, (layer_operations, head_operations), repeats, token_count in (
            ("prefill", counts[0], 1,
             builder.workload.batch_size * builder.workload.prompt_tokens),
            ("decode", counts[1], builder.workload.max_new_tokens,
             builder.workload.sessions)):
        for layer, (nbytes, weight) in enumerate(zip(layer_sizes[:split], left_weights)):
            builder.compute(left_name, layer, weight, left_activation, left_kv, nbytes,
                            layer_operations, stage, repeats)
            builder.barrier(left_name, layer, stage, repeats)
        boundary_bytes = max(1, builder.inventory.hidden_size * token_count * dtype_bytes)
        builder.transfer(left_name, right_name, left_activation, right_activation,
                         boundary_bytes, split, stage, repeats, allocate=False)
        ...
        for layer, (nbytes, weight) in enumerate(
            ...
            builder.compute(right_name, layer, weight, right_activation, right_kv, nbytes,
                            layer_operations, stage, repeats)
            builder.barrier(right_name, layer, stage, repeats)
        builder.compute(right_name, -1, final_weight, right_activation, right_kv,
                        global_bytes, head_operations, stage, repeats)
        builder.barrier(right_name, -1, stage, repeats)


def lower_model_execution(report: ModelCostReport, candidate: PlacementCandidate,
                          inventory: TensorInventory, hardware: HardwareEnvelope,
                          workload: ModelWorkloadSpec) -> LoweredModelExecution:
    """Create an artifact only after every available deterministic law accepts it."""
    if report.inventory_digest != inventory.header_digest or report.hardware_digest != hardware.digest \
    ...
    if workload.mode != "inference":
        raise ValueError("model execution-plan lowering currently supports inference workloads only")
    ...
    pack = hydrate_pipelined(builder.module, result,
                             plan=f"model:{candidate.candidate_id}", depth=2)
