...
from .model_microbench import run_bounded_model_microbench, run_bounded_model_microbench_native
...
def _config(path: Path) -> dict:
    value = strict_json_loads(
        read_bounded_text(str(path), "model config", max_bytes=4 << 20),
        "model config",
        max_bytes=4 << 20,
    )
...
def _shards(model_dir: Path, explicit: list[str]) -> list[str]:
    ...
    if index.is_file():
        doc = strict_json_loads(
            read_bounded_text(str(index), "Safetensors index", max_bytes=16 << 20),
            "Safetensors index",
            max_bytes=16 << 20,
        )
        ...
        if any(
            not isinstance(value, str) or not value or value != os.path.basename(value)
            for value in values
        ):
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--microbench", action="store_true", help="run bounded local prefill/decode calibration"
    )
    parser.add_argument(
        "--microbench-engine",
        choices=("native", "python-oracle"),
        default="native",
...
def _run(args) -> int:
    ...
    workload = ModelWorkloadSpec.from_json(
        read_bounded_text(args.workload, "model workload", max_bytes=1 << 20)
    )
    if args.microbench:
        evidence = list(hardware.benchmarks)
        measure = (
            run_bounded_model_microbench_native
            if args.microbench_engine == "native"
            else run_bounded_model_microbench
        )
        for fmt in workload.formats:
            measured = measure(
                channel=args.microbench_channel,
                weight_format=fmt,
                dimension=args.microbench_dimension,
        ...
        hardware = HardwareEnvelope(
            hardware.name, hardware.architecture, hardware.banks, hardware.links, tuple(evidence)
        )
    ...
    if args.candidate:
        selected = next(
            (row for row in report.candidates if row.candidate_id == args.candidate), None
        )
        if selected is None:
            raise ValueError(f"candidate {args.candidate!r} is absent from the report")
        if not selected.feasible:
            raise ValueError(
                f"candidate {args.candidate!r} is infeasible: " + "; ".join(selected.blockers)
            )
    else:
        try:
