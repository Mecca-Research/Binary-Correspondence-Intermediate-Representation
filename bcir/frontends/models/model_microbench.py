...
def run_bounded_model_microbench_native(*, channel: str = "host",
    ...
    if not compiler or any(not source.is_file() for source in sources):
        raise RuntimeError("native model microbenchmark requires a C11 compiler and runtime sources")
    with tempfile.TemporaryDirectory() as directory:
        executable = Path(directory) / ("bcir-ai-microbench.exe" if os.name == "nt"
                                        else "bcir-ai-microbench")
        command = [compiler, "-std=c11", "-O3", "-ffp-contract=off", "-Wall",
                   "-Wextra", "-Wpedantic", "-Werror", "-I", str(_RUNTIME),
        ...
        run = subprocess.run([str(executable), str(dimension), str(repeats), weight_format],
    ...
    if (not isinstance(document, dict) or set(document) != fields
            ...
            or document["dimension"] != dimension or document["repeats"] != repeats
    ...
    for operation in ("prefill", "decode"):
        ...
        if (not isinstance(row, dict) or set(row) != sample_fields
                or any(type(row[field]) is not int or row[field] < 1
    ...
    def result(operation: str) -> KernelBenchmark:
        ...
        return KernelBenchmark(operation, channel, weight_format,
                               row["lower_ns"], row["median_ns"], row["upper_ns"],
                               tokens, operations, n * n * element_bytes,
                               workspace, repeats)
