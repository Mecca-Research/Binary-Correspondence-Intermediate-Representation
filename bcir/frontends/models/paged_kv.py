...
from .serve import decode_session_module
...
class PagedKV:
    ...
    def __init__(self, spec: DecoderSpec, capacity: int, page_size: int,
        ...
        if capacity < 1 or page_size < 1:
            raise ValueError("PagedKV needs capacity >= 1 and page_size >= 1")
        ...
        self.row_elems = 2 * max(1, spec.n_layers * spec.kv_dim)
        self.n_pages = -(-capacity // page_size)
        page_bytes = page_size * self.row_elems * 4
        self.views: list = []
        self.pages: list = []
        for p in range(self.n_pages):
            view = StridedView(bank=bank, offset_bytes=p * page_bytes,
                               shape=(page_size, self.row_elems),
                               strides=(self.row_elems, 1), elem_bytes=4)
            bad = check_strided_view(view, man)
            if bad:                                    # D-R4, live: no view, no memory
                raise ValueError(f"PagedKV page {p} refused: " + "; ".join(bad))
            self.views.append(view)
            self.pages.append(Resource(rid=base_rid + p, domain=b.domain,
                                       shape=(page_size, self.row_elems),
                                       name=f"KVPAGE{p}"))
...
def generate_paged(prompt_ids: list, spec: DecoderSpec, w: DecoderWeights, max_new: int,
    ...
    if not prompt_ids or max_new < 0:
        raise ValueError("generate_paged needs a non-empty prompt and max_new >= 0")
    kv = PagedKV(spec, len(prompt_ids) + max_new, page_size, man, bank)
    hrow: list = []
    for tid in prompt_ids:                             # the prefill fills pages in order
        hrow = kv.step_row(w.embedding.row(int(tid)), spec, w)
    out: list = []
...
def batched_sessions_module(spec: DecoderSpec, sessions: tuple) -> Module:
    ...
    prefill = 1000*s + 1, decode t = 1000*s + 100 + t (session 0 matches
    `decode_session_module`'s bands)."""
    if not sessions:
...
def certify_batch(spec: DecoderSpec, sessions: tuple, h: HProfile, theta: Theta,
    ...
    cert = BatchCertificate(sessions=len(sessions), serial=sum(dur.values()),
