...
class DecoderSpec:
    ...
    relu/gelu contract as `feedforward_reference`."""
...
class LayerWeights:
    ...
    the four d_model x d_model attention projections, and the FF weights/biases."""

    g_attn: tuple          # d_model            (attention pre-norm gamma)
    w_q: tuple             # d_model x d_model
    w_k: tuple             # d_model x d_model
    w_v: tuple             # d_model x d_model
...
def check_decoder_weights(spec: DecoderSpec, w: DecoderWeights) -> list[str]:
    ...
    want = {"g_attn": d, "w_q": d * d, "w_k": d * d, "w_v": d * d, "w_o": d * d,
...
def decoder_layer_reference(x: list, seq: int, spec: DecoderSpec, lw: LayerWeights) -> list:
    ...
    multi-head attention -> residual -> RMSNorm -> FF -> residual. Composed from the existing
    references (see the module docstring); returns a fresh seq x d_model buffer."""
    d, nh, dk = spec.d_model, spec.n_heads, spec.d_k
    h = rmsnorm_reference(x, seq, d, list(lw.g_attn))
    q = matmul_reference(h, lw.w_q, seq, d, d)
    k = matmul_reference(h, lw.w_k, seq, d, d)
    v = matmul_reference(h, lw.w_v, seq, d, d)
    mask = causal_mask(seq)
    aspec = AttentionSpec(seq, dk)
    concat = [0.0] * (seq * d)
    for hd in range(nh):
        qh = rope_reference(_split_head(q, seq, hd, nh, dk), seq, dk, spec.rope_base)
        kh = rope_reference(_split_head(k, seq, hd, nh, dk), seq, dk, spec.rope_base)
        vh = _split_head(v, seq, hd, nh, dk)
        s = scores_reference(qh, kh, aspec)                       # (Q_h @ K_h^T) / sqrt(d_k)
        s = [s[i] + mask[i] for i in range(seq * seq)]            # causal: exp(-inf) = 0
        a = softmax_reference(s, axis_len=seq)
        ctx = matmul_reference(a, vh, seq, dk, seq)               # A @ V_h
        for i in range(seq):
            concat[i * d + hd * dk:i * d + (hd + 1) * dk] = ctx[i * dk:(i + 1) * dk]
...
class KVCache:
    """Per-layer, per-head cached K/V rows (K stored POST-RoPE at each token's absolute
    ...
    def __init__(self, spec: DecoderSpec) -> None:
        self.k = [[[] for _ in range(spec.n_heads)] for _ in range(spec.n_layers)]
        self.v = [[[] for _ in range(spec.n_heads)] for _ in range(spec.n_layers)]
    ...
    def _step_row(self, x_row: list, spec: DecoderSpec, w: DecoderWeights) -> list:
        ...
        layer, appending each layer's roped K row + V row, and return the final-norm row."""
        d, nh, dk = spec.d_model, spec.n_heads, spec.d_k
        ...
        for li, lw in enumerate(w.layers):
            ...
            k = matmul_reference(h, lw.w_k, 1, d, d)
            v = matmul_reference(h, lw.w_v, 1, d, d)
            concat = [0.0] * d
            for hd in range(nh):
                qh = rope_reference(_split_head(q, 1, hd, nh, dk), 1, dk, spec.rope_base,
                                    pos_offset=self.pos)
                kh = rope_reference(_split_head(k, 1, hd, nh, dk), 1, dk, spec.rope_base,
                                    pos_offset=self.pos)
                self.k[li][hd].append(kh)
                self.v[li][hd].append(_split_head(v, 1, hd, nh, dk))
                t_len = len(self.k[li][hd])
                ...
                ctx = [0.0] * dk                                 # A @ V_h, ascending j like matmul
                for c in range(dk):
                    acc = 0.0
                    for j in range(t_len):
                        acc += a[j] * self.v[li][hd][j][c]
                    ctx[c] = acc
