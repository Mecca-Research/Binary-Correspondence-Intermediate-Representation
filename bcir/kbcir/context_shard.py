...
class ContextShardManifest:
    ...
    def __post_init__(self) -> None:
        ...
        _name(self.classification, "context shard classification", choices=_CLASSIFICATIONS)
        for value, field in (
        ...
        _integer(
            self.payload_bytes, "context shard payload_bytes", minimum=1, maximum=_MAX_PAYLOAD_BYTES
        )
        ...
        expected_format = {
            "bcirq8": "bcirq8-v1",
            "q8-table": "bcir-q8-table",
            "lora": "safetensors",
            "weight-delta": "safetensors",
            "execution-plan": "bcir-streampack",
        }[self.shard_kind]
        if self.tensor_format != expected_format:
            raise ValueError(
                f"context shard {self.shard_kind!r} requires tensor_format {expected_format!r}"
            )
        ...
        if self.shard_kind in {"q8-table", "execution-plan"} and self.classification != "exact":
            raise ValueError("Q8-table and execution-plan context shards must be exact")

    def to_dict(self) -> dict:
        return {
            "schema": self.schema,
            **{key: value for key, value in asdict(self).items() if key != "schema"},
        }
    ...
    def to_ham_resource(
        ...
        *,
        ...
        kind = {
            ...
            "q8-table": "adapter",
            "lora": "adapter",
            "weight-delta": "adapter",
            "execution-plan": "program",
        }[self.shard_kind]
        return HAMResource(
            resource_id,
            f"context-shard:{self.digest[:16]}",
            kind,
            self.payload_bytes,
            home_bank,
            self.payload_sha256,
...
class ContextShardCatalog:
    ...
    def __post_init__(self) -> None:
        ...
        if len({(row.selector_sha256, row.shard_sha256) for row in self.entries}) != len(
            self.entries
        ):
    ...
    def from_json(cls, text: str) -> "ContextShardCatalog":
        ...
        fields = {"schema", "base_model_sha256", "target_hardware_sha256", "generation", "entries"}
        if (
...
def certify_context_activation(
    catalog: ContextShardCatalog,
    manifest: ContextShardManifest,
    selector_sha256: str,
    *,
    current_shard_sha256: str = "",
    activation_generation: int,
    quiescent: bool,
) -> ContextShardActivation:
    """Bind one catalog decision at a quiescent boundary; preserve rollback identity."""
    if not isinstance(catalog, ContextShardCatalog) or not isinstance(
        manifest, ContextShardManifest
    ):
    ...
    return ContextShardActivation(
        catalog.digest,
        selector_sha256,
        manifest.digest,
        current_shard_sha256,
        activation_generation,
        rollback,
    )
...
def write_context_shard_manifest(path: str | os.PathLike, manifest: ContextShardManifest) -> None:
...
def read_context_shard_manifest(path: str | os.PathLike) -> ContextShardManifest:
    return ContextShardManifest.from_json(
        read_bounded_text(os.fspath(path), "context-shard manifest", max_bytes=_MAX_JSON)
    )


def write_context_shard_catalog(path: str | os.PathLike, catalog: ContextShardCatalog) -> None:
...
def read_context_shard_catalog(path: str | os.PathLike) -> ContextShardCatalog:
    return ContextShardCatalog.from_json(
        read_bounded_text(os.fspath(path), "context-shard catalog", max_bytes=_MAX_JSON)
    )


def verify_context_payload(manifest: ContextShardManifest, path: str | os.PathLike) -> None:
