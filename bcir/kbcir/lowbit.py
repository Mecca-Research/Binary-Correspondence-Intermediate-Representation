...
class PackedQ4Tensor:
    ...
    def __post_init__(self) -> None:
        shape = _checked_shape(self.shape)
        ...
        if (
            not isinstance(self.exponents, (tuple, list))
            or len(self.exponents) != groups
            or any(
                type(value) is not int or not -32768 <= value <= 32767 for value in self.exponents
            )
        ):
    ...
    def groups(self) -> tuple[QGroup, ...]:
        codes = self.codes
        return tuple(
            QGroup(tuple(codes[index : index + self.group_size]), exponent, 4)
            for index, exponent in zip(range(0, len(codes), self.group_size), self.exponents)
        )
    ...
    @classmethod
    def from_values(
        cls, values, shape, *, source_sha256: str, calibration_sha256: str
    ) -> "PackedQ4Tensor":
        ...
        return cls(
            shape,
            tuple(group.scale_exp for group in groups),
    ...
    @classmethod
    def from_values_native(
        cls, values, shape, *, source_sha256: str, calibration_sha256: str, native_kernels
    ) -> "PackedQ4Tensor":
        ...
        return cls(shape, native.exponents, native.codes, source_sha256, calibration_sha256)
...
def _encode_q4(tensor: PackedQ4Tensor) -> bytes:
    ...
    body = (
        exponent_bytes
        + bytes(code_offset - exponent_offset - len(exponent_bytes))
        + tensor.packed_codes
    )
    ...
    header = _HEADER.pack(
        *arguments, 0, bytes.fromhex(tensor.source_sha256), bytes.fromhex(tensor.calibration_sha256)
    )
    header_crc = zlib.crc32(header) & 0xFFFFFFFF
    header = _HEADER.pack(
        *arguments,
        header_crc,
        bytes.fromhex(tensor.source_sha256),
...
def read_packed_q4(path: os.PathLike | str) -> PackedQ4Tensor:
    ...
    dimensions = tuple(tail[:4])
    exponent_offset, code_offset, body_crc, header_crc = tail[4:8]
    source_hash, calibration_hash = tail[8:10]
    if (
        magic != _MAGIC
        or version != _VERSION
        or header_bytes != _HEADER_BYTES
        or flags != _FLAG_SIGNED_SYMMETRIC
        or group_size != _GROUP_SIZE
        or not 1 <= rank <= 4
        or any(dim != 0 for dim in dimensions[rank:])
    ):
    ...
    groups = (count + _GROUP_SIZE - 1) // _GROUP_SIZE
    exponent_bytes = groups * 2
    code_bytes = (count + 1) // 2
    if (
        exponent_offset != _HEADER_BYTES
        or code_offset != _align8(exponent_offset + exponent_bytes)
    ...
    exponents = struct.unpack(
        f"<{groups}h", raw[exponent_offset : exponent_offset + exponent_bytes]
    )
...
class SmoothQuantPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            isinstance(self.outlier_threshold, bool)
...
def calibrate_smoothquant(
    activations, weights, policy: SmoothQuantPolicy = SmoothQuantPolicy()
) -> SmoothQuantCalibration:
    activations = _matrix(activations, "activations")
    ...
    candidates = [
        index
        for index, value in enumerate(activation_max)
    ...
    return SmoothQuantCalibration(
        tuple(exponents),
        tuple(candidates),
        _matrix_digest(activations),
        _matrix_digest(weights),
        policy_sha,
    )
...
def calibrated_q4q8_dot(activation, weight, calibration: SmoothQuantCalibration) -> float:
    ...
    for index, (left, right, scale) in enumerate(zip(activation, weight, calibration.scales)):
        if index in outliers:
            residual += left * right
            smooth_activation.append(0.0)
            smooth_weight.append(0.0)
        else:
            smooth_activation.append(left / scale)
            smooth_weight.append(right * scale)
...
def admit_low_bit_format(
    ...
    *,
    ...
    if max_logit_error < 0.0 or max_size_ratio <= 0.0 or max_error < 0.0 or max_nll_delta < 0.0:
