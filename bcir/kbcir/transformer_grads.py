...
def rmsnorm_reference(
    x: list[float], rows: int, dim: int, gamma: list[float], eps: float = 1e-6
) -> list[float]:
...
def rmsnorm_grad(
    x: list[float], rows: int, dim: int, gamma: list[float], gout: list[float], eps: float = 1e-6
...
def rope_reference(
    x: list[float], rows: int, dim: int, base: float = 10000.0, pos_offset: int = 0
) -> list[float]:
...
def rope_grad(
    gout: list[float], rows: int, dim: int, base: float = 10000.0, pos_offset: int = 0
) -> list[float]:
...
def ff_ln_tail_grads(
    ...
    for r in range(rows):
        for j in range(d_ff):
            a[r * d_ff + j] = b1[j] + sum(
                h[r * d_model + i] * W1[i * d_ff + j] for i in range(d_model)
            )
    relu = [v if v > 0.0 else 0.0 for v in a]
    ff = [0.0] * n
    for r in range(rows):
        for c in range(d_model):
            ff[r * d_model + c] = b2[c] + sum(
                relu[r * d_ff + j] * W2[j * d_model + c] for j in range(d_ff)
            )
...
def mha_projection_grads(
    ...
    for h in range(n_heads):
        ...
        dkh = [
            sc * g for g in matmul_reference(_t(ds, seq, seq), qh, seq, dk, seq)
        ]  # dK = sc*dS^T@Q
        for i in range(seq):
            for t in range(dk):
