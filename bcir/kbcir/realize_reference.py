"""The pre-G17 K_BCIR planner, verbatim: the reference `planner.parity` holds the compact
planner to.

GEM+ G17 (S4-A) replaced the object planner in `realize` -- a `Candidate` and a
`CostVector` built and rebuilt per claim, a 12-way function call per coupling, the phase
weights re-derived per claim, a full DAG materialized as tuples -- with compact indexed
arrays behind the same API (`realize.fused_offer`, `realize.optimize`). A rewrite that is
supposed to change nothing needs a witness that it changed nothing, so the code it replaced
is kept here exactly as it stood on the parent (`731373df`): the candidate enumeration, the
cost arithmetic, the CSE and deforestation discounts, the context coupling and the min-plus
path over `semiring.dag_shortest_path`, including the two shared predicates the compact
planner rewrote for speed (`cse_eligible`, `cse_identity`), so a defect in either rewrite
is a divergence rather than a shared blind spot.

Read by nothing but the parity gate (`tools/perf/gemplus_baseline.py::measure_planner`,
`bcir/tests/test_planner_compact.py`), like `gem.overlap.price_waves_legacy`: it plans,
and its plans are the "before" of "identical plan bytes before and after". Do not import
it from product code, and do not "fix" it -- a change here moves the reference, not the
planner.
"""
...
from .realize import Candidate, ChosenStep, RealizationResult
