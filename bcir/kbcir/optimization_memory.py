...
class OptimizationPattern:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not isinstance(self.embedding_q15, tuple)
            or not self.embedding_q15
            or len(self.embedding_q15) > _MAX_DIMENSION
            or any(
                type(value) is not int or not _Q15_MIN <= value <= _Q15_MAX
                for value in self.embedding_q15
            )
        ):
            raise ValueError("optimization embedding must be a bounded Q15 integer tuple")
        if (
            not isinstance(self.required_facts, tuple)
...
class OptimizationMemory:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not isinstance(self.patterns, tuple)
        ...
        if tuple(sorted(self.patterns, key=lambda row: row.pattern_sha256)) != self.patterns or len(
            {row.pattern_sha256 for row in self.patterns}
        ) != len(self.patterns):
    ...
    def from_json(cls, text: str) -> "OptimizationMemory":
        ...
        fields = {"schema", "model_revision_sha256", "embedding_model_sha256", "patterns", "facts"}
        if (
            not isinstance(doc, dict)
            or set(doc) != fields
            or not isinstance(doc["patterns"], list)
            or not doc["patterns"]
...
def query_optimization_memory(
    memory: OptimizationMemory, embedding_q15, *, top_k: int = 1, active_facts=None
    ...
    for pattern, admitted in zip(memory.patterns, eligible):
        if not admitted:
            continue
        distance = sum(
            (left - right) * (left - right) for left, right in zip(values, pattern.embedding_q15)
        )
        matches.append(
            OptimizationMatch(pattern.pattern_sha256, pattern.context_shard_sha256, distance)
        )
    matches.sort(key=lambda row: (row.squared_distance, row.pattern_sha256))
    return tuple(matches[:top_k])


def _prepare_optimization_query(
    memory: OptimizationMemory, embedding_q15, *, top_k: int, active_facts
    ...
    if (
        not isinstance(embedding_q15, tuple | list)
        or len(embedding_q15) != memory.dimension
        or any(
            type(value) is not int or not _Q15_MIN <= value <= _Q15_MAX for value in embedding_q15
        )
    ):
    ...
    elif not isinstance(active_facts, tuple | list | set | frozenset) or any(
        not isinstance(row, OptimizationFact) for row in active_facts
    ):
    ...
    return tuple(embedding_q15), tuple(
        set(pattern.required_facts) <= admitted for pattern in memory.patterns
    )
