...
from .device_manifest import (DeviceManifest, MemoryBank, check_bank_moves,
...
_ACTION_KINDS = frozenset((
    "transfer", "prefetch", "barrier", "access", "invalidate", "evict"))
...
class HAMResource:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.dependencies, tuple) \
                or any(type(value) is not int or value < 1 for value in self.dependencies):
...
class HAMWorkload:
    ...
    def __post_init__(self) -> None:
        ...
        _int(self.max_route_hops, "HAM max_route_hops", minimum=1,
             maximum=_MAX_ROUTE_HOPS)
        ...
        if (not isinstance(self.accesses, tuple) or not self.accesses
        ...
        if tuple(row.sequence for row in self.accesses) != tuple(
                range(1, len(self.accesses) + 1)):
        ...
        if any(dependency not in known for row in self.resources
        ...
        def visit(resource_id: int) -> None:
            ...
            for dependency in graph[resource_id]:
                visit(dependency)
            active.remove(resource_id); done.add(resource_id)
    ...
    def to_dict(self) -> dict:
        return {"schema": self.schema, "hardware_digest": self.hardware_digest,
                "resources": [{**asdict(row), "dependencies": list(row.dependencies)}
                              for row in self.resources],
    ...
    def from_json(cls, text: str) -> "HAMWorkload":
        ...
        fields = {"schema", "hardware_digest", "resources", "accesses",
                  "prefetch_distance", "max_route_hops", "policy_digest"}
        if not isinstance(doc, dict) or set(doc) != fields:
            raise ValueError("HAM workload has missing or unknown fields")
        if (not isinstance(doc["resources"], list) or not doc["resources"]
                or len(doc["resources"]) > _MAX_RESOURCES
                or not isinstance(doc["accesses"], list) or not doc["accesses"]
        ...
        try:
            ...
            for row in doc.pop("resources"):
                ...
                value = dict(row); value["dependencies"] = tuple(value["dependencies"])
...
class HAMAction:
    ...
    def __post_init__(self) -> None:
        ...
        if self.kind == "transfer":
            if (len(self.route) < 2 or self.bank != self.route[-1]
                    or self.intent not in _TRANSFER_INTENTS or self.mode
                    or self.nbytes < 1 or self.generation_before != self.generation_after
                    or self.predicted_ns < 1 or self.route_class not in _ROUTE_CLASSES):
            ...
            if self.kind == "access":
                if (self.mode not in {"read", "write"} or self.intent or self.nbytes < 1
                        or (self.mode == "read"
                            and self.generation_before != self.generation_after)
                        or (self.mode == "write"
                            and self.generation_after != self.generation_before + 1)):
                    raise ValueError("HAM access action fields are inconsistent")
            elif self.kind == "prefetch":
                if (self.intent != "prefetch" or self.mode or self.nbytes < 1
                ...
                if (self.intent or self.mode or self.nbytes < 1
...
class HAMExecutionPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if any(right.sequence < left.sequence for left, right in
               zip(self.actions, self.actions[1:])):
            raise ValueError("HAM plan action sequences must be nondecreasing")
        if (not isinstance(self.peaks, tuple) or not self.peaks
        ...
        for field in ("logical_transfer_bytes", "wire_transfer_bytes",
                      "direct_peer_bytes", "staged_transfer_bytes", "host_bounce_bytes",
        ...
        if self.direct_peer_bytes > self.logical_transfer_bytes \
    ...
    def to_dict(self) -> dict:
        return {"schema": self.schema, "hardware_digest": self.hardware_digest,
                ...
                "policy_digest": self.policy_digest,
                "actions": [{**asdict(row), "route": list(row.route)}
    ...
    def from_json(cls, text: str) -> "HAMExecutionPlan":
        ...
        fields = {"schema", "hardware_digest", "workload_digest", "policy_digest",
                  "actions", "peaks", "logical_transfer_bytes", "wire_transfer_bytes",
                  "direct_peer_bytes", "staged_transfer_bytes", "host_bounce_bytes",
                  "predicted_transfer_ns", "module_digest", "streampack_sha256",
                  "streampack_bytes"}
        if not isinstance(doc, dict) or set(doc) != fields:
            raise ValueError("HAM execution plan has missing or unknown fields")
        if (not isinstance(doc["actions"], list) or not doc["actions"]
                or len(doc["actions"]) > _MAX_ACTIONS
                or not isinstance(doc["peaks"], list) or not doc["peaks"]
                or len(doc["peaks"]) > 32):
        ...
        try:
            ...
            for row in doc.pop("actions"):
                ...
                value = dict(row); value["route"] = tuple(value["route"])
...
def _hardware_inventory(hardware, workload: HAMWorkload) -> tuple[dict, dict, dict]:
    ...
    for link in links:
        ...
        link_map[key] = link; adjacency[key[0]].append(key[1])
    for values in adjacency.values():
        values.sort()
    for resource in workload.resources:
        if resource.home_bank not in by_name:
            raise ValueError(f"HAM resource {resource.resource_id} names unknown home bank "
    ...
    for access in workload.accesses:
        ...
        if access.mode == "write" and not workload.resource(access.resource_id).mutable:
            raise ValueError(f"HAM access {access.sequence} writes immutable resource "
                             f"{access.resource_id}")
    return by_name, link_map, adjacency


def _link_ns(link, nbytes: int) -> int:
    bandwidth = _int(getattr(link, "bandwidth_bps", None), "HAM link bandwidth",
                     minimum=1)
    latency = _int(getattr(link, "latency_ns", None), "HAM link latency")
    transfer = (_checked_mul(nbytes, 1_000_000_000, "HAM link transfer")
                + bandwidth - 1) // bandwidth
    return _checked_add(latency, max(1, transfer), "HAM link latency")


def _shortest_route(source: str, destination: str, nbytes: int, *, link_map: dict,
                    adjacency: dict, max_hops: int) -> tuple[tuple[str, ...], int]:
    ...
    while queue:
        ...
        for target in adjacency[node]:
            ...
            heapq.heappush(queue, (_checked_add(cost, edge_cost, "HAM route cost"),
                                   hops + 1, path + (target,), target))
    raise ValueError(f"no declared HAM route from {source!r} to {destination!r} "
...
class _Planner:
    def __init__(self, workload: HAMWorkload, hardware) -> None:
        self.workload = workload; self.hardware = hardware
        ...
        self.required = {row.sequence: _closure(workload, row.resource_id)
                         for row in workload.accesses}
        ...
        self.copies = {row.resource_id: {row.home_bank: (0, row.content_sha256)}
        ...
        for name, value in self.used.items():
            if value > self.banks[name].allocatable_bytes:
                raise ValueError(f"HAM pinned home resources need {value} bytes in {name!r}, "
                                 f"above its {self.banks[name].allocatable_bytes}-byte envelope")
        self.actions: list[HAMAction] = []
        self.logical_transfer_bytes = 0; self.wire_transfer_bytes = 0
        self.direct_peer_bytes = 0; self.staged_transfer_bytes = 0
        self.host_bounce_bytes = 0; self.predicted_transfer_ns = 0

    def add(self, sequence: int, kind: str, resource_id: int, bank: str, *, route=(),
            intent="", mode="", nbytes=0, before=None, after=None, version=None,
            predicted_ns=0, route_class="", host_bounce=False, reason: str) -> None:
        generation = self.generations[resource_id]
        self.actions.append(HAMAction(
            len(self.actions) + 1, sequence, kind, resource_id, bank, tuple(route),
            intent, mode, nbytes, generation if before is None else before,
            generation if after is None else after,
            self.versions[resource_id] if version is None else version,
            predicted_ns, route_class, host_bounce, reason))
    ...
    def evict(self, resource_id: int, bank: str, sequence: int, reason: str,
        ...
        self.add(sequence, "invalidate" if invalidate else "evict", resource_id, bank,
                 nbytes=resource.nbytes, before=generation,
                 after=self.generations[resource_id], version=version, reason=reason)

    def make_space(self, bank: str, nbytes: int, sequence: int,
                   protect: frozenset[tuple[int, str]]) -> None:
        ...
        while self.used[bank] + nbytes > capacity:
            ...
            for resource_id, copies in self.copies.items():
                resource = self.resources[resource_id]
                if (bank not in copies or bank == resource.home_bank
                        or (resource_id, bank) in protect):
                    continue
                next_use = self._next_use(resource_id, bank, sequence)
                candidates.append((next_use is None, next_use or 0,
                                   -resource.priority, resource.nbytes, -resource_id,
                                   resource_id))
            if not candidates:
                raise ValueError(f"HAM bank {bank!r} cannot make {nbytes} bytes of space "
        ...
        for source, (copy_generation, _version) in self.copies[resource_id].items():
            ...
            try:
                path, cost = _shortest_route(
                    source, target, self.resources[resource_id].nbytes,
                    link_map=self.links, adjacency=self.adjacency,
    ...
    def ensure(self, resource_id: int, target: str, sequence: int, intent: str, *,
               protect_extra: frozenset[tuple[int, str]] = frozenset()) -> bool:
        generation = self.generations[resource_id]
        ...
        host_bounce = any(self.banks[name].channel.lower() == "host"
                          for name in route[1:-1])
        if len(route) > 2:
            route_class = "staged"
        elif (self.banks[route[0]].channel.lower() != "host"
              and self.banks[route[-1]].channel.lower() != "host"):
            route_class = "direct-peer"
        else:
            route_class = "direct-dma"
        self.add(sequence, "transfer", resource_id, target, route=route, intent=intent,
                 nbytes=resource.nbytes, predicted_ns=predicted_ns,
                 route_class=route_class, host_bounce=host_bounce,
                 reason=f"{intent}:{route[0]}->{target}")
        self.copies[resource_id][target] = (generation, self.versions[resource_id])
        self.used[target] = _checked_add(self.used[target], resource.nbytes,
                                         "HAM destination occupancy")
        ...
        if intent == "prefetch":
            self.add(sequence, "prefetch", resource_id, target, intent="prefetch",
                     nbytes=resource.nbytes, reason="lookahead:resident-before-use")
        return True

    def _snapshot(self):
        return ({resource_id: dict(copies) for resource_id, copies in self.copies.items()},
                dict(self.used), dict(self.peaks), len(self.actions),
                self.logical_transfer_bytes, self.wire_transfer_bytes,
                self.direct_peer_bytes, self.staged_transfer_bytes,
                self.host_bounce_bytes, self.predicted_transfer_ns)

    def _restore(self, snapshot) -> None:
        (self.copies, self.used, self.peaks, length,
         self.logical_transfer_bytes, self.wire_transfer_bytes,
         self.direct_peer_bytes, self.staged_transfer_bytes,
         self.host_bounce_bytes, self.predicted_transfer_ns) = snapshot
    ...
    def _prefetch(self, index: int) -> None:
        ...
        for future in self.workload.accesses[index + 1:stop]:
            for resource_id in self.required[future.sequence]:
                ...
                try:
                    self.ensure(resource_id, future.bank,
                                self.workload.accesses[index].sequence, "prefetch",
    ...
    def run(self) -> None:
        for index, access in enumerate(self.workload.accesses):
            ...
            if access.mode == "write":
                after = before + 1
                version = _sha({"previous_version": self.versions[access.resource_id],
                                "sequence": access.sequence,
                                "operation": access.operation})
            else:
                after = before; version = self.versions[access.resource_id]
            self.add(access.sequence, "barrier", access.resource_id, access.bank,
                     before=before, after=before, version=self.versions[access.resource_id],
                     reason="residency:complete-before-access")
            self.add(access.sequence, "access", access.resource_id, access.bank,
                     mode=access.mode, nbytes=resource.nbytes, before=before, after=after,
                     version=version, reason=access.operation)
            if access.mode == "write":
                ...
                for bank in sorted(tuple(self.copies[access.resource_id])):
                    if bank != access.bank:
                        self.evict(access.resource_id, bank, access.sequence,
                                   "write:invalidate-stale-copy", invalidate=True)
                if access.bank != resource.home_bank:
                    self.ensure(access.resource_id, resource.home_bank,
                                access.sequence, "writeback")
                    self.add(access.sequence, "barrier", access.resource_id,
                             resource.home_bank, reason="writeback:quiescent-home-copy")
...
def _route_properties(route: tuple[str, ...], nbytes: int, banks: dict,
                      links: dict) -> tuple[int, str, bool]:
    predicted = 0
    ...
    if len(route) > 2:
        route_class = "staged"
    elif (banks[route[0]].channel.lower() != "host"
          and banks[route[-1]].channel.lower() != "host"):
        route_class = "direct-peer"
    else:
        route_class = "direct-dma"
...
def _replay(actions: tuple[HAMAction, ...], peaks: tuple[HAMBankPeak, ...],
            summary: tuple[int, int, int, int, int, int], workload: HAMWorkload,
            hardware) -> list[str]:
    errors: list[str] = []
    try:
    ...
    copies = {row.resource_id: {row.home_bank: (0, row.content_sha256)}
              for row in workload.resources}
    used = {name: 0 for name in banks}; observed_peaks = dict(used)
    for row in workload.resources:
        used[row.home_bank] += row.nbytes
        observed_peaks[row.home_bank] = max(observed_peaks[row.home_bank],
                                            used[row.home_bank])
    access_index = 0
    logical = wire = direct = staged = bounced = predicted_total = 0
    required = {row.sequence: _closure(workload, row.resource_id)
                for row in workload.accesses}
    pending_access_barrier = None
    pending_transfer_marker = None
    for action in actions:
        if pending_access_barrier is not None and action.kind != "access":
            errors.append(f"action {action.order}: residency barrier is not followed "
        ...
        if pending_transfer_marker is not None:
            ...
            if (action.kind, action.resource_id, action.bank, action.sequence) != (
                    expected_kind, expected_resource, expected_bank, expected_sequence):
                errors.append(f"action {action.order}: transfer is missing its "
        ...
        if action.kind == "transfer":
            ...
            if (source_generation != generations[action.resource_id]
            ...
            try:
                ...
                errors.append(f"action {action.order}: {exc}"); continue
            if (expected_ns != action.predicted_ns or route_class != action.route_class
            ...
            for intermediate in action.route[1:-1]:
                ...
                if transient > banks[intermediate].allocatable_bytes:
                    errors.append(f"action {action.order}: transient capacity exceeded in "
                                  f"{intermediate!r}")
            ...
            copies[action.resource_id][destination] = (
                generations[action.resource_id], versions[action.resource_id])
            observed_peaks[destination] = max(observed_peaks[destination], used[destination])
            logical += resource.nbytes; wire += resource.nbytes * (len(action.route) - 1)
            predicted_total += expected_ns
            if route_class == "direct-peer": direct += resource.nbytes
            if route_class == "staged":
                staged += resource.nbytes
                if host_bounce: bounced += resource.nbytes
            if action.intent == "prefetch":
                pending_transfer_marker = ("prefetch", action.resource_id,
                                           destination, action.sequence)
            elif action.intent == "writeback":
                pending_transfer_marker = ("barrier", action.resource_id,
                                           destination, action.sequence)
            ...
            if access_index >= len(workload.accesses):
                ...
                continue
            expected = workload.accesses[access_index]; access_index += 1
            if pending_access_barrier != (
                    expected.sequence, expected.resource_id, expected.bank):
            ...
            if (action.sequence, action.resource_id, action.bank, action.mode, action.reason) != (
                    expected.sequence, expected.resource_id, expected.bank,
                    expected.mode, expected.operation):
            ...
            for dependency in required[expected.sequence]:
                if copies[dependency].get(expected.bank) != (
                        generations[dependency], versions[dependency]):
                    errors.append(f"action {action.order}: resource {dependency} is not current "
                                  f"in {expected.bank!r}")
            if action.generation_before != generations[action.resource_id]:
                errors.append(f"action {action.order}: access generation is stale")
            if action.mode == "write":
                expected_version = _sha({"previous_version": versions[action.resource_id],
                                         "sequence": action.sequence,
                                         "operation": action.reason})
                if (action.generation_after != generations[action.resource_id] + 1
                ...
                copies[action.resource_id][action.bank] = (
                    generations[action.resource_id], expected_version)
            elif (action.generation_after != generations[action.resource_id]
                  or action.version_sha256 != versions[action.resource_id]):
            ...
            if copy is None:
                ...
                if (copy_generation != action.generation_before
    ...
    for resource in workload.resources:
        ...
        if copies[resource.resource_id] != {resource.home_bank: expected}:
            errors.append(f"HAM plan does not finish with one current home copy of resource "
                          f"{resource.resource_id}")
    expected_peaks = tuple(HAMBankPeak(name, observed_peaks[name], banks[name].allocatable_bytes)
...
def verify_ham_plan(plan: HAMExecutionPlan, workload: HAMWorkload, hardware) -> tuple[str, ...]:
    ...
    errors = []
    if plan.hardware_digest != workload.hardware_digest \
            or plan.hardware_digest != getattr(hardware, "digest", None):
    ...
    errors += _replay(
        plan.actions, plan.peaks,
        (plan.logical_transfer_bytes, plan.wire_transfer_bytes,
         plan.direct_peer_bytes, plan.staged_transfer_bytes,
         plan.host_bounce_bytes, plan.predicted_transfer_ns), workload, hardware)
    return tuple(errors)


def _device_manifest(hardware) -> DeviceManifest:
    banks = tuple(MemoryBank(row.name, Domain[row.domain], row.capacity_bytes,
                             row.native_tile, row.alignment) for row in hardware.banks)
    distance = tuple(tuple(0 if left == right else 256 for right in range(len(banks)))
...
def _lower(actions: tuple[HAMAction, ...], workload: HAMWorkload, hardware):
    ...
    for (resource_id, bank_name), rid in pair_rids.items():
        semantic = resources[resource_id]; bank = hardware.bank(bank_name)
        module.add_resource(Resource(rid, Domain[bank.domain], 1, (semantic.nbytes,),
                                     align=bank.alignment, access="ham",
                                     priority=semantic.priority,
                                     name=f"ham:{resource_id}:{bank_name}"))
    previous_phase = 0; phase_id = 0; claim_id = 0
    ...
    def emit(claim: Claim, channel: str) -> None:
        ...
        previous_phase = phase_id; claim_channels[claim.id] = channel

    def cid() -> int:
        nonlocal claim_id
        claim_id += 1; return claim_id
    ...
    for action in actions:
        if action.kind == "transfer":
            ...
            emit(Claim(cid(), Opcode.LOAD, Lane.U, StrideClass.UNIT,
                       count=action.nbytes, rd=(rid,), domain=Domain[bank.domain],
                       bounds="assumed_safe", op="ham.prefetch", cost_class="bandwidth"),
                 bank.channel)
        elif action.kind == "barrier":
            bank = hardware.bank(action.bank)
            emit(Claim(cid(), Opcode.BARRIER, Lane.H, StrideClass.SCALAR,
                       hazard="barriered", domain=Domain[bank.domain],
                       bounds="assumed_safe", op="ham.barrier", cost_class="latency"),
                 bank.channel)
        elif action.kind == "access":
            rid = pair_rids[(action.resource_id, action.bank)]
            bank = hardware.bank(action.bank)
            opcode = Opcode.LOAD if action.mode == "read" else Opcode.STORE
            emit(Claim(cid(), opcode, Lane.U, StrideClass.UNIT, count=action.nbytes,
                       rd=(rid,), wr=() if action.mode == "read" else (rid,),
                       hazard="barriered" if action.mode == "write" else "unique",
                       domain=Domain[bank.domain], bounds="assumed_safe",
                       op=f"ham.{action.mode}:{action.reason}", cost_class="bandwidth"),
                 bank.channel)
        else:  # explicit eviction / stale-copy invalidation
            rid = pair_rids[(action.resource_id, action.bank)]
            bank = hardware.bank(action.bank)
            emit(Claim(cid(), Opcode.NOP, Lane.H, StrideClass.SCALAR,
                       rd=(rid,), domain=Domain[bank.domain], bounds="assumed_safe",
                       op=f"ham.{action.kind}", cost_class="bandwidth"), bank.channel)
...
def lower_ham_workload(workload: HAMWorkload, hardware) -> LoweredHAMExecution:
    ...
    planner = _Planner(workload, hardware); planner.run()
    peaks = tuple(HAMBankPeak(name, planner.peaks[name],
                              planner.banks[name].allocatable_bytes)
                  for name in sorted(planner.banks))
    summary = (planner.logical_transfer_bytes, planner.wire_transfer_bytes,
               planner.direct_peer_bytes, planner.staged_transfer_bytes,
               planner.host_bounce_bytes, planner.predicted_transfer_ns)
    ...
    action_digest = _sha([{**asdict(row), "route": list(row.route)}
                          for row in planner.actions])
    module_digest = hashlib.sha256(
        f"{hash_module(module)}:{action_digest}".encode()).hexdigest()
    artifact = HAMExecutionPlan(
        workload.hardware_digest, workload.digest, workload.policy_digest,
        tuple(planner.actions), peaks, *summary, module_digest,
        hashlib.sha256(pack_data).hexdigest(), len(pack_data))
...
__all__ = ["HAMAccess", "HAMAction", "HAMBankPeak", "HAMExecutionPlan", "HAMResource",
           "HAMWorkload", "LoweredHAMExecution", "lower_ham_workload", "verify_ham_plan"]
