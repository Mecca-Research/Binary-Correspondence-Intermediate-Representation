...
from .device_manifest import DeviceManifest, StridedView, check_strided_view, move_claim, move_cost
...
def dma_descriptors(src: StridedView, dst: StridedView, man: DeviceManifest) -> list:
    ...
    if tuple(src.shape) != tuple(dst.shape):
        raise ValueError(
            f"dma shape mismatch: src {tuple(src.shape)} != dst "
            f"{tuple(dst.shape)} (a truncated transfer is a lie)"
        )
    if src.elem_bytes != dst.elem_bytes:
        raise ValueError(
            f"dma element width mismatch: src {src.elem_bytes} != dst "
    ...
    if src.bank == dst.bank:
        ...
        if a0 < b1 and b0 < a1:
            raise ValueError(
                f"dma overlap on bank {src.bank}: src [{a0},{a1}) vs dst "
    ...
    while i < len(sruns) and j < len(druns):
        ...
        if top and top.src_off + top.nbytes == so + si and top.dst_off + top.nbytes == do + dj:
...
def dma_cost(
    ...
    *,
    ...
    if not isinstance(desc_setup, int) or isinstance(desc_setup, bool) or desc_setup < 0:
        raise ValueError(
            f"descriptor setup cost must be a non-negative integer; got {desc_setup!r}"
        )
    for index, desc in enumerate(descs):
        if (
            not isinstance(desc, Descriptor)
            or any(
...
def dma_transfer_module(
    man: DeviceManifest, src: StridedView, dst: StridedView, *, engine: str = "dma0"
) -> Module:
    ...
    for r in (
        Resource(
            rid=_SRCB, domain=man.bank(src.bank).domain, shape=(max(1, nbytes // eb),), name="SRCB"
        ),
        Resource(
            rid=_DSTB, domain=man.bank(dst.bank).domain, shape=(max(1, nbytes // eb),), name="DSTB"
        ),
    ...
    program = Claim(
        ...
        count=max(1, len(descs)),
        wr=(_RING,),
        op=f"dma.program:{src.bank}->{dst.bank}",
        domain=Domain.RAM,
    )
    ...
    kick = Claim(
        ...
        rd=(_RING,),
        wr=(_CTRL,),
        op=f"dma.kick:{engine}",
        domain=Domain.MMIO,
        hazard="atomic",
        volatile=True,
    )
    m.add_phase(Phase(phase_id=1, deps=(0,), claims=[unmask_claim(line, _CTRL, cid=2), kick]))
    ...
    m.add_phase(
        Phase(
            phase_id=2,
            deps=(1,),
            claims=[mask_claim(line, _CTRL, cid=4), poll, unmask_claim(line, _CTRL, cid=6)],
        )
    )
    return m
