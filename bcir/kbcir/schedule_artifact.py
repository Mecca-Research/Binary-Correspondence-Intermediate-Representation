...
def _plan_from_dict(value) -> TilePlan:
    if not isinstance(value, dict) or set(value) != {
            "tile_m", "tile_n", "tile_k", "loop_order", "compute_cost", "mem_cost",
            "bottleneck", "fits_cache"}:
    ...
    for field in ("tile_m", "tile_n", "tile_k", "compute_cost", "mem_cost", "bottleneck"):
        _positive(getattr(plan, field), field, zero=field != "tile_m" and field != "tile_n"
                  and field != "tile_k")
    if plan.loop_order not in _LOOP_ORDERS or type(plan.fits_cache) is not bool \
...
def target_fingerprint(target: TargetProfile) -> str:
    ...
    memory = [{"name": tier.name, "latency_cyc": tier.latency_cyc,
               "bw_factor": tier.bw_factor, "lat_factor": tier.lat_factor,
               "capacity": tier.capacity} for tier in target.mem.tiers]
    value = {field: getattr(target, field) for field in (
        "name", "triple", "cacheline", "elem_bytes", "lane_widths", "warp", "scalable",
        "gather_penalty", "mem_unit", "base_overhead", "thermal_density", "power_density",
        "per_op_heat", "fma", "affinity_domains", "mem_channels", "cal_gen")}
    value["lane_widths"] = list(value["lane_widths"])
    value["isa_features"] = sorted(target.isa_features); value["memory"] = memory
...
class MeasuredScheduleCandidate:
    ...
    def __post_init__(self) -> None:
        ...
        for field in ("wall_ns", "cpu_ns", "minor_faults", "major_faults",
    ...
    def to_dict(self) -> dict:
        value = asdict(self); value["plan"] = _plan_dict(self.plan); return value
...
class MeasuredCostConstants:
    ...
    def __post_init__(self) -> None:
        for field in ("ns_per_mac_q20", "cycles_per_instruction_q20",
                      "cache_misses_per_kmac_q20"):
            _positive(getattr(self, field), field, zero=True)
...
class ScheduleArtifact:
    ...
    def __post_init__(self) -> None:
        ...
        _plan_from_dict(_plan_dict(self.analytic_plan)); _plan_from_dict(_plan_dict(self.measured_plan))
        if not isinstance(self.candidates, (tuple, list)) or not self.candidates \
                or any(not isinstance(candidate, MeasuredScheduleCandidate)
    ...
    def to_dict(self) -> dict:
        return {"schema": self.schema, "workload": self.workload, "shape": list(self.shape),
                "target_name": self.target_name, "target_sha256": self.target_sha256,
                ...
                "constants": asdict(self.constants), "source_commit": self.source_commit}
    ...
    def analytic_regret_q20(self) -> int:
        ...
        return (measured[self.analytic_plan] << 20) // measured[self.measured_plan] \
            if self.analytic_plan in measured else 0


def measure_schedule_candidates(M: int, N: int, K: int, target: TargetProfile,
                                candidates, work, *, workload: str, source_commit: str,
                                repeats: int = 3) -> ScheduleArtifact:
    ...
    if not plans or any(not isinstance(plan, TilePlan) for plan in plans) \
    ...
    for plan in plans:
        ...
        for _ in range(repeats):
            sampler = CounterSampler(); work(plan); laps.append(sampler.lap())
        hardware = read_hw_counters(lambda: work(plan))
        evidence.append(MeasuredScheduleCandidate(
            plan, int(statistics.median(lap.wall_ns for lap in laps)),
            int(statistics.median(lap.cpu_ns for lap in laps)),
            sum(lap.minor_faults for lap in laps), sum(lap.major_faults for lap in laps),
            sum(lap.vol_ctx for lap in laps), sum(lap.invol_ctx for lap in laps),
    ...
    winner = min(evidence, key=lambda item: (
        item.wall_ns, item.cpu_ns, item.plan.loop_order, -item.plan.tile_m,
        -item.plan.tile_n, -item.plan.tile_k))
    macs = M * N * K
    constants = MeasuredCostConstants(
    ...
    return ScheduleArtifact(workload, (M, N, K), target.name, target_fingerprint(target),
                            platform.machine() or "unknown", analytic, winner.plan,
                            tuple(evidence), constants, source_commit)
...
def read_schedule_artifact(path: os.PathLike | str) -> ScheduleArtifact:
    value = strict_json_loads(read_bounded_text(str(path), "schedule artifact"),
                              "schedule artifact")
    expected = {"schema", "workload", "shape", "target_name", "target_sha256",
                "host_architecture", "analytic_plan", "measured_plan", "candidates",
                "constants", "source_commit"}
    if not isinstance(value, dict) or set(value) != expected or not isinstance(value["candidates"], list):
        raise ValueError("schedule artifact has missing or unknown fields")
    try:
        candidates = tuple(MeasuredScheduleCandidate(
            plan=_plan_from_dict(row.pop("plan")), **row) for row in value["candidates"]
                           if isinstance(row, dict))
        if len(candidates) != len(value["candidates"]):
            raise ValueError("malformed measured candidate")
        return ScheduleArtifact(
            workload=value["workload"], shape=tuple(value["shape"]),
            target_name=value["target_name"], target_sha256=value["target_sha256"],
            host_architecture=value["host_architecture"],
            analytic_plan=_plan_from_dict(value["analytic_plan"]),
            measured_plan=_plan_from_dict(value["measured_plan"]), candidates=candidates,
            constants=MeasuredCostConstants(**value["constants"]),
            source_commit=value["source_commit"], schema=value["schema"])
    except (KeyError, TypeError) as exc:
        raise ValueError(f"malformed schedule artifact: {exc}") from exc


def export_matmul_buffer_mlir(artifact: ScheduleArtifact, *, use_measured: bool = True,
                              function_name: str = "scheduled_matmul") -> str:
    if not isinstance(artifact, ScheduleArtifact) or not isinstance(function_name, str) \
            or not function_name.replace("_", "").isalnum():
        raise ValueError("artifact or MLIR function name is invalid")
    M, N, K = artifact.shape; plan = artifact.measured_plan if use_measured else artifact.analytic_plan
    return (f"func.func @{function_name}(%A: memref<{M}x{K}xf32>, "
