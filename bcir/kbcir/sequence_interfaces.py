...
class AlignedTokenChunk:
    ...
    def __post_init__(self) -> None:
        ...
        _integer(self.student_end, "student_end", minimum=self.student_start + 1,
                 maximum=_MAX_SEQUENCE_TOKENS)
        _integer(self.teacher_end, "teacher_end", minimum=self.teacher_start + 1,
                 maximum=_MAX_SEQUENCE_TOKENS)
        _integer(self.byte_length, "byte_length", minimum=1,
                 maximum=_MAX_SEQUENCE_BYTES)
...
def align_decoded_tokens(student, teacher, *,
                         max_bytes: int = _MAX_SEQUENCE_BYTES) -> tuple[AlignedTokenChunk, ...]:
    ...
    while student_index < len(student) and teacher_index < len(teacher):
        ...
        chunks.append(AlignedTokenChunk(
            student_start, student_index, teacher_start, teacher_index,
            len(student_payload), hashlib.sha256(student_payload).hexdigest()))
...
class CrossTokenizerProjection:
    ...
    def __post_init__(self) -> None:
        if not self.projected_log_probabilities or \
                len(self.projected_log_probabilities) != len(self.advantages):
            raise ValueError("cross-tokenizer projection arrays are malformed")
        for field in ("projected_log_probabilities", "advantages",
...
def project_aligned_log_probabilities(
        chunks: Sequence[AlignedTokenChunk], student_log_probabilities: Sequence[float],
        teacher_log_probabilities: Sequence[float]) -> CrossTokenizerProjection:
    ...
    if not isinstance(student_log_probabilities, (tuple, list)) \
    ...
    if not isinstance(teacher_log_probabilities, (tuple, list)) \
    ...
    student_logs = tuple(_finite(value, "student log probability", maximum=0.0)
                         for value in student_log_probabilities)
    teacher_logs = tuple(_finite(value, "teacher log probability", maximum=0.0)
                         for value in teacher_log_probabilities)
    if chunks[0].student_start != 0 or chunks[0].teacher_start != 0 \
    ...
    for chunk in chunks:
        ...
        if not math.isclose(math.fsum(projected[chunk.student_start:chunk.student_end]),
                            teacher_sum, rel_tol=1e-12, abs_tol=1e-12):
    ...
    return CrossTokenizerProjection(
        tuple(projected), tuple(advantages), tuple(student_sums), tuple(teacher_sums),
        _digest(identity))
...
def thunder_candidate_score(delta_log_likelihood: float,
                            delta_branching_entropy: float) -> float:
...
def segment_unigram(payload: bytes, pieces: Sequence[UnigramPiece], *,
                    max_pieces: int = 65535) -> tuple[int, ...]:
    ...
    if not isinstance(pieces, (tuple, list)) or not pieces \
    ...
    if len(set(token_ids)) != len(token_ids) or len({piece.payload for piece in pieces}) != len(pieces):
    ...
    for start in range(len(payload)):
        ...
        for piece in by_first.get(payload[start], ()):
            ...
            if previous_score is None or candidate_score > previous_score \
                    or candidate_score == previous_score and candidate_count < counts[end] \
                    or candidate_score == previous_score and candidate_count == counts[end] \
...
class ExpansionRow:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.source_token_ids, tuple) or not self.source_token_ids \
...
class ContinuedBPEExpansionPlan:
    ...
    def __post_init__(self) -> None:
        _integer(self.base_vocab_size, "base_vocab_size", minimum=1,
                 maximum=(1 << 31) - 1)
        _integer(self.source_vocab_size, "source_vocab_size",
                 minimum=self.base_vocab_size, maximum=(1 << 31) - 1)
        _integer(self.target_vocab_size, "target_vocab_size",
                 minimum=self.source_vocab_size + 1, maximum=(1 << 31) - 1)
        if not isinstance(self.special_token_ids, tuple) \
                or any(type(token) is not int or not 0 <= token < self.base_vocab_size
        ...
        if not isinstance(self.new_rows, tuple) \
        ...
        if any(any(token >= self.source_vocab_size for token in row.source_token_ids)
               for row in self.new_rows):
            raise ValueError("new token decomposition must terminate in the source vocabulary")
        decomposition_elements = 0
        for row in self.new_rows:
            decomposition_elements = _checked_add(
                decomposition_elements, len(row.source_token_ids))
...
def plan_continued_bpe_expansion(source_merges, target_merges, *,
                                 base_vocab_size: int,
                                 special_token_ids: Sequence[int] = ()) \
        -> ContinuedBPEExpansionPlan:
    """Prove prefix-preserving BPE continuation and derive each new row's source mean."""
    _integer(base_vocab_size, "base_vocab_size", minimum=1,
             maximum=(1 << 31) - 1)
    ...
    if any(type(token) is not int or not 0 <= token < base_vocab_size for token in special) \
    ...
    def validate(rows: tuple[tuple[int, int, int], ...], field: str) -> None:
        ...
        for index, (left, right, token) in enumerate(rows):
            if left not in known or right not in known \
                    or token != base_vocab_size + index:
    ...
    source_sha = _digest({"base_vocab_size": base_vocab_size, "merges": source,
                          "special_token_ids": special})
    target_sha = _digest({"base_vocab_size": base_vocab_size, "merges": target,
                          "special_token_ids": special})
    identity = {"source": source_sha, "target": target_sha,
                "rows": [asdict(row) for row in rows]}
    return ContinuedBPEExpansionPlan(
        base_vocab_size, source_vocab_size, base_vocab_size + len(target), special,
        tuple(rows), source_sha, target_sha, _digest(identity))
...
class TokenizerExpansionCost:
    ...
    def __post_init__(self) -> None:
        for field in ("source_vocab_size", "target_vocab_size", "added_parameter_bytes",
                      "additional_head_macs_per_token", "source_tokens_per_unit_q20",
                      "target_tokens_per_unit_q20", "source_head_macs_per_unit",
...
def assess_tokenizer_expansion(plan: ContinuedBPEExpansionPlan, *, hidden_width: int,
                               bytes_per_element: int, tied_head: bool,
                               source_tokens_per_unit_q20: int,
                               target_tokens_per_unit_q20: int) -> TokenizerExpansionCost:
    ...
    source_macs = _checked_mul(plan.source_vocab_size, hidden_width,
                               source_tokens_per_unit_q20) // _Q20
    target_macs = _checked_mul(plan.target_vocab_size, hidden_width,
                               target_tokens_per_unit_q20) // _Q20
    identity = {
        "plan": plan.plan_sha256, "hidden_width": hidden_width,
        "bytes_per_element": bytes_per_element, "tied_head": tied_head,
    ...
    return TokenizerExpansionCost(
        plan.source_vocab_size, plan.target_vocab_size, parameter_bytes,
        additional_head_macs, source_tokens_per_unit_q20,
        target_tokens_per_unit_q20, source_macs, target_macs,
        target_macs < source_macs, _digest(identity))
...
class SequenceInterfaceEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        for field in ("artifact_bytes", "encode_ns", "decode_ns",
                      "reconstruction_error_q20"):
            _integer(getattr(self, field), field)
...
def pareto_sequence_interfaces(evidence: Sequence[SequenceInterfaceEvidence]) \
    ...
    def dominates(left: SequenceInterfaceEvidence,
                  right: SequenceInterfaceEvidence) -> bool:
        left_min = (left.tokens_per_unit_q20, left.artifact_bytes, left.encode_ns,
                    left.decode_ns, left.reconstruction_error_q20)
        right_min = (right.tokens_per_unit_q20, right.artifact_bytes, right.encode_ns,
                     right.decode_ns, right.reconstruction_error_q20)
        left_max = (left.linear_probe_q20, left.nonlinear_probe_q20,
                    left.domain_probe_q20, int(left.exact_roundtrip))
        right_max = (right.linear_probe_q20, right.nonlinear_probe_q20,
                     right.domain_probe_q20, int(right.exact_roundtrip))
        no_worse = all(a <= b for a, b in zip(left_min, right_min)) \
            and all(a >= b for a, b in zip(left_max, right_max))
        strictly = any(a < b for a, b in zip(left_min, right_min)) \
            or any(a > b for a, b in zip(left_max, right_max))
...
class CausalSeriesCodecSpec:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.quantizer, FiniteScalarQuantizerSpec) \
        ...
        _integer(self.max_values, "max_values", minimum=self.warmup_values,
                 maximum=_MAX_SEQUENCE_TOKENS)
    ...
    @property
    def digest(self) -> str:
        return _digest({"schema": "bcir.causal_series_codec.v1",
...
class CausalSeriesEncoding:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.tokens, tuple) or not isinstance(self.token_ends, tuple) \
                or not isinstance(self.reconstructed_values, tuple) \
                or not self.tokens or not self.token_ends or not self.reconstructed_values \
        ...
        if self.token_ends[-1] != len(self.tokens) \
                or any(left >= right for left, right in zip((0,) + self.token_ends,
                                                            self.token_ends)):
...
def encode_causal_series(values: Sequence[float], spec: CausalSeriesCodecSpec) \
    ...
    for index, value in enumerate(source):
        if index < spec.warmup_values:
            ...
            reconstructed_value = _float32(
                mean + decoded * scale * spec.normalized_clip,
                f"reconstructed_values[{index}]")
    ...
    identity = {"spec": spec.digest, "tokens": tokens,
                "token_ends": ends,
                "reconstructed": [value.hex() for value in reconstructed]}
    return CausalSeriesEncoding(tuple(tokens), tuple(ends), tuple(reconstructed),
                                max(errors), _digest(identity))


def decode_causal_series(tokens: Sequence[int], spec: CausalSeriesCodecSpec) \
        -> tuple[float, ...]:
    if not isinstance(spec, CausalSeriesCodecSpec):
        raise ValueError("spec must be CausalSeriesCodecSpec")
...
class FrozenTokenCodeSpec:
    ...
    def __post_init__(self) -> None:
        ...
        _integer(self.model_width, "model_width", minimum=self.code_bits,
                 maximum=1 << 16)
    ...
    def code(self, token_id: int) -> tuple[float, ...]:
        ...
        bits = tuple(1.0 if token_id & (1 << bit) else -1.0
...
class GrowthStage:
    ...
    def __post_init__(self) -> None:
        ...
        _integer(self.new_block_end, "new_block_end", minimum=self.new_block_start,
        ...
        for field in ("trainable_blocks", "lora_layers"):
            ...
            if not isinstance(values, tuple) or any(
                    ...
                    for layer in values) or len(set(values)) != len(values):
        ...
        if self.kind == "dense_growth":
            if self.new_block_start == self.new_block_end \
                    or self.trainable_blocks != tuple(
                        range(self.new_block_start, self.new_block_end)) \
                    or self.lora_layers or self.lora_rank:
                raise ValueError("dense growth must train exactly its newly added blocks")
        elif self.trainable_blocks or self.lora_layers != tuple(range(self.total_layers)) \
                or self.lora_rank < 1 or self.new_block_start != self.new_block_end:
...
class ProgressiveGrowthSpec:
    ...
    def __post_init__(self) -> None:
        ...
        _integer(self.final_layers, "final_layers", minimum=self.initial_layers,
                 maximum=1024)
        _integer(self.growth_layers, "growth_layers", minimum=1, maximum=1024)
        for field in ("block_parameter_elements", "head_parameter_elements",
                      "fixed_interface_elements", "interface_width",
        ...
        _integer(self.optimizer_state_bytes_per_parameter,
                 "optimizer_state_bytes_per_parameter", minimum=1, maximum=64)
        _integer(self.lora_rank, "lora_rank", maximum=1 << 16)
        _integer(self.lora_elements_per_rank_per_layer,
                 "lora_elements_per_rank_per_layer")
    ...
    def stages(self) -> tuple[GrowthStage, ...]:
        ...
        while current < self.final_layers:
            added = self.initial_layers if current == 0 else min(
                self.growth_layers, self.final_layers - current)
            end = current + added
            active = _checked_add(_checked_mul(added, self.block_parameter_elements),
            ...
            rows.append(GrowthStage(
                stage_index, "dense_growth", end, current, end,
                tuple(range(current, end)), (), 0, active,
            ...
            if self.lora_rank and current > self.initial_layers:
                ...
                rows.append(GrowthStage(
                    stage_index, "lora_readjustment", current, current, current,
                    (), tuple(range(current)), self.lora_rank, lora_active,
                    _checked_mul(lora_active,
    ...
    def inference_elements(self) -> int:
        return _checked_add(self.fixed_interface_elements,
                            _checked_mul(self.final_layers, self.block_parameter_elements),
                            self.head_parameter_elements)

    @property
    def digest(self) -> str:
        return _digest({"schema": "bcir.progressive_growth.v1", **asdict(self),
...
def lower_growth_stage(spec: ProgressiveGrowthSpec, stage_index: int, *,
                       sequence_length: int, batch: int = 1,
                       target: TargetProfile | None = None) -> LoweredGrowthStage:
    ...
    frozen = base_elements if stage.kind == "lora_readjustment" else max(
        1, base_elements - stage.active_parameter_elements)
    ...
    module.add_resource(Resource(2, Domain.RAM, 4,
                                 (spec.fixed_interface_elements,), name="fixed_interface"))
    ...
    module.add_resource(Resource(5, Domain.RAM, 1,
                                 (max(1, stage.optimizer_state_bytes),),
    ...
    module.add_resource(Resource(7, Domain.RAM, 4, (rows, vocabulary),
                                 name="loss_gradient"))
    ...
    emit(Opcode.LOAD, Lane.GGG, StrideClass.RANDOM,
         ...
         reads=(1, 2), writes=(6,), op="model.interface.fixed_lookup",
         cost_class="bandwidth")
    if frozen > 1:
        emit(Opcode.T_MACC, Lane.T, StrideClass.TILE,
             count=_checked_mul(rows, frozen), reads=(3, 6), writes=(6,),
             op="model.growth.forward.frozen", cost_class="compute")
    stage_op = "model.growth.forward.new_blocks" if stage.kind == "dense_growth" \
        else "model.growth.forward.lora_adapters"
    emit(Opcode.T_MACC, Lane.T, StrideClass.TILE,
         count=_checked_mul(rows, active), reads=(4, 6), writes=(6,),
         op=stage_op, cost_class="compute")
    emit(Opcode.T_MACC, Lane.T, StrideClass.TILE,
         count=_checked_mul(rows, active), reads=(1, 3, 4, 6), writes=(7,),
         op="model.growth.backward.active_only", cost_class="compute")
    emit(Opcode.ADD, Lane.U, StrideClass.UNIT, count=active,
         reads=(4, 5, 7), writes=(4, 5), op="model.growth.optimizer.update",
         cost_class="compute")
    emit(Opcode.BARRIER, Lane.H, StrideClass.SCALAR, count=1,
         reads=(4, 5), writes=(), op="model.growth.stage.freeze_boundary",
         cost_class="latency")
    ...
    data = encode(pack)
    artifact = _digest({"spec": spec.digest, "stage": asdict(stage),
...
__all__ = [
    "AlignedTokenChunk", "CausalSeriesCodecSpec", "CausalSeriesEncoding",
    "ContinuedBPEExpansionPlan", "CrossTokenizerProjection", "DecodedToken",
    "ExpansionRow", "FiniteScalarQuantizerSpec", "FrozenTokenCodeSpec",
    "GrowthStage", "LoweredGrowthStage", "ProgressiveGrowthSpec",
    "SequenceInterfaceEvidence", "TokenizerExpansionCost", "UnigramPiece",
    "align_decoded_tokens", "assess_tokenizer_expansion",
    "canonical_substring_candidate", "decode_causal_series", "encode_causal_series",
    "lower_growth_stage", "pareto_sequence_interfaces",
    "plan_continued_bpe_expansion", "project_aligned_log_probabilities",
    "segment_unigram", "thunder_candidate_score",
]
