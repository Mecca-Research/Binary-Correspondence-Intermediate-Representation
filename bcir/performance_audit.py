...
def _validate_result(value, field: str = "result"):
    ...
    if isinstance(value, dict):
        ...
        return {key: _validate_result(value[key], f"{field}.{key}") for key in sorted(value)}
...
def _case_phase_dag(scale: int) -> dict:
    ...
    return {
        "claims": count,
        "diagnostics": 0,
        "first_phase": result.phase_order[0],
        "last_phase": result.phase_order[-1],
        "order_sum": sum(result.order),
    }
...
def _case_gem_schedulers(scale: int) -> dict:
    ...
    for index in range(count):
        module.add_resource(
            Resource(1000 + index, Domain.RAM, shape=(17 + index % 13,), name=f"out{index}")
        )
    claims = []
    for index in range(count):
    ...
    return {
        "claims": count,
        "waves": len(waves.waves),
        "max_parallelism": waves.max_parallelism(),
        "await_edges": edge_count,
        "makespan": eft.makespan,
        "slot_checksum": sum(
            (slot.claim_id * 31 + slot.start * 7 + slot.finish) for slot in eft.slots
        ),
    }
...
def _case_kbcir_streampack(scale: int) -> dict:
    ...
    return {
        ...
        "prefetches": len(pack.prefetches),
        "blocks": len(pack.blocks),
        "trace_notes": len(pack.trace_notes),
    }
...
def _case_static_memory(scale: int) -> dict:
    ...
    phases = [Phase(index, () if index == 0 else (index - 1,), []) for index in range(phase_count)]
    claim_id = 1
    ...
    return {
        "resources": resources,
        "phases": phase_count,
        "extent_bytes": bank.extent_bytes,
        "naive_bytes": bank.naive_bytes,
...
def _case_lowbit(scale: int) -> dict:
    ...
    q4 = PackedQ4Tensor.from_values(
        values, (count,), source_sha256=source, calibration_sha256=calibration
    )
    ...
    return {
        "elements": count,
        "q8_groups": len(q8),
        "q4_bytes": len(q4.packed_codes),
        "q8_error_q9": _q(max(abs(left - right) for left, right in zip(values, restored))),
...
def _case_unsupervised(scale: int) -> dict:
    ...
    from .kbcir.unsupervised import (
        EmbeddingTable,
        embedding_lookup,
        kmeans_fit,
        kmeans_inertia,
    ...
    for index in range(samples):
        ...
        data.extend(
            cluster * 4.0 + ((index * 13 + feature * 7) % 17) / 17.0 for feature in range(features)
        )
    centroids, labels = kmeans_fit(data, samples, features, clusters, list(range(clusters)), 6)
    inertia = kmeans_inertia(data, samples, features, centroids, labels)
    model = KnnModel(
        tuple(data), tuple(float(index % clusters) for index in range(samples)), features
    )
    ...
    return {
        ...
        "features": features,
        "clusters": clusters,
        "inertia_q9": _q(inertia),
        "label_checksum": sum(labels),
        "neighbour_checksum": sum(neighbours),
        "standardized_sum_q9": _q(sum(standardized)),
        "embedding_values": len(embedded),
        "embedding_sum_q9": _q(sum(embedded)),
    }
...
def _case_ols_pca(scale: int) -> dict:
    ...
    for row in range(rows):
        design.extend(
            (1.0 if row % columns == column else 0.0) + ((row * 11 + column * 5) % 19) / 100.0
            for column in range(columns)
        )
    truth = [float(index + 1) for index in range(columns)]
    rhs = [
        sum(design[row * columns + column] * truth[column] for column in range(columns))
        for row in range(rows)
    ]
    estimate = ols_reference(design, rhs, rows, columns)
    ols_residual = normal_equation_residual(design, estimate, rhs, rows, columns)
    observations = [
        ((row * 17 + column * 29) % 101) / 37.0 + column * (row % 7) / 50.0
        for row in range(rows)
        for column in range(columns)
    ]
    ...
    return {
        "rows": rows,
        "columns": columns,
        "ols_residual_q15": _q(ols_residual, 15),
...
def _case_transformer(scale: int) -> dict:
    ...
    matrix = lambda count, shift: [
        ((index * 13 + shift) % 23 - 11) / 128.0 for index in range(count)
    ]
    params = TransformerBlockParams(
        matrix(d_model * d_model, 1),
        matrix(d_model * d_model, 2),
        matrix(d_model * d_model, 3),
        matrix(d_model * d_model, 4),
        matrix(d_model * d_ff, 5),
        [0.0] * d_ff,
        matrix(d_ff * d_model, 6),
        [0.0] * d_model,
        [1.0] * d_model,
        [0.0] * d_model,
        [1.0] * d_model,
    ...
    output = transformer_block_reference(
        _values(spec.token_count), spec, params, activation="gelu", pre_ln=True
    )
...
def _case_recurrent(scale: int) -> dict:
    ...
    bias = [0.0] * hidden_dim
    lstm = LstmParams(
        *(list(input_weights), list(recurrent_weights), list(bias)) * 4, input_dim, hidden_dim
    )
    gru = GruParams(
        *(list(input_weights), list(recurrent_weights), list(bias)) * 3, input_dim, hidden_dim
    )
    sequence = _values(steps * input_dim, modulus=43, divisor=64.0)
    hidden, cell = lstm_unroll(sequence, [0.0] * hidden_dim, [0.0] * hidden_dim, lstm)
    ...
    return {
        ...
        "lstm_hidden_q9": _q(sum(hidden)),
        "lstm_cell_q9": _q(sum(cell)),
...
def _case_training(scale: int) -> dict:
    ...
    if (
        not result.train_loss
        or result.train_loss[-1] >= result.train_loss[0]
...
def _case_hardware_search(scale: int) -> dict:
    ...
    priors = {candidate: Q20 + index * 97 for index, candidate in enumerate(candidates)}
    utility = {
        candidate: Q20 - abs(index - 7) * (Q20 // 16) for index, candidate in enumerate(candidates)
    }
    result = bounded_mcts(candidates, priors, utility.__getitem__, simulations=64 * scale)
    if (
        result.candidate_id not in candidates
        or sum(row.visits for row in result.visits) != 64 * scale
    ):
    ...
    return {
        ...
        "visit_checksum": sum((index + 1) * row.visits for index, row in enumerate(result.visits)),
    }
...
def _case_definitions(scale: int):
    return (
        ...
        ("ml-unsupervised", "kmeans-knn-scaler-embedding", 256 * scale, _case_unsupervised),
...
def _measure(group: str, name: str, items: int, function, scale: int, repeats: int) -> AuditSample:
...
def run_tmsao_audit(*, scale: int = 1, repeats: int = 3) -> TMSAOAuditReport:
    ...
    samples = tuple(
        _measure(group, name, items, function, scale, repeats)
        for group, name, items, function in definitions
    )
    correctness = [
        {"group": row.group, "name": row.name, "result_sha256": row.result_sha256}
        for row in samples
    ]
    host = {
    ...
    return TMSAOAuditReport(host, scale, repeats, _sha(definition), _sha(correctness), samples)
...
def main(argv: list[str] | None = None, prog: str = "bcir-performance-audit") -> int:
    ...
    print(
        f"performance audit: {len(report.samples)} cases; "
...
def main_compat(argv: list[str] | None = None) -> int:
    ...
    print(
        "bcir-tmsao-audit: GEM+/TMSAO certificates are not implemented; "
        + _NOT_A_CERTIFICATE
        + ". Use bcir-performance-audit.",
        file=sys.stderr,
    )
