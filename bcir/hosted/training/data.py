...
class DataPreparationSpec:
    ...
    def __post_init__(self) -> None:
        if (
            not isinstance(self.allowed_licenses, (tuple, list))
        ...
        require_int(self.max_document_bytes, "max_document_bytes", minimum=1, maximum=1 << 30)
        require_int(self.validation_permyriad, "validation_permyriad", minimum=0, maximum=9999)
...
def prepare_corpus(documents: Iterable[RawDocument], spec: DataPreparationSpec) -> PreparedCorpus:
    ...
    for index, document in enumerate(documents):
        ...
        input_digest.update(canonical_json(asdict(document)).encode("utf-8"))
        ...
        split_value = int(
            hashlib.sha256(
                f"{spec.split_salt}\0{document.document_id}\0{content_sha}".encode()
            ).hexdigest()[:16],
            16,
        )
...
def write_prepared_corpus(corpus: PreparedCorpus, output_dir: os.PathLike | str) -> Path:
    ...
    try:
        ...
        manifest = {
            "schema": "bcir.prepared_corpus.v1",
            "files": files,
