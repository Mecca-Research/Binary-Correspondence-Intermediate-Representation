...
class SearchBudget:
    ...
    def __post_init__(self) -> None:
        for field in ("rounds", "candidates_per_round", "max_candidate_tokens", "max_total_tokens"):
...
class ReasoningCandidate:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self, "trace_ids", token_tuple(self.trace_ids, "trace_ids", allow_empty=True)
        )
    ...
    def digest(self) -> str:
        raw = canonical_json(
            {"trace_ids": self.trace_ids, "answer_ids": self.answer_ids, "producer": self.producer}
        )
...
def bounded_reasoning_search(
    ...
    for round_index in range(budget.rounds):
        ...
        if (
            ...
            or len(candidates) > budget.candidates_per_round
        ...
        for candidate in candidates:
            ...
            if (
                candidate.token_count > budget.max_candidate_tokens
    ...
    return SearchResult(
        candidate, verdict, proposed, verified, rejected, duplicates, total, exhausted
    )
