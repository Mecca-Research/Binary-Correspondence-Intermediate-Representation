...
try:
    ...
    raise ModuleNotFoundError(
        "sequence-interface probes require PyTorch; install bcir[model-lab]"
    ) from exc
...
def _validate_float_rows(rows: torch.Tensor, expected_rows: int, field: str) -> None:
    if (
        ...
        or rows.shape[1] < 1
        or rows.numel() > _MAX_ROW_ELEMENTS
...
@torch.no_grad()
def initialize_expanded_rows(
    source_rows: torch.Tensor, plan: ContinuedBPEExpansionPlan
) -> torch.Tensor:
...
class ExpandedRowTable(nn.Module):
    ...
    def __init__(self, source_rows: torch.Tensor, plan: ContinuedBPEExpansionPlan):
        ...
        self.plan = plan
        self.source_rows = nn.Parameter(
            expanded[: plan.source_vocab_size].clone(), requires_grad=False
        )
...
def clipped_cross_tokenizer_opd_loss(
    ...
    projection: CrossTokenizerProjection,
    *,
    ...
    if (
        not isinstance(current_student_log_probabilities, torch.Tensor)
    ...
    if (
        ...
        or not isinstance(clip_epsilon, (int, float))
        or not math.isfinite(float(clip_epsilon))
        or not 0.0 < clip_epsilon < 1.0
    ):
        raise ValueError("clip_epsilon must be finite and in (0, 1)")
    advantages = torch.tensor(
        projection.advantages,
        dtype=current_student_log_probabilities.dtype,
        device=current_student_log_probabilities.device,
    )
    ratio = torch.exp(current_student_log_probabilities - old_student_log_probabilities.detach())
...
class _GrowthBlock(nn.Module):
    def __init__(self, width: int, heads: int, ff_width: int):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(width, ff_width, bias=False),
...
class HostedProgressiveLanguageModel(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        block_elements = (
            4 * interface.model_width * interface.model_width
            + 2 * interface.model_width * ff_width
            + 4 * interface.model_width
        )
        head_elements = 2 * interface.model_width + interface.vocab_size * interface.model_width
        parameter_elements = max_layers * block_elements + head_elements
        if interface_elements > _MAX_ROW_ELEMENTS or position_elements > _MAX_ROW_ELEMENTS:
        ...
        codebook = torch.tensor(
            [interface.code(token) for token in range(interface.vocab_size)], dtype=torch.float32
        )
        ...
        frequencies = torch.exp(
            -math.log(10000.0) * (2 * torch.floor(dimensions / 2)) / interface.model_width
        )
        ...
        self.blocks = nn.ModuleList(
            _GrowthBlock(interface.model_width, heads, ff_width) for _ in range(max_layers)
        )
    ...
    def block_parameter_elements(self) -> int:
        counts = [
            sum(parameter.numel() for parameter in block.parameters()) for block in self.blocks
        ]
        if len(set(counts)) != 1:
    ...
    def head_parameter_elements(self) -> int:
        return sum(parameter.numel() for parameter in self.final_norm.parameters()) + sum(
            parameter.numel() for parameter in self.lm_head.parameters()
        )
    ...
    def _validate_ids(self, input_ids: torch.Tensor) -> None:
        if (
            ...
            or input_ids.shape[1] > self.context_length
    ...
    def activate_stage(self, growth: ProgressiveGrowthSpec, stage_index: int) -> GrowthStage:
        if (
            not isinstance(growth, ProgressiveGrowthSpec)
        ...
        if (
            growth.final_layers > len(self.blocks)
...
@torch.no_grad()
def _growth_loss(
    model: HostedProgressiveLanguageModel, input_ids: torch.Tensor, targets: torch.Tensor
) -> float:
...
def train_progressive_growth_stage(
    ...
    if (
        not isinstance(model, HostedProgressiveLanguageModel)
