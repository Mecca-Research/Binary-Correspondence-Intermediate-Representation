...
try:
    ...
    raise ModuleNotFoundError(
        "hosted alignment stages require PyTorch; install bcir[model-lab]") from exc
...
from .contracts import (PPOExample, PreferenceExample, ReasoningExample, SFTExample,
                        StageRunReport, StageTrainEvent, StageTrainSpec, canonical_json,
                        sha256_text)
...
def _optimizer(spec: StageTrainSpec, parameters):
    return torch.optim.AdamW(parameters, lr=spec.learning_rate,
...
def _validate_model_and_spec(model: HostedLlama, spec: StageTrainSpec,
...
def _emit(sink, stage: str, step: int, loss: float, norm: float,
...
def _mean_sft_loss(model: HostedLlama, examples) -> float:
    ...
    model.eval()
    try:
        value = sum(float(_sft_loss(model, example).item())
...
def train_sft(model: HostedLlama, examples, spec: StageTrainSpec,
    ...
    with _deterministic_stage(spec.seed):
        ...
        for step in range(spec.steps):
            ...
            for _ in range(count):
                ...
                cursor += 1; running += float(loss.detach().item())
                loss.backward()
        ...
        return StageRunReport(spec.stage, spec.digest, _input_digest(examples), initial, final,
                              spec.steps, len(examples), _module_digest(model), _EMPTY_DIGEST)
...
class HostedRewardModel(nn.Module):
    ...
    def __init__(self, policy: HostedLlama):
        ...
        self.score = nn.Linear(policy.spec.d_model, 1, bias=False,
...
class HostedValueModel(nn.Module):
    ...
    def __init__(self, policy: HostedLlama):
        ...
        self.policy = copy.deepcopy(policy)
        self.value = nn.Linear(policy.spec.d_model, 1, bias=False,
                               device=_device(policy))
...
class HostedEmbeddingStudent(nn.Module):
    ...
    def __init__(self, policy: HostedLlama, output_dim: int):
        ...
        self.policy = copy.deepcopy(policy)
        self.projection = nn.Linear(policy.spec.d_model, output_dim, bias=False,
                                    device=_device(policy))
...
def train_embedding_distillation(model: HostedEmbeddingStudent, sequences,
                                 relational_targets, spec: StageTrainSpec,
                                 telemetry_sink=None) -> StageRunReport:
    if not isinstance(model, HostedEmbeddingStudent) or not isinstance(spec, StageTrainSpec) \
    ...
    if any(not sequence or any(type(token) is not int or token < 0
                               or token >= model.policy.spec.vocab_size for token in sequence)
           for sequence in sequences):
        raise ValueError("embedding sequences contain invalid token IDs")
    if not isinstance(relational_targets, (tuple, list)) or len(relational_targets) != len(sequences) \
            or any(not isinstance(row, (tuple, list)) or len(row) != len(sequences)
    ...
    if any(not math.isfinite(value) or not -1.0 <= value <= 1.0
           for row in targets for value in row):
    ...
    with _deterministic_stage(spec.seed):
        with torch.no_grad(): initial = float(_embedding_loss(model, sequences, targets).item())
        ...
        for step in range(spec.steps):
            ...
            loss = _embedding_loss(model, sequences, targets); loss.backward()
            norm = torch.nn.utils.clip_grad_norm_(model.parameters(), spec.grad_clip)
            if not torch.isfinite(norm):
                raise RuntimeError("embedding distillation produced a non-finite gradient norm")
            optimizer.step()
            _emit(telemetry_sink, "embedding", step + 1, float(loss.detach().item()),
                  float(norm.item()), (step + 1) * len(sequences))
        with torch.no_grad(): final = float(_embedding_loss(model, sequences, targets).item())
        if not math.isfinite(final):
            raise RuntimeError("embedding distillation produced a non-finite final loss")
        return StageRunReport("embedding", spec.digest, input_sha, initial, final, spec.steps,
                              len(sequences), _module_digest(model), _EMPTY_DIGEST)
...
def _mean_reward_loss(model, examples) -> float:
    ...
    model.eval()
    try:
        value = sum(float(_reward_loss(model, example).item())
                    for example in examples) / len(examples)
    finally:
        model.train(was_training)
    if not math.isfinite(value):
...
def train_reward_model(model: HostedRewardModel, examples, spec: StageTrainSpec,
                       telemetry_sink=None) -> StageRunReport:
    if not isinstance(model, HostedRewardModel) or not isinstance(spec, StageTrainSpec) \
    ...
    with _deterministic_stage(spec.seed):
        ...
        optimizer = _optimizer(spec, model.parameters()); cursor = 0; model.train()
        for step in range(spec.steps):
            optimizer.zero_grad(set_to_none=True); running = 0.0
            count = spec.batch_size * spec.gradient_accumulation
            for _ in range(count):
                loss = _reward_loss(model, examples[cursor % len(examples)]) / count
                cursor += 1; running += float(loss.detach().item()); loss.backward()
            norm = torch.nn.utils.clip_grad_norm_(model.parameters(), spec.grad_clip)
            if not torch.isfinite(norm):
        ...
        return StageRunReport("reward", spec.digest, _input_digest(examples), initial, final,
                              spec.steps, len(examples), _module_digest(model), _EMPTY_DIGEST)
...
def _dpo_loss(policy, reference, example, beta: float) -> torch.Tensor:
    policy_margin = (_token_log_probs(policy, example.prompt_ids, example.chosen_ids).sum()
...
def _mean_dpo_loss(policy, reference, examples, beta) -> float:
    policy_training = policy.training; reference_training = reference.training
    policy.eval(); reference.eval()
    try:
        value = sum(float(_dpo_loss(policy, reference, example, beta).item())
                    for example in examples) / len(examples)
    finally:
        policy.train(policy_training); reference.train(reference_training)
    if not math.isfinite(value):
        raise RuntimeError("DPO evaluation produced a non-finite loss")
    return value


def train_dpo(policy: HostedLlama, reference: HostedLlama, examples,
    ...
    with _deterministic_stage(spec.seed):
        ...
        optimizer = _optimizer(spec, policy.parameters()); cursor = 0; policy.train()
        for step in range(spec.steps):
            optimizer.zero_grad(set_to_none=True); running = 0.0
            count = spec.batch_size * spec.gradient_accumulation
            for _ in range(count):
                loss = _dpo_loss(policy, reference, examples[cursor % len(examples)],
                                 spec.preference_beta) / count
                cursor += 1; running += float(loss.detach().item()); loss.backward()
        ...
        return StageRunReport("dpo", spec.digest, _input_digest(examples), initial, final,
                              spec.steps, len(examples), _module_digest(policy),
                              _module_digest(reference))


def generalized_advantage_estimate(rewards: torch.Tensor, values: torch.Tensor,
    ...
    advantages = torch.zeros_like(rewards); following_value = torch.zeros_like(values[0])
    ...
    for index in range(rewards.numel() - 1, -1, -1):
        ...
        advantages[index] = following_advantage; following_value = values[index]
...
def _ppo_loss(policy, value_model, example, snapshot, spec):
    ...
    value_loss = 0.5 * torch.maximum((values - returns).square(),
...
def train_ppo(policy: HostedLlama, reference: HostedLlama, value_model: HostedValueModel,
              examples, spec: StageTrainSpec, telemetry_sink=None) -> StageRunReport:
    ...
    if telemetry_sink is not None and not callable(telemetry_sink):
        raise ValueError("telemetry_sink must be callable or None")
    with _deterministic_stage(spec.seed):
        snapshots = [_ppo_snapshot(policy, reference, value_model, example, spec)
                     for example in examples]
        initial = sum(float(_ppo_loss(policy, value_model, example, snapshot, spec).item())
                      for example, snapshot in zip(examples, snapshots)) / len(examples)
        if not math.isfinite(initial):
            raise RuntimeError("PPO evaluation produced a non-finite initial loss")
        parameters = [*policy.parameters(), *value_model.parameters()]
        optimizer = _optimizer(spec, parameters); cursor = 0
        policy.train(); value_model.train()
        for step in range(spec.steps):
            optimizer.zero_grad(set_to_none=True); running = 0.0
            count = spec.batch_size * spec.gradient_accumulation
            for _ in range(count):
                index = cursor % len(examples); cursor += 1
                loss = _ppo_loss(policy, value_model, examples[index], snapshots[index], spec) / count
                running += float(loss.detach().item()); loss.backward()
        ...
        final = sum(float(_ppo_loss(policy, value_model, example, snapshot, spec).item())
                    for example, snapshot in zip(examples, snapshots)) / len(examples)
        if not math.isfinite(final):
            raise RuntimeError("PPO evaluation produced a non-finite final loss")
        return StageRunReport("ppo", spec.digest, _input_digest(examples), initial, final,
                              spec.steps, len(examples), _module_digest(policy),
                              _module_digest(value_model))


def train_reasoning_sft(model: HostedLlama, examples, spec: StageTrainSpec,
                        telemetry_sink=None) -> StageRunReport:
    ...
    return StageRunReport("reasoning", report.spec_sha256, _input_digest(reasoning),
                          report.initial_loss, report.final_loss, report.steps,
                          len(reasoning), report.model_sha256, report.auxiliary_sha256)
