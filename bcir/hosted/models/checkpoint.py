...
try:
    ...
    raise ModuleNotFoundError(
        "safe hosted checkpoints require torch and safetensors") from exc
...
def _tensor_bytes(value: torch.Tensor) -> bytes:
    ...
    return (value.detach().to("cpu").contiguous().reshape(-1)
...
def tensor_mapping_digest(values: dict[str, torch.Tensor]) -> str:
    ...
    for name in sorted(values):
        ...
        digest.update(len(dtype).to_bytes(2, "little")); digest.update(dtype)
...
def _tensor_is_finite(value: torch.Tensor) -> bool:
    return not (value.is_floating_point() or value.is_complex()) \
...
def _validate_scaler_state(value, *, enabled: bool) -> dict:
    ...
    if float(value["scale"]) <= 0.0 or float(value["growth_factor"]) <= 1.0 \
            or not 0.0 < float(value["backoff_factor"]) < 1.0:
        raise ValueError("checkpoint scaler factors are outside their valid ranges")
    if type(value["growth_interval"]) is not int or value["growth_interval"] < 1 \
    ...
    for parameter_name, parameter in sorted(model.named_parameters()):
        ...
        for state_name in sorted(state):
            ...
            if not isinstance(value, torch.Tensor):
                raise ValueError(
                    f"optimizer state {parameter_name}.{state_name} is not a tensor")
            tensor_name = f"optimizer.{len(index):08d}"
            tensor = value.detach().to("cpu").contiguous()
            if state_name not in _ADAMW_STATE_NAMES or not _tensor_is_finite(tensor):
                raise ValueError(
                    f"optimizer state {parameter_name}.{state_name} is invalid")
            out[tensor_name] = tensor
            index.append({"parameter": parameter_name, "state": state_name,
                          "tensor": tensor_name})
    return out, index
...
def _validate_optimizer(model: HostedLlama, optimizer, spec: HostedTrainSpec) -> None:
    ...
    optimizer_parameters = [parameter for group in optimizer.param_groups
    ...
    for group in optimizer.param_groups:
        if tuple(group.get("betas", ())) != (spec.beta1, spec.beta2) \
...
def save_safe_checkpoint(root: os.PathLike | str, model: HostedLlama, optimizer, *,
                         spec: HostedTrainSpec, corpus: CorpusManifest,
                         global_step: int, batch_index: int, initial_loss: float,
                         scaler_state: dict | None = None,
                         failure_injector: Callable[[str], None] | None = None
                         ) -> SavedCheckpoint:
    """Publish one immutable checkpoint generation, then atomically advance ``latest``."""
    if not isinstance(model, HostedLlama) or not isinstance(spec, HostedTrainSpec) \
    ...
    if isinstance(initial_loss, bool) or not isinstance(initial_loss, (int, float)) \
    ...
    try:
        ...
        if model_device.type == "cuda":
            for index, state in enumerate(torch.cuda.get_rng_state_all()):
                ...
                optimizer_values[name] = state.to("cpu"); cuda_names.append(name)
        if global_step and not optimizer_index:
            raise ValueError("optimizer checkpoint has no AdamW state after step zero")
        save_file({name: model_values[name] for name in sorted(model_values)},
                  str(temporary / "model.safetensors"))
        _fsync(temporary / "model.safetensors"); hook("after-model")
        save_file({name: optimizer_values[name] for name in sorted(optimizer_values)},
                  str(temporary / "optimizer.safetensors"))
        _fsync(temporary / "optimizer.safetensors"); hook("after-optimizer")
        ...
        manifest = {
            "schema": "bcir.hosted_checkpoint.v1", "files": files,
            "model_sha256": model_digest, "optimizer_sha256": optimizer_digest,
        }
        _write_json(temporary / "manifest.json", manifest); hook("after-manifest")
        manifest_digest = sha256_file(temporary / "manifest.json")
        generation = (f"step-{global_step:08d}-{model_digest[:12]}-"
                      f"{optimizer_digest[:12]}")
        ...
        if published.exists():
            if published.is_symlink() or not published.is_dir() \
                    ...
                    or any(not (published / name).is_file()
...
def _resolve_checkpoint(path: os.PathLike | str) -> tuple[Path, str | None]:
    ...
    if pointer.is_file():
        ...
        if not isinstance(value, dict) or set(value) != {
                "schema", "generation", "manifest_sha256"} \
        ...
        if candidate is None or len(candidate.parts) != 2 \
        ...
        try:
            if os.path.commonpath((str(path.resolve()), str(directory.resolve()))) \
                    != str(path.resolve()):
                raise ValueError("checkpoint pointer escapes its root")
...
def load_safe_checkpoint(path: os.PathLike | str, model: HostedLlama, optimizer, *,
                         spec: HostedTrainSpec, corpus: CorpusManifest,
                         device: torch.device) -> LoadedCheckpoint:
    if not isinstance(model, HostedLlama) or not isinstance(spec, HostedTrainSpec) \
            or not isinstance(corpus, CorpusManifest) or not isinstance(device, torch.device):
    ...
    if device.type == "cuda" and (cuda_device_count < 1 or device.index is not None
    ...
    if not isinstance(manifest, dict) or set(manifest) != {
            "schema", "files", "model_sha256", "optimizer_sha256"} \
            or manifest["schema"] != "bcir.hosted_checkpoint.v1":
        raise ValueError("malformed hosted checkpoint manifest")
    if not _is_sha256(manifest["model_sha256"]) \
            or not _is_sha256(manifest["optimizer_sha256"]):
    ...
    if not isinstance(manifest["files"], dict) or set(manifest["files"]) != {
            "config.json", "model.safetensors", "optimizer.safetensors", "state.json"}:
    ...
    for name, record in manifest["files"].items():
        ...
        if type(record["bytes"]) is not int or not 0 <= record["bytes"] \
                <= _CHECKPOINT_FILE_LIMITS[name] or not _is_sha256(record["sha256"]):
            raise ValueError(f"checkpoint file record {name!r} has invalid size/hash fields")
        file = directory / name
        if file.is_symlink() or not file.is_file() or file.stat().st_size != record["bytes"] \
    ...
    required = {"schema", "batch_index", "corpus_sha256", "cuda_rng_tensors",
                "dependencies", "global_step", "initial_loss", "optimizer_index",
                "rng_cpu_tensor", "scaler_state", "scheduler_state", "spec_sha256",
                "tokenizer_sha256"}
    if not isinstance(state, dict) or set(state) != required \
            or state["schema"] != "bcir.hosted_checkpoint_state.v1":
        raise ValueError("malformed hosted checkpoint state")
    if state["spec_sha256"] != spec.digest or state["corpus_sha256"] != corpus.digest \
    ...
    if not isinstance(state["scaler_state"], dict) or state["scheduler_state"] != {
            "kind": "warmup_cosine_v1", "last_completed_step": global_step}:
        raise ValueError("checkpoint scheduler/scaler state is malformed")
    if not isinstance(state["optimizer_index"], list) \
    ...
    optimizer_values = {name: value for name, value in all_values.items()
                        if not name.startswith("rng.")}
    ...
    for index, row in enumerate(state["optimizer_index"]):
        ...
        if not all(isinstance(value, str) for value in (
                parameter_name, state_name, tensor_name)) \
                or parameter_name not in by_name or state_name not in _ADAMW_STATE_NAMES \
        ...
        if state_name == "step":
            valid = value.ndim == 0 and value.dtype == torch.float32 \
    ...
    if cpu_name != "rng.cpu" or cuda_names != [
            f"rng.cuda.{index}" for index in range(len(cuda_names))] \
    ...
    if list(missing) != allowed_missing or unexpected:  # defensive; prevalidation pins this
        raise ValueError(f"checkpoint model keys mismatch: missing={missing}, unexpected={unexpected}")
    model.to(device)
