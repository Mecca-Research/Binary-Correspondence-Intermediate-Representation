...
try:
    ...
    raise ModuleNotFoundError(
        "hosted training requires PyTorch; install bcir[model-lab]") from exc
...
def _device_and_dtype(device_name: str, dtype_name: str):
    ...
    if device_name == "cuda" and dtype_name == "bfloat16" \
...
def _batch(source: HostedTokenSource, batch_index: int, spec: HostedTrainSpec,
           device: torch.device) -> tuple[torch.Tensor, torch.Tensor]:
    rows = source.batch(batch_index, spec.batch_size, spec.context_length)
    if not isinstance(rows, list) or len(rows) != spec.batch_size \
            or any(not isinstance(row, list) or len(row) != spec.context_length + 1
                   for row in rows):
...
@torch.no_grad()
def _loss(model: HostedLlama, source: HostedTokenSource, spec: HostedTrainSpec,
...
def _train_hosted_impl(spec: HostedTrainSpec, source: HostedTokenSource,
                       output_dir: os.PathLike | str, *, device: str,
                       resume_from: os.PathLike | str | None,
                       telemetry_sink) -> HostedRunReport:
    ...
    model = HostedLlama(spec.decoder,
                        activation_checkpointing=spec.activation_checkpointing,
                        context_length=spec.context_length).to(target_device)
    ...
    scaler = torch.amp.GradScaler("cuda", enabled=(target_device.type == "cuda"
                                                   and spec.dtype == "float16"))
    global_step = batch_index = 0
    initial_loss = _loss(model, source, spec, target_device, compute_dtype)
    if resume_from is not None:
        loaded = load_safe_checkpoint(resume_from, model, optimizer, spec=spec,
                                      corpus=corpus, device=target_device)
        global_step, batch_index, initial_loss = (
            loaded.global_step, loaded.batch_index, loaded.initial_loss)
    ...
    while global_step < spec.steps:
        ...
        global_step += 1
        tokens = (global_step * spec.batch_size * spec.context_length
                  * spec.gradient_accumulation)
        peak = (int(torch.cuda.max_memory_allocated(target_device))
                if target_device.type == "cuda" else 0)
        event = HostedTrainEvent(global_step, loss_sum / spec.gradient_accumulation,
                                 lr, grad_norm, tokens, peak)
        if telemetry_sink is not None:
            telemetry_sink(event)
        if global_step % spec.checkpoint_every == 0 or global_step == spec.steps:
            last_saved = save_safe_checkpoint(
                output, model, optimizer, spec=spec, corpus=corpus,
                global_step=global_step, batch_index=batch_index,
                initial_loss=initial_loss, scaler_state=scaler.state_dict())
    ...
    report = HostedRunReport(
        schema="bcir.hosted_run.v1", spec_sha256=spec.digest,
        corpus_sha256=corpus.digest, tokenizer_sha256=corpus.tokenizer_sha256,
        steps=global_step, batch_index=batch_index,
        tokens_processed=(global_step * spec.batch_size * spec.context_length
                          * spec.gradient_accumulation),
...
def train_hosted(spec: HostedTrainSpec, source: HostedTokenSource,
                 output_dir: os.PathLike | str, *, device: str = "cpu",
    ...
    try:
        return _train_hosted_impl(
            spec, source, output_dir, device=device, resume_from=resume_from,
