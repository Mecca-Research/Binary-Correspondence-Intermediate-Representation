...
def decode_boolean(content: bytes, *, der: bool = False) -> bool:
    ...
    if der and octet not in (0x00, 0xFF):
        raise Asn1Error(f"DER boolean TRUE must be 0xFF (X.690 11.1), got 0x{octet:02x}")
    return octet != 0
...
def decode_integer(content: bytes) -> int:
    ...
    if len(content) > 1:
        ...
        if first == 0x00 and not second & 0x80:
            raise Asn1Error("integer has a redundant leading 0x00 octet (X.690 8.3.2 b)")
        if first == 0xFF and second & 0x80:
            raise Asn1Error("integer has a redundant leading 0xFF octet (X.690 8.3.2 a)")
...
def encode_real(value: float) -> bytes:
    ...
    exponent_octets = exponent.to_bytes((exponent.bit_length() // 8) + 1, "big", signed=True)
...
def _decode_real_special(content: bytes) -> float:
    ...
    if len(content) != 1:
        raise Asn1Error("a SpecialRealValue encoding has exactly one contents octet (X.690 8.5.9)")
    octet = content[0]
    ...
    if octet == REAL_MINUS_ZERO:
        return -0.0
    raise Asn1Error(f"0x{octet:02x} is reserved in the SpecialRealValue space (X.690 8.5.9)")
...
def _require_der_binary(base_bits: int, scale: int, exp_octets: bytes, man_octets: bytes) -> None:
    ...
    if scale != 0:
        raise Asn1Error(f"the binary scaling factor F shall be zero; got {scale} (X.690 11.3.1)")
    ...
    if int.from_bytes(man_octets, "big") % 2 == 0:
        raise Asn1Error(
            "the mantissa shall be odd, so that one real value has one encoding (X.690 11.3.1)"
        )
    if man_octets[0] == 0x00:
        raise Asn1Error("the mantissa is not in the fewest octets necessary (X.690 11.3.1)")
    if (
        len(exp_octets) > 1
        and exp_octets[0] in (0x00, 0xFF)
        and (exp_octets[1] & 0x80) == (0x80 if exp_octets[0] == 0xFF else 0x00)
    ):
        raise Asn1Error("the exponent is not in the fewest octets necessary (X.690 11.3.1)")
...
def _decode_real_decimal(content: bytes, *, der: bool = False) -> float:
    ...
    if form not in _NR_FORMS:
        raise Asn1Error(
            f"ISO 6093 number-representation selector {form:06b} is reserved (X.690 8.5.8)"
        )
    try:
        raw = content[1:].decode("ascii", errors="strict")
    except UnicodeDecodeError as exc:
        raise Asn1Error(
            "an ISO 6093 field is ASCII; a non-ASCII octet is not a digit (X.690 8.5.8)"
        ) from exc
    if not raw:
    ...
    if der and raw != raw.strip(" "):
        raise Asn1Error(f"decimal real {raw!r} contains SPACE; DER forbids it (X.690 11.3.2.2)")
    if not _NR_GRAMMAR[form].match(text):
        raise Asn1Error(f"{text!r} is not an ISO 6093 {_NR_FORMS[form]} field (X.690 8.5.8)")
    if der:
...
def _require_der_nr3(form: int, text: str) -> None:
    ...
    if text[0] == "+":
        raise Asn1Error(
            "a non-negative decimal real begins with a digit, not PLUS SIGN (X.690 11.3.2.3)"
        )
    ...
    if exponent.startswith("+"):
        raise Asn1Error(
            f"a non-zero exponent does not use PLUS SIGN; got {exponent!r} (X.690 11.3.2.6)"
        )
    if exponent.lstrip("-")[0] == "0":
        raise Asn1Error(
            f"a non-zero exponent's first digit is not zero; got {exponent!r} (X.690 11.3.2.6)"
        )
...
class BitString:
    ...
    def __post_init__(self) -> None:
        if not 0 <= self.unused <= 7:
            raise Asn1Error(f"unused-bit count must be 0..7 (X.690 8.6.2.2), got {self.unused}")
        if not self.octets and self.unused:
            raise Asn1Error("an empty bitstring must declare zero unused bits (X.690 8.6.2.3)")
...
def decode_bitstring(tlv: Tlv, *, der: bool = False) -> BitString:
    ...
    if tlv.constructed:
        if der:
            raise Asn1Error(
                "DER forbids the constructed form for bitstring (X.690 10.2)", tlv.offset
            )
        octets = bytearray()
        unused = 0
    ...
    if not content:
        raise Asn1Error(
            "bitstring contents need an initial unused-bit octet (X.690 8.6.2)", tlv.offset
        )
    unused = content[0]
    if unused > 7:
        raise Asn1Error(f"unused-bit count must be 0..7 (X.690 8.6.2.2), got {unused}", tlv.offset)
    body = content[1:]
    if not body and unused:
        raise Asn1Error(
            "an empty bitstring must declare zero unused bits (X.690 8.6.2.3)", tlv.offset
        )
    if der and body and body[-1] & ((1 << unused) - 1):
        raise Asn1Error(
            "DER requires each unused bit in the final octet to be zero (X.690 11.2.1)", tlv.offset
        )
...
def decode_octetstring(tlv: Tlv, *, der: bool = False) -> bytes:
    ...
    if not tlv.constructed:
        return tlv.content
    if der:
        raise Asn1Error("DER forbids the constructed form for octetstring (X.690 10.2)", tlv.offset)
    out = bytearray()
...
def decode_null(content: bytes) -> None:
    ...
    if content:
        raise Asn1Error(f"null contents must be empty (X.690 8.8.2), got {len(content)} octet(s)")
    return None
    ...
    for i, octet in enumerate(content):
        if not started and octet == 0x80:
            raise Asn1Error(
                ...
                f"(X.690 8.19.2: encoded in the fewest possible octets)",
                i,
            )
        started = True
...
def encode_oid(arcs: tuple[int, ...] | list[int]) -> bytes:
    ...
    arcs = tuple(arcs)
    if len(arcs) < 2:
        raise Asn1Error("an object identifier has at least two components (X.690 8.19.4)")
    if any(a < 0 for a in arcs):
        raise Asn1Error("object identifier components must be non-negative")
    first, second = arcs[0], arcs[1]
    if first > 2:
        raise Asn1Error(
            f"the first object identifier component is 0, 1 or 2 (X.690 8.19.4 NOTE), got {first}"
        )
    if first < 2 and second >= 40:
        raise Asn1Error(
            f"under arc {first} the second component is 0..39 (X.690 8.19.4 NOTE), got {second}"
        )
...
def encode_oid_iri(value: str) -> bytes:
    ...
    if not value.startswith("/"):
        raise Asn1Error(f"an OID-IRI value begins with a solidus (X.680 34.3), got {value!r}")
...
def decode_string(tlv: Tlv, *, der: bool = False) -> str:
    ...
    if codec is None:
        raise Asn1Error(f"UNIVERSAL {number} is not a restricted character string", tlv.offset)
    if tlv.constructed and der:
        raise Asn1Error(
            "DER forbids the constructed form for character strings (X.690 10.2)", tlv.offset
        )
    ...
    octets = tlv.flatten_content(Universal.OCTET_STRING) if tlv.constructed else tlv.content
    if number in (Universal.UNIVERSAL_STRING, Universal.BMP_STRING):
        ...
        if len(octets) % width:
            raise Asn1Error(
                ...
                f"octets (X.690 8.23.{7 if width == 4 else 8})",
                tlv.offset,
            )
    try:
        ...
        raise Asn1Error(
            f"{Universal(number).name} contents are not valid {codec}: {exc}", tlv.offset
        ) from exc
...
def _check_repertoire(number: int, value: str, offset: int = -1) -> None:
    if number == Universal.PRINTABLE_STRING:
        ...
        if bad:
            raise Asn1Error(
                f"character(s) {bad!r} are outside the PrintableString repertoire (X.680 41.4)",
                offset,
            )
        ...
        if bad:
            raise Asn1Error(
                f"character(s) {bad!r} are outside the NumericString repertoire (X.680 41.2)",
                offset,
            )
        ...
        if bad:
            raise Asn1Error(
                f"character(s) {bad!r} are outside the {Universal(number).name} repertoire", offset
            )
...
def decode_utctime(content: bytes, *, der: bool = False) -> str:
    ...
    if der and not _UTCTIME_DER.match(value):
        raise Asn1Error(f"DER UTCTime must be YYMMDDHHMMSSZ (X.690 11.8), got {value!r}")
    return value
...
def encode_generalizedtime(value: str) -> bytes:
    ...
    if not _GENTIME_DER.match(value):
        raise Asn1Error(
            f"DER GeneralizedTime must be YYYYMMDDHHMMSS[.f]Z (X.690 11.7.1-11.7.2), got {value!r}"
        )
...
def decode_generalizedtime(content: bytes, *, der: bool = False) -> str:
    ...
    if der:
        if not _GENTIME_DER.match(value):
            raise Asn1Error(
                f"DER GeneralizedTime must be YYYYMMDDHHMMSS[.f]Z (X.690 11.7), got {value!r}"
            )
        if "." in value and value.rstrip("Z").endswith("0"):
            raise Asn1Error(
                f"DER GeneralizedTime omits trailing fractional zeros (X.690 11.7.3), got {value!r}"
            )
    return value
