...
def _hardware(*, direct=True, vram_bytes=1024) -> HardwareEnvelope:
    banks = (
        BankEnvelope("ssd", "NVM", "storage", 1 << 20, 1 << 20, 2_000_000_000, 2_000_000_000),
        BankEnvelope("ram", "RAM", "host", 1 << 20, 1 << 20, 20_000_000_000, 20_000_000_000),
        BankEnvelope(
            "vram", "VRAM", "gpu", vram_bytes, vram_bytes, 100_000_000_000, 100_000_000_000
        ),
    )
    links = [
    ...
    if direct:
        links.extend(
            (
                LinkEnvelope("ssd", "vram", 10_000_000_000, 100),
...
def _workload(hardware: HardwareEnvelope, *, mutable=False, prefetch=1) -> HAMWorkload:
    resources = (
        ...
        HAMResource(2, "context", "adapter", 300, "ssd", _hash("context"), (1,), mutable, 20),
    )
    accesses = (
        HAMAccess(1, 1, "vram", "read", "gem.prefill"),
        HAMAccess(2, 2, "vram", "write" if mutable else "read", "gem.context"),
...
def test_ham_workload_is_strict_canonical_and_dependency_acyclic():
    ...
    _raises(
        "contiguous",
        lambda: HAMWorkload(
            hardware.digest, workload.resources, (HAMAccess(2, 1, "vram", "read", "read"),)
        ),
    )
    ...
    _raises("entries must be objects", lambda: HAMWorkload.from_json(json.dumps(nested_array)))
    _raises(
        "immutable",
        lambda: lower_ham_workload(
            HAMWorkload(
                hardware.digest,
                workload.resources,
                (HAMAccess(1, 1, "vram", "write", "bad-write"),),
            ),
            hardware,
        ),
    )
...
def test_direct_peer_route_lowers_to_verified_claims_and_deterministic_streampack():
    ...
    assert all(row.route_class == "direct-peer" and not row.host_bounce for row in transfers)
...
def test_staged_route_is_explicit_and_reports_host_bounce_without_claiming_p2p():
    ...
    no_path = HardwareEnvelope(
        hardware.name,
        hardware.architecture,
        hardware.banks,
        tuple(link for link in hardware.links if link.destination != "vram"),
        (),
    )
...
def test_host_endpoint_route_is_direct_dma_not_peer_or_bounce():
    ...
    workload = HAMWorkload(
        ...
        (HAMAccess(1, 1, "ram", "read", "host-stage"),),
        0,
        1,
    )
    lowered = lower_ham_workload(workload, hardware)
...
def test_capacity_uses_explicit_farthest_next_use_eviction_and_never_home_eviction():
    ...
    assert (
        sum(row.kind == "transfer" and row.resource_id == 1 for row in lowered.artifact.actions)
        == 2
    )
...
def test_capacity_ties_evict_lower_priority_before_equal_next_use():
    ...
    resources = (
        HAMResource(1, "important", "tensor", 190, "ssd", _hash("important"), (), False, 10),
        HAMResource(2, "disposable", "tensor", 190, "ssd", _hash("disposable"), (), False, 1),
        HAMResource(3, "temporary", "tensor", 200, "ssd", _hash("temporary")),
        HAMResource(4, "join", "tensor", 1, "ssd", _hash("join"), (1, 2)),
    )
    accesses = (
        HAMAccess(1, 1, "vram", "read", "important-now"),
        HAMAccess(2, 2, "vram", "read", "disposable-now"),
        HAMAccess(3, 3, "vram", "read", "temporary"),
        HAMAccess(4, 4, "vram", "read", "equal-next-use-join"),
    )
    lowered = lower_ham_workload(HAMWorkload(hardware.digest, resources, accesses, 0, 3), hardware)
    capacity_evictions = [
        row
        for row in lowered.artifact.actions
        if row.kind == "evict" and row.reason == "capacity:farthest-next-use"
    ]
    assert capacity_evictions[0].resource_id == 2
    assert not verify_ham_plan(
        lowered.artifact, HAMWorkload(hardware.digest, resources, accesses, 0, 3), hardware
    )


def test_mutable_generation_invalidates_stale_copies_and_writes_back_home():
    hardware = _hardware()
    workload = _workload(hardware, mutable=True)
    lowered = lower_ham_workload(workload, hardware)
    write = next(
        row for row in lowered.artifact.actions if row.kind == "access" and row.mode == "write"
    )
    assert (write.generation_before, write.generation_after) == (0, 1)
    assert any(row.kind == "invalidate" and row.bank == "ssd" for row in lowered.artifact.actions)
    assert any(
        row.kind == "transfer" and row.intent == "writeback" and row.route == ("vram", "ssd")
        for row in lowered.artifact.actions
    )
    assert not verify_ham_plan(lowered.artifact, workload, hardware)
...
def test_independent_replay_rejects_route_size_and_peak_tampering():
    ...
    actions[index] = replace(
        actions[index], route=("ssd", "vram"), route_class="direct-peer", host_bounce=False
    )
    tampered = replace(lowered.artifact, actions=tuple(actions))
    assert any(
        "undeclared HAM route" in row for row in verify_ham_plan(tampered, workload, hardware)
    )
    ...
    peaks = tuple(
        replace(row, peak_bytes=max(0, row.peak_bytes - 1)) if row.bank == "vram" else row
        for row in lowered.artifact.peaks
    )
    ...
    actions = [
        row
        for row in lowered.artifact.actions
        if not (row.kind == "barrier" and row.reason == "residency:complete-before-access")
    ]
    actions = [replace(row, order=index + 1) for index, row in enumerate(actions)]
    tampered = replace(lowered.artifact, actions=tuple(actions))
    assert any(
        "lacks its residency barrier" in row
    ...
    tampered = replace(lowered.artifact, actions=tuple(actions))
    assert any("prefetch lineage" in row for row in verify_ham_plan(tampered, workload, hardware))
...
def test_context_catalog_activation_is_quiescent_bound_and_rollback_capable():
    hardware = _hardware()
    manifest = _manifest(b"artifact", hardware.digest)
    ...
    activation = certify_context_activation(
        ...
        current_shard_sha256=_hash("old"),
    ...
    assert (
        activation.rollback_token_sha256
        == certify_context_activation(
            catalog,
            manifest,
            manifest.selector_sha256,
    ...
    _raises(
        "quiescent",
        lambda: certify_context_activation(
            catalog, manifest, manifest.selector_sha256, activation_generation=3, quiescent=False
        ),
    )
    _raises(
        "advance",
        lambda: certify_context_activation(
            catalog, manifest, manifest.selector_sha256, activation_generation=2, quiescent=True
        ),
    )
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        duplicate = catalog.to_json().replace('"generation":2', '"generation":2,"generation":2')
...
def test_dual_memory_similarity_can_rank_but_hard_facts_veto_candidates():
    ...
    shard_near = _hash("near-shard")
    shard_far = _hash("far-shard")
    near = OptimizationPattern(
        _hash("near-pattern"), (10, 10, 10), shard_near, _hash("near-evidence"), (p2p,)
    )
    far = OptimizationPattern(
        _hash("far-pattern"), (100, 100, 100), shard_far, _hash("far-evidence"), (enough_vram,)
    )
    ...
    _raises(
        "entries must be objects", lambda: OptimizationMemory.from_json(json.dumps(nested_array))
    )
    ...
    matches = query_optimization_memory(memory, (11, 11, 11), top_k=2, active_facts=(enough_vram,))
    assert len(matches) == 1 and matches[0].context_shard_sha256 == shard_far
    _raises(
        "outside hard memory",
        lambda: query_optimization_memory(
