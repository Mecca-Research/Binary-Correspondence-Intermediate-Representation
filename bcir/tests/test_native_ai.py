...
from bcir.kbcir.native_ai import (NativeAIKernels, NativeOptimizationIndex,
                                  NativeQuantizedTensor)
from bcir.kbcir.optimization_memory import (OptimizationFact, OptimizationMemory,
                                             OptimizationPattern,
...
def test_native_quantization_is_byte_identical_for_q8_q4_odd_groups_and_extremes():
    ...
    fixtures = (
        (), (0.0,), (math.ldexp(1.0, -1074), -math.ldexp(1.0, -1074)),
        (math.ldexp(127.0, -20), math.nextafter(math.ldexp(127.0, -20), 0.0),
         math.nextafter(math.ldexp(127.0, -20), math.inf)),
        (math.ldexp(7.0, 20), math.nextafter(math.ldexp(7.0, 20), 0.0),
...
def test_native_q8_matrix_kernels_match_increasing_coordinate_oracle_exactly():
    ...
    assert native.q8_matvec(inputs, quantized.codes, quantized.exponents,
                            output_count=11, group_size=13) == tuple(expected_matvec)
    ...
    assert native.q8_rows_dot(inputs, quantized.codes, quantized.exponents,
                              output_count=11, group_size=13) == tuple(expected_rows)
...
def test_native_boundary_rejects_malformed_numbers_extents_and_symmetric_codes():
    ...
    bad_calls = (
        ...
        lambda: native.q8_matvec((1.0,), b"\x01\x02", (0,),
                                 output_count=1, group_size=32),
        lambda: native.q8_matvec((1.0,), b"\x80", (0,),
                                 output_count=1, group_size=32),
        lambda: native.q8_rows_dot((math.nan,), b"\x01", (0,),
                                   output_count=1, group_size=32),
    )
...
def test_native_q4_and_q8_exports_are_identical_to_python_artifacts():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        write_q8_decoder(python_q8, spec, weights, source_hashes=_HASHES,
                         tokenizer_ids=_TOKENS)
        write_q8_decoder(native_q8, spec, weights, source_hashes=_HASHES,
                         tokenizer_ids=_TOKENS, native_kernels=native)
        assert python_q8.read_bytes() == native_q8.read_bytes()
        python_q4 = PackedQ4Tensor.from_values(
            values, (65,), source_sha256=_hash("source"),
            calibration_sha256=_hash("calibration"))
        native_q4 = PackedQ4Tensor.from_values_native(
            values, (65,), source_sha256=_hash("source"),
            calibration_sha256=_hash("calibration"), native_kernels=native)
        assert native_q4 == python_q4


def _optimization_memory() -> OptimizationMemory:
    facts = tuple(sorted((OptimizationFact("host", "isa", "x86_64"),
                          OptimizationFact("gpu", "format", "q8"))))
    patterns = []
    for index in range(257):
        required = (facts[index & 1],)
        embedding = tuple(((index * 97 + coordinate * 31) % 65536) - 32768
                          for coordinate in range(17))
        patterns.append(OptimizationPattern(
            _hash(f"pattern-{index:04d}"), embedding, _hash(f"shard-{index}"),
            _hash(f"evidence-{index}"), required))
    return OptimizationMemory(_hash("model"), _hash("embedder"),
                              tuple(sorted(patterns, key=lambda row: row.pattern_sha256)), facts)
...
def test_native_q15_index_matches_hard_fact_filtered_python_oracle():
    ...
    assert index.query(query, top_k=13, active_facts=(memory.facts[0],)) == \
        query_optimization_memory(memory, query, top_k=13,
                                  active_facts=(memory.facts[0],))
