...
def test_octetstring_constructed_segments_concatenate():
    ...
    raw = bytes.fromhex("24800403 0102030402 04050000".replace(" ", ""))
    tlv = decode_one(raw)
...
def test_sequence_worked_example_is_byte_identical():
    ...
    want = bytes.fromhex("300a1605536d6974680101ff")
    built = Tlv(
...
def test_visible_string_jones_in_all_three_sender_option_forms():
    ...
    definite = bytes.fromhex("3a0904034a6f6e04026573")
    indefinite = bytes.fromhex("3a8004034a6f6e040265730000")
...
def test_ber_to_der_conversion_is_idempotent():
    ...
    for raw in (
        bytes.fromhex("3a8004034a6f6e040265730000"),
...
def test_decoder_is_total_over_malformed_input():
    ...
    corpus = [
        ...
        b"\x1f\x80\x01",
        b"\x02\xff",
        b"\x30\x80\x01\x01\xff",
        b"\x05\x80",
        b"\x02\x01",
        b"\xff" * 32,
        b"\x30\x84\xff\xff\xff\xff",
        b"\x24\x80" + b"\x04\x01a" * 3,
    ]
    for raw in corpus:
        try:
            decode_one(raw)
