...
def _independent_solve(a, b, n, nrhs):
    ...
    aug = [
        [float(a[i * n + j]) for j in range(n)] + [float(b[i * nrhs + r]) for r in range(nrhs)]
        for i in range(n)
    ]
    for col in range(n):
...
def test_solve_reference_matches_an_independent_solver_and_has_tiny_residual():
    ...
    a = [10.0, 1.0, -2.0, 0.0, 1.0, 8.0, 1.0, -1.0, -2.0, 1.0, 12.0, 2.0, 0.0, -1.0, 2.0, 9.0]
    b = [7.0, -3.0, 14.0, 5.0]
    n, nrhs = 4, 1
...
def test_emit_wraps_lapack_with_a_reference_fallback():
    ...
    assert (
        "LAPACKE_sgesv(LAPACK_ROW_MAJOR, 4, 1, A, 4, ipiv, b, 1)" in c
...
def test_fallback_path_compiles_runs_and_solves_correctly():
    ...
    a = [10.0, 2.0, -1.0, -3.0, 9.0, 1.0, 1.0, -2.0, 8.0]
    b = [11.0, 7.0, 7.0]
    n, nrhs = 3, 1
    ref = solve_reference(a, b, n, nrhs)
    got = _run_solve_kernel(
        cc, emit_lapack_solve_c(n, nrhs, "solve"), a, b, n, nrhs
    )  # NO LAPACK lib
...
def test_fallback_path_solves_multiple_right_hand_sides():
    ...
    if not cc:
        return
    a = [6.0, 1.0, 2.0, 1.0, 7.0, 1.0, 2.0, 1.0, 9.0]
    b = [10.0, 1.0, 9.0, 2.0, 12.0, 3.0]
    n, nrhs = 3, 2
    ref = solve_reference(a, b, n, nrhs)
...
def test_linked_lapack_path_agrees_when_lapack_is_present():
    ...
    got = _run_solve_kernel(
        cc,
        emit_lapack_solve_c(n, nrhs, "solve"),
        a,
        b,
        n,
        nrhs,
        define="-DBCIR_USE_LAPACK",
        libs=libs,
    )
