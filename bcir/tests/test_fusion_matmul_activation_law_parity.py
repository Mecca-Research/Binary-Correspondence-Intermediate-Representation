...
def test_law_decision_matches_oracle_optimize_fused_adopt_or_noop():
    ...
    for kind in _FUSIBLE:
        ...
        assert len(certs) == 1 and certs[0].gain > 0, f"{kind}: oracle must ADOPT a strict-win fusion"
...
def _matmul_activation_mlir(kind: str, dtype: str = "f32", *, with_reader: bool = False) -> str:
    ...
    reader = ('  bcir.gem.block @reader { base = 0 : i64, count = 4 : i64, strideA = 1 : i64, '
...
def test_law_rail_fuses_iff_oracle_adopts_via_bcir_opt():
    ...
    for kind in ("relu", "gelu"):
        ...
        proc = subprocess.run([bo, "-bcir-fuse-matmul-activation"], input=_matmul_activation_mlir(kind),
        ...
        assert 'dtype = "f32"' in line, f"{kind}: the quarantine dtype did not carry through\n{line}"
        ...
        exp_un, exp_fu, exp_ga, adopt = _law_fused_decision(200, (24 if kind == "gelu" else 2), 32, 32)
...
def test_law_rail_is_a_noop_on_softmax_and_non_sole_consumer():
    ...
    proc = subprocess.run([bo, "-bcir-fuse-matmul-activation"], input=_matmul_activation_mlir("softmax"),
                          capture_output=True, text=True)
    assert proc.returncode == 0, proc.stderr
    assert "gem.fused_matmul_activation" not in proc.stdout, "softmax must NOT fuse (a row reduction)"
    ...
    proc = subprocess.run([bo, "-bcir-fuse-matmul-activation"],
...
def test_law_rail_rejects_a_fused_softmax_and_a_transcendental_on_int():
    ...
    proc = subprocess.run([bo], input=base.format(kind="softmax", dtype="f32"),
                          capture_output=True, text=True)
    ...
    proc = subprocess.run([bo], input=base.format(kind="gelu", dtype="i32"),
                          capture_output=True, text=True)
