...
def _token(sequence=0):
    ...
    return TelemetryToken.from_data_dna(
        event,
        sequence=sequence,
        register_pressure=25 + sequence,
        bandwidth_pressure=35,
        throttled=False,
    )
...
def _outcome(context, candidate, latency, energy, *, provenance="simulated"):
    return HardwareOutcome(
        context, candidate, latency, energy, 20, 15, 18, 25, False, True, provenance, 5, "1" * 64
    )


def test_telemetry_availability_and_episode_roundtrip_are_strict():
    token = TelemetryToken.from_data_dna(
        DataDNA("s", 1, cycles=4, bytes=8, misses=1, thermal=2, utilization=3), sequence=0
    )
    ...
    episode = HardwareEpisode(
        report, hardware, workload, (token,), (_outcome(context, "a", 10, 4),), policy.digest
    )
...
def test_reward_preferences_follow_exact_kbcir_metrics_and_reject_bad_outputs():
    ...
    assert pairs and all(
        policy.score(next(row for row in (fast, hot, slow) if row.candidate_id == chosen))
        < policy.score(next(row for row in (fast, hot, slow) if row.candidate_id == rejected))
    ...
    try:
        ...
        pass
    assert policy.score(replace(fast, cache_miss_pressure=0, bandwidth_pressure=90)) > policy.score(
        replace(fast, cache_miss_pressure=0, bandwidth_pressure=0)
    )
    try:
        HardwareRewardPolicy(latency_reference_ns=1).score(replace(fast, latency_ns=(1 << 63) - 1))
...
def _lifetime_module():
    ...
    module.add_phase(
        Phase(1, deps=(0,), claims=[Claim(2, Opcode.LOAD, rd=(2,), domain=Domain.RAM)])
    )
    module.add_phase(
        Phase(2, deps=(1,), claims=[Claim(3, Opcode.ADD, rd=(2, 3), wr=(3,), domain=Domain.RAM)])
    )
    return module
...
def test_static_memory_reuses_only_dead_ranges_and_detects_alias_tampering():
    ...
    tampered = replace(
        plan,
        allocations=tuple(
            replace(row, offset=rows[2].offset) if row.rid == 3 else row for row in plan.allocations
        ),
    )
    assert any(
        "alias" in error
        for error in verify_static_memory_plan(tampered, module, bindings, hardware)
    )
    unknown = replace(
        plan,
        allocations=tuple(
            replace(row, bank="unknown") if row.rid == 1 else row for row in plan.allocations
        ),
    )
...
def test_model_candidates_lower_to_streampack_static_addresses_and_measured_promotion():
    with tempfile.TemporaryDirectory() as directory:
        ...
        assert (
            tuple(row.name for row in topology.nodes)
            == tuple(row.name for row in reversed_topology.nodes)
            == ("ram", "vram")
        )
        try:
        ...
        assert not verify_static_memory_plan(
            static, lowered.module, lowered.resource_banks, hardware
        )
        token = _token()
        context = hardware_context_digest(report.digest, hardware.digest, workload.digest, (token,))
        selected = _outcome(context, candidate.candidate_id, 500_000, 500, provenance="measured")
        baseline = _outcome(context, "source:resident:ram", 2_000_000, 2_000, provenance="measured")
        policy = HardwareRewardPolicy()
        episode = HardwareEpisode(
            report.digest,
            hardware.digest,
        ...
        assert (
            cert.generation == 2
            and cert.improvement_q20 > 0
            and cert.episode_digest == episode.digest
        )
        assert type(cert).from_json(cert.to_json()) == cert
        wrong_lowered = replace(lowered, artifact=replace(lowered.artifact, report_digest="e" * 64))
        try:
            certify_hardware_promotion(
                wrong_lowered,
                static,
        ...
        try:
            certify_hardware_promotion(
                ...
                static,
                hardware,
                replace(selected, provenance="simulated"),
                baseline,
                policy,
                episode=episode,
                generation=2,
