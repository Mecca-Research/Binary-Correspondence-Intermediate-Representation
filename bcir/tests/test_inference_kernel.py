...
from bcir.lower.inference import (InferenceModel, Layer, compile_and_run_inference_c,
                                  emit_inference_kernel_c, quantize_model)
...
def test_layer_validates_shapes_and_forward():
    ...
    for bad in (lambda: Layer(0, 2, (), b=()),
        ...
        try:
            bad(); assert False, "expected a validation error"
        except ValueError:
            pass
...
def test_baked_weights_are_static_const_in_the_emit():
    rng = random.Random(3)
    m = InferenceModel((_float_layer(rng, 4, 8, "relu"), _float_layer(rng, 8, 3, "gelu")), name="bake")
...
def test_attestation_reports_the_r17_bound_only_when_quantized():
    rng = random.Random(4)
    m = InferenceModel((_float_layer(rng, 4, 6, "relu"), _float_layer(rng, 6, 3, "relu")), name="att")
...
def test_all_relu_mlp_kernel_is_bit_exact_to_the_reference():
    ...
    model = InferenceModel((_int_layer(rng, 4, 8, "relu"),
                            _int_layer(rng, 8, 8, "relu"),
                            _int_layer(rng, 8, 3, "relu")), name="relu_mlp")
...
def test_mixed_relu_gelu_mlp_kernel_matches_the_reference_to_roundoff():
    ...
    model = InferenceModel((_float_layer(rng, 4, 8, "relu"),
                            _float_layer(rng, 8, 8, "relu"),
                            _float_layer(rng, 8, 3, "gelu")), name="mixed_mlp")
...
def test_quantized_path_is_within_the_r17_bound():
    ...
    base = InferenceModel((_float_layer(rng, 4, 8, "relu"),
                           _float_layer(rng, 8, 8, "relu"),
                           _float_layer(rng, 8, 3, "relu")), name="q8_mlp")
...
def test_large_weights_bake_via_embed_and_still_match():
    ...
    model = InferenceModel((_float_layer(rng, 20, 20, "relu"),
                            _float_layer(rng, 20, 3, "relu")), name="big_mlp")
...
def test_softmax_final_layer_is_a_separate_pass_and_normalizes():
    ...
    model = InferenceModel((_float_layer(rng, 4, 8, "relu"),
                            _float_layer(rng, 8, 5, "softmax")), name="sm_mlp")
    src = emit_inference_kernel_c(model)
...
def test_no_bias_layer_is_supported():
    ...
    model = InferenceModel((_float_layer(rng, 4, 6, "relu", bias=False),
                            _float_layer(rng, 6, 2, "relu", bias=False)), name="nobias")
    src = emit_inference_kernel_c(model)
...
def test_emit_is_deterministic():
    ...
    m = InferenceModel((_float_layer(rng, 4, 8, "relu"), _float_layer(rng, 8, 3, "gelu")), name="det")
