...
def _claim(cid, rd, wr, phase, n=64):
    return Claim(id=cid, opcode=Opcode.ADD, count=n, rd=tuple(rd), wr=tuple(wr),
...
def _module():
    ...
    m.add_phase(Phase(phase_id=0, claims=[
        _claim(1, [10], [12], 0), _claim(2, [12], [10], 0),
        _claim(3, [10, 11], [12], 0)]))
...
def test_trained_placer_learns_hot_vs_cold_and_is_frozen_q8():
    hot = ResourceProfile(rid=1, size_bytes=256, lifespan=1, frequency=8, declared_tier=MemTier.DRAM)
    cold = ResourceProfile(rid=2, size_bytes=1 << 22, lifespan=8, frequency=1, declared_tier=MemTier.DRAM)
    ...
    assert isinstance(DEFAULT_PLACER, FrozenPlacer) and DEFAULT_PLACER.heat(hot) > DEFAULT_PLACER.heat(cold)
