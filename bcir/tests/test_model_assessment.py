...
def _write_shard(path, tensors, *, sparse=False):
    ...
    with open(path, "wb") as stream:
        ...
        if offset:
            if sparse:
                stream.seek(offset - 1, os.SEEK_CUR)
                stream.write(b"\0")
            else:
...
def _hardware(*, host_capacity=1 << 28, device_capacity=1 << 28):
    banks = (
        BankEnvelope(
            "ram", "RAM", "host", host_capacity, host_capacity, 20_000_000_000, 20_000_000_000
        ),
        BankEnvelope(
            "vram",
            "VRAM",
    ...
    links = (
        LinkEnvelope("ram", "vram", 8_000_000_000, 5000),
    ...
    for channel, multiplier in (("host", 4), ("gpu", 1)):
        for fmt, fmt_scale in (("source", 2), ("bcirq8-group32", 1)):
            for operation, tokens, operations in (("prefill", 8, 100_000), ("decode", 1, 20_000)):
...
def test_synthetic_multi_billion_parameter_header_lowers_a_streaming_plan():
    ...
    manifest_module.os.fstat = lambda _fd: type("Stat", (), {"st_size": len(prefix) + offset})()
    try:
        config = {
            "architectures": ["LlamaForCausalLM"],
            "vocab_size": vocab,
    ...
    workload = _workload(
        sessions=1, prompt_tokens=16, context_tokens=128, max_new_tokens=16, formats=("source",)
    )
    report = assess_model(inventory, hardware, workload)
    resident = next(row for row in report.candidates if row.candidate_id == "source:resident:vram")
    streamed = next(
        row for row in report.candidates if row.candidate_id == "source:stream:ram->vram"
    )
...
def test_exact_format_kv_training_and_per_bank_costs():
    with tempfile.TemporaryDirectory() as directory:
        ...
        expected_kv = (
            2 * 8 * 2 * 2 * 2 * 2 * 2
        )  # sessions, context, layers, K/V, kv heads, head dim, BF16
        ...
        params = inventory.parameter_count
        ...
        assert all(
            not row.feasible and "full-weight training" in " ".join(row.blockers)
            for row in report.candidates
            if row.weight_format == "bcirq8-group32"
        )
        assert all(
            peak.peak_bytes
            == peak.weights_bytes
            + peak.kv_bytes
            + peak.activation_bytes
            + peak.training_bytes
            + peak.staging_bytes
...
def test_lora_state_counts_only_target_matrices():
    with tempfile.TemporaryDirectory() as directory:
        ...
        report = assess_model(
            inventory,
            _hardware(),
            _workload(mode="lora", optimizer="adamw", lora_rank=2, formats=("source",)),
        )
        expected = 0
        targets = {
            "attention.q",
...
def test_candidates_include_resident_streaming_split_and_intervals():
    with tempfile.TemporaryDirectory() as directory:
        ...
        assert any(
            row.weight_format == "source" and row.classification == "exact"
            for row in report.candidates
        )
        assert any(
            row.weight_format == "bcirq8-group32" and row.classification == "quantized"
            for row in report.candidates
        )
        for candidate in report.candidates:
            ...
            for interval in candidate.predictions:
                ...
                assert (
                    interval.tokens_per_second_lower_q16
                    <= interval.tokens_per_second_median_q16
                    <= interval.tokens_per_second_upper_q16
                )
        resident = next(
            row for row in report.candidates if row.candidate_id == "source:resident:vram"
        )
        streamed = next(
            row for row in report.candidates if row.candidate_id == "source:stream:ram->vram"
        )
        assert next(
            row.median_ns for row in streamed.predictions if row.operation == "prefill"
...
def test_candidate_selection_minimizes_complete_workload_latency():
    with tempfile.TemporaryDirectory() as directory:
        ...
        left = next(row for row in report.candidates if row.candidate_id == "source:resident:ram")
        right = next(row for row in report.candidates if row.candidate_id == "source:resident:vram")

        def interval(operation, latency):
            return PredictionInterval(operation, latency, latency, latency, 1, 1, 1, 1, "0" * 64)

        balanced = replace(
            left,
            candidate_id="balanced-worst-stage",
            predictions=(interval("prefill", 100), interval("decode", 100)),
        )
        lower_total = replace(
            right,
            candidate_id="lower-complete-workload",
...
def test_stream_lowering_emits_and_verifies_every_action_class_deterministically():
    with tempfile.TemporaryDirectory() as directory:
        ...
        inventory = build_tensor_inventory([path], config)
        hardware = _hardware()
        workload = _workload(formats=("source",))
        report = assess_model(inventory, hardware, workload)
        ...
        assert (
            sum(
                row.nbytes * row.repeats
                for row in first.artifact.actions
        ...
        stage_resources = sorted(
            resource.name
            for resource in first.module.resources.values()
...
def test_cpu_gpu_split_lowering_accounts_each_boundary_stage_exactly():
    with tempfile.TemporaryDirectory() as directory:
        ...
        report = assess_model(inventory, hardware, workload)
        candidate = next(
            row for row in report.candidates if row.candidate_id == "source:split:ram:1:vram"
        )
        ...
        assert [(row.stage, row.repeats) for row in moves] == [
            ("prefill", 1),
            ("decode", workload.max_new_tokens),
        ]
        ...
        assert all(
            row.operations > 0 and row.repeats >= 1
...
def test_capacity_and_missing_evidence_fail_closed():
    with tempfile.TemporaryDirectory() as directory:
        ...
        hardware = HardwareEnvelope(
            hardware.name, hardware.architecture, hardware.banks, hardware.links, ()
        )
        report = assess_model(inventory, hardware, _workload(formats=("source",)))
        assert not any(row.feasible for row in report.candidates)
        assert all(
            any("allocatable" in reason or "benchmark" in reason for reason in row.blockers)
            for row in report.candidates
        )
        ...
        partial = HardwareEnvelope(
            ...
            tuple(
                row
                for row in complete.benchmarks
                if not (row.channel == "host" and row.operation == "decode")
            ),
        )
        partial_report = assess_model(inventory, partial, _workload(formats=("source",)))
        host_resident = next(
            row for row in partial_report.candidates if row.candidate_id == "source:resident:ram"
        )
        split = next(
            row
            for row in partial_report.candidates
...
def test_cli_writes_report_plan_and_pack_without_model_payload_outputs():
    with tempfile.TemporaryDirectory() as directory:
        ...
        assert all(
            ...
            for path in (inventory_path, report_path, plan_path, pack_path)
        )
        assert not any(
            name.endswith((".safetensors", ".bcirq8")) and name != "model.safetensors"
