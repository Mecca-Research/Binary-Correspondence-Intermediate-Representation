...
from bcir.kbcir.unsupervised import (
    ...
    StandardScaler,
    autoencoder_forward,
    autoencoder_via_bridge,
    check_unsupervised,
    embedding_lookup,
    k_fold_indices,
    k_fold_is_partition,
    kmeans_assign,
    kmeans_assign_via_bridge,
    kmeans_fit,
    kmeans_inertia,
    minmax_fit,
    minmax_transform,
...
def test_kmeans_assign_tie_break_is_lowest_index():
    ...
    assert (
        kmeans_assign(centroids, [0.0], k=2, n_feat=1) == 0
    )  # equidistant (d=1 each) -> lower index 0
...
def test_kmeans_validates():
    for fn, args in [
        (kmeans_assign, ([0.0, 0.0], [0.0], 1, 2)),  # point length mismatch
        (kmeans_assign, ([0.0, 0.0], [0.0, 0.0], 0, 2)),  # k < 1
        (kmeans_fit, ([0.0, 0.0], 1, 2, 2, [0], 1)),  # k > n_samples
        (kmeans_fit, ([0.0], 1, 1, 1, [5], 1)),
    ]:  # init index out of range
        try:
            fn(*args)
...
def test_standard_scaler_standardizes_training_data_to_mean0_std1():
    ...
    transformed = [
        standard_scaler_transform(X[i * nf : (i + 1) * nf], sc.mean, sc.std) for i in range(n)
    ]
    for j in range(nf):
...
def test_autoencoder_nontrivial_has_positive_error():
    ...
    recon = autoencoder_forward(
        x, n_in, n_hidden, W_enc, [0.0] * n_hidden, W_dec, [0.0] * n_in, "relu"
    )
...
def test_autoencoder_validates():
    ...
    W = _identity_flat(n)
    for args in [
        ([0.0, 0.0], n, n, W, [0.0] * n, W, [0.0] * n),  # x length mismatch
        ([0.0] * n, n, n, W[:-1], [0.0] * n, W, [0.0] * n),
    ]:  # W_enc length mismatch
        try:
...
def test_check_unsupervised_accepts_well_formed_claims():
    ...
    assert (
        check_unsupervised("kmeans", n_feat=2, n_unit=3, in_dtype="i32", out_dtype="i32") == []
    )  # assign exact -> i32 ok
    assert (
        check_unsupervised("scaler", n_feat=4, n_unit=4, in_dtype="i32", out_dtype="i32") == []
    )  # transform exact -> i32 ok
    assert (
        check_unsupervised("embedding", n_feat=8, n_unit=100, in_dtype="i32", out_dtype="i32") == []
    )  # gather exact
    assert (
        check_unsupervised(
            "autoencoder", n_feat=5, n_unit=2, in_dtype="i32", out_dtype="i32", activation="relu"
        )
        == []
    )  # relu exact -> i32 ok
    assert (
        check_unsupervised(
            "autoencoder", n_feat=5, n_unit=2, in_dtype="f32", out_dtype="f32", activation="sigmoid"
        )
        == []
    )
...
def test_check_unsupervised_rejects_malformed_claims():
    assert any(
        "not a known organ" in e
        for e in check_unsupervised("foo", n_feat=2, n_unit=2, in_dtype="f32", out_dtype="f32")
    )
    assert any(
        ">= 1" in e
        for e in check_unsupervised("kmeans", n_feat=0, n_unit=2, in_dtype="f32", out_dtype="f32")
    )
    assert any(
        ">= 1" in e
        for e in check_unsupervised("kmeans", n_feat=2, n_unit=0, in_dtype="f32", out_dtype="f32")
    )
    assert any(
        "not preserved" in e
    ...
    assert any(
        "needs f32" in e
        for e in check_unsupervised(
            "autoencoder", n_feat=2, n_unit=2, in_dtype="i32", out_dtype="i32", activation="tanh"
        )
    )
    # an unknown autoencoder activation is rejected.
    assert any(
        "not a known activation" in e
        for e in check_unsupervised(
            "autoencoder", n_feat=2, n_unit=2, in_dtype="f32", out_dtype="f32", activation="swish"
        )
    )
...
def test_kmeans_c_emit_routes_and_validates():
    ...
    assert (
        "math.h" not in src and "expf" not in src and "sqrtf" not in src
    )  # EXACT -- no transcendental
...
def test_autoencoder_training_drives_reconstruction_loss_down():
    ...
    for _ in range(12):  # each input lies near a 2-D subspace...
        ...
        inputs.append(
            [a * u[i] + b * v[i] + 0.3 for i in range(n_in)]
        )  # ...+0.3 keeps it positive for relu
