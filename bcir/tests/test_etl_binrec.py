...
_FIELDS = [(0, 8, "u", "little"), (16, 16, "u", "little"), (32, 32, "u", "little"),
           (16, 16, "s", "little"), (0, 16, "u", "big"), (0, 64, "s", "little")]
_OK, _OOB = 0, 3
...
def _build_harness(tmp: str) -> str | None:
    ...
    build = subprocess.run(
        [cc, "-std=c23", "-O2", "-Wall", "-Wextra", "-I", _RUNTIME_C,
         os.path.join(_RUNTIME_C, "bcir_binrec.c"),
         os.path.join(_RUNTIME_C, "test_binrec.c"), "-o", exe],
...
def test_c_decoder_builds_freestanding():
    ...
    for std in ("c11", "c23"):
        r = subprocess.run([cc, "-ffreestanding", "-nostdlib", f"-std={std}", "-Wall",
                            ...
                            os.path.join(_RUNTIME_C, "bcir_binrec.c"), "-o", os.devnull],
                           capture_output=True, text=True)
