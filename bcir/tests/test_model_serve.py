...
from bcir.frontends.models.serve import (
    SessionCertificate,
    TokenDFA,
    certify_session,
    check_token_dfa,
...
def test_the_session_module_is_law_clean_and_the_log_replays():
    ...
    rep = replay(
        r.manifest, decode_session_module(SPEC, 3, 4), AVX, COOL, artifacts=r.manifest.artifacts
    )
    assert rep.score > 0
...
def test_the_stream_is_generate_token_by_token():
    ...
    ev = list(generate_stream([3, 9, 1], SPEC, w, 5, h=AVX, theta=COOL, eos_id=batch.ids[0]))
...
def test_schema_constrained_emission_obeys_the_dfa_and_refuses_deadlock():
    ...
    for bad, needle in (
        (TokenDFA(allowed=((a,),), edges=({},)), "no edge"),
...
def test_schema_validation_is_total_and_generation_owns_its_snapshot():
    ...
    stream = generate_stream(
        [3, 9, 1], SPEC, _toy_weights(SPEC), 3, h=AVX, theta=COOL, schema=schema
    )
