...
from bcir.lower.optimizers import (
    adam_step,
    compile_and_run_optimizer_c,
    emit_adam_step_c,
    emit_momentum_step_c,
    emit_rmsprop_step_c,
    momentum_step,
...
def test_rmsprop_step_matches_hand_computation():
    ...
    exp_p = [
        1.0 - lr * 0.5 / (math.sqrt(exp_s[0]) + eps),
...
def test_momentum_emit_is_pure_arithmetic_no_libm():
    ...
    assert (
        "void mom(float *params, const float *grad, float *velocity, float lr, float beta)" in src
    )
...
def test_rmsprop_emit_rides_libm_sqrtf():
    ...
    assert (
        "void rms(float *params, const float *grad, float *sq_avg, float lr, float beta, float eps)"
        in src
    )
...
def test_momentum_c_matches_reference():
    _assert_c_matches_reference(
        "momentum", [1.0, -2.0, 0.5], [0.4, -0.3, 0.8], lr=0.05, steps=10, beta=0.9
    )


def test_rmsprop_c_matches_reference():
    _assert_c_matches_reference(
        "rmsprop", [1.0, -2.0, 0.5], [0.4, -0.3, 0.8], lr=0.05, steps=10, beta=0.9, eps=1e-8
    )
...
def test_adam_c_matches_reference_with_state_roundtrip():
    ...
    err = _assert_c_matches_reference(
        "adam",
        [1.0, -2.0, 0.5, 3.0],
        [0.4, -0.3, 0.8, -1.2],
        lr=0.05,
...
def test_well_conditioned_both_adam_and_sgd_converge():
    ...
    for opt, cfg in (
        ...
        assert abs(traj[-1][0] - w_star) < 5e-2 and abs(traj[-1][1] - b_star) < 5e-2, (
            opt,
            traj[-1],
        )
...
def test_convergence_loss_curves_match_in_compiled_c():
    ...
    g0 = grad_at(p0)
    ok, c_traj, out = compile_and_run_optimizer_c(
        "adam", p0, g0, lr=0.1, steps=6, beta1=0.9, beta2=0.999, eps=1e-8
    )
    assert ok, out
    ref_traj = reference_optimizer_trajectory(
        "adam", p0, lambda s, p: g0, lr=0.1, steps=6, beta1=0.9, beta2=0.999, eps=1e-8
    )
