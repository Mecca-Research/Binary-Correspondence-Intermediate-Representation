...
def _spec(*, patcher=None, generation_mode="autoregressive", diffusion=None,
          speculation=None, backbone="transformer", learned_policy=None):
...
def test_patch_layouts_are_bounded_incremental_and_content_addressed():
    ...
    entropy = BytePatchSpec(
        method="entropy_global", min_patch_bytes=2, max_patch_bytes=4,
    ...
    rising = BytePatchSpec(
        method="entropy_monotonic", min_patch_bytes=1, max_patch_bytes=5,
        entropy_delta_millibits=500)
    assert rising.layout(6, entropy_millibits=(100, 200, 900, 1000, 1700, 1800)).starts \
        == (0, 2, 4)
    learned = BytePatchSpec(
        method="learned", min_patch_bytes=2, max_patch_bytes=4,
        learned_threshold_q20=1 << 19)
    assert learned.layout(7, learned_scores_q20=(0, 0, 1 << 20, 0, 0, 0, 0)).starts \
        == (0, 2, 6)
...
def test_diffusion_corruption_and_selection_are_deterministic_and_progress_bounded():
    ...
    confidence = ByteDiffusionSpec(
        block_size=4, max_steps=4, unmask_strategy="confidence",
        confidence_threshold_q20=900_000)
    first = confidence.corrupt(
        (1, 2, 3, 4), vocabulary, mask_probability_q20=1, seed=17)
    assert first == confidence.corrupt(
        (1, 2, 3, 4), vocabulary, mask_probability_q20=1, seed=17)
    assert vocabulary.mask_id in first
    assert confidence.select_positions(
        (0, 2), confidences_q20=(10, 0, 20, 0),
        entropies_millibits=(5, 0, 4, 0)) == (2,)

    entropy = ByteDiffusionSpec(
        block_size=4, max_steps=2, unmask_strategy="entropy_bounded",
        entropy_budget_millibits=7)
    assert entropy.select_positions(
        (0, 1, 2), confidences_q20=(0, 0, 0),
        entropies_millibits=(5, 3, 4)) == (1, 2)
    _refuse(lambda: confidence.corrupt(
        (), vocabulary, mask_probability_q20=1, seed=1), "malformed")
    _refuse(lambda: confidence.corrupt(
        (1, vocabulary.mask_id), vocabulary,
        mask_probability_q20=1, seed=1), "malformed")
    _refuse(lambda: confidence.select_positions(
        (0, 0), confidences_q20=(1,), entropies_millibits=(1,)), "distinct")
    _refuse(lambda: confidence.select_positions(
        (0,), confidences_q20=(1, 2), entropies_millibits=(1,)), "cover")


def test_exact_byte_draft_verification_preserves_greedy_target_semantics():
    mismatch = verify_byte_draft(
        (10, 11, 12), (10, 99, 12), free_prediction=7, vocabulary_size=260)
    assert mismatch.accepted == (10,)
    assert mismatch.emitted == (10, 99)
    assert mismatch.rejected == 2 and not mismatch.matched_all
    complete = verify_byte_draft(
        (10, 11), (10, 11), free_prediction=12, vocabulary_size=260)
    assert complete.emitted == (10, 11, 12)
    assert complete.matched_all and complete.rejected == 0
    _refuse(lambda: verify_byte_draft(
        (), (), free_prediction=0, vocabulary_size=260), "nonzero")
...
def test_learned_boundary_advantages_are_future_discounted_and_batch_centered():
    ...
    assert all(abs(sum(row[index] for row in advantages)) < 1.0e-12
               for index in range(2))
    _refuse(lambda: discounted_batch_advantages(((1.0,), (1.0, 2.0)), 0.9),
            "rectangular")
    _refuse(lambda: discounted_batch_advantages((range((1 << 20) + 1),), 0.9),
            "bounded")
...
def test_byte_model_and_mambabyte_costs_are_exact_sizes_with_honest_lower_bounds():
    ...
    mamba = MambaByteSpec(
        context_bytes=64, width=16, layers=2, state_size=4,
    ...
    decoder_block = 2 * block * (4 * local * local + 3 * local * ff) \
        + 4 * local * block * block
    vocabulary_projection = 2 * block * local * one_layer.vocabulary.size
    assert one_report.diffusion_flops_lower_bound \
        == steps * (decoder_block + vocabulary_projection)
    assert two_report.diffusion_flops_lower_bound \
        == steps * (2 * decoder_block + vocabulary_projection)
...
def test_byteified_global_transplant_is_shape_exact_and_generator_safe():
    ...
    _refuse(lambda: plan_byteified_transplant(
        spec, source_sha256=source_sha,
        source_shapes=source,
        target_shapes=(TensorShape(
            "global_blocks.0.attention.q_proj.weight", (24, 16)),),
        mappings={"global_blocks.0.attention.q_proj.weight": "layer.0.q"}), "exactly")
    _refuse(lambda: plan_byteified_transplant(
        spec, source_sha256=source_sha,
        source_shapes=source,
        target_shapes=target,
        mappings={"embed_bytes.weight": "layer.0.q"}), "only global_blocks")
    _refuse(lambda: plan_byteified_transplant(
        spec, source_sha256=source_sha,
        source_shapes=source, target_shapes=target,
        mappings={1: "layer.0.q"}), "only global_blocks")
    incomplete_target = target + (
        TensorShape("global_blocks.0.attention.k_proj.weight", (24, 24)),)
    _refuse(lambda: plan_byteified_transplant(
        spec, source_sha256=source_sha,
        source_shapes=source,
...
def test_measured_ingest_selects_device_only_with_parity_capacity_and_advantage():
    ...
    fast = ByteIngestProfile(
        digest, host_ns_per_byte_q20=10 << 20,
        device_ns_per_byte_q20=1 << 20, device_launch_ns=100,
        device_pool_bytes=4096, max_chunk_bytes=1024, exact_roundtrip=True)
    ...
    no_parity = ByteIngestProfile(
        digest, 10 << 20, 1 << 20, 1, 4096, 1024, False)
    assert choose_byte_ingest(no_parity, 1024).provider == "host"
    assert choose_byte_ingest(fast, 4097).provider == "host"
    huge_chunk = ByteIngestProfile(
        digest, 10 << 20, 1 << 20, 0, (1 << 63) - 1,
        (1 << 63) - 1, True)
...
def test_byte_latent_lowering_is_complete_verified_and_deterministic():
    ...
    spec = _spec(
        generation_mode="diffusion_verify", diffusion=diffusion,
        speculation=speculation)
    ...
    full_precision = lower_byte_latent(
        spec, layout, batch=2, bytes_per_element=4, target=target)
    assert full_precision.module.resources[3].elem_bytes == 4
    assert full_precision.report.local_cache_bytes \
        == 2 * first.report.local_cache_bytes
...
def test_learned_and_selective_scan_lowering_paths_are_explicit():
    ...
    spec = _spec(
        patcher=patcher, backbone="selective_ssm",
        learned_policy=LearnedPatchPolicySpec())
    layout = patcher.layout(
        8, learned_scores_q20=(1 << 20, 0, 1 << 20, 0, 0, 0, 1 << 20, 0))
