...
def test_two_paths_that_agree_commute():
    sq = CommutingSquare("ok", phi=lambda x: x + 3, psi=lambda x: x + 1, lam=lambda y: y + 2)
    assert verify_commutativity(sq, [0, 1, 2, 10]) == []


def test_disagreeing_paths_are_R12():
    sq = CommutingSquare("bad", phi=lambda x: x + 3, psi=lambda x: x + 1, lam=lambda y: y + 5)
...
def test_manifest_json_round_trip_is_a_commuting_square():
    ...
    sq = CommutingSquare(
        "manifest_json",
        phi=lambda x: x,
        psi=lambda x: x.to_json(),
        lam=ProvenanceManifest.from_json,
    )
...
def test_a_custom_equality_can_be_supplied():
    ...
    sq = CommutingSquare("mod10", phi=lambda x: x, psi=lambda x: x + 10, lam=lambda y: y)
