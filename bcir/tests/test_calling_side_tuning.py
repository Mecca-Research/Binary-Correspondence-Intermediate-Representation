...
from bcir.kbcir.calling_side import (COL_MAJOR, ROW_MAJOR, CallingSideCertificate, CallingSidePlan,
                                     _PREFETCH_FACTOR, _order_cost, certify_calling_side,
                                     plan_calling_side, tune_wrapped_gemm)
...
def test_is_deterministic():
    ...
    assert (a_cert.untuned_cost, a_cert.tuned_cost, a_cert.gain) == \
           (b_cert.untuned_cost, b_cert.tuned_cost, b_cert.gain)
...
def test_emit_column_major_uses_the_swapped_call_and_matching_leading_dim():
    ...
    assert "5, 6, 7, 1.0f, B, 5, A, 7, 0.0f, C, 5" in c          # m=N, n=M, k=K; B(lda=N=5), A(ldb=K=7), C(ldc=N=5)
...
def test_emit_rejects_bad_dims():
    for bad in [(0, 1, 1), (1, 0, 1), (1, 1, 0)]:
        plan = CallingSidePlan(M=bad[0], N=bad[1], K=bad[2], major=ROW_MAJOR,
                               block=TilePlan(1, 1, 1, "ijk", 0, 0, 0, True),
                               prefetch_distance=0, channel="host", channel_kind="cpu")
        try:
...
def _run_gemm(kernel: str, fn: str, M: int, N: int, K: int, a, b, fmt: str = ".1f"):
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        bld = subprocess.run([cc, "-std=c11", "-O2", "-Wall", "-Wextra", src, "-o", exe],
...
def test_tuned_column_major_and_prefetch_are_bit_exact_to_the_untuned_reference():
    ...
    for plan in (base,
                 ...
                 replace(base, major=COL_MAJOR, prefetch_distance=1)): # both
        tuned = _run_gemm(emit_tuned_gemm_c(plan, "g"), "g", M, N, K, a, b)
...
def test_linked_cblas_column_major_agrees_when_a_blas_is_present():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        open(src, "w").write(kernel + main)
        exe = os.path.join(d, "g")
        bld = subprocess.run([cc, "-std=c11", "-O2", "-DBCIR_USE_CBLAS", src, lib, "-o", exe],
                             capture_output=True, text=True)
