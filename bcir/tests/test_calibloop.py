...
def _hot_telemetry(n=4):
    ...
    return [DataDNA(segment_id="s", claim_id=1000, thermal=100, utilization=90,
                    voltage=70, misses=20) for _ in range(n)]
...
def test_hot_telemetry_drives_a_measured_replan_win():
    cert = close_loop(vector_add(1024), AVX, events=_hot_telemetry(),
...
def test_real_host_measurement_freezes_a_wellformed_table():
    ...
    cert, table = measure_and_close(vector_add(1024), AVX, n=1 << 14, repeats=3,
                                    cal_gen=2)
    assert table.cal_gen == 2 and table.samples >= 1
    assert table.stream_q8 == 256                       # streaming baseline (R8)
    assert all(r >= 256 for r in (table.stream_q8, table.strided_q8,
                                  table.random_q8, table.compute_q8))
