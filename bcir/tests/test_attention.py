...
from bcir.kbcir.attention import (AttentionSpec, attention_realize, attention_reference,
...
def test_fuzz_realization_tracks_the_reference():
    ...
    for _ in range(40):
        seq = rng.randint(1, 6); d = rng.randint(1, 6)
        spec = plan_attention(seq, d, target=_HOST)
        q, k, v = _mat(rng, seq * d), _mat(rng, seq * d), _mat(rng, seq * d)
...
def test_r17_certifies_the_bridge_on_a_quantized_attention_call():
    ...
    def call(qbits):
        return Claim(id=7300, opcode=Opcode.MUL, lane=Lane.U, stride_class=StrideClass.UNIT, count=1,
                     rd=(80, 81, 82), wr=(83,), op="gem.attention", domain=Domain.RAM, quantized_bits=qbits)
...
def test_attention_c_emit_routes_softmax_through_the_libm_edge():
    ...
    for bad in [(0, 4), (4, 0)]:
        try:
            emit_attention_c(*bad); assert False, f"{bad} should raise"
        except ValueError:
            pass
