...
from bcir.kbcir.lowbit import (PackedQ4Tensor, SmoothQuantPolicy, admit_low_bit_format,
                               calibrate_smoothquant, calibrated_q4q8_dot, pack_signed_int4,
                               read_packed_q4, unpack_signed_int4, write_packed_q4)
...
def _must_refuse(call, fragment=""):
    try:
        call(); raise AssertionError("invalid low-bit value was accepted")
...
def test_packed_q4_wire_roundtrip_is_deterministic_compact_and_crc_guarded():
    ...
    tensor = PackedQ4Tensor.from_values(values, (32, 32), source_sha256=_HASH_A,
                                        calibration_sha256=_HASH_B)
    with tempfile.TemporaryDirectory() as td:
        first = Path(td) / "first.q4"; second = Path(td) / "second.q4"
        write_packed_q4(first, tensor); write_packed_q4(second, tensor)
        assert first.read_bytes() == second.read_bytes()
        assert read_packed_q4(first) == tensor
        assert first.stat().st_size < len(values) * 4 * 0.25
        corrupt = bytearray(first.read_bytes()); corrupt[-1] ^= 1
        bad = Path(td) / "bad.q4"; bad.write_bytes(corrupt)
...
def test_smoothquant_records_outliers_and_preserves_their_exact_residual():
    ...
    calibration = calibrate_smoothquant(
        activations, weights, SmoothQuantPolicy(outlier_threshold=4.0,
                                                max_outlier_channels=1))
    ...
    _must_refuse(lambda: calibrate_smoothquant(
        activations, weights, SmoothQuantPolicy(outlier_threshold=4.0,
                                                outlier_mode="reject")), "outlier")
...
def test_calibrated_q4q8_dot_is_bounded_over_random_vectors():
    ...
    calibration = calibrate_smoothquant(activations, weights,
                                        SmoothQuantPolicy(outlier_threshold=100.0))
...
def test_format_admission_requires_implementation_provenance_size_and_drift():
    accepted = admit_low_bit_format(
        "bcirq4", artifact_bytes=40, reference_bytes=100, max_logit_error=0.02,
        nll_delta=0.01, source_sha256=_HASH_A, calibration_sha256=_HASH_B,
        max_size_ratio=0.5, max_error=0.05, max_nll_delta=0.02)
    assert accepted.accepted and not accepted.reasons
    refused = admit_low_bit_format(
        "nf4", artifact_bytes=40, reference_bytes=100, max_logit_error=0.02,
        nll_delta=0.01, source_sha256=_HASH_A, calibration_sha256=_HASH_B,
        max_size_ratio=0.5, max_error=0.05, max_nll_delta=0.02)
    assert not refused.accepted and "not present" in refused.reasons[0]
    _must_refuse(lambda: admit_low_bit_format(
        "bcirq4", artifact_bytes=40, reference_bytes=100, max_logit_error=-0.01,
        nll_delta=0.0, source_sha256=_HASH_A, calibration_sha256=_HASH_B,
        max_size_ratio=0.5, max_error=0.05, max_nll_delta=0.02), "magnitudes")
...
int main(void) {{
  ...
  return 0;
    ...
    with tempfile.TemporaryDirectory() as td:
        src = Path(td) / "test.c"; exe = Path(td) / ("test.exe" if os.name == "nt" else "test")
        src.write_text(source, "utf-8")
        build = subprocess.run(host_link_args([
            compiler, "-std=c11", "-O2", "-Wall", "-Wextra", "-Werror",
            "-I", str(_RUNTIME), str(src), str(_RUNTIME / "bcir_q4_kernel.c"),
...
def test_c_q4_avx2_object_contains_real_packed_dot_instruction():
    ...
    compiler = _compiler(); objdump = shutil.which("objdump") or shutil.which("llvm-objdump")
    if compiler is None or objdump is None or machine not in ("x86_64", "amd64"):
        return
    with tempfile.TemporaryDirectory() as td:
        obj = Path(td) / "q4.o"
        build = subprocess.run([
            compiler, "-std=c11", "-O3", "-mavx2", "-Wall", "-Wextra", "-Werror",
            "-I", str(_RUNTIME), "-c", str(_RUNTIME / "bcir_q4_kernel.c"), "-o", str(obj)],
