...
def _oracle_plan(in_c, in_h, in_w, out_c, kh, kw, stride, pad, dtype, quant_bits):
    ...
    return {
        "in_c": in_c, "in_h": in_h, "in_w": in_w, "out_c": out_c, "kh": kh, "kw": kw,
        "stride": stride, "pad": pad, "dtype": dtype,
        "out_h": spec.out_h, "out_w": spec.out_w, "gemm_m": m, "gemm_n": n, "gemm_k": k,
        "strategy": spec.strategy, "tile_m": tm, "tile_n": tn, "tile_k": tk, "loop_order": lo,
...
def test_oracle_conv_plan_is_self_consistent_and_priced_through_the_matmul_roofline():
    ...
    for case in _CASES:
        ...
        assert (p["compute"], p["mem"]) == (mc, mm), f"{case}: cost != the matmul roofline (bespoke term?)"
...
def test_law_rail_reproduces_oracle_conv_plan_via_bcir_opt():
    ...
    for case in _CASES:
        p = _oracle_plan(*case)
        proc = subprocess.run([bo, "-bcir-lower-gem-conv"], input=_conv_mlir(p),
        ...
        n_tiles = (_ceil_div(p["gemm_m"], p["tile_m"]) * _ceil_div(p["gemm_n"], p["tile_n"])
        ...
        assert f'kbcir.compute_cost = {p["compute"]} : i64' in t0, f"{case}: compute != oracle\n{t0}"
        ...
        assert f'kbcir.loop_order = "{p["loop_order"]}"' in t0, f"{case}: loop_order != oracle\n{t0}"
