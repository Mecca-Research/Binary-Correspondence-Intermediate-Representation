...
def _raw(random_ns):
    ...
    return MicrobenchRaw(
        stream_ns=100, strided_ns=100, random_ns=random_ns, compute_ns=100, n=1024, repeats=5
    )
...
def test_bayes_table_json_rejects_ambiguous_or_invalid_artifacts():
    ...
    bad_documents.append(
        table.to_json().replace(
            '"coverage_milli": 800', '"coverage_milli": 800, "coverage_milli": 900'
        )
    )
