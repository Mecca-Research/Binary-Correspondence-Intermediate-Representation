...
def test_json_semantic_range_covers_the_identifier():
    ...
    assert (
        rep.source[d["range"]["start"] : d["range"]["end"]] == "m"
    )  # span starts at the identifier
...
def test_line_map_corrects_the_line_after_an_inlined_include():
    ...
    out = rep.render()
    assert (
        "main.c:2:" in out and "In file included from" not in out
    )  # top-level: no frame, true line
