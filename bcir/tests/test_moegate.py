...
def test_freeze_carries_a_stable_fingerprint_and_gen():
    ...
    assert fz.fingerprint == freeze(train_gate(_SEPARABLE, EXPERTS, epochs=120),
                                    gate_gen=3).fingerprint
...
def test_ledger_labels_pick_the_hindsight_best_expert():
    ...
    samples, experts = ledger_labels(vector_add(1024), AVX, thetas,
...
def test_deploying_a_regressing_gate_is_R13():
    ...
    # verifier disposes.
    regressing = ReplayCertificate(candidate="moe_gate", incumbent="classify",
                                   episodes=5, regressions=2)
