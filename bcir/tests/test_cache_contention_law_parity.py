...
_CASES = [
    ...
    (
        ...
        4,
        64,
    ),  # bank-colliding stride (== banks): gcd(4,4)=4 -> conflict (4-1)*256; reach min(4,16)=4
    (
        16,
        4,
        64,
    ),  # strided gather (stride == cacheline/elem 16): reach 16 -> max waste; gcd(16,4)=4 conflict
]
...
def test_oracle_contention_signal_is_self_consistent_and_quarantined():
    ...
    for stride, eb, count in _CASES:
        ...
        assert p["bank_conflict_q8"] == (gcd(max(1, stride), p["banks"]) - 1) * 256, (
            f"{stride}: conflict model"
        )
    ...
    cv = CachePredictor(cacheline=64, banks=4).cost_vector(
        AccessShape(stride=16, elem_bytes=4, count=64)
    )
    for axis in range(N):
        if axis != CONTENTION:
...
def test_contention_never_gates_legality_verify_plan_ignores_it():
    ...
    assert "CONTENTION" not in src, (
        "the legality verifier must NEVER read the CONTENTION axis (the quarantine)"
    )
...
def test_law_rail_reproduces_oracle_contention_signal_via_bcir_opt():
    ...
    for case in _CASES:
        ...
        proc = subprocess.run(
            [bo, "-bcir-cache-contention"],
            input=_contention_mlir(p),
            capture_output=True,
            text=True,
        )
        ...
        assert f"kbcir.line_waste_q8 = {p['line_waste_q8']} : i64" in line, (
            f"{case}: waste != oracle\n{line}"
        )
        assert f"kbcir.bank_conflict_q8 = {p['bank_conflict_q8']} : i64" in line, (
            f"{case}: conflict != oracle\n{line}"
        )
        assert f"kbcir.contention_q8 = {p['contention_q8']} : i64" in line, (
            f"{case}: q8 != oracle\n{line}"
        )
        assert f"kbcir.contention_cost = {p['contention_cost']} : i64" in line, (
            f"{case}: cost != oracle\n{line}"
        )
        ...
        assert 'kbcir.contention_axis = "CONTENTION"' in line, (
            f"{case}: missing the CONTENTION axis tag\n{line}"
        )
        # a cost producer, NOT a lowering: the carrier op is NOT erased.
        assert "bcir.gem.contention" in out, (
            f"{case}: the contention op must NOT be erased (it is a producer)"
        )
