...
from bcir.hosted.models import (CorpusManifest, HostedRunReport, HostedTrainEvent,
...
def _spec() -> HostedTrainSpec:
    return HostedTrainSpec(
        ...
        steps=64, checkpoint_every=32, learning_rate=3e-3,
        min_learning_rate=0.0, warmup_steps=4, weight_decay=0.0)


def _manifest(token_count: int = 8) -> CorpusManifest:
    return CorpusManifest(
        name="generated-byte-sequence", revision="v1", split="train",
        license="LicenseRef-BCIR-NC-1.0", source_sha256=_HASH_A,
        tokenizer_sha256=_HASH_B, token_count=token_count, vocab_size=260,
        bos_token_id=257, eos_token_id=258, pad_token_id=259)
...
def test_hosted_manifest_and_reports_have_content_identity():
    ...
    report = HostedRunReport(
        schema="bcir.hosted_run.v1", spec_sha256=_HASH_A,
        corpus_sha256=_HASH_A, tokenizer_sha256=_HASH_B,
        steps=1, batch_index=1, tokens_processed=64,
        initial_loss=5.5, final_loss=5.0, model_sha256=_HASH_A,
        optimizer_sha256=_HASH_B, checkpoint_sha256=_HASH_A)
...
def test_tinystories_32m_profile_and_source_inventory_are_pinned():
    ...
    effective_tokens = (spec.context_length * spec.batch_size
    ...
    assert set(pins) == {"dataset", "files", "license", "revision", "schema",
                         "tokenizer_plan"}
    ...
    assert all(len(row["sha256"]) == 64 and row["sha256"] == row["sha256"].lower()
               for row in pins["files"])
    assert pins["tokenizer_plan"] == {
        "add_dummy_prefix": True, "bos_id": 1, "byte_fallback": True,
        "eos_id": 2, "hard_vocab_limit": True,
        "implementation": "sentencepiece", "model_type": "bpe",
        "normalization_rule_name": "identity", "pad_id": 3,
        "remove_extra_whitespaces": False, "shuffle_input_sentence": False,
        "unk_id": 0, "version": "0.2.2", "vocab_size": 16_384}
