...
def test_r19_a_well_formed_timing_block_is_legal():
    tm = Timing(clock_domain="core0", sync_type="synchronous", clock_frequency_mhz=3200,
                latency_cycles=12, setup_hold_margin=2, critical_path=True)
...
def test_r19_setup_hold_margin_cannot_exceed_the_stage_latency():
    tm = Timing(sync_type="synchronous", clock_frequency_mhz=1000, latency_cycles=4, setup_hold_margin=9)
    d = verify_timing(_mod([Claim(id=0, opcode=Opcode.LOAD, rd=(0,), wr=(1,), timing=tm)]))
...
def test_r19_rejects_an_unknown_sync_type_and_negative_cycles():
    ...
    laws = [x.law for x in verify_timing(_mod([Claim(id=0, opcode=Opcode.LOAD, rd=(0,), wr=(1,), timing=tm)]))]
...
def _cdc(consumer_timing):
    ...
    producer = Claim(id=0, opcode=Opcode.STORE, rd=(0,), wr=(1,), timing=Timing(clock_domain="core0"))
...
def test_r20_is_cleared_by_a_barriered_synchronizer():
    producer = Claim(id=0, opcode=Opcode.STORE, rd=(0,), wr=(1,), timing=Timing(clock_domain="core0"))
    consumer = Claim(id=1, opcode=Opcode.LOAD, rd=(1,), wr=(0,), hazard="barriered",
...
def test_r20_same_clock_domain_is_not_a_crossing():
    producer = Claim(id=0, opcode=Opcode.STORE, rd=(0,), wr=(1,), timing=Timing(clock_domain="core0"))
    consumer = Claim(id=1, opcode=Opcode.LOAD, rd=(1,), wr=(0,),
