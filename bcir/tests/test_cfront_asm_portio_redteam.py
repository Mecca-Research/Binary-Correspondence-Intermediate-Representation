...
def _assert_clean_or_diagnoses(src: str) -> str:
    ...
    try:
        ...
        raise AssertionError(
            ...
            f"(CParseError/CLowerError/...) or lower, never crash on this malformed input") from e
    ...
    try:
        ...
        raise AssertionError(
            ...
            f"{type(e).__name__}: {e} -- diagnose must be total (render the error), never crash") from e
    if outcome == "LOWERED":
        assert rep.ok, (src, "compile_unit lowered but diagnose reported errors", rep.diagnostics)
    else:
        assert not rep.ok and rep.diagnostics, (
            src, "compile_unit raised a clean diagnostic but diagnose reported ok", outcome)
    return outcome
...
def test_asm_empty_constraint_strings_are_clean():
    ...
    assert _assert_clean_or_diagnoses(
        'unsigned f(unsigned x){ unsigned y=0; asm("" : ""(y) : ""(x)); return y; }') == "LOWERED"
...
def test_asm_volatile_memory_clobber_lowers_barriered():
    ...
    assert _assert_clean_or_diagnoses('void f(void){ __asm__ __volatile__("" ::: "memory"); }') == "LOWERED"
...
def test_asm_duplicate_output_operands_are_clean():
    ...
    assert _assert_clean_or_diagnoses(
        'unsigned f(unsigned x){ unsigned y=0; asm("" : "=r"(y), "=r"(y) : "r"(x)); return y; }') == "LOWERED"
...
def test_portio_struct_port_is_a_clean_diagnostic():
    ...
    out = _assert_clean_or_diagnoses('struct S { int a; }; unsigned f(struct S s){ return inb(s); }')
    assert out == "CLowerError", out
...
def test_portio_outb_result_used_as_a_value_is_clean():
    ...
    assert _assert_clean_or_diagnoses(
        'unsigned f(unsigned v){ unsigned y = outb(v, 0x60); return y; }') == "LOWERED"
...
def test_portio_outb_result_in_arithmetic_is_clean():
    ...
    assert _assert_clean_or_diagnoses('void f(unsigned v){ unsigned y = outb(v, 0x60) + 1; (void)y; }') in (
        "LOWERED", "CLowerError")
...
def test_portio_deeply_nested_reads_are_a_clean_depth_diagnostic_not_recursion():
    ...
    assert out in ("CParseError", "LOWERED"), out  # a clean depth diagnostic (or lowers if under the guard)
...
def test_parse_unit_rejects_malformed_asm_cleanly():
    ...
    for bad in (
        ...
        try:
            ...
            raise AssertionError(
                f"UNCAUGHT INTERNAL EXCEPTION from parse_unit on {bad!r}: "
...
def test_parse_unit_accepts_asm_goto_grammar_then_lowering_rejects():
    ...
    out = _assert_clean_or_diagnoses('void f(int x){ asm goto("" : : "r"(x) : : L1, L2); L1: ; L2: ; }')
    assert out == "CLowerError", out
