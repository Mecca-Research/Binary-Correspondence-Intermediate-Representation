...
from bcir.asn1.constraints import (Extensible, Intersection, PermittedAlphabet, SingleValue,
                                   Size, Union, ValueRange, effective_size_constraint,
                                   effective_value_constraint, is_unsatisfiable)
...
def test_a_size_constrained_sequence_of_still_carries_its_quantity_field():
    ...
    kind = SequenceOf(Primitive(_INT, "INTEGER", ValueRange(0, 255)),
                      "SEQUENCE OF INTEGER", Size(SingleValue(2)))
...
def test_adding_a_constraint_changes_no_der_octet():
    ...
    plain = Primitive(_INT, "INTEGER")
    for constraint in (ValueRange(0, 255), Extensible(ValueRange(0, 255)),
                       Size(ValueRange(1, 4)), SingleValue(5),
...
def test_a_constraint_form_the_model_cannot_represent_leaves_the_type_unconstrained():
    ...
    for text in ("(WITH COMPONENTS { a PRESENT })", "(PATTERN \"[0-9]#(3)\")",
                 "(0..255 EXCEPT 128)"):
...
def test_an_unsatisfiable_constraint_is_detected():
    ...
    assert is_unsatisfiable(PermittedAlphabet(Union((SingleValue("a"), SingleValue("b")))
                                             )) is False
