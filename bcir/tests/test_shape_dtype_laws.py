...
def _gem_mod(counts, ops=("gem.matmul", "gem.activation:relu")):
    ...
    c1 = Claim(id=1, opcode=Opcode.T_MACC, lane=Lane.T, stride_class=StrideClass.TILE,
               count=counts[0], rd=(1, 2), wr=(3,), op=ops[0], domain=Domain.RAM)
    c2 = Claim(id=2, opcode=Opcode.MUL, lane=Lane.U, stride_class=StrideClass.UNIT,
               count=counts[1], rd=(3,), wr=(4,), op=ops[1], domain=Domain.RAM)
...
def test_r22_flags_a_shape_inconsistent_gem_seam():
    ...
    assert [x.law for x in verify_smart_lowering(_gem_mod((1024, 512))) if x.law == "R22"] == ["R22"]
...
def test_r22_a_non_gem_rewrite_breaks_the_seam():
    ...
    m = _gem_mod((1024, 512))
    mid = Claim(id=3, opcode=Opcode.STORE, lane=Lane.U, stride_class=StrideClass.UNIT,
                count=1, rd=(5,), wr=(3,), op="c.store", domain=Domain.RAM)
...
def test_r23_spec_promotion_classifies_dtype_vs_shape():
    ...
    d = verify_ml_spec("transformer", ["d_model 5 not divisible by n_heads 2",
                                       "softmax/layernorm need f32, got i32"])
...
def test_r23_promotes_the_real_e3_checker():
    ...
    rows = good.rows
    assert verify_ml_spec("transformer",
                          check_transformer(good, (rows, 4), "f32", (rows, 4), "f32")) == []
    laws = {d.law for d in verify_ml_spec(
