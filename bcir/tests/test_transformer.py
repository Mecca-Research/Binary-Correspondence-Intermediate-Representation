...
from bcir.kbcir.transformer import (
    ...
    causal_mask,
    causal_weights_upper_triangle_max,
    check_transformer,
    feedforward_reference,
    layernorm_reference,
    layernorm_stats,
    multihead_attention_reference,
    multihead_concat_residual,
...
def _params(rng, d_model, d_ff, *, identity_ln=True, zeros=False):
    ...
    return TransformerBlockParams(
        ...
        W1=w(d_model * d_ff),
        b1=([0.0] * d_ff if zeros else _vec(rng, d_ff)),
        W2=w(d_ff * d_model),
        b2=([0.0] * d_model if zeros else _vec(rng, d_model)),
        gamma1=g(),
        beta1=be(),
        gamma2=g(),
        beta2=be(),
...
def test_layernorm_gamma_beta_affine_after_normalization():
    ...
    for mean, var in layernorm_stats(y, rows, dim):
        ...
        assert abs(var - gconst * gconst) < 1e-2, (
            var
        )  # variance scales by gamma^2 (var~1 -> gamma^2)
...
def test_causal_mask_zeros_the_upper_triangle_exactly():
    ...
    for h in range(spec.n_heads):
        qh = _split_head(
            _project(
                [rng.uniform(-1, 1) for _ in range(seq * spec.d_model)],
                params.w_q,
                seq,
                spec.d_model,
            ),
...
def test_multihead_reuses_the_existing_softmax_and_scores():
    ...
    full = multihead_attention_reference(
        x, TransformerBlockSpec(d_model, n_heads, 16, seq, 1), params.mha
    )
    ...
    concat = multihead_attention_reference(
        x,
        TransformerBlockSpec(d_model, n_heads, 16, seq, 1),
        TransformerBlockParams(
            params.w_q,
            params.w_k,
            params.w_v,
...
def test_feedforward_matches_a_hand_composition():
    ...
    assert (
        len(feedforward_reference(x, rows, d_model, d_ff, W1, b1, W2, b2, "gelu")) == rows * d_model
    )
...
def _naive_block(x, spec, params, mask, activation, eps, pre_ln):
    ...
    def ff(z):
        return feedforward_reference(
            z, rows, d_model, spec.d_ff, params.W1, params.b1, params.W2, params.b2, activation
        )
...
def test_zero_weight_block_reduces_to_layernorm_of_layernorm():
    ...
    ln1 = layernorm_reference(
        x, spec.rows, spec.d_model, [1.0] * spec.d_model, [0.0] * spec.d_model
    )
    ln2 = layernorm_reference(
        ln1, spec.rows, spec.d_model, [1.0] * spec.d_model, [0.0] * spec.d_model
    )
...
def test_block_bridge_is_block_of_the_roundtrip():
    ...
    assert transformer_block_via_bridge(
        x, spec, params, spec.d_model, 8
...
def test_layernorm_c_kernel_matches_the_reference():
    ...
    assert all(
        abs(cm - rm) < 1e-3 and abs(cv - rv) < 1e-3 for (cm, cv), (rm, rv) in zip(cstats, rstats)
    )
...
def test_layernorm_c_emit_routes_rsqrt_through_the_libm_edge():
    ...
    assert (
        "c.call.libm: edge" in src or "c.call.libm:sqrtf" in src
    )  # the edge label in the docstring comment
    assert (
        "void ln(const float *restrict x, const float *restrict gamma," in src
    )  # the baked-in signature
    for bad in [(0, 4), (4, 0)]:
        try:
...
def test_transformer_block_training_drives_loss_down():
    ...
    for _ in range(24):
        ...
        ys.append(
            sum(coef[j] * f[j] for j in range(dm)) + 0.1
        )  # an exactly-linear target over the features
