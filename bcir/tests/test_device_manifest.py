...
from bcir.kbcir.device_manifest import (DeviceManifest, MemoryBank, StridedView,
                                        check_bank_moves, check_device_manifest,
                                        check_strided_view, load_device_manifest,
                                        move_claim, move_cost, probe_agree,
                                        save_device_manifest)
from bcir.model import Domain, Module, Phase, Resource

_MAN = DeviceManifest(
    device="toy_npu",
    banks=(MemoryBank("sram0", Domain.RAM, 1 << 16, native_tile=16, align=64),
...
def test_the_manifest_is_checked_digested_and_tamper_refused():
    ...
    bad_dist = dataclasses.replace(_MAN, distance=((0, 512, 640), (512, 0, 256),
                                                   (999, 256, 0)))
    assert any("asymmetric" in m for m in check_device_manifest(bad_dist))
    bad_cap = dataclasses.replace(_MAN, banks=_MAN.banks[:2] + (
        MemoryBank("hbm1", Domain.HBM, 0),))
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for mutate, needle in ((lambda d: d.update(kind="nope"), "not a"),
    ...
    if with_move:
        ...
        if malformed:                                            # two sources: not a move
            c = Claim(id=1, opcode=Opcode.ADD, lane=Lane.U, stride_class=StrideClass.UNIT,
                      count=64, rd=(1, 3), wr=(2,), op="mem.move.far:sram0->hbm0",
                      domain=Domain.HBM, bounds="assumed_safe")
    else:                                                        # an IMPLICIT crossing
        c = Claim(id=1, opcode=Opcode.ADD, lane=Lane.U, stride_class=StrideClass.UNIT,
                  count=64, rd=(1,), wr=(2,), op="gem.matmul", domain=Domain.HBM,
                  bounds="assumed_safe")
...
def test_bank_typing_requires_explicit_moves_and_is_corpus_vacuous():
    ...
    assert any("exactly one" in m
               for m in check_bank_moves(_two_bank_module(True, malformed=True)))
    ...
    for mod in (train_step_module(TrainStepSpec()),
                ...
                decode_session_module(DecoderSpec(vocab_size=8, d_model=8, n_heads=2,
                                                  n_layers=1, d_ff=4), 2, 2)):
