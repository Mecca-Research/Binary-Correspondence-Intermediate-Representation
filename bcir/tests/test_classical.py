...
from bcir.kbcir.classical import (
    GaussianNbModel,
    KnnModel,
    SvmModel,
    TreeModel,
    check_classical,
    gaussian_nb_log_posterior,
    gaussian_nb_log_posterior_independent,
    knn_classify,
    knn_classify_via_bridge,
    knn_neighbours,
    knn_neighbours_independent,
    knn_regress,
    nb_predict,
    nb_predict_via_bridge,
    squared_distance,
    svm_decision_linear,
    svm_decision_linear_independent,
    svm_decision_rbf,
    svm_decision_rbf_independent,
    svm_predict,
    svm_predict_linear_via_bridge,
    tree_leaves_reachable,
    tree_predict,
    tree_predict_recursive,
    tree_predict_via_bridge,
)
...
def test_tree_recursive_verifier_matches_the_iterative_walk():
    ...
    for _ in range(200):
        ...
        assert tree_predict(t, x) == tree_predict_recursive(
            t, x
        )  # independent control flow, same leaf
...
def test_svm_linear_independent_verifier_matches():
    ...
    nf, nsv = 5, 8
    m = SvmModel(
        tuple(rng.uniform(-2, 2) for _ in range(nsv * nf)),
        tuple(rng.uniform(-1, 1) for _ in range(nsv)),
        rng.uniform(-1, 1),
        nf,
    )
    for _ in range(30):
...
def test_svm_rbf_decision_matches_an_independent_kernel_sum():
    ...
    nf, nsv = 4, 6
    m = SvmModel(
        tuple(rng.uniform(-1, 1) for _ in range(nsv * nf)),
        tuple(rng.uniform(-1, 1) for _ in range(nsv)),
        rng.uniform(-1, 1),
        nf,
    )
...
def test_nb_independent_full_log_likelihood_verifier_matches():
    ...
    nc, nf = 3, 4
    m = GaussianNbModel(
        tuple(math.log(1.0 / nc) for _ in range(nc)),
        tuple(rng.uniform(-3, 3) for _ in range(nc * nf)),
        tuple(rng.uniform(0.5, 2.0) for _ in range(nc * nf)),
        nf,
    )
    for _ in range(30):
...
def test_nb_model_validates():
    for args in [
        ((math.log(0.5),), (0.0,), (1.0,), 0),  # n_feat < 1
        ((math.log(0.5),), (0.0, 0.0), (1.0,), 1),  # mean length mismatch
        ((math.log(0.5),), (0.0,), (0.0,), 1),
    ]:  # var <= 0
        try:
...
def test_check_classical_accepts_well_formed_claims():
    ...
    assert (
        check_classical("tree", n_feat=2, n_param=5, in_dtype="i32", out_dtype="i32") == []
    )  # tree exact -> i32 ok
    assert (
        check_classical("knn", n_feat=3, n_param=10, in_dtype="i32", out_dtype="i32") == []
    )  # knn exact -> i32 ok
    assert (
        check_classical(
            "svm", n_feat=5, n_param=8, in_dtype="i32", out_dtype="i32", svm_kernel="linear"
        )
        == []
    )  # linear svm exact -> i32 ok
    assert (
        check_classical(
            "svm", n_feat=5, n_param=8, in_dtype="f32", out_dtype="f32", svm_kernel="rbf"
        )
        == []
    )
...
def test_check_classical_rejects_malformed_claims():
    assert any(
        "not a known predictor" in e
        for e in check_classical("foo", n_feat=2, n_param=2, in_dtype="f32", out_dtype="f32")
    )
    assert any(
        ">= 1" in e
        for e in check_classical("knn", n_feat=0, n_param=2, in_dtype="f32", out_dtype="f32")
    )
    assert any(
        ">= 1" in e
        for e in check_classical("knn", n_feat=2, n_param=0, in_dtype="f32", out_dtype="f32")
    )
    assert any(
        "not preserved" in e
    ...
    assert any(
        "needs f32" in e
        for e in check_classical(
            "svm", n_feat=2, n_param=2, in_dtype="i32", out_dtype="i32", svm_kernel="rbf"
        )
    )
    assert any(
        "needs f32" in e
        for e in check_classical("nb", n_feat=2, n_param=2, in_dtype="i32", out_dtype="i32")
    )
    # an unknown svm kernel is rejected.
    assert any(
        "not a known kernel" in e
        for e in check_classical(
            "svm", n_feat=2, n_param=2, in_dtype="f32", out_dtype="f32", svm_kernel="poly"
        )
    )
...
def test_classical_c_emit_routes_and_validates():
    ...
    assert (
        "math.h" not in tree_src and "expf" not in tree_src
    )  # the tree is exact -- NO transcendental, NO libm
...
def test_classical_link_flag_rule_reuses_the_existing_libm_rule():
    ...
    assert (
        library_for_callee("logf") == "-lm"
    )  # the Gaussian-NB normaliser edge (baked, but logf maps)
...
def test_classical_nb_fit_quality_on_toy_data():
    ...
    model = GaussianNbModel(
        log_prior=tuple(_m.log(1.0 / nc) for _ in range(nc)),
        mean=tuple(mean),
        var=tuple(var),
        n_feat=nf,
    )
