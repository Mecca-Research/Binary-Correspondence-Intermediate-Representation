...
def test_composition_preserves_labels():
    ...
    assert op.get(s).label == f_label(("MEMORY", "op", "add"), (op.get(b).label, op.get(c).label))
...
def test_a_dangling_source_is_R13():
    ...
    op.ops[123] = EnrichedOp(
        name="mul", arity=1, label=("MEMORY", "op", "mul"), index=123, children=(999,)
    )
...
def test_a_tampered_index_is_R13():
    ...
    op.ops[7] = EnrichedOp(name="mul", arity=0, label=("MEMORY", "op", "mul"), index=7, children=())
    assert "R13" in _laws(verify_enriched(op))
...
def test_enrich_memory_lifts_the_fixpoint_into_a_labeled_indexed_tree():
    ...
    a_nodes = [i for i, o in operad.ops.items() if o.label == ("MEMORY", "var", "a")]
