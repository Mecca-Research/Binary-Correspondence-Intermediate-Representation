...
def _numpy_free_dft(signal):
    ...
    n = len(signal)
    return [
        sum(signal[k] * cmath.exp(-2j * math.pi * j * k / n) for k in range(n)) for j in range(n)
    ]
...
def test_emit_wraps_fftw_with_a_reference_fallback():
    ...
    assert (
        "fftwf_plan_dft_1d(8, fin, fout, FFTW_FORWARD, FFTW_ESTIMATE)" in c
    )  # the trusted external
    ...
    assert (
        "re += xr * c - xi * s" in c and "im += xr * s + xi * c" in c
    )  # the reference DFT fallback
    for bad in (0, -1):
        try:
            emit_fftw_fft_c(bad)
            assert False, f"{bad} should raise"
