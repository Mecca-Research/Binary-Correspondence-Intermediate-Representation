...
def test_emit_wraps_cblas_with_a_reference_fallback():
    ...
    assert "cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans" in c   # the trusted external kernel
    ...
    for bad in [(0, 1, 1), (1, 0, 1), (1, 1, 0)]:
        try:
            emit_blas_gemm_c(*bad); assert False, f"{bad} should raise"
        except ValueError:
            pass
...
def test_r17_certifies_the_bridge_on_a_quantized_gemm_call():
    ...
    def call(qbits):
        return Claim(id=7000, opcode=Opcode.MUL, lane=Lane.U, stride_class=StrideClass.UNIT, count=1,
                     rd=(50, 51), wr=(52,), op="call.gemm", domain=Domain.RAM, quantized_bits=qbits)
