...
def test_the_tower_spans_every_hardware_class():
    ...
    assert {"x86_avx512", "x86_avx2", "arm64_neon", "arm64_sve", "riscv_rvv", "nvidia_ptx"} <= set(
        CHANNELS
    )
...
def test_x86_and_arm_rules_do_not_collide():
    ...
    assert (
        x.runtime.perf_syscall_nr == 298 and a.runtime.perf_syscall_nr == 241
    )  # the #255 collision
...
def test_a_cpu_only_tower_keeps_everything_on_the_cpu():
    ...
    assert (
        plan.total_cost
        == optimize(matmul_tiled(), CHANNELS["x86_avx512"].profile, Theta.cool()).score
    )
...
def test_orchestration_tags_one_streampack_across_the_tower():
    ...
    pack = apply_channels(
        hydrate(m, optimize(m, CHANNELS["x86_avx512"].profile, Theta.cool())), plan
    )
    placed = {p.claim_id: p.channel for p in plan.placements}
...
def test_adding_a_channel_does_not_touch_the_others():
    ...
    try:
        accel = HardwareChannel(
            "tpu_test",
            "accelerator",
            CHANNELS["nvidia_ptx"].profile,
            "",
            0,
            RuntimeChannel(),
            modeled=True,
