...
def _uart_rx_module(
    *,
    armed: bool = True,
    masked_consumer: bool = True,
    atomic_consumer: bool = False,
    deps_on_event: bool = False,
) -> Module:
    ...
    for r in (
        Resource(rid=_IER, domain=Domain.MMIO, shape=(1,), name="IER"),
    ...
    body = (
        [mask_claim("uart_rx", _IER, cid=2), consume, unmask_claim("uart_rx", _IER, cid=4)]
        if masked_consumer
        else [consume]
    )
    ...
    m.add_phase(
        Phase(
            phase_id=10,
            deps=(0,) if deps_on_event else (),
            claims=[dispatch, fill],
            event="uart_rx",
        )
    )
    return m
