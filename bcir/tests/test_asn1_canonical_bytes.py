...
def test_a_schema_directed_decode_refuses_another_types_encoding() -> None:
    ...
    """
    for hexs, what in (("0101ff", "a BOOLEAN encoding"),
                       ("040141", "an OCTET STRING encoding"),
                       ("0500", "a NULL encoding")):
        message = _refused(
            lambda h=hexs: _INT.decode(decode_one(bytes.fromhex(h)),
                                       strictness=Strictness.DER),
            f"{what} for an INTEGER schema")
        assert "8.1.2" in message, message

    # The type it actually names still decodes.
    assert _INT.decode(decode_one(bytes.fromhex("020105")),
                       strictness=Strictness.DER) == 5
...
def test_der_refuses_a_component_encoded_at_its_default() -> None:
    ...
    kind = Sequence((Component("version", _INT, tag=0, default=1),
                     Component("body", _INT, tag=1)), name="D")
    omitted = encode_tlv(kind.encode({"version": 1, "body": 7}))
    assert omitted.hex() == "3003810107", "the encoder has always omitted it"

    written = encode_tlv(Tlv(Tag(TagClass.UNIVERSAL, Universal.SEQUENCE, True), b"", [
        Tlv(Tag(TagClass.CONTEXT, 0), b"\x01"),
        Tlv(Tag(TagClass.CONTEXT, 1), b"\x07")]))
    ...
    assert kind.decode(decode_one(written), strictness=Strictness.BER) == \
        {"version": 1, "body": 7}
    assert kind.decode(decode_one(omitted), strictness=Strictness.DER) == \
        {"version": 1, "body": 7}
...
def test_a_set_refuses_a_component_encoded_at_its_default() -> None:
    ...
    kind = Set((Component("v", _INT, tag=0, default=1),
                Component("b", _INT, tag=1)), name="T")
    omitted = encode_tlv(kind.encode({"v": 1, "b": 7}))
    assert kind.decode(decode_one(omitted), strictness=Strictness.DER) == {"v": 1, "b": 7}

    written = encode_tlv(Tlv(Tag(TagClass.UNIVERSAL, Universal.SET, True), b"", [
        Tlv(Tag(TagClass.CONTEXT, 0), b"\x01"),
        Tlv(Tag(TagClass.CONTEXT, 1), b"\x07")]))
...
def test_der_real_is_base_two_with_an_odd_mantissa() -> None:
    ...
    for hexs, why in (("0903900101", "base 8"),
                      ("0903a00101", "base 16"),
    ...
    assert from_tlv(decode_one(bytes.fromhex("0903900101")),
                    strictness=Strictness.BER) == 8.0
...
def test_der_real_decimal_is_iso_6093_not_a_python_literal() -> None:
    ...
    for text, why in (("1_0", "a PEP 515 underscore"),
        ...
        assert "11.3" in _refused(lambda h=decimal(3, text): _der(h),
                                  f"{text!r} ({why}) under strict DER")

    # The NR selector must MEAN something: NR1 is ISO 6093's integer form.
    assert "8.5.8" in _refused(lambda: _der(decimal(1, "1.5e3")),
                               "a floating-point field declared NR1")
    assert from_tlv(decode_one(bytes.fromhex(decimal(1, "42"))),
                    strictness=Strictness.BER) == 42.0
...
def test_every_real_the_encoder_writes_survives_the_new_check() -> None:
    ...
    values = [0.0, 1.0, 2.0, 4.0, -4.0, 0.5, 0.125, 1.5, 1e10, 1e-10, -1e-10,
              3.14159265358979, 1e300, 1e-300, math.inf, -math.inf, -0.0,
              2.0 ** 53 + 1.0, 123456.789]
    for value in values:
...
def test_a_constructed_string_segment_must_be_the_same_type() -> None:
    ...
    """
    for hexs in ("1a054a6f6e6573",                      # primitive
                 "3a09" "04034a6f6e" "04026573",       # 8.23.5's constructed-definite
                 "3a80" "04034a6f6e" "04026573" "0000"):  # ... and indefinite
        assert decode_string(decode_one(bytes.fromhex(hexs))) == "Jones", hexs

    for hexs, why in (("3a03020105", "an INTEGER segment"),
                      ("3a05" "1a03616263", "a VisibleString segment (8.7.3.2 says 4)"),
                      ("3a05" "2403020105", "an INTEGER nested under an OCTET STRING")):
    ...
    assert "8.7.3.2" in _refused(
        lambda: decode_string(decode_one(bytes.fromhex("3a03" "02017f"))),
...
def test_int_to_chars_inverts_over_the_clauses_repertoire_only() -> None:
    ...
    for text, want in (("42", 42), ("  42  ", 42), ("+42", 42), ("-0042", -42),
                       ("007", 7), ("0", 0), ("-0", 0)):
        assert transform.inverse_one(text) == want, text

    for text, why in (("4_2", "a PEP 515 underscore"),
                      ("1_000", "a PEP 515 underscore"),
                      ("۴۲", "ARABIC-INDIC digits"),
                      ("４２", "FULLWIDTH digits"),
                      ("4٠2", "an ARABIC-INDIC zero mid-string")):
        assert "24.7.4" in _refused(
            lambda t=text: transform.inverse_one(t),
            f"{text!r} ({why}) as an INT-TO-CHARS field")
