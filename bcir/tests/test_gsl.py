...
def test_stats_reference_rejects_malformed_inputs():
    try:
        stats_reference([], "mean")
        assert False, "empty mean should raise"
    except ValueError:
        pass
    for bad_kind in ("variance", "sd"):
        try:
            stats_reference([1.0], bad_kind)
            assert False, f"n<2 {bad_kind} should raise"
        except ValueError:
            pass
    try:
        stats_reference([1.0, 2.0], "median")
...
def test_linked_gsl_path_agrees_when_gsl_is_present():
    ...
    for kind in GSL_STATS:
        ref = stats_reference(data, kind)
        got = _run_stats_kernel(
            cc, emit_gsl_stats_c(kind, len(data), "stat"), data, define="-DBCIR_USE_GSL", libs=libs
        )
