...
from bcir.kbcir.autodiff_program import (
    ADProgramFeatures,
    ADQuarantineReason,
    ADQuarantinedError,
    MutationStatement,
    admit_ad_program,
    compare_ad_ordering,
    defunctionalize_call,
    functionalize_scalar_mutation,
    grad_with_rematerialization,
    trace_bounded_while,
)
...
def test_admission_rewrites_supported_features_and_quarantines_escapes():
    supported = admit_ad_program(
        ADProgramFeatures(
            mutation="local_scalar", dynamic_loop=True, loop_max_steps=8, higher_order="finite"
        )
    )
    assert supported.admitted and len(supported.rewrites) == 3
    refused = admit_ad_program(
        ADProgramFeatures(
            mutation="aliased",
            dynamic_loop=True,
            higher_order="dynamic",
            recursion=True,
            unknown_calls=True,
        )
    )
...
def test_local_scalar_mutation_is_functionalized_into_ssa_nodes():
    program = functionalize_scalar_mutation(
        ("x", "y"), (MutationStatement("x", "add", "y"), MutationStatement("x", "mul", 2.0)), "x"
    )
...
def test_dynamic_while_is_traced_with_a_hard_budget_and_remains_differentiable():
    ...
    other = Tape()
    start = other.const(0.0)
    one2 = other.const(1.0)
    three2 = other.const(3.0)
    try:
        trace_bounded_while(
            other,
            start,
            lambda graph, carry: graph.sub(three2, carry),
            lambda graph, carry: graph.add(carry, one2),
            {},
            max_steps=2,
        )
...
def test_finite_higher_order_calls_are_defunctionalized_by_tag():
    ...
    x = tape.var("x")
    table = {
        "square": lambda graph, value: graph.mul(value, value),
