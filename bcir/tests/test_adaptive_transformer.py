...
def _uniform(*, layers=3, width=16, context=16, repeats=1,
             residual_mode="pre_norm", window=None, anchor=None,
             gated=False):
    schedule = WidthScheduleSpec(
        (width,) * layers, (2,) * layers, (2 * width,) * layers)
    return AdaptiveLanguageSpec(
        64, context, schedule, repeats=repeats,
        residual_mode=residual_mode, reference_window=window,
        anchor=anchor, gated_attention=gated)


def test_geometric_width_schedule_is_symmetric_aligned_and_strict():
    schedule = WidthScheduleSpec.symmetric_u(
        layers=5, outer_width=32, inner_width=16,
        head_dim=8, ff_multiplier=3, quantum=8)
    ...
    even = WidthScheduleSpec.symmetric_u(
        layers=6, outer_width=32, inner_width=16,
    ...
    _refuse(lambda: WidthScheduleSpec.symmetric_u(
        layers=3, outer_width=30, inner_width=16, head_dim=8), "align")
    _refuse(lambda: WidthScheduleSpec.symmetric_u(
        layers=2, outer_width=32, inner_width=16, head_dim=8), "fewer than three")
...
def test_reference_sliding_attention_has_global_prefix_and_bounded_tail():
    ...
    assert window.visible_pairs(8) == sum(
        len(window.visible_keys(index, 8)) for index in range(8))
...
def test_exogenous_anchor_contract_quarantines_unstudied_combinations():
    ...
    _refuse(lambda: _uniform(
        layers=2, repeats=2, residual_mode="loop_deepnorm",
        anchor=anchor, gated=True), "pre-norm")
    variable = WidthScheduleSpec((16, 8, 16), (2, 1, 2), (32, 16, 32))
    _refuse(lambda: AdaptiveLanguageSpec(
        64, 16, variable, gated_attention=True, anchor=anchor), "fixed-width")
    _refuse(lambda: _uniform(
        layers=1, anchor=ExogenousAnchorSpec(source_depth=1), gated=True), "precede")
...
def test_variable_width_and_reference_window_costs_are_honest_lower_bounds():
    ...
    sliding = _uniform(
        layers=3, context=32, window=ReferenceWindowSpec(4, 6))
...
def test_adaptive_lowering_is_verified_complete_and_deterministic():
    schedule = WidthScheduleSpec.symmetric_u(
        ...
        head_dim=8, quantum=8, ff_multiplier=2)
    spec = AdaptiveLanguageSpec(
        64, 16, schedule, repeats=2, residual_mode="loop_deepnorm",
    ...
    assert len(first.realization.steps) == sum(
        len(phase.claims) for phase in first.module.phases)
    ...
    attention_projection = [
        claim for phase in first.module.phases for claim in phase.claims
...
def test_multipatch_costs_coarse_attention_without_claiming_runtime_speed():
    spec = MultiPatchSpec(
        image_size=16, input_channels=3, hidden_width=16, heads=4,
        coarse_patch=4, fine_patch=2, coarse_layers=3, fine_layers=1,
        condition_tokens=2)
