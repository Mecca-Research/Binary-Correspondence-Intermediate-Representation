...
def test_ols_supports_multiple_right_hand_sides():
    ...
    m, n, nrhs = 6, 2, 2
    a = _design(m, n)
    x_true = [3.0, 1.0,                                   # rhs0 = [3,1] ... laid out n x nrhs row-major
              -2.0, 0.5]                                  # ... per column: x[j*nrhs + r]
...
def test_ols_reference_rejects_a_rank_deficient_design():
    ...
    m, n = 4, 2
    a = [1.0, 1.0,                                        # column 0 == column 1 -> rank 1
         2.0, 2.0,
         3.0, 3.0,
         4.0, 4.0]
    b = [1.0, 2.0, 3.0, 4.0]
...
def test_bridged_ols_tracks_the_reference_within_quant_error():
    ...
    for i in range(m):
        a.append(1.0)                                     # bias column
        for j in range(1, n):
            a.append(rng.uniform(-2.0, 2.0) + 5.0 * j)    # spread, well-separated -> well-conditioned A^T A
...
def test_emit_wraps_lapack_sgels_with_a_normal_equations_fallback():
    ...
    assert "LAPACKE_sgels(LAPACK_ROW_MAJOR, 'N', 8, 3, 1, qa, 3, bx, 1)" in c   # the trusted QR least-squares kernel
    ...
    assert "void ols(const float *A, const float *b, float *x)" in c           # the signature bakes m,n,nrhs in
    ...
    for bad in [(0, 0), (3, 5), (4, 0)]:                                        # n<1 (m=0), m<n (3<5), nrhs handled below
        try:
            ...
            pass
    try:
        emit_lapack_ols_c(4, 2, 0); assert False, "nrhs<1 should raise"
    except ValueError:
        pass
...
def test_fallback_path_compiles_runs_and_matches_the_reference():
    ...
    assert normal_equation_residual(a, got, b, m, n) < 1e-2, normal_equation_residual(a, got, b, m, n)
...
def test_linked_lapack_path_agrees_when_lapack_is_present():
    ...
    b = _matvec(a, x_true, m, n)
    ref = ols_reference(a, b, m, n)
    got = _run_ols_kernel(cc, emit_lapack_ols_c(m, n, 1, "ols"), a, b, m, n, 1,
...
def test_ols_link_flag_rule_reuses_the_existing_lapack_rule():
    ...
    assert library_for_callee("LAPACKE_sgels") == "-llapack"        # the OLS callee (the wrapper's actual symbol)
    assert library_for_callee("LAPACKE_sgesv") == "-llapack"        # the square-solve sibling still maps
    assert library_for_callee("sgels_") == "-llapack" or library_for_callee("sgels_") is None  # sgels not in the Fortran set; either way no surprise flag
...
def test_ols_touches_no_verifier_and_emits_no_diagnostic():
    ...
    assert isinstance(normal_equation_residual([1.0, 0.0, 1.0, 1.0, 1.0, 2.0], x,
                                               [1.0, 2.0, 3.0], 3, 2), float)
