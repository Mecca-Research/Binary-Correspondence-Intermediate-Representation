...
def test_a_dummy_carries_exactly_the_governor_its_kind_requires():
    ...
    Parameter(
        "obj", ParameterKind.ENCODING_OBJECT, GovernorKind.DEFINED_OR_BUILTIN_ENCODING_CLASS, "#INT"
    )
    _object_set_param()  # governed by #ENCODINGS

    _refuses(
        "C.1 a)", lambda: Parameter("#D", ParameterKind.ENCODING_CLASS, GovernorKind.REFERENCE)
    )
    _refuses("shall be governed by REFERENCE", lambda: Parameter("ref", ParameterKind.IDENTIFIER))
    _refuses(
        "shall be governed by #ENCODINGS",
        lambda: Parameter("set", ParameterKind.ENCODING_OBJECT_SET, GovernorKind.REFERENCE),
    )
    _refuses(
        "a Parameter needs a DummyReference", lambda: Parameter("", ParameterKind.ENCODING_CLASS)
    )
...
def test_a_keyword_governor_has_nothing_to_name_and_a_class_governor_must():
    ...
    _refuses(
        "keyword governor with nothing to name",
...
def test_dummy_governors_are_legal_asn1_and_refused_in_ecn():
    ...
    _refuses(
        "DummyGovernors are not allowed in ECN",
...
def test_each_dummy_kind_takes_its_own_actual_alternative():
    ...
    check_actuals(
        params, ActualParameterList((ActualParameter(ActualKind.ENCODING_CLASS, "#INT"),))
    )
    _refuses(
        "C.4",
        lambda: check_actuals(
            params,
            ActualParameterList((ActualParameter(ActualKind.ENCODING_OBJECT, "int-object"),)),
        ),
    )
...
def test_a_reference_dummy_has_four_spellings_and_one_of_them_lives_a_clause_away():
    ...
    for actual in (
        ActualParameter(ActualKind.IDENTIFIER, "length"),
    ...
    _refuses(
        "C.4",
        lambda: check_actuals(
            params, ActualParameterList((ActualParameter(ActualKind.VALUE, "3"),))
        ),
    )
...
def test_a_component_id_list_is_a_dotted_path_and_its_parts_are_identifiers():
    ...
    _refuses("is not a ComponentIdList", lambda: ActualParameter(ActualKind.OUTER).components())
...
def test_the_wrong_count_and_the_wrong_kinds_are_named_as_two_different_faults():
    ...
    check_actuals(
        params,
        ActualParameterList(
            (ActualParameter(ActualKind.ENCODING_CLASS, "#INT"), ActualParameter(ActualKind.OUTER))
        ),
    )
...
def test_only_an_object_assignment_carries_a_governor_before_its_assignment_sign():
    ...
    _refuses("C.2", lambda: ParameterizedAssignment("obj", AssignmentKind.ENCODING_OBJECT, params))
    _refuses(
        "C.2",
        lambda: ParameterizedAssignment(
...
def test_a_dummy_reaches_backwards_across_the_assignment_sign_only_for_an_object():
    ...
    ParameterizedAssignment(
        ...
        AssignmentKind.ENCODING_OBJECT,
        params,
        governor="#New-component",
        governor_actuals=borrowed,
    )
    _refuses(
        "8.4",
        lambda: ParameterizedAssignment(
...
def test_instantiation_checks_the_correspondence_and_hands_back_bindings():
    ...
    bindings = assignment.instantiate(
        ActualParameterList((ActualParameter(ActualKind.ENCODING_CLASS, "#INT"),))
    )
...
def _replacement(**changes) -> ReplacementParameterization:
    defaults = dict(
        ...
        governor_actuals=ActualParameterList((ActualParameter(ActualKind.ENCODING_CLASS, "#D"),)),
    )
...
def test_a_replacement_structure_has_exactly_one_encoding_class_parameter():
    ...
    _refuses(
        "22.1.2.2",
        lambda: _replacement(structure=ParameterList((_class_param(), _class_param("#E")))),
    )
    _refuses("22.1.2.2", lambda: _replacement(structure=ParameterList((_object_set_param(),))))
...
def test_the_encoded_by_objects_governor_is_the_structure_instantiated_with_its_own_dummy():
    ...
    **instantiated with `#D`**".
    ...
    _refuses(
        "22.1.2.4",
        lambda: _replacement(encoded_by=ParameterList((_reference_param(),)), insert_at_head=True),
    )
...
def test_the_reference_parameter_is_present_if_and_only_if_insert_at_head_is():
    ...
    _refuses(
        "if and only if",
        lambda: _replacement(encoded_by=ParameterList((_class_param(), _reference_param()))),
    )
...
def test_the_encoded_by_object_takes_at_most_one_object_set_and_one_reference():
    ...
    both = _replacement(
        encoded_by=ParameterList((_class_param(), _object_set_param(), _reference_param())),
        insert_at_head=True,
    )
    ...
    _refuses(
        ...
        lambda: _replacement(
            encoded_by=ParameterList(
                (_class_param(), _reference_param("r1"), _reference_param("r2"))
            ),
            insert_at_head=True,
        ),
    )
...
def test_a_head_end_structure_has_no_dummy_parameters_at_all():
    ...
    _refuses(
        "22.1.2.7",
        lambda: _replacement(encoded_by=None, governor_actuals=None, insert_at_head=True),
    )
...
def test_inside_replace_the_name_is_bare_even_though_the_definition_is_parameterized():
    ...
    """
    assert (
        bare_use(ParameterizedReference("#Length-prefixed"), clause="§22.1.2.2")
        == "#Length-prefixed"
    )
    _refuses(
        "22.1.2.2",
        lambda: bare_use(
            ParameterizedReference(
                "#Length-prefixed",
                ActualParameterList((ActualParameter(ActualKind.ENCODING_CLASS, "#D"),)),
            ),
            clause="§22.1.2.2",
        ),
    )
...
def test_the_first_identifier_is_found_breadth_first_and_not_by_a_recursive_walk():
    ...
    **That is breadth-first.** A recursive walk over `header` first would reach
    ...
    assert resolve_component({"header": {"flags": {"width": "d"}, "width": "s"}}, ("width",)) == (
        "header",
        "width",
    )
...
def test_three_governors_determine_a_kind_and_two_do_not():
    ...
    _refuses("stands for no dummy kind", lambda: Parameter("t", None, GovernorKind.TYPE, "INTEGER"))
...
def test_an_undetermined_dummy_accepts_every_actual_its_governor_admits():
    ...
    shared = ParameterList(
        (Parameter("obj", None, GovernorKind.DEFINED_OR_BUILTIN_ENCODING_CLASS, "#INT"),)
    )
    ...
    _refuses(
        "C.4",
        lambda: check_actuals(
            shared, ActualParameterList((ActualParameter(ActualKind.ENCODING_CLASS, "#INT"),))
        ),
    )
