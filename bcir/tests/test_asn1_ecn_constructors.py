...
def test_a_handle_value_set_reads_as_ranges_over_the_conceptual_field():
    ...
    assert HandleValueSet.of_ranges(((0, 2), (2, 5), (9, 9))).ranges_over(4) == ((0, 5), (9, 9))
...
def test_positions_are_taken_after_pre_alignment():
    ...
    objects = _concat(
        {
            "a": PadSpec(width=3),
            "v": IntSpec(width=8, exhibits=handle, pre_alignment=PreAlignment(unit=UNIT_OCTET)),
        },
        ("a", "v"),
        padding=("a",),
    )
...
def test_tag_any_is_only_legal_for_a_tag_object():
    ...
    objects = _objects(
        ConcatenationSpec(
            fields={"t": TagSpec(width=4, number=6, exhibits=tag_handle)},
            order=("t",),
            padding=("t",),
        )
    )
    ...
    _refuses("22.9.1.9", lambda: encode_with_user(bad, "#C", {"v": 1}, outer=_pad_to_octet()))
...
def test_the_registry_enforces_the_two_specification_wide_rules():
    ...
    registry.declare(
        IdentificationHandle("k", (0, 1), HandleValueSet.of_number(0)),
        where="'a'",
        alignment_unit=UNIT_OCTET,
    )
    _refuses(
        "22.9.2.1",
        lambda: registry.declare(
            IdentificationHandle("k", (0, 2), HandleValueSet.of_number(1)),
            where="'b'",
            alignment_unit=UNIT_OCTET,
        ),
    )
    _refuses(
        "22.9.2.3",
        lambda: registry.declare(
            IdentificationHandle("k", (0, 1), HandleValueSet.of_number(1)),
            where="'c'",
            alignment_unit=4,
        ),
    )
    ...
    plain.declare(
        IdentificationHandle("k", (0,), HandleValueSet.of_number(1)), where="'b'", alignment_unit=1
    )
...
def test_presence_is_mandatory_and_not_defaulted():
    ...
    _refuses("#OPTIONAL object wraps", lambda: OptionalSpec(presence=Optionality(reference="p")))
    ...
    objects = _concat(
        {
            ...
            "v": OptionalSpec(component=IntSpec(width=7), presence=Optionality(reference="p")),
        },
        ("p", "v"),
    )
    ...
    objects = _concat(
        {
            ...
            "v": OptionalSpec(
                ...
                presence=Optionality(
                    reference="p", encoder_transforms=TransformChain((BoolToInt(true_zero=True),))
                ),
            ),
        },
        ("p", "v"),
...
def test_the_five_optionality_restrictions_are_each_enforced():
    ...
    _refuses(
        "22.5.2.3",
        lambda: Optionality(determination=OptionalityDetermination.HANDLE, reference="p"),
    )
    _refuses(
        "22.5.2.6",
        lambda: Optionality(
            determination=OptionalityDetermination.CONTAINER,
            reference="c",
    ...
    _refuses(
        "22.5.2.8",
        lambda: Optionality(reference="p", decoder_transforms=TransformChain((BoolToInt(),))),
    )
    _refuses(
        "22.5.2.4",
        lambda: OptionalSpec(
            component=_OCTET, presence=Optionality(determination=OptionalityDetermination.POINTER)
        ),
    )
    ...
    objects = _concat(
        {
            "v": OptionalSpec(
                ...
                presence=Optionality(
                    determination=OptionalityDetermination.HANDLE, handle_id="lead", handle_set=True
                ),
            ),
        },
        ("v",),
...
def test_replace_optionals_and_non_optionals_now_have_something_to_sort_by():
    ...
    only_optional = ConcatenationSpec(
        fields=fields,
        order=("p", "m", "o"),
        replacement=Replacement(action=ReplaceAction.OPTIONALS, structure=_length_prefixed()),
    )
    assert only_optional.transmission_order() == ("p", "m", "o$len", "o")
    only_mandatory = ConcatenationSpec(
        fields=fields,
        order=("p", "m", "o"),
        replacement=Replacement(action=ReplaceAction.NON_OPTIONALS, structure=_length_prefixed()),
    )
    assert only_mandatory.transmission_order() == ("p$len", "p", "m$len", "m", "o")
    assert encode_with_user(_objects(only_optional), "#C", {"p": 1, "m": 0x11, "o": 0x22}) == bytes(
        (1, 0x11, 1, 0x22)
    )
...
def test_a_replaced_optional_component_becomes_mandatory():
    ...
    spec = ConcatenationSpec(
        fields={"p": IntSpec(width=8), "o": optional},
        order=("p", "o"),
        replacement=Replacement(action=ReplaceAction.OPTIONALS, structure=_length_prefixed()),
    )
...
def test_the_alternative_index_is_positional_and_the_order_property_fixes_it():
    ...
    spec = AlternativesSpec(
        alternatives={
            "a": TagSpec(width=8, number=9, tagged=_OCTET),
...
def test_alternatives_by_handle_require_disjoint_sets_from_every_alternative():
    ...
    spec = AlternativesSpec(
        alternatives={"a": IntSpec(width=8, exhibits=low), "b": IntSpec(width=8, exhibits=high)},
        selection=AlternativeSelection(
            determination=AlternativeDetermination.HANDLE, handle_id="kind", handle_set=True
        ),
    )
    ...
    _refuses(
        "21.6.6",
        lambda: AlternativesSpec(
            alternatives={
                "a": IntSpec(width=8, exhibits=low),
...
def test_alternative_ordering_has_two_values_where_concatenation_has_three():
    ...
    _refuses(
        "22.6.1.1", lambda: AlternativeSelection(reference="k", ordering=ComponentOrder.RANDOM)
    )
...
def test_tag_ordering_requires_distinct_tags_on_every_alternative():
    ...
    _refuses(
        ...
        lambda: AlternativesSpec(
            alternatives={
                "a": TagSpec(width=8, number=1, tagged=_OCTET),
...
def test_an_optional_component_is_transparent_to_the_component_tag():
    ...
    optional = OptionalSpec(
        component=TagSpec(width=8, number=3, tagged=_OCTET), presence=Optionality(reference="p")
    )
    ...
    _refuses(
        "22.10.2.4",
        lambda: ConcatenationSpec(
            fields={"p": AuxIntSpec(width=8), "a": optional},
            order=("p", "a"),
            concatenation=Concatenation(order=ComponentOrder.TAG),
        ),
    )
...
def test_an_alternatives_object_needs_alternatives_and_a_selection():
    _refuses(
        ...
        lambda: AlternativesSpec(selection=AlternativeSelection(reference="k")),
    )
    ...
    _refuses(
        "22.6.2.3",
        lambda: AlternativeSelection(determination=AlternativeDetermination.HANDLE, reference="k"),
    )
...
def test_tag_order_sorts_the_components_by_component_tag():
    ...
    spec = ConcatenationSpec(
        fields={
            "z": TagSpec(width=8, number=9, tagged=_OCTET),
...
def test_random_order_requires_a_handle_on_every_component():
    ...
    ok = ConcatenationSpec(
        ...
        order=("x", "y"),
        concatenation=Concatenation(order=ComponentOrder.RANDOM),
    )
...
def test_alignment_aligned_applies_the_class_pre_alignment_before_each_component():
    ...
    assert encode_with_user(
        _objects(packed), "#C", {"a": 5, "b": 3}, outer=_pad_to_octet()
    ) == bytes((0b101_011_00,))
    ...
    assert encode_with_user(
        _objects(spread), "#C", {"a": 5, "b": 3}, outer=_pad_to_octet()
    ) == bytes((0b101_00000, 0b011_00000))
...
def test_an_absent_concatenation_group_means_every_default():
    ...
    explicit = ConcatenationSpec(fields=fields, order=("a", "b"), concatenation=Concatenation())
    value = {"a": 5, "b": 3}
    assert encode_with_user(_objects(bare), "#C", value, outer=_pad_to_octet()) == encode_with_user(
        _objects(explicit), "#C", value, outer=_pad_to_octet()
    )
...
def test_a_repetition_can_end_at_a_handle_now_that_handles_exist():
    ...
    space = RepetitionSpace(determination=RepetitionSpaceDetermination.HANDLE, handle_id="more")
    ...
    _refuses(
        "21.7.10",
        lambda: RepetitionSpace(determination=RepetitionSpaceDetermination.HANDLE, reference="n"),
    )
