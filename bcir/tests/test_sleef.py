...
def _sleef_link():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        open(src, "w").write("#include <sleef.h>\n"
...
def test_exp_reference_rejects_empty():
    try:
        exp_reference([]); assert False, "empty exp should raise"
    except ValueError:
        pass
...
def test_emit_wraps_sleef_with_a_libm_fallback():
    ...
    try:
        emit_sleef_exp_c(0, "vexp"); assert False, "n=0 should raise"
    except ValueError:
        pass
...
def test_linked_sleef_path_agrees_when_sleef_is_present():
    ...
    data = [2.5, -1.0, 3.0, 0.5, 0.0, -2.0, 1.0, -0.25, 1.75, -3.0]
    ref = exp_reference(data)
    got = _run_exp_kernel(cc, emit_sleef_exp_c(len(data), "vexp"), data,
                          define="-DBCIR_USE_SLEEF", libs=libs)
...
def test_r17_certifies_the_bridge_on_a_quantized_exp_call():
    ...
    def call(qbits):
        return Claim(id=6300, opcode=Opcode.MUL, lane=Lane.U, stride_class=StrideClass.UNIT, count=1,
                     rd=(63,), wr=(64,), op="call.vexp", domain=Domain.RAM, quantized_bits=qbits)
...
def test_sleef_link_flag_rule():
    assert library_for_callee("Sleef_expf1_u10") == "-lsleef"       # the rule (the wrapper's actual callee)
