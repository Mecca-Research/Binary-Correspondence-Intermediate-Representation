...
_ADD, _SUB, _MUL, _DIV = (lambda a, b: a + b), (lambda a, b: a - b), (lambda a, b: a * b), (lambda a, b: a / b)


def _interp_jvm(lines):
    return _interp_mnemonics(lines, "fload ", "fstore ", "ldc ",
                             {"fadd": _ADD, "fsub": _SUB, "fmul": _MUL, "fdiv": _DIV})


def _interp_cil(lines):
    return _interp_mnemonics(lines, "ldloc.", "stloc.", "ldc.r4 ",
                             {"add": _ADD, "sub": _SUB, "mul": _MUL, "div": _DIV})


def _interp_wasm(lines):
    return _interp_mnemonics(lines, "local.get ", "local.set ", "f32.const ",
...
class _ConstPool:
    ...
    def nat(self, name, desc):
        return self._add(("nt", name, desc), bytes([12]) + struct.pack(">HH", self.utf8(name), self.utf8(desc)))

    def fieldref(self, cls, name, desc):
        return self._add(("fr", cls, name, desc),
                         bytes([9]) + struct.pack(">HH", self.class_(cls), self.nat(name, desc)))

    def methodref(self, cls, name, desc):
        return self._add(("mr", cls, name, desc),
...
def _assemble_run_code(cp, jvm_mnemonics):
    ...
    for m in jvm_mnemonics:
        if m.startswith("fload "):
            ...
            if fi > 0xFF:
                raise ValueError("constant pool too large for 1-byte ldc (expand to ldc_w if this ever fires)")
...
def build_class(class_name, jvm_program):
    ...
    main_code = (bytes([0xB2]) + struct.pack(">H", sys_out)         # getstatic System.out
                 + bytes([0xB8]) + struct.pack(">H", run_mref)      # invokestatic run()F
                 + bytes([0xB6]) + struct.pack(">H", println)       # invokevirtual println(F)V
                 + bytes([0xB1]))                                   # return
    methods = (_method(code_attr, 0x0009, run_n, run_d, run_code, 8, run_locals)
               + _method(code_attr, 0x0009, main_n, main_d, main_code, 2, 1))
    out = struct.pack(">IHH", 0xCAFEBABE, 0, 52)                    # magic, minor, major(=52, Java 8)
    out += cp.to_bytes()
    out += struct.pack(">HHHHHH", 0x0021, this_c, obj_c, 0, 0, 2)   # access, this, super, ifaces, fields, methods
    out += methods
...
def _rand_expr(rng, nloc, depth):
    if depth <= 0 or rng.random() < 0.4:
        return Local(rng.randrange(nloc)) if rng.random() < 0.5 else Const(round(rng.uniform(-3.0, 3.0), 3))
...
def test_text_encoders_handle_division():
    ...
    prog = assign(0, Const(6.0)) + assign(1, Const(4.0)) + stackify(BinOp("div", Local(0), Local(1)))
