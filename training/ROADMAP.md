...
| 1.10 | ~~**Close the LangRef surface**~~ — **landed**: the sweep's second pass found that the attribute check credited a name appearing in PROSE while the dialect check required code, so `corpus_code` is now one predicate for both. Under it, every one of the three LangRef surfaces is accounted for: **67/67 instructions named in code** (no disposition table, because an instruction nobody writes down is a hole rather than a judgement), **104 attributes** each named or dispositioned, and **524 intrinsics** each named or assigned to one of 26 classes in [`llvm/reference/langref-intrinsic-dispositions.json`](llvm/reference/langref-intrinsic-dispositions.json). `llvm.va_copy`, the common scalar builtins and the `returned` parameter attribute were taught rather than declared, because "out of scope" would not have been true of them |
...
produce claims no gate checks. Slices 1.8 and 1.9 exist because the answer to
"how much of MLIR does this corpus cover?" turned out to be measurable, and the
measurement said 1.9% of operations — a number that means nothing on its own,
which is why each dialect now answers for itself instead. Slice 1.6 is not in that class — its claims are
