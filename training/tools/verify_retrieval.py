...
this enforces:
  ...
  5. **Metric sanity.** Recall is monotonic in the cutoff and MRR never exceeds
     recall@10. These are properties of the arithmetic; violating one means the
     harness is miscounting, whatever the model does.
  6. **The evaluation can tell a broken index from a working one.** A shuffled
  ...
  7. **Determinism.** Two runs agree exactly.
...
def check_metrics(report_data: dict, manifest: dict, report: Report) -> None:
    for scope, block in [("overall", report_data["overall"])] + sorted(
        ...
        report.require(
            model[MRR_KEY] <= model["recall@10"] + 1e-9,
            f"{scope}: MRR exceeds recall@10, which is arithmetically impossible",
        )

    for family, block in sorted(report_data["families"].items()):
        if not manifest["families"][family]["control"]:
            continue
        report.require(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        shuffled = evaluator.evaluate(
            queries, embedding_set, search, rankers=("model",), permutation=permutation
        )
        ...
        if "memory" in measured["overall"]:
            base = measured["overall"]["random"][MRR_KEY]
            lift = measured["overall"]["memory"][MRR_KEY] / base if base else float("inf")
            report.require(
                lift >= MIN_MEMORY_LIFT,
                f"concept memory MRR is only {lift:.1f}x random over the "
                f"{len(scored)} query/queries it may score (floor {MIN_MEMORY_LIFT}x)",
            )
        ...
        again = evaluator.evaluate(queries, embedding_set, search, rankers=("model",))
        report.require(
