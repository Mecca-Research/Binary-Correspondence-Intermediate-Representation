...
`training/llvm/tools/run-eval.py` connects four deliberately separate stages:
...
`training/llvm/autograder/exercises.json` and joins topic/difficulty metadata
from the matching files in `training/llvm/autograder/manifests/`.
...
```bash
python3 training/llvm/tools/run-eval.py grade \
