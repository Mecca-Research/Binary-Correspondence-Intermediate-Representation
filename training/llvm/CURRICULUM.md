...
8. Run `./training/llvm/tools/verify-bcir-mapping.sh` and `./training/llvm/tools/verify-examples.sh` after editing any checked source-like `.bcir.txt` or lowered `.ll` output under [`bcir-mapping/examples/`](bcir-mapping/examples).
...
4. Run `./training/llvm/tools/verify-invalid-fixtures.sh` for broken inputs, `./training/llvm/tools/verify-exercises.sh` for fixed `.solution.ll` outputs, and `./training/llvm/tools/verify-bcir-mapping.sh` when a repair touches BCIR mapping fixtures.
...
Practice next: run
`python3 training/llvm/tools/verify-frontend-lowering.py`, then compile
...
`./training/llvm/tools/build-pass-plugin.sh`, then break the collision in
...
11. [`25-ml-dialects/README.md`](25-ml-dialects/README.md) — the ML dialects: the ladder from `tosa` to `linalg` to an `scf.for` nest, and what a quantized type is once the type is gone.
...
Practice next: run
`python3 training/llvm/tools/analyze-benchmark-samples.py` over each fixture in
