...
```bash
training/llvm/tools/demo-debug-pipeline.sh
```
...
```bash
training/llvm/tools/demo-mem2reg.sh
```
...
```bash
training/llvm/tools/demo-o2.sh
```
...
```bash
opt -passes=verify training/llvm/07-optimization/examples/mem2reg-diamond-before.ll -o /dev/null
opt -S -passes='print<function>,mem2reg,print<function>' \
  training/llvm/07-optimization/examples/mem2reg-diamond-before.ll \
  -disable-output
opt -S -passes=mem2reg \
  training/llvm/07-optimization/examples/mem2reg-diamond-before.ll \
  -o /tmp/mem2reg-diamond-after.ll
diff -u training/llvm/07-optimization/examples/mem2reg-diamond-before.ll /tmp/mem2reg-diamond-after.ll
```
...
opt -S -passes='print<function>,instcombine,print<function>' \
  training/llvm/07-optimization/examples/instcombine-canonical-before.ll \
  -disable-output
opt -S -passes=instcombine \
  training/llvm/07-optimization/examples/instcombine-canonical-before.ll \
  -o /tmp/instcombine-canonical-after.ll
diff -u training/llvm/07-optimization/examples/instcombine-canonical-after.ll /tmp/instcombine-canonical-after.ll
```
...
opt -S -passes='print<module>,simplifycfg,print<module>' \
  training/llvm/07-optimization/examples/simplifycfg-select-before.ll \
  -disable-output
opt -S -passes=simplifycfg \
  training/llvm/07-optimization/examples/simplifycfg-select-before.ll \
  -o /tmp/simplifycfg-select-after.ll
diff -u training/llvm/07-optimization/examples/simplifycfg-select-before.ll /tmp/simplifycfg-select-after.ll
```
...
opt -S -passes='print<function>,gvn,print<function>' \
  training/llvm/07-optimization/examples/gvn-load-before.ll \
  -disable-output
opt -S -passes=gvn \
  training/llvm/07-optimization/examples/gvn-load-before.ll \
  -o /tmp/gvn-load-after.ll
diff -u training/llvm/07-optimization/examples/gvn-load-before.ll /tmp/gvn-load-after.ll
```
...
opt -disable-output -debug-pass-manager -passes='loop-rotate' \
  training/llvm/07-optimization/examples/loop-rotate-while-before.ll
opt -S -passes='print<loops>,loop-rotate,print<loops>' \
  training/llvm/07-optimization/examples/loop-rotate-while-before.ll \
  -disable-output
opt -S -passes='loop-unroll' \
  training/llvm/07-optimization/examples/loop-unroll-count2-before.ll \
...
opt -disable-output -debug-pass-manager -passes='default<O2>' \
  training/llvm/07-optimization/examples/o2-pipeline-inspection.ll \
  2> /tmp/o2-pipeline.log
opt -S -passes='default<O2>' \
  training/llvm/07-optimization/examples/o2-pipeline-inspection.ll \
  -o /tmp/o2-pipeline-after.ll
diff -u training/llvm/07-optimization/examples/o2-pipeline-inspection.ll /tmp/o2-pipeline-after.ll
```
