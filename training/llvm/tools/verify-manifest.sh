...
expected=$(mktemp "${TMPDIR:-/tmp}/training-llvm-manifest-expected.XXXXXX")
actual=$(mktemp "${TMPDIR:-/tmp}/training-llvm-manifest-actual.XXXXXX")
missing=$(mktemp "${TMPDIR:-/tmp}/training-llvm-manifest-missing.XXXXXX")
extra=$(mktemp "${TMPDIR:-/tmp}/training-llvm-manifest-extra.XXXXXX")
artifact_expected=$(mktemp "${TMPDIR:-/tmp}/training-llvm-artifacts-expected.XXXXXX")
artifact_actual=$(mktemp "${TMPDIR:-/tmp}/training-llvm-artifacts-actual.XXXXXX")
artifact_missing=$(mktemp "${TMPDIR:-/tmp}/training-llvm-artifacts-missing.XXXXXX")
artifact_extra=$(mktemp "${TMPDIR:-/tmp}/training-llvm-artifacts-extra.XXXXXX")
trap 'rm -f "$expected" "$actual" "$missing" "$extra" "$artifact_expected" "$artifact_actual" "$artifact_missing" "$artifact_extra"' EXIT

sed -n 's/^| `\(training\/llvm\/.*\/examples\/[^`]*\.ll\)` |.*/\1/p' "$MANIFEST" | sort -u > "$expected"
