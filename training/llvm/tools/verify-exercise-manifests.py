#!/usr/bin/env python3
"""Validate declarative manifests for the graded training/llvm exercise set."""
...
def repository_path(repo_root: Path, value: Any, label: str, errors: list[str]) -> Path | None:
    if not isinstance(value, str) or not value.startswith("training/llvm/"):
        errors.append(f"{label}: expected a repository-relative path below training/llvm/")
        return None
...
def main() -> int:
    ...
    parser.add_argument(
        "--schema",
        type=Path,
        default=repo_root / "training/llvm/autograder/schema/exercise.schema.json",
    )
    parser.add_argument(
        "--manifests", type=Path, default=repo_root / "training/llvm/autograder/manifests"
    )
    ...
    graded_prompts = {
        ...
        for path in (repo_root / "training/llvm/exercises").glob("[0-9][0-9][0-9]-*.prompt.md")
    }
