...
run_case() {
  ...
  temp=$(mktemp "${TMPDIR:-/tmp}/training-llvm-opt-diff.XXXXXX.ll")
  actual=$(mktemp "${TMPDIR:-/tmp}/training-llvm-opt-diff.actual.XXXXXX.ll")
  expected=$(mktemp "${TMPDIR:-/tmp}/training-llvm-opt-diff.expected.XXXXXX.ll")
