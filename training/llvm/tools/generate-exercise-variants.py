...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="training-llvm-variants-") as temp_name:
        temp = Path(temp_name)
        for family in selected:
            ...
            for attempt in range(args.max_attempts_per_family):
                ...
                variant_id = f"training-llvm-exercise-{family.parent_id}-variant-{args.seed}-{len(accepted) + 1:02d}"
                hashes = artifact_hashes(prompt, solution, manifest, normalized_hash)
                record = {
                    "record_schema_version": RECORD_SCHEMA_VERSION,
                    "id": variant_id,
                    "parent_exercise_id": f"training-llvm-exercise-{family.parent_id}",
                    "generator": {
