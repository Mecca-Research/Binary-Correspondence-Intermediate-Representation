...
def discover_context_paths(repo_root: Path, metadata: dict[str, Any]) -> list[str]:
    ...
    for match in re.findall(r"(?:training/llvm/)?[A-Za-z0-9_./-]+\.(?:ll|mlir|txt|md|c|cpp)", text):
        value = match.strip("`'\"()[]{}.,:;")
    ...
    for entry in entries:
        ...
        generator_input = {
            ...
            "exercise_metadata": {
                ...
                "topics": [tag for tag in metadata["tags"] if tag != "training/llvm"],
...
def grade(
    ...
    for entry in entries:
        ...
        record: dict[str, Any] = {
            ...
            "topics": [tag for tag in entry["metadata"]["tags"] if tag != "training/llvm"],
