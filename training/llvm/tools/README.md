...
| `verify-frontend-lowering.py` | Compiles the checked-in `20-clang-frontend/examples/` sources and asserts the chapter's lowering claims structurally across x86-64 SysV and AArch64 AAPCS: aggregate layout, packed alignment, bit-field erasure, short-circuit branching, `volatile`, mangling, vtable dispatch, template linkage, and per-target argument classification. `--update` refreshes the normalized `.ll` snapshots; `--require-tools` fails instead of skipping. Normalization strips parameter attributes newer than the corpus's LLVM 15 baseline, and each snapshot is then assembled by the **oldest** `llvm-as` on `PATH` at or above that baseline — a snapshot checked only against the newest assembler is one nobody has checked. That check says nothing when the host offers only a recent assembler; CI's host-portability job is the backstop. | `clang`, `clang++` (optional without `--require-tools`); optional `llvm-as-NN` for the baseline check |
