...
CHAPTER = REPO_ROOT / "training/llvm/15-binary-analysis"
...
def collect_rows(entry: dict[str, Any], object_path: Path) -> list[list[str]]:
    ...
    functions = sorted(
        (
            ...
            if symbol.name
            and symbol.section_index == text.index
            and symbol.info & 0x0F == 2
            and symbol.size > 0
        ),
    ...
    for function in functions:
        rows.append(
            [fixture_id, "symbol", "artifact", function.name, "1", "present", classification]
        )
        ...
        for instruction in instructions:
            if instruction.conditional_branch:
                if (
                    ...
                    and function.value <= instruction.target < function_end
                ):
            ...
            if instruction.call_instruction:
                target_name = by_address.get(
                    instruction.target if instruction.target is not None else -1
                )
    ...
    for section in sections:
        if section.name not in {".text", ".data", ".bss"}:
            continue
        flags = (
            "".join(letter for bit, letter in ((2, "A"), (1, "W"), (4, "X")) if section.flags & bit)
            or "none"
        )
...
def compare_or_write(path: Path, generated: str, check: bool, provenance: bool = False) -> bool:
    ...
    try:
        ...
        return False
    equal = (
        stable_provenance(current) == stable_provenance(generated)
        if provenance
...
def generated_entries(manifest: dict[str, Any], fixture: str | None) -> list[dict[str, Any]]:
    entries = [
        entry for entry in manifest["evidence"] if entry.get("classification") == "deterministic"
    ]
...
def main() -> int:
    ...
    mode.add_argument(
        "--check", action="store_true", help="regenerate in a temporary directory and fail on drift"
    )
    mode.add_argument(
        "--write", action="store_true", help="write checked-in CSV and provenance files (default)"
    )
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="bcir-binary-evidence-") as temporary:
            ...
            for entry in entries:
                ...
                command = [
                    clang,
                    f"--target={entry['target_triple']}",
                    "-Wno-unused-command-line-argument",
                    *entry["optimization_flags"],
                    "-c",
                    str(source),
                    "-o",
                    str(artifact),
                ...
                success &= compare_or_write(
                    repository_path(entry["checked_in_csv"]), csv_text, args.check
                )
                success &= compare_or_write(
                    repository_path(entry["provenance_path"]),
                    provenance_text,
                    args.check,
                    provenance=True,
                )
