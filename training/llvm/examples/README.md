...
| `training/llvm/18-mlir-lowering-to-llvm/examples/complex-and-math-to-llvm-lowered.ll` | `llvm-as training/llvm/18-mlir-lowering-to-llvm/examples/complex-and-math-to-llvm-lowered.ll -o /dev/null` |
| `training/llvm/18-mlir-lowering-to-llvm/examples/index-width-to-llvm-lowered.ll` | `llvm-as training/llvm/18-mlir-lowering-to-llvm/examples/index-width-to-llvm-lowered.ll -o /dev/null` |
...
| `training/llvm/18-mlir-lowering-to-llvm/examples/pdl-pattern-as-data.mlir` | Tier 3 registered MLIR conversion fixture | The registry compiles a declarative `pdl` pattern into the explicit `pdl_interp` matcher; applying the pattern needs a driver release mlir-opt does not ship, so that step is not gated. |
| `training/llvm/18-mlir-lowering-to-llvm/examples/shape-broadcast-to-std.mlir` | Tier 3 registered MLIR conversion fixture | The registry runs `shape-to-shape-lowering` then `convert-shape-to-std` and requires no `shape` operation to survive; the forbid list includes the bridge ops, so it pins the pass order. |
| `training/llvm/18-mlir-lowering-to-llvm/examples/shape-constraints-to-assert.mlir` | Tier 3 registered MLIR conversion fixture | The registry converts a `shape.cstr_broadcastable` witness into a runtime `cf.assert` and requires `shape.is_broadcastable` with it; the require half is the safety claim, since both discharge passes emit `shape.const_witness`. |
| `training/llvm/18-mlir-lowering-to-llvm/examples/shape-one-pass-is-not-enough.mlir` | Tier 3 registered MLIR conversion fixture, expected failure | Deliberately incomplete: the single converter leaves `shape.num_elements` behind, declared as an `illegal-ops-remaining` outcome so the chapter's claim is checked. |
| `training/llvm/25-ml-dialects/examples/tosa-matmul-to-linalg.mlir` | Tier 3 registered MLIR conversion fixture | The registry converts `tosa.matmul` to `linalg.batch_matmul` and requires the `tensor.empty`/`linalg.fill` destination the named operation concealed. |
| `training/llvm/25-ml-dialects/examples/linalg-to-loops.mlir` | Tier 3 registered MLIR conversion fixture | The registry generalizes, bufferizes, then lowers to an `scf.for` nest with `memref` loads and stores; no `linalg` may survive. |
| `training/llvm/25-ml-dialects/examples/linalg-to-loops-needs-buffers.mlir` | Tier 3 registered MLIR conversion fixture, expected failure | Deliberately incomplete: `convert-linalg-to-loops` on tensors matches nothing and exits zero, so the surviving `linalg.generic` is the declared outcome. |
| `training/llvm/25-ml-dialects/examples/sparse-matvec-to-loops.mlir` | Tier 3 registered MLIR conversion fixture | The registry sparsifies one `linalg.generic` whose only sparse marking is an encoding on a type, and requires all three `sparse_tensor` accessors in the output; it forbids `linalg.yield` rather than `linalg.generic`, which survives as a provenance attribute. |
| `training/llvm/25-ml-dialects/examples/gpu-launch-outlining.mlir` | Tier 3 registered MLIR conversion fixture | The registry outlines a `gpu.launch` region into a `gpu.module` and requires `gpu.thread_id` in the kernel; it forbids `gpu.launch blocks`, since `gpu.launch_func` contains the short spelling. Runs on any host: no device, no vendor toolkit. |
| `training/llvm/25-ml-dialects/examples/quant-cast-arithmetic.mlir` | Tier 3 registered MLIR conversion fixture | The registry lowers `quant.qcast`/`quant.dcast` to plain arithmetic and forbids `arith.maxsi`/`arith.minsi`: at the full storage range no clamp is emitted. |
| `training/llvm/25-ml-dialects/examples/quant-clamp-only-when-narrowed.mlir` | Tier 3 registered MLIR conversion fixture | The same lowering over `i8<-100:100>`, which does emit the clamp. Only meaningful paired with the fixture above. |
| `training/llvm/25-ml-dialects/examples/quant-round-trip-folds-away.mlir` | Tier 3 registered MLIR conversion fixture | Pins the canonicalizer fold that turns a lossy quantize/dequantize round trip into the identity, so a warning about it cannot outlive the behaviour. |
| `training/llvm/20-clang-frontend/examples/emitc-mlir-to-c.mlir` | Tier 3 registered MLIR conversion fixture | The registry converts arith/func to the emitc dialect; the C++ emission itself is shown in the chapter from a verified run and is not re-run here. |
| `training/llvm/18-mlir-lowering-to-llvm/examples/index-width-to-llvm.mlir` | Tier 4 registered MLIR conversion fixture | The registry lowers index/func to LLVM dialect at the default 64-bit index width, translates, assembles and verifies it. |
| `training/llvm/18-mlir-lowering-to-llvm/examples/complex-and-math-to-llvm.mlir` | Tier 4 registered MLIR conversion fixture | The registry lowers complex/math/func to LLVM dialect, translates, assembles and verifies it. |
