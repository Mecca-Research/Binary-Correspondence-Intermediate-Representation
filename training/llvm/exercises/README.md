...
```sh
training/llvm/tools/verify-adversarial-fixtures.sh
```
...
```sh
llvm-as -disable-output training/llvm/exercises/001-add.solution.ll
```
...
- Broken repair inputs are intentionally named `*.invalid.ll.txt`; they may be
  ...
  `; training-llvm-invalid-kind: semantic-only` marker. Adversarial fixtures also use the
