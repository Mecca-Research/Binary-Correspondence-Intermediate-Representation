...
```sh
training/llvm/exercises/022-predict-loop-vectorizer.input.ll
```
...
```sh
opt -S -passes=loop-vectorize -force-vector-width=4 -force-vector-interleave=1 training/llvm/exercises/022-predict-loop-vectorizer.input.ll -o -
```
...
```sh
opt -S -passes=loop-vectorize -force-vector-width=4 -force-vector-interleave=1 -pass-remarks-missed=loop-vectorize training/llvm/exercises/022-predict-loop-vectorizer.input.ll -o /dev/null
```

For a stable teaching snapshot of vectorized IR, compare the chapter example:

```sh
llvm-as -disable-output training/llvm/09-vectorization/examples/sum-loop-after-loop-vectorize.ll
```
