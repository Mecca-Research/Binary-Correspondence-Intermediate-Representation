...
LLVM IR examples and exercises in `training/llvm/`. The goal is to make it clear
...
files in `training/llvm/15-binary-analysis/examples/` are schema examples, not
LLVM IR examples:
...
- Validate intentional failures with
  `./training/llvm/tools/verify-invalid-fixtures.sh` instead of broad `llvm-as`
  loops.
...
- Use `./training/llvm/tools/verify-mlir-examples.sh` only when the local
...
`training/llvm/examples/broken-example.ll.txt` as a deliberately malformed
trip-wire fixture; `training/llvm/tools/verify-examples.sh` checks that LLVM
...
`training/llvm/examples/broken-example.ll.txt` as the invalid-example tripwire:
...
```bash
find training/llvm -path '*/examples/*.ll' -type f \
...
After configuring the training-only CMake project with `cmake -S training/llvm -B build/training/llvm`, the same batch checks are available through
...
`training/llvm/exercises/*.solution.ll` reference answer. Use targeted commands
