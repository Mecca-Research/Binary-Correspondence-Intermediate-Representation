...
```bash
opt -S -passes='loop-vectorize' training/llvm/09-vectorization/examples/sum-loop.ll -o /tmp/sum-loop-vectorized.ll
...
```bash
opt -passes=verify training/llvm/09-vectorization/examples/sum-loop-before.ll -o /dev/null
```
...
opt -S -passes='loop-vectorize' \
  ...
  training/llvm/09-vectorization/examples/sum-loop-before.ll \
...
```bash
diff -u \
  training/llvm/09-vectorization/examples/sum-loop-after-loop-vectorize.ll \
...
opt -S -passes='loop-vectorize' \
  -pass-remarks-missed=loop-vectorize \
  training/llvm/09-vectorization/examples/not-vectorizable-dependency.ll \
...
opt -S -passes='loop-vectorize' \
  -pass-remarks-missed=loop-vectorize \
  training/llvm/09-vectorization/examples/not-vectorizable-call.ll \
