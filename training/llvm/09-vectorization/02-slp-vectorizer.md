...
```bash
opt -S -passes='slp-vectorizer' training/llvm/09-vectorization/examples/slp-scalars.ll -o /tmp/slp.ll
...
```bash
opt -passes=verify training/llvm/09-vectorization/examples/slp-scalars-before.ll -o /dev/null
opt -S -passes='slp-vectorizer' -slp-threshold=-999 \
  training/llvm/09-vectorization/examples/slp-scalars-before.ll \
...
diff -u training/llvm/09-vectorization/examples/slp-scalars-before.ll /tmp/slp-scalars-canonicalized.ll
```
