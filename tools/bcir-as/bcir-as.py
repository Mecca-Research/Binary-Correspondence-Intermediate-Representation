...
OPMAP = {
    "NOP": (0, 0), "LOAD_U": (1, 0), "LOAD_UX": (1, 1), "STORE_U": (2, 0), "STORE_UX": (2, 1),
    "ADD_U": (3, 0), "ADD_UX": (3, 1), "SUB_U": (4, 0), "SUB_UX": (4, 1), "MUL_U": (5, 0), "MUL_UX": (5, 1),
    "ATOMIC_ADD": (32, 4), "ATOMIC_SUB": (33, 4), "ATOMIC_XOR": (34, 4), "CMPXCHG": (35, 4),
    "BARRIER": (48, 5), "PHASE_ENTER": (49, 5), "PHASE_LEAVE": (50, 5),
}
...
def parse(src: str):
    ...
    rid_map = {
        ...
        for m in re.finditer(r"RECORD\s+(\w+)\s*:\s*RES\s*\{[^}]*rid\s*=\s*(\d+)", src, re.S)
    }
    claims = []
    phase = 0
    ...
    while i < len(lines):
        ...
        if cm:
            ...
            for k in req:
                if not re.search(rf"\b{k}\s*=", c):
                    raise ValueError(f"missing contract field {k} for claim {cid}")
...
def emit(module, rids, claims):
    ...
    for c in claims:
        ...
        imm0 = int(c["args"].get("off", 0))
        imm1 = int(c["args"].get("count", 8))
        rd4 = (c["rd"] + [0, 0, 0, 0])[:4]
        wr4 = (c["wr"] + [0, 0, 0, 0])[:4]
        ctrl = control(opcode, lane, c["phase"])
        hz = encode_hazard(c)
    ...
    rrows = [
        f'  %bcir.res {{ i32 {rid}, i32 {DOMAIN_BITS.get("ram", 0)}, ptr null, i64 4096, i64 0, i64 0, i64 0 }}'
        for _, rid in sorted(rids.items(), key=lambda kv: kv[1])
    ]
    ...
    while i < len(claims):
        j = i + 1
        op, lane = OPMAP[claims[i]["op"]]
        phase = claims[i]["phase"]
        while j < len(claims) and claims[j]["phase"] == phase and OPMAP[claims[j]["op"]] == (op, lane):
            j += 1
        batch_rows.append(f'  %bcir.batch {{ i32 0, i32 {phase}, i32 {lane}, i32 {op}, i32 0, i32 0, i64 {i}, i64 {j - i}, i64 0 }}')
        i = j
    if batch_rows:
        bs = []
        for txt in batch_rows:
            m = re.search(r"i32 (\d+), i32 (\d+), i32", txt)
            bs.append(int(m.group(2)))
        pstart = 0
        while pstart < len(bs):
            pend = pstart + 1
            while pend < len(bs) and bs[pend] == bs[pstart]:
                pend += 1
...
def main():
    ...
    ap.add_argument("input", help="Path to BCIR textual input file")
    ap.add_argument("-o", "--output", required=True, help="Output LLVM .ll path")
    args = ap.parse_args()
    try:
        m, r, c = parse(Path(args.input).read_text())
        Path(args.output).write_text(emit(m, r, c))
    except Exception as exc:
        print(f"bcir-as error: {exc}", file=sys.stderr)
        raise SystemExit(2)
