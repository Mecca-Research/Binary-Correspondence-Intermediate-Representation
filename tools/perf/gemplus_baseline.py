...
BASELINE_HOST = ("WSL2, AMD Ryzen 5 2600 (6c/12t, AVX2, 8 MiB L3), 7.7 GiB RAM, "
...
class Metric:
    ...
    __slots__ = ("key", "group", "what", "baseline", "unit", "kind", "bound",
                 "bound_source", "lower_is_better", "slice_owner", "noise")

    def __init__(self, key, group, what, baseline, unit, kind, *, bound=None,
                 bound_source="", lower_is_better=True, slice_owner="", noise=None):
        self.key = key
        self.group = group
        self.what = what
...
METRICS: tuple[Metric, ...] = (
    ...
    Metric("optimize_scheduled.512", "planner",
           "optimize_scheduled at 512 claims", 1703.66, "ms", "wall",
           bound=24.61, bound_source="serial optimize at 512 claims (§6.3) -- the sweep "
                                     "should approach O(claims x candidates), i.e. the "
                                     "serial cost times a small constant",
           slice_owner="G2"),
    Metric("optimize_scheduled.256", "planner",
           "optimize_scheduled at 256 claims", 435.73, "ms", "wall",
           bound=12.19, bound_source="serial optimize at 256 claims (§6.3)",
           slice_owner="G2"),
    Metric("optimize_scheduled.slowdown.512", "planner",
           "optimize_scheduled / serial optimize at 512", 69.2, "x", "ratio",
           bound=4.0, bound_source="a linear sweep over a constant candidate set should "
                                   "cost a small multiple of the serial pass, not a "
                                   "growing one (§6.3)",
           slice_owner="G2"),
    Metric("optimize_scheduled.quality", "planner",
           "one-sweep makespan / exact makespan, 4-claim 3-candidate fixture",
           1.00696, "x", "exact",
           bound=1.0, bound_source="exhaustive enumeration of the same fixture (§6.3)",
           slice_owner="G4"),

    # --- §6.1: HEFT-lite against an exact branch-and-bound scheduler.
    Metric("eft.suboptimal.2domains", "scheduler",
           "fraction of 1,716 six-job instances where EFT is suboptimal, 2 domains",
           0.1107, "fraction", "exact",
           bound=0.0, bound_source="exact branch-and-bound over the same corpus (§6.1)",
           slice_owner="G4"),
    Metric("eft.worst.2domains", "scheduler",
           "worst EFT/optimal makespan ratio, 2 domains", 1.1333, "x", "exact",
           bound=1.0, bound_source="exact branch-and-bound (§6.1)", slice_owner="G4"),
    Metric("eft.mean.2domains", "scheduler",
           "mean EFT/optimal makespan ratio, 2 domains", 1.0078, "x", "exact",
           bound=1.0, bound_source="exact branch-and-bound (§6.1)", slice_owner="G4"),
    ...
    Metric("pricing.eft.divergence", "scheduler",
           "price_scheduled makespan / schedule_eft makespan on the §6.2 fixture",
           51200 / 25700, "x", "exact",
           bound=1.0, bound_source="one canonical schedule artifact read by both (§6.2) -- "
    ...
    Metric("memory.suboptimal.fraction", "memory",
           "fraction of 500 seven-resource fixtures where first-fit is suboptimal",
           0.386, "fraction", "exact",
           bound=0.0, bound_source="exact integer backtracking (§6.4)", slice_owner="G5"),
    Metric("memory.worst.ratio", "memory",
           "worst first-fit extent / proved optimum", 21 / 13, "x", "exact",
           bound=1.0, bound_source="exact integer backtracking, 21 vs 13 units (§6.4)",
           slice_owner="G5"),
    Metric("memory.real.bytes", "memory",
           "real planner extent on the §6.4 fixture at 64-byte alignment", 1344, "bytes", "exact",
           bound=832, bound_source="exact layout on the same fixture (§6.4)",
           slice_owner="G5"),
    ...
    Metric("static_memory.plan.2048", "memory",
           "plan_static_memory (includes verify) at 2,048 resources", 301.02, "ms", "wall",
           bound=88.05, bound_source="the module digest alone at the same size (§5.2) -- "
                                     "one canonical digest computed once is the floor the "
                                     "planner cannot go below while it still hashes",
           slice_owner="G0"),
    Metric("static_memory.digest.2048", "memory",
           "module digest at 2,048 resources", 88.05, "ms", "wall", slice_owner="G0"),
    Metric("static_memory.verify.2048", "memory",
           "external verify at 2,048 resources", 157.88, "ms", "wall",
           bound=88.05, bound_source="an identity-bound API lets an independent verifier "
    ...
    Metric("audit.kbcir-streampack.scale4", "audit",
           "K_BCIR->StreamPack at scale 4 (4,096 claims)", 275.22, "ms", "wall",
           slice_owner="G2"),
    Metric("audit.static-lifetime-planner.scale4", "audit",
           "static lifetime planner at scale 4 (2,048 resources)", 566.35, "ms", "wall",
           slice_owner="G0"),
    Metric("audit.mixed-wave-token-eft.scale4", "audit",
           "mixed wave/token/EFT at scale 4 (2,048 claims)", 48.05, "ms", "wall",
           slice_owner="G1"),
    Metric("audit.iterative-phase-dag.scale4", "audit",
           "iterative phase DAG at scale 4 (2,048 claims)", 34.62, "ms", "wall",
           slice_owner="G3"),
    ...
    Metric("native.gather-avoidance", "native",
           "blocked realization vs gather form", 5.58, "x", "ratio",
           lower_is_better=False,
           bound=6.05, bound_source="the upper end of the observed 5.58-6.05x band (§4.4)",
           slice_owner="G6"),
    Metric("native.blocked-reduction", "native",
           "blocked reduction vs naive", 11.68, "x", "ratio", lower_is_better=False,
           bound=11.72, bound_source="the upper end of the observed band (§4.4)",
           slice_owner="G6"),
    Metric("native.direct-stride", "native",
           "direct strided access vs gather form", 1.27, "x", "ratio",
           lower_is_better=False,
           bound=1.33, bound_source="the upper end of the observed band (§4.4)",
           slice_owner="G6"),
    Metric("native.dense-parity", "native",
           "dense streaming BCIR vs equivalent compiler loop", 0.98, "x", "ratio",
           lower_is_better=False,
           bound=1.01, bound_source="the observed 0.98-1.01x parity band (§4.4). Parity is "
...
def measure_planner() -> dict[str, float]:
    ...
    for count in (256, 512):
        ...
        module.add_phase(Phase(phase_id=0, claims=[
            Claim(id=index + 1, opcode=Opcode.ADD, lane=Lane.U,
                  stride_class=StrideClass.UNIT, count=64, rd=(1,), wr=(1,),
                  op="vector.add")
    ...
    if "optimize_scheduled.512" in out and out.get("_serial.512"):
        out["optimize_scheduled.slowdown.512"] = (
            out["optimize_scheduled.512"] / out["_serial.512"])
...
def measure_exact() -> dict[str, float]:
    ...
    try:
        ...
        module.add_phase(Phase(phase_id=0, claims=[
            Claim(id=index + 1, opcode=Opcode.ADD, lane=Lane.U,
                  stride_class=StrideClass.UNIT, count=count,
                  rd=(2 * index + 1,), wr=(2 * index + 2,), op="vector.add")
...
def render(rows: list[dict]) -> str:
    ...
    lines += ["", f"{len(gains)} gain, {len(flat)} no-change, {len(bad)} regression, "
    ...
    if flat:
        lines += ["",
                  "NO-CHANGE rows are findings, not passes. For each, the roadmap requires "
                  "one of:",
...
def render_list() -> str:
    ...
    for group in sorted({m.group for m in METRICS}):
        ...
        for metric in [m for m in METRICS if m.group == group]:
            floor = ("no bound computed yet" if metric.bound is None
                     else f"bound {_fmt(metric.bound, metric.unit)} {metric.unit}")
            lines.append(f"  {metric.key:<40} {_fmt(metric.baseline, metric.unit):>12} "
    ...
    if unbounded:
        lines += ["Rows with no lower bound yet -- no optimality claim is available for "
...
def main(argv: list[str]) -> int:
    ...
    if args.json:
        with open(args.json, "w", encoding="utf-8") as handle:
            json.dump({"source": SOURCE, "baseline_host": BASELINE_HOST,
                       "host": platform.platform(), "scale": args.scale,
