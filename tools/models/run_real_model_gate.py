...
def _ingest_with_counts(model_dir: Path):
    ...
    return (spec, weights, int(report.decoder_element_count),
            int(report.auxiliary_element_count))
...
def _build_c_cli(output: Path) -> None:
    ...
    command = [compiler, "-std=c11", "-O2", "-ffp-contract=off", "-Wall", "-Wextra",
               "-I", str(runtime), str(runtime / "bcir_q8_model.c"),
...
def run_gate(*, cache_dir: str | None = None, output_dir: str | os.PathLike | None = None,
    ...
    if decoder_elements != int(pin["decoder_elements"]):
        raise AssertionError(f"decoder element census {decoder_elements} != {pin['decoder_elements']}")
    if auxiliary_elements != int(pin["auxiliary_elements"]):
        raise AssertionError(f"auxiliary element census {auxiliary_elements} != {pin['auxiliary_elements']}")
    ...
    hashes = {"model": pin["files"]["model.safetensors"]["sha256"],
              "config": pin["files"]["config.json"]["sha256"],
              "tokenizer": pin["files"]["tokenizer.model"]["sha256"]}
    tokenizer_ids = {"bos": int(config["bos_token_id"]), "eos": int(config["eos_token_id"]),
    ...
    metadata = write_q8_decoder(artifact, spec, weights, group_size=32,
                                source_hashes=hashes, tokenizer_ids=tokenizer_ids)
    repeat_metadata = write_q8_decoder(repeat, spec, weights, group_size=32,
                                       source_hashes=hashes, tokenizer_ids=tokenizer_ids)
    if metadata.artifact_sha256 != repeat_metadata.artifact_sha256 \
    ...
    command = [str(executable), "--model", str(artifact), "--prompt-ids",
               ",".join(str(value) for value in prompt_ids), "--max-new", "1",
    ...
    report = {
        ...
        "source": {
            "repo": pin["repository"], "revision": pin["revision"],
            "license": pin["license"],
            "files": {name: {"bytes": int(info["bytes"]), "sha256": info["sha256"]}
                      for name, info in sorted(pin["files"].items())},
        },
        "model": {
            "decoder_elements": decoder_elements,
            "auxiliary_elements": auxiliary_elements,
            "spec": {"vocab_size": spec.vocab_size, "d_model": spec.d_model,
        ...
        "artifact": {"format": "BCIRQ8", "version": metadata.version,
                     "group_size": metadata.group_size, "bytes": metadata.file_size,
                     "sha256": metadata.artifact_sha256,
                     "body_crc32": metadata.body_crc32},
        "decode": {"float_ids": [float_id], "q8_python_ids": [q8_id], "q8_c_ids": c_ids,
                   "max_abs_logit_error_c_vs_q8": max_c_error,
                   "max_abs_logit_drift_q8_vs_float": drift,
                   "nll_f32": nll_f32, "nll_q8": nll_q8,
...
def main() -> int:
    ...
    print(json.dumps({"artifact_sha256": report["artifact"]["sha256"],
                      "generated_ids": report["decode"]["q8_c_ids"], "passed": True},
                     sort_keys=True))
    return 0
