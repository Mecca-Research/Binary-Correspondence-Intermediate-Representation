...
def _pretrain(seed: int, directory: Path):
    documents = (
        RawDocument(
            "micro-a", "abcde abcde abcde", "generated-fixture", "CC0-1.0", _sha("micro-a-source")
        ),
        RawDocument(
            "micro-b",
    ...
    corpus = prepare_corpus(documents, DataPreparationSpec(("CC0-1.0",), validation_permyriad=0))
    tokenizer = BytePairTokenizer.train(
        [document.text for document in corpus.split("train")], vocab_size=272, min_frequency=2
    )
    ...
    loaded = load_safe_checkpoint(
        directory, model, optimizer, spec=spec, corpus=source.manifest, device=torch.device("cpu")
    )
...
def _alignment_run(seed: int, scratch: Path) -> dict:
    ...
    sft = _run_stage(
        lambda: train_sft(
            policy,
            sft_examples,
            StageTrainSpec("sft", 2, 1e-2, batch_size=2, seed=seed + 1),
            events.append,
        )
    )
    ...
    reward = _run_stage(
        lambda: train_reward_model(
            reward_model,
            preferences,
            StageTrainSpec("reward", 2, 1e-2, batch_size=2, seed=seed + 2),
            events.append,
        )
    )
    ...
    dpo = _run_stage(
        lambda: train_dpo(
            ...
            reference,
            preferences,
            StageTrainSpec("dpo", 2, 1e-2, batch_size=2, seed=seed + 3),
            events.append,
        )
    )
    ...
    ppo = _run_stage(
        lambda: train_ppo(
            ...
            value_model,
            ppo_examples,
            StageTrainSpec("ppo", 2, 1e-3, batch_size=2, seed=seed + 4),
            events.append,
        )
    )
    ...
    reasoning = _run_stage(
        lambda: train_reasoning_sft(
            policy,
            reasoning_examples,
            StageTrainSpec("reasoning", 2, 1e-2, seed=seed + 5),
            events.append,
        )
    )
    ...
    embedding = _run_stage(
        lambda: train_embedding_distillation(
            ...
            sequences,
            targets,
            StageTrainSpec("embedding", 2, 1e-2, seed=seed + 6),
            events.append,
        )
    )
    ...
    for index, family in enumerate(("mlp", "gru", "transformer_encoder")):
        ...
        model = HostedSmallModel(SmallModelSpec(family, 4, 8, 2, heads=2, sequence_length=3))
        report = _run_stage(
            lambda model=model, index=index: train_small_supervised(
                model,
                features,
                labels,
    ...
    ledger = TrainingPipelineLedger(
        (
            ...
            PipelineStageRecord(
                ...
                (corpus.digest,),
                tokenizer.digest,
                _sha(tokenizer.to_json()),
                "byte-bpe",
            ),
            PipelineStageRecord(
                "pretrain",
                (tokenizer.digest,),
                pretrain.model_sha256,
                _sha(pretrain.to_json()),
                "safe-checkpoint",
            ),
            PipelineStageRecord(
                "sft",
            ...
            PipelineStageRecord(
                ...
                (sft.model_sha256,),
                dpo.model_sha256,
                _stage_report_digest(dpo),
                "hosted-state",
            ),
            PipelineStageRecord(
                "ppo",
                (dpo.model_sha256, reward.model_sha256),
                ppo.model_sha256,
                _stage_report_digest(ppo),
                "policy-value-state",
            ),
            PipelineStageRecord(
                "reasoning",
                (ppo.model_sha256,),
                reasoning.model_sha256,
                _stage_report_digest(reasoning),
                "hosted-state",
            ),
            PipelineStageRecord(
                "embedding",
                (sft.model_sha256,),
                embedding.model_sha256,
                _stage_report_digest(embedding),
                "embedding-state",
            ),
        )
    )
    reports = {
        name: _report_dict(report)
        for name, report in (
            ("pretrain", pretrain),
            ("sft", sft),
            ("reward", reward),
