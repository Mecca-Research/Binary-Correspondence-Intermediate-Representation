...
def _hardware():
    banks = (
        BankEnvelope("ram", "RAM", "host", 1 << 28, 1 << 28, 20_000_000_000, 20_000_000_000),
        BankEnvelope(
            "vram",
            "VRAM",
            "gpu",
    ...
    return HardwareEnvelope("tiny-simulated-tower", "x86_64", banks, links, tuple(benchmarks))
    ...
    for episode_index, (thermal, bandwidth, register, throttled) in enumerate(scenarios):
        ...
        for position in range(4):
            event = DataDNA(
                ...
                position + 1,
                cycles=1000 + position * 100,
                bytes=4096 + position * 256,
                misses=min(100, bandwidth // 2 + position),
                thermal=thermal,
            ...
            telemetry.append(
                TelemetryToken.from_data_dna(
                    event,
                    sequence=position,
                    register_pressure=min(100, register + position),
        ...
        for feature in candidates:
            ...
            energy = (
                (320 if primary_gpu else 450)
                + thermal * (5 if primary_gpu else 1)
                + (120 if stream else 0)
            )
            ...
            source = hashlib.sha256(
                f"{context}:{feature.candidate_id}:{latency}:{energy}".encode()
            ).hexdigest()
            outcomes.append(
                HardwareOutcome(
                    context,
...
def run(output_dir: Path) -> dict:
    ...
    model_source = output_dir / "synthetic-model"
    model_source.mkdir()
    ...
    model_spec = HardwarePolicySpec(
        hidden_dim=32, heads=4, transformer_layers=1, graph_layers=2, max_telemetry_tokens=8
    )
    ...
    second = train_hardware_policy(
        model_spec, train_spec, topology, candidates, episodes, policy, output_dir / "artifacts-b"
    )
    if (
    ...
    if (
        ...
        or first.artifact.manifest_sha256 != second.artifact.manifest_sha256
    ):
    ...
    if (
        first.report.final_top1_correct < first.report.initial_top1_correct
    ...
    shutil.copytree(first.artifact.directory, corrupt)
    ...
    scores = {
        candidate_id: policy.score(outcome) for candidate_id, outcome in outcome_by_id.items()
    }
    worst = max(scores.values())
    search = bounded_mcts(
        scores, priors, lambda candidate_id: (worst - scores[candidate_id]) * Q20, simulations=64
    )
    ...
    try:
        certify_hardware_promotion(
            ...
            memory,
            hardware,
            outcome_by_id[oracle],
            outcome_by_id[baseline_id],
            policy,
            episode=episode,
            generation=2,
            quiescent=True,
    ...
    result = {
        ...
        "training": {
            "episodes": first.report.episodes,
            "candidates": first.report.candidates,
