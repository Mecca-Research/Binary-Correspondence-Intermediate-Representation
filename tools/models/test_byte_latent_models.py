...
def _latent_spec(
    *,
    ...
    patch_method: str = "stride",
    backbone: str = "transformer",
    joint_diffusion: bool = False,
):
    ...
    return ByteLatentSpec(
        ...
        diffusion=ByteDiffusionSpec(block_size=4, max_steps=2, unmask_strategy="confidence")
        if needs_diffusion
        else None,
        speculation=ByteSpeculationSpec(draft_bytes=3)
        if mode in ("self_speculative", "diffusion_verify")
        else None,
        learned_policy=LearnedPatchPolicySpec() if patch_method == "learned" else None,
    )
...
def _training_cases(inputs, targets) -> dict:
    ...
    latent_report = train_byte_latent_pretrain(
        ...
        inputs,
        targets,
        StageTrainSpec("pretrain", steps=3, learning_rate=3.0e-3, weight_decay=0.0, seed=1729),
    ...
    mamba_spec = MambaByteSpec(
        context_bytes=32, width=16, layers=1, state_size=4, expansion=2, conv_width=4
    )
    ...
    mamba_report = train_mambabyte_pretrain(
        ...
        inputs,
        targets,
        StageTrainSpec("pretrain", steps=3, learning_rate=3.0e-3, weight_decay=0.0, seed=1730),
    )
...
def _objective_cases(inputs, targets) -> dict:
    ...
    if (
        not all(
            ...
            for value in (objective.total, objective.autoregressive, objective.diffusion)
        )
...
def _causality_case() -> dict:
    ...
    mamba = HostedMambaByteModel(MambaByteSpec(context_bytes=32, width=16, layers=1, state_size=4))
    with torch.no_grad():
...
def _generation_cases() -> dict:
    ...
    _must_refuse(
        lambda: greedy_generate_byte_latent(
            speculative, prompt + (speculative.spec.vocabulary.mask_id,), 1
        ),
        "sentinels",
    )
    ...
    _must_refuse(lambda: self_speculative_generate(diffusion_verify, prompt, 1), "generation_mode")
    ...
    if any(
        forbidden.intersection(values[len(prompt) :])
        for values in (verified, verified_dv, generated, mamba_generated)
    ):
        raise AssertionError("byte generation emitted a non-output control sentinel")
    if (
        speculative.training is not True
        or diffusion_verify.training is not True
        or diffusion.training is not True
        or mamba.training is not True
    ):
...
def _transplant_case() -> dict:
    ...
    mapped_targets = sorted(name for name in targets if name.startswith("global_blocks.0."))
    source_state = {
    ...
    plan = plan_byteified_transplant(
        model.spec,
        source_sha256=tensor_mapping_digest(source_state),
    ...
    if any(
        not torch.equal(model.state_dict()[target], source_state[source])
    ...
    bad_plan = plan_byteified_transplant(
        model.spec,
        source_sha256=tensor_mapping_digest(bad_source),
        source_shapes=source_shapes,
        target_shapes=target_shapes,
        mappings=mappings,
    )
    ...
    before = tensor_mapping_digest(dict(model.state_dict()))
    _must_refuse(
        lambda: apply_byteified_transplant(source_state, model, tampered_plan), "element counts"
    )
    if tensor_mapping_digest(dict(model.state_dict())) != before:
    ...
    frozen_groups = byteified_parameter_groups(model, freeze_plan, base_learning_rate=1.0e-3)
...
def _lowering_and_ingest_case() -> dict:
    ...
    repeated = lower_byte_latent(spec, layout, target=TargetProfile.x86_avx2())
    if (
        lowered.artifact_sha256 != repeated.artifact_sha256
