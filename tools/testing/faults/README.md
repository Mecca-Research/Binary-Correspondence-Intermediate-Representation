...
| `movement.json` | `tools/perf/check_movement.py --require-cc` (G8: the joint movement planner, the movement laws and the ExecutionPlan v3 wire on both rails; needs a C compiler) | the exact planner's comparison; the transform's read remapping, staged-route kind, move spelling, final writeback and remat and accuracy certificates; the producer's identity plan and spec binding; each of MV1–MV11 and the laws' totality; R9's lifetime cover; the Python codec's version, window, home and same-bank laws and its move-version field; the ASN.1 projection's wire laws; the C twin's binding, generation locator, trailer, version field, flags, absent references, producer, self-replay, codec bits, certificate, windows, writeback home, same-bank and lossy-writeback laws |
