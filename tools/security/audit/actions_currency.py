...
_USES = re.compile(
    r"^\s*(?:-\s*)?uses:\s*([A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+)(?:/[^@\s]+)?@([0-9a-f]{40})"
    r"(?:\s*#\s*(\S+))?",
    re.M,
)
_HOOK_REPO = re.compile(
    r"^\s*-\s*repo:\s*https://github\.com/([A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+)\s*$"
    r"\n\s*rev:\s*(\S+)",
    re.M,
)
    ...
    if os.path.exists(hooks):
        with open(hooks, encoding="utf-8") as handle:
            for repo, rev in _HOOK_REPO.findall(handle.read()):
                found.append(
                    {"repo": repo, "pin": rev, "comment": "rev", "where": ".pre-commit-config.yaml"}
                )
    return found
...
def assess(repo: str, pin: str, tags: dict[str, str]) -> dict:
    ...
    pin_release = next(
        (t for t in pin_tags if _RELEASE.match(t)), pin_tags[0] if pin_tags else None
    )
    same_major = None
    majors_behind = None
    ...
    return {
        ...
        "pin": pin,
        "pin_sha": pin_sha,
        "is_release_commit": bool(pin_tags),
        "pin_release": pin_release,
        "pin_tags": pin_tags,
        "latest_release": latest,
        "latest_release_sha": releases.get(latest) if latest else None,
...
def main(argv: list[str] | None = None) -> int:
    ...
    for r in rows:
        if r["state"] != "OK":
            ...
            continue
        print(
            f"  {r['repo']:36s} {r['pin'][:12]:12s} {str(r['comment']):8s} uses={r['uses']:2d} "
