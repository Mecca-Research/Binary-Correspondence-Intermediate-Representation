...
#
# The corpus is generated by BCIR's OWN PER encoder and read back by the C twin, so this is a
# real differential rather than a self-consistency check: if the two disagree about a bit, the
# decoder is wrong about the rule the encoder implements. Both variants are covered, because
# ALIGNED and UNALIGNED differ at every field boundary and a decoder correct on one can be
# wrong on the other in a way no single-variant corpus would show.
