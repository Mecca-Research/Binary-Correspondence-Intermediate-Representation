...
then
  ...
  # becomes exit 141 for the whole pipeline -- a MATCH reported as a failure. It hides while
  # the disassembly fits the 64 KiB pipe buffer and appears when it does not, so it surfaces
  # as the object grows rather than as a change in what is being tested.
  : > "${tmp}/cross.asm"
